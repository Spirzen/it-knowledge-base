...
} from './shared/databaseSchemaEngine';
import styles from './DatabaseSchemaViewerPlay.module.css';
...
export default function DatabaseSchemaViewerPlay({
  title = 'Просмотр схемы базы данных',
  subtitle = 'Имитация подключения к СУБД: параметры, проверка, чтение метаданных и интерактивная ER-диаграмма',
}) {
  return (
    <BrowserOnly fallback={demoLoadingFallback('Загрузка просмотрщика схемы…')}>
      {() => <DatabaseSchemaViewerPlayInner title={title} subtitle={subtitle} />}
    </BrowserOnly>
  );
}
