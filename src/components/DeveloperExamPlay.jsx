...
import styles from './DeveloperExamPlay.module.css';
...
function DeveloperExamPlayInner({level = 'junior'}) {
  ...
  const loadFromArticle = useCallback(() => {
    const article = getArticleElement();
    const {sections: secs, questions} = extractExamQuestions(article);
    if (!questions.length) {
      setLoadStatus('empty');
      return;
    }
  ...
  useEffect(() => {
    const t = window.setTimeout(loadFromArticle, 320);
