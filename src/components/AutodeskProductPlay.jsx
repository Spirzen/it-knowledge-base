...
import {
  ...
  drawTinkerCAD,
  getProductMeta,
} from './shared/autodeskProductsEngine';
import styles from './AutodeskProductPlay.module.css';
...
export default function AutodeskProductPlay({product = 'autocad'}) {
  ...
  return (
    <BrowserOnly fallback={demoLoadingFallback(`Загрузка ${getProductMeta(valid).label}…`)}>
      {() => <AutodeskProductPlayInner product={valid} />}
    </BrowserOnly>
  );
}
