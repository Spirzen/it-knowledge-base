---
title: Финтех — о разделе
description: "Регуляторика, платежи, комплаенс и архитектура финансовых систем"
slug: /context/fintech/intro
sidebar_label: Финтех — о разделе
---

import ExternalPlayEmbed from '@site/src/components/ExternalPlayEmbed';


import DocCardList from '@theme/DocCardList';

# Финтех — о разделе

Финтех требует идемпотентности транзакций, низких задержек и согласованности с PSD2, ПОД/ФТ и защитой персональных данных. Статьи покрывают стандарты сообщений, KYC/AML и типовые архитектурные зоны. Универсальные основы — [финансовые потоки в IT](/encyclopedia/1-basics/1-04-kak-vidyat-it-obychnye-lyudi/4) и [безопасность API](/encyclopedia/2-system-network/2-09-osnovy-integratsionnogo-vzaimodeystviya/132).

<ExternalPlayEmbed example="context/context-domain-play" title="Отраслевое демо" minHeight={420} playProps={{ domain: 'fintech' }} />

## С чего начать

1. [Финтех](/context/fintech/1)
2. [Платёжные системы](/context/fintech/2)
3. [Регулирование](/context/fintech/3)
4. [Транзакции](/context/fintech/7)
5. [Безопасность в финансах](/context/fintech/8)

## Смежные разделы

- [Торговля](/context/commerce/intro)
- [Государство](/context/government/intro)

<DocCardList />

{/* context-encyclopedia */}

---

## В энциклопедии

Раздел **Контекст** дополняет [Энциклопедию](/encyclopedia/1-basics/basics) отраслевой призмой — те же технологии (API, БД, безопасность, интеграции), но с регуляторикой и бизнес-процессами домена. Рекомендуемые главы: [Финансовые потоки в IT-индустрии](/encyclopedia/1-basics/1-04-kak-vidyat-it-obychnye-lyudi/4), [Государство и цифровая экономика](/encyclopedia/1-basics/1-29-gosudarstvo-i-biznes/1), [Аутентификация и авторизация](/encyclopedia/2-system-network/2-08-osnovy-informatsionnoy-bezopasnosti/111), [12 советов по безопасности API](/encyclopedia/2-system-network/2-09-osnovy-integratsionnogo-vzaimodeystviya/132).

{/* /context-encyclopedia */}

---
