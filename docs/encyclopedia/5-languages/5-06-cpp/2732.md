...
## Плавный старт: как читать QML без страха

QML часто воспринимают как "новый язык", но на практике это декларативное описание экрана:

- задаёте элемент (`Button`, `Label`, `Column`);
- указываете свойства (`text`, `anchors`, `spacing`);
- описываете реакцию на событие (`onClicked`).

Логику домена при этом оставляете в C++-классе, который экспортируете в QML.
