...
<div class="callout callout--tip">
  ...
  **Исключение** — способ сообщить "здесь операция не удалась" и **передать** обработку наверх по стеку вызовов, минуя промежуточные `return`. В C++ это связано с **RAII**: при раскрутке стека вызываются деструкторы локальных объектов. Иерархия стандартных типов — [191](./191.md). Раскрутка и архитектура: [ошибки и исключения](/encyclopedia/4-code-dev/4-06-arhitektura-vypolneniya/111), [память](./19.md). Zero-cost EH: [28](./28.md).
</div>
  </div>
...
`std::ifstream file(...)` — **RAII**: при unwinding файл **закроется** в деструкторе, даже если `throw` случился сразу после открытия. Поэтому в C++ предпочитают RAII вместо `try/finally` как в Java: [30](./30.md), [14 — жизненный цикл](./14.md).
...
Практическое правило: **деструкторы только освобождают ресурсы**; ошибки внутри — логировать или глотать, но не `throw`. Подробнее: [191](./191.md).
...
Контейнеры STL документируют гарантии для `push_back`, `resize` и т.д. — см. [14 — STL](./14.md), [19](./19.md).
...
В embedded часто компилируют с `-fno-exceptions` — [21](./21.md).
