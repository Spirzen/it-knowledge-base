...
| `snprintf` | Безопасный форматированный вывод с ограничением длины | `snprintf(buf, size, "Данные: %s", input);` |
| `fopen` | Открытие файла для чтения или записи | `FILE* f = fopen("Данные.txt", "r");` |
| `fclose` | Закрытие ранее открытого файла | `fclose(file);` |
| `fread` | Чтение блока данных из файла | `fread(buffer, 1, 1024, file);` |
| `fwrite` | Запись блока данных в файл | `fwrite(Данные, sizeof(int), 10, file);` |
