...
После [первой программы](./20.md) вы уже знаете `cargo new` и `cargo run`. Реальные проекты редко укладываются в один файл: появляются **библиотека + бинарник**, несколько crate в одном репозитории, опциональные модули (**features**), разные режимы оптимизации (**profiles**), иногда скрипт **`build.rs`** перед компиляцией.
