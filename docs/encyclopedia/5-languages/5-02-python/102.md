...
Ниже — короткая линейка "игрушек", которые хорошо закрывают типовые темы главы. Попробуйте интерактивные версии — они работают в браузере.

<LangPracticeToysHub theme="python" toys={['tamagotchi', 'bugquiz', 'lootbox']} />

---

### Жанры видеоигр на Python

<GameGenreToysStack theme="python" genres={['platformer', 'roguelike', 'turnbased']} />

---

### 1) Тамагочи-сервер

- **Что тренирует:** классы, состояние объекта, методы, таймеры.
- **Идея:** у вас есть "сервер-питомец" с параметрами `load`, `temperature`, `uptime`; действия игрока влияют на выживание.
- **Расширение:** фоновые события через `threading` и сохранение прогресса в JSON.

---

### 2) Детектив "Угадай баг"

- **Что тренирует:** списки, словари, условия, подсчёт очков.
- **Идея:** игра показывает "симптомы", а игрок выбирает причину бага из вариантов.
- **Расширение:** добавьте чтение сценариев из файла и режим "случайный раунд".

---

### 3) Таверна-чат

<TavernChatPlay theme="python" />

- **Что тренирует:** сокеты, многопоточность, клиент-серверную модель.
- **Идея:** мини-чат с комнатами ("Таверна", "Гильдия", "Рейд"), где игроки обмениваются сообщениями.
- **Расширение:** добавьте команды `/roll`, `/whisper`, `/party`.
