...
В `plot` и `scatter` параметр `marker=` задаёт форму точки:

| Маркер | Символ | Когда уместен |
| --- | --- | --- |
| `'o'` | круг | универсальный scatter |
| `'s'` | квадрат | категории, сетка |
| `'^'` | треугольник вверх | акцент на максимумах |
| `'*'` | звезда | выделение выбросов |
| `'x'` | крест | ошибки измерений |
| `'.'` | точка | плотные облака данных |

```python
plt.scatter([1, 2, 3], [2, 5, 3], marker="s", s=80)
```

Больше примеров — [Lab 1112 — Matplotlib](/lab/Примеры/1112).
