...
1. Переопределение `__eq__` без корректного `__hash__` для неизменяемых объектов.
2. Слишком "умный" `__getattr__`, скрывающий реальные ошибки в именах атрибутов.
3. Побочные эффекты в `__repr__` и `__str__`.
4. Использование `__del__` в бизнес-логике вместо явного освобождения ресурса.
...
- Базовые классы и объектная модель: [ООП в Python](./26.md)
- Итерация и контекстные менеджеры: [Итераторы, генераторы и контекстные менеджеры](./25.md)
- Проверки условий и сравнения: [Управляющие конструкции](./23.md)
