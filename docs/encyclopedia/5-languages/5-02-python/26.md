...
После классов, наследования и утиной типизации (см. [выше](#утиная-типизация)) удобно перейти к **именованным схемам** из каталога GoF. В Python многие из них выражаются компактно: фабрика — функция или класс, возвращающий нужный тип; стратегия — вложенные классы или callable; наблюдатель — список колбэков или `asyncio` pub/sub.

| Паттерн | Типичный Python-приём |
|---------|------------------------|
| Factory | функция `create_shape(kind)` или `@classmethod` |
| Strategy | передача callable в конструктор |
| Observer | список подписчиков + `notify()` |
| Decorator | `@decorator` над функцией (идиома языка) и класс-обёртка (паттерн GoF) |

Краткая шпаргалка с ролями на диаграмме и ссылками на Java/C#: [Частые паттерны GoF](/encyclopedia/7-project/7-06-proektirovanie-i-arhitektura/design-patterns/141). Общая теория ООП без языка — [раздел 4-08-oop](/encyclopedia/4-code-dev/4-08-oop/intro).

---

## Связанные материалы

- [Магические методы и дандер-методы](./173.md) — шпаргалка и полный справочник dunder-методов
- [Итераторы, генераторы и контекстные менеджеры](./25.md) — `__iter__`, `__next__`, `with`
- [Управляющие конструкции](./23.md) — условия и сравнения
