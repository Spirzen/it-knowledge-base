...
description: Flask — это микрофреймворк для создания веб-приложений на Python. Слово микро здесь не означает неполноценный — оно указывает на отсутствие жёстких требований к структуре проекта, отсутствие…
---
...
def create_user():
    ...
    user = {'id': user_id, 'name': Данные['name'], 'email': Данные.get('email')}
    users[user_id] = user
...
def update_user(user_id):
    ...
    users[user_id].update({
        'name': Данные.get('name', users[user_id]['name']),
        'email': Данные.get('email', users[user_id]['email'])
    })
