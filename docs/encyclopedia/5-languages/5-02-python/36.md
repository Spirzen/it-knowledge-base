...
Компактные таблицы в [§1.5](#15-компактные-шпаргалки) частично восходят к шпаргалке [OverAPI Python](https://overapi.com/python) (MIT, [исходники](https://github.com/code9media/overthecs.github.io)); здесь они переведены и приведены к **CPython 3.12** (без устаревших конструкций Python 2).

**Учебные статьи:** [Типы данных](./19.md) · [Работа с типами](./21.md) · [Даты и время](./232.md) · [Магические методы](./173.md).
...
#### 1.5 Компактные шпаргалки

Таблицы для быстрого поиска по синтаксису. Подробнее о типах — в [§1.1](#11-встроенные-типы); о датах — в [Даты и время](./232.md); о дандер-методах — в [Магические методы](./173.md).

##### Индексы и срезы (`list`, `tuple`, `str`, `bytes`)
...
| `a[1:3]` | `[1, 2]` |
| `a[1:-1]` | `[1, 2, 3, 4]` |
| `b = a[:]` | поверхностная копия списка |

Отрицательный индекс считается с конца: `-1` — последний элемент. Срез `[start:stop:step]` не включает `stop`.

---

##### Форматирование через `%` (устаревший стиль)

`"значение: %s" % x` или `"%.2f" % (3.14159,)`. В новом коде предпочтительнее f-строки (`f"{x}"`) и `str.format()`.

| Код | Назначение |
|-----|------------|
| `d`, `i` | целое со знаком (десятичное) |
| `o` | целое в восьмеричной системе |
| `x`, `X` | целое в шестнадцатеричной (`x` — нижний регистр) |
| `e`, `E` | экспоненциальная запись `float` |
| `f`, `F` | десятичная запись `float` |
| `g`, `G` | кратчайшая из `f` и `e` |
| `c` | один символ (целое или строка длины 1) |
| `r` | `repr()` объекта |
| `s` | `str()` объекта |
| `%` | литеральный символ `%` |

Модификаторы: `%(width)d`, `%(width).(prec)f`, `%-10s` (выравнивание влево), `%+d` (знак у числа).

---

##### `strftime` — коды формата даты и времени

Работают в `datetime.strftime()`, `date.strftime()`, `time.strftime()` и `time.strptime()`. Локаль влияет на `%a`, `%A`, `%b`, `%B`, `%c`, `%x`, `%X`.

| Код | Значение |
|-----|----------|
| `%a` | сокращённый день недели (`Sun`) |
| `%A` | полное название дня недели (`Sunday`) |
| `%b` | сокращённое название месяца (`Jan`) |
| `%B` | полное название месяца (`January`) |
| `%c` | дата и время в локали |
| `%d` | день месяца `01`–`31` |
| `%H` | час (24 ч) `00`–`23` |
| `%I` | час (12 ч) `01`–`12` |
| `%j` | день года `001`–`366` |
...
| `%M` | минуты `00`–`59` |
| `%p` | `AM` / `PM` |
| `%S` | секунды `00`–`59` (до `61` при високосной секунде) |
| `%U` | номер недели (воскресенье — первый день), `00`–`53` |
| `%w` | день недели `0` (вс) – `6` (сб) |
| `%W` | номер недели (понедельник — первый день), `00`–`53` |
| `%x` | дата в локали |
| `%X` | время в локали |
| `%y` | год без века `00`–`99` |
| `%Y` | год с веком (`2026`) |
| `%z`, `%Z` | смещение / имя часового пояса (если есть) |
| `%%` | литеральный `%` |

Регистр важен: `%m` — месяц, `%M` — минуты; `%d` — день, `%D` — дата `мм/дд/гг` (расширение, не POSIX).
...
- , `join([timeout])`, `is_alive()`, `terminate()`, `kill()`, `close()`
...
##### Атрибуты  
- `argv` — аргументы командной строки (`argv[0]` — имя скрипта или `-c`)  
...
Пример `sys.argv` при запуске `python foo.py bar -c qux --h`:
...
Для разбора флагов удобнее [`argparse`](#22-argparse--парсинг-аргументов-командной-строки), чем ручной обход `argv`.
...
#### 2.27 `math` — Математические функции

Модуль `import math` — функции с плавающей точкой (аргументы и результат обычно `float`). Для целых степеней и точной арифметики смотрите также встроенный `pow()` и модуль `decimal`.
...
| `math.e` | e ≈ 2.718281… |
| `math.tau` | 2π |
| `math.inf`, `math.nan` | бесконечность и «не число» |

##### Числовые и целочисленные

`ceil(x)`, `floor(x)`, `trunc(x)`, `fabs(x)`, `copysign(x, y)`, `fmod(x, y)`, `factorial(n)`, `fsum(iterable)`, `gcd(a, b)`, `isclose(a, b, *, rel_tol=1e-09, abs_tol=0.0)`, `isfinite(x)`, `isinf(x)`, `isnan(x)`, `ldexp(x, i)`, `modf(x)`, `frexp(x)`, `remainder(x, y)`, `prod(iterable)` (≥ 3.8).

##### Степени и логарифмы

`exp(x)`, `expm1(x)`, `log(x[, base])`, `log1p(x)`, `log2(x)`, `log10(x)`, `pow(x, y)`, `sqrt(x)`, `hypot(x, y)`, `dist(p, q)` (≥ 3.8, евклидово расстояние).

##### Тригонометрия и гиперболика

`sin`, `cos`, `tan`, `asin`, `acos`, `atan`, `atan2(y, x)`, `sinh`, `cosh`, `tanh`, `asinh`, `acosh`, `atanh`, `degrees(x)`, `radians(x)`.

Углы в радианах, если не указано иное.

---

#### 2.28 `random` — Псевдослучайные числа

Модуль `import random` — детерминированный ГПСЧ (не криптостойкий). Для паролей и токенов — модуль `secrets` и `os.urandom()` (см. [§2.13](#213-os--интерфейс-к-операционной-системе)).

##### Базовые операции

`seed(a=None, version=2)`, `getstate()`, `setstate(state)`, `getrandbits(k)`, `randrange(stop)` / `randrange(start, stop[, step])`, `randint(a, b)`, `choice(seq)`, `choices(population, weights=None, *, cum_weights=None, k=1)`, `shuffle(x)`, `sample(population, k, *, counts=None)`.

##### Распределения `float`

`random()` → `[0.0, 1.0)`, `uniform(a, b)`, `triangular(low, high, mode)`, `betavariate(alpha, beta)`, `expovariate(lambd)`, `gammavariate(alpha, beta)`, `gauss(mu, sigma)`, `lognormvariate(mu, sigma)`, `normalvariate(mu, sigma)`, `vonmisesvariate(mu, kappa)`, `paretovariate(alpha)`, `weibullvariate(alpha, beta)`.

Отдельный генератор с собственным состоянием: `rng = random.Random()`; метод `jumpahead` удалён в Python 3 — для параллельных потоков используйте разные экземпляры `Random` с разными `seed`.
...
```sql
with Progress() as progress: task = progress.add_task("...", total=100)
```
...
<ExternalCodeEmbed example="yaml/py-502-36-001" title="3.15 DevOps и инфраструктура" minHeight={354} />
...
- Недостатки: устаревший внешний вид, ограниченная функциональность — больше примеров: [Tkinter и GUI](./311.md), [Lab — окна и виджеты](/lab/Примеры/1124)
