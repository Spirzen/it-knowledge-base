...
| Задача | Команда |
|--------|---------|
| Версия | `python --version` / `python3 --version` |
| venv | `python -m venv .venv` → активация: `.venv/Scripts/activate` (Win) или `source .venv/bin/activate` |
...
- `Система(command)`, `popen(cmd, mode='r', buffering=-1)` — устаревшее, использовать `subprocess`  
...
##### `pandas`  
- `DataFrame(Данные=None, index=None, columns=None, dtype=None, copy=False)`  
- Конструкторы:  
