...
Путеводитель: [Инструменты и среды](//tools/development/1). HTTP в Python: [requests](./34.md), файлы и JSON: [Работа с файлами, сетью и внешними API](./31.md).
...
<ExternalCodeEmbed example="python/py-502-317-001" title="Минимальный бот" minHeight={570} />
...
<ExternalCodeEmbed example="python/py-502-317-002" title="Inline-кнопки (под сообщением)" minHeight={336} />
...
<ExternalCodeEmbed example="python/py-502-317-003" title="Пошаговый диалог (состояния)" minHeight={480} />
...
<ExternalCodeEmbed example="python/py-502-317-004" title="SQLite" minHeight={318} />



Подробнее про слой доступа к данным — [Работа с базами данных в Python](./314.md) и кейс [Telegram Bot MVP](/lab/Кейсы/6) (шаг с `storage/sqlite.py`).
...
Один и тот же [Bot API](https://core.telegram.org/bots/api) — меняется только обёртка на Python. Уведомления без входящих команд (мониторинг) — в [Веб-разработка и REST API на Python](./34.md#telegram-боты--python-telegram-bot).
...
Продакшен-чеклист (Docker, webhook, тесты handlers) — [Примеры решений для Python бота](/lab/Примеры/122).
...
- Мониторинг и алерты — [Автоматизация задач и DevOps-скрипты](./35.md).
