...
description: В JavaScript очень часто приходится работать с объектами, чья структура неочевидна — это может быть объект из API, библиотеки, фреймворка (React, Vue, Express), или даже this в сложной цепочке…
---
...
// Цепочка промисов
fetch("https://api.example.com/Данные")
...
function processFile() {
    using file = new FileHandle("Данные.txt");
