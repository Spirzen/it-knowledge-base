...
description: Функция Значение Пример --------------------------- Array.isArray() Проверяет, является ли значение массивом Array.isArray(1, 2) concat() Объединяет массивы 1, 2.concat(3, 4) push() Добавляет элемент…
---
...
| `console.table()` | Выводит данные в виде таблицы | `console.table(Данные)` |
...
| `crypto.subtle.encrypt()` | Шифрует данные | `crypto.subtle.encrypt(algorithm, key, Данные)` |
| `crypto.subtle.decrypt()` | Расшифровывает данные | `crypto.subtle.decrypt(algorithm, key, Данные)` |
| `crypto.subtle.sign()` | Создает цифровую подпись | `crypto.subtle.sign(algorithm, key, Данные)` |
| `crypto.subtle.verify()` | Проверяет цифровую подпись | `crypto.subtle.verify(algorithm, key, signature, Данные)` |
| `crypto.subtle.digest()` | Вычисляет хеш данных | `crypto.subtle.digest(algorithm, Данные)` |
