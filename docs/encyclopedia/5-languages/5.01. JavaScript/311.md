...
- [🔹 Система данных: `Ext.Данные.*`](#-система-данных-extданные)
...
- Связывается с `Ext.Данные.Store`.
- Ключевые конфиги:
...
- Не отправляет данные напрямую — требует FormДанные или iframe в старых версиях.
...
| Конфиг | Описание |
|--------|--------|
| `store` | Экземпляр `Ext.Данные.Store` с данными |
...
#### Особенности:
- Хранит данные в `Ext.Данные.TreeStore`.
- Каждый узел — экземпляр `Ext.Данные.Model` с полями: `text`, `leaf`, `expanded`, `children`.
...
|--------|--------|
| `model` | Класс модели |
| `Данные` | Начальные данные (массив объектов) |
...
##### Пример хранилища:
```js
Ext.create('Ext.Данные.Store', {
    model: 'App.model.User',
    autoLoad: true,
    proxy: {
        type: 'ajax',
        url: '/api/users',
        reader: { type: 'json', rootProperty: 'Данные' }
    }
});
```
