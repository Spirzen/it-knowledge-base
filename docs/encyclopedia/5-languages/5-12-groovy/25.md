...
import JobDslPlayground from '@site/src/components/JobDslPlayground.jsx';
...
Тот же сценарий, что минимальный [Jenkinsfile в 22.md](./22.md), но **определение job** — код:
...
- `publishers { junit(...) }` — тот же glob, что в [22.md](./22.md) `post { junit ... }`.
...
Для Multibranch (авто-ветки) смотрите в API Viewer `multibranchPipelineJob` — типичный паттерн для GitFlow ([22.md](./22.md) § Multibranch).
...
- **секреты не храните** в DSL — credentials в Jenkins + `withCredentials` в `Jenkinsfile` ([22.md](./22.md));
