...
## Зачем нужен Groovy, если движок "на Java"
...
- опционально **IDE** с поддержкой Groovy (IntelliJ IDEA — см. [Первая программа на Groovy](./2.md)).
...
Шаблон собирается через **Groovy DSL** Gradle — подробнее в [Gradle Groovy DSL — первая сборка](./23.md).



<ExternalCodeEmbed example="groovy/groovy-512-24-001" title="build.gradle — ключевые блоки" minHeight={408} />
...
<ExternalCodeEmbed example="groovy/groovy-512-24-002" title="Минимальная игра — сцена и квадрат" minHeight={720} />
...
Без умножения на delta при 144 Hz игрок "летает", при 30 FPS — "ползёт". Тот же принцип — в [игровом практикуме](/encyclopedia/9-spinoff/9-04-razrabotka-igr/praktikum-razrabotki-igr/intro).
...
**Живая теория.** Игра — **симуляция во времени**. Каждый проход цикла:
...
| Hot path | `@CompileStatic` на классе `CollisionGrid` | [Особенности и расширения языка Groovy](./16.md) |
...
Покройте [Spock](./21.md)-спецификацией с `where:` — таблица граничных значений. Production-класс может остаться на Java, тест — на Groovy: см. [Spock — первая спецификация](./21.md).
...
| `MissingPropertyException` | Забыли `def` в скрипте | [Первая программа на Groovy](./2.md) |
...
4. Stage `Build` в Jenkins — `./gradlew test` + `./gradlew build` ([Jenkins Pipeline — первый Jenkinsfile](./22.md)).
