...
В таблицах ниже — не отдельные "глобальные функции" языка, а **методы GDK** (Groovy Development Kit): они добавляются к стандартным классам Java (`List`, `String`, `File`, `Map`) при запуске в среде Groovy. В чистой Java вызов `[1,2,3].each { }` не скомпилируется.

Полный список: [справочник § GDK](/encyclopedia/5-languages/5-12-groovy/3). Метапрограммирование (`metaClass`, AST): [основы](/encyclopedia/5-languages/5-12-groovy/11), [особенности](/encyclopedia/5-languages/5-12-groovy/16).

> **Интерактивное демо** — вызов функции и стек на примере **JavaScript**. В Groovy объявление другое, но вызов, локальные переменные и возврат устроены так же. Обобщённо: [функции в коде](/encyclopedia/4-code-dev/4-02-chto-takoe-kod-i-kak-on-rabotaet/4).
