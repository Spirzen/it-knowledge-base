...
В таблицах ниже — **методы GDK** (Groovy Development Kit) — они добавляются к стандартным классам Java (`List`, `String`, `File`, `Map`) при запуске в среде Groovy. В чистой Java вызов `[1,2,3].each { }` не скомпилируется.
