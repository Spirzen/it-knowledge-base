...
- **Gradle Groovy DSL** — сборка, зависимости, задача `run` ([23.md](./23.md));
- **groovy-swing** — декларативный UI через `SwingBuilder`, тот же приём delegate, что в Gradle ([11.md](./11.md));
- **groovy-json** — автоформатирование JSON в ответе ([103.md](./103.md));
...
Альтернатива на чистом Groovy — `HttpURLConnection` или Java 11+ `HttpClient` ([103.md](./103.md)). JMeter оправдан, если вы уже работаете с JMeter в CI или хотите единый стек "ручной запрос → нагрузочный сценарий".
...
- **Gradle** или wrapper (`gradlew`) — см. [23.md](./23.md);
- **сеть** — для примеров с [httpbin.org](https://httpbin.org);
- опционально **IntelliJ IDEA** с плагином Groovy — [2.md](./2.md).
...
- каталог `src/main/groovy` — стандарт Gradle для Groovy ([23.md](./23.md)), аналог `src/main/java`;
...
**Живая теория.** Файл `build.gradle` — Groovy-скрипт с **делегированием замыканий** ([11.md](./11.md)). Когда вы пишете `dependencies { implementation ... }`, Gradle передаёт closure объекту `DependencyHandler`, и вызов `implementation` резолвится у делегата — не в глобальной области. Тот же механизм в `application { }` и `tasks.register(...) { }`.
...
- `.tap { addArgument('API_TEST', 'true') }` — Groovy-идиома ([18.md](./18.md)) — настроить `Arguments` и вернуть тот же объект;
...
`SwingBuilder` ставит closure.delegate = builder; вызов `label(...)` уходит в builder — тот же **delegate**, что `dependencies { implementation ... }` в Gradle ([11.md](./11.md)). Именованные параметры `gridx`, `fill` — Map-аргументы Groovy.
...
- `readLines().each` — GDK на `String` ([172.md](./172.md));
...
- `JsonSlurper.parseText` — разбор JSON в Map/List без Jackson ([103.md](./103.md));
...
- `System.exit(1)` — ненулевой код для CI; Jenkins/GitHub Actions помечают stage как failed ([22.md](./22.md)).
...
|--------------|--------|--------|
| `HttpRequestConfig` | POGO, defaults, `[:]` | [12.md](./12.md) |
| `new HttpResponseResult(success: false, ...)` | Named args конструктора | [15.md](./15.md) |
| `.tap { addArgument(...) }` | Настройка объекта in-place | [18.md](./18.md) |
| `method in ['POST', 'PUT']` | `in` для коллекций | [13.md](./13.md) |
| `SwingBuilder` nested blocks | Delegate / DSL | [11.md](./11.md) |
| `JsonSlurper` / `JsonOutput` | JSON без Jackson | [103.md](./103.md) |
| `urlField.text?.trim()` | Safe navigation | [20.md](./20.md) |
| `body ?: ''` | Elvis-оператор | [13.md](./13.md) |
| последнее выражение метода | Неявный return | [16.md](./16.md) |
...
| `mainClass not found` | Расхождение package и Gradle | `application.mainClass` ([23.md](./23.md)) |
...
1. **Spock-тест** на `parseHeaders` с таблицей `where:` — [21.md](./21.md).
...
6. **@CompileStatic** на `parseHeaders` — если профилируете hot path ([16.md](./16.md)).
