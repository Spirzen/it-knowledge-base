...
description: Groovy не навязывает жёсткой структуры, но следует общепринятым соглашениям JVM-экосистемы, особенно если используется Gradle или Maven.
...
**Учебные статьи:** [Типы данных](./12.md).

---

## Краткое пояснение

Groovy — **Java-подобный** язык для JVM с **динамической и статической** типизацией, встроенными литералами коллекций, регулярных выражений и **замыканиями** (появились в Groovy раньше, чем лямбды в Java). Компиляция в байт-код; стандарт — **JSR 241**; разработка под эгидой **Apache** (Apache License 2.0). Расширения файлов — `.groovy`, `.gvy`, `.gsh`, `.gy`.

Компоненты и ключевые особенности языка — в разделах ниже.
...
| Конструкция | Пример |
|-------------|--------|
| Переменная | `def x = 1` или `int x = 1` |
| Строка | `'literal'`, `"Hello, $name"` |
| Список / карта | `[1, 2]`, `[a: 1, b: 2]` |
| Цикл | `(1..5).each { println it }` |
| Условие | `if (x) { ... }` |
| Безопасный доступ | `user?.profile?.name` |
| Значение по умолчанию | `name ?: 'Guest'` |
| Приведение | `value as Integer` |
| Класс | `class User { String name }` |
| Статическая проверка | `@CompileStatic` на классе или методе |

Полный перечень — в разделах ниже; учебный курс: [типы данных](/encyclopedia/5-languages/5-12-groovy/12), [операторы](/encyclopedia/5-languages/5-12-groovy/13).
...
- [4. Методы и функции](#4-методы-и-функции)
- [5. Классы и объекты](#5-классы-и-объекты)
- [6. Замыкания (Closures)](#6-замыкания-closures)
- [7. Коллекции и их методы](#7-коллекции-и-их-методы)
- [8. Метапрограммирование](#8-метапрограммирование)
- [9. Компиляция и настройки](#9-компиляция-и-настройки)
- [10. Интеграция с Java](#10-интеграция-с-java)
- [11. DSL и скриптование](#11-dsl-и-скриптование)
- [12. Распространённые утилиты и расширения](#12-распространённые-утилиты-и-расширения)
- [13. Безопасность и ограничения](#13-безопасность-и-ограничения)
- [14. AST-трансформации (Compile-time Metaprogramming)](#14-ast-трансформации-compile-time-metaprogramming)
- [15. Расширенное метапрограммирование](#15-расширенное-метапрограммирование)
- [16. Работа с данными](#16-работа-с-данными)
- [17. Шаблоны](#17-шаблоны)
- [18. Интеграция с Gradle](#18-интеграция-с-gradle)
- [19. Тестирование: Spock Framework](#19-тестирование-spock-framework)
- [20. Производительность и оптимизация](#20-производительность-и-оптимизация)
- [21. Практическое применение Groovy](#21-практическое-применение-groovy)
- [22. Версии и совместимость](#22-версии-и-совместимость)
- [23. Ограничения и предостережения](#23-ограничения-и-предостережения)
- [24. Встроенные методы расширений (GDK — Groovy Development Kit)](#24-встроенные-методы-расширений-gdk--groovy-development-kit)
- [25. Регулярные выражения](#25-регулярные-выражения)
- [26. Многопоточность и параллелизм](#26-многопоточность-и-параллелизм)
- [27. Управление ресурсами и замыкания](#27-управление-ресурсами-и-замыкания)
- [28. Практические паттерны использования](#28-практические-паттерны-использования)
- [29. Инструменты и экосистема](#29-инструменты-и-экосистема)
- [30. Советы по стилю и читаемости](#30-советы-по-стилю-и-читаемости)
- [31. Структура типичного Groovy-проекта](#31-структура-типичного-groovy-проекта)
- [32. Организация кода](#32-организация-кода)
- [33. Управление зависимостями](#33-управление-зависимостями)
- [34. Конфигурация приложения](#34-конфигурация-приложения)
- [35. Настройка компилятора Groovy](#35-настройка-компилятора-groovy)
- [36. Обработка ошибок и логирование](#36-обработка-ошибок-и-логирование)
- [37. Тестирование и проверка качества](#37-тестирование-и-проверка-качества)
- [38. Запуск и развёртывание](#38-запуск-и-развёртывание)
- [39. Безопасность при выполнении скриптов](#39-безопасность-при-выполнении-скриптов)
- [40. Профилирование и отладка](#40-профилирование-и-отладка)
- [41. Обновление и миграция](#41-обновление-и-миграция)
- [42. Когда использовать Groovy — и когда нет](#42-когда-использовать-groovy-и-когда-нет)
...
Проверка существования ссылки перед доступом — см. [Проверка и валидация](/encyclopedia/4-code-dev/4-14-razrabotka-i-otladka/118).
...
#### Делегирование
- Замыкание может иметь `delegate`, который определяет контекст выполнения (основа Gradle DSL — см. [основы языка § делегирование](/encyclopedia/5-languages/5-12-groovy/11)):
```groovy
...
- , `curry()`, `rcurry()`, `memoize()`, `trampoline()`
...
|----------|-------------------|
| Добавить в конец | `add(value)`, `leftShift(value)` (`<<`) |
| Вставить по индексу | `add(index, value)` |
| Прочитать | `get(index)`, `list[index]` |
| Заменить | `putAt(index, value)`, `list[index] = value` |
| Удалить по индексу | `remove(index)` |

Плюс — `each`, `collect`, `findAll`, `sort`, `reverse`, `unique`, `flatten`, `sum`, `min`, `max`.
...
| Действие | Метод / синтаксис |
|----------|-------------------|
| Записать | `put(key, value)`, `map[key] = value` |
| Прочитать | `get(key)`, `map[key]` |
| Удалить | `remove(key)` |

Плюс — `keySet()`, `values()`, `entrySet()`, `each`, `collectEntries`, `subMap`.
...
| Действие | Метод |
|----------|--------|
| Добавить | `add(value)`, `leftShift(value)` |
...
DSL на Groovy для стандартизации CI/CD. Подробный разбор: [Jenkins Shared Library](./26.md). Создание jobs — [Job DSL Playground](./25.md).
...
| **Griffon** | Настольные приложения (JavaFX/Swing) |

#### Среды разработки

Groovy поддерживают основные IDE:

| IDE | Поддержка |
|-----|-----------|
| **IntelliJ IDEA** | Встроенная (с версии 7); для старых версий — плагин JetGroovy |
| **Eclipse** | Плагин Groovy-Eclipse |
| **NetBeans** | Встроенная |
| **VS Code** | Расширение Groovy |
| **Vim** | Расширения сообщества |

#### Файлы и ресурсы

- Расширения исходников — `.groovy`, `.gvy`, `.gsh`, `.gy`.
- Официальные сайты — [groovy-lang.org](https://groovy-lang.org/), [groovy.apache.org](https://groovy.apache.org/).
- Спецификация — [JSR 241](https://jcp.org/en/jsr/detail?id=241).
...
**Справочники** — [Справочник по языку Ruby](/encyclopedia/5-languages/5-11-ruby/3), [Справочник по языку Rust](/encyclopedia/5-languages/5-13-rust/3), [Справочник по языку Go](/encyclopedia/5-languages/5-10-go/3), [Справочник по языку Swift](/encyclopedia/5-languages/5-14-swift/3), [Справочник по Kotlin](/encyclopedia/5-languages/5-09-kotlin/3), [Справочник по Lua](/encyclopedia/5-languages/5-15-lua-i-luau/3).
