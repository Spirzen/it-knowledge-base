...
Стандартизация завершена в **Java Community Process** ([JSR 241](https://jcp.org/en/jsr/detail?id=241)). С 2015 года Groovy — **проект верхнего уровня Apache** (лицензия Apache 2.0). Официальные ресурсы — [groovy-lang.org](https://groovy-lang.org/) и [groovy.apache.org](https://groovy.apache.org/).
...
| Кто вы | Маршрут |
|--------|---------|
| **Уже пишете на Java/Kotlin** | [Что требуется знать перед началом изучения языка программирования Groovy](./1001.md) → [Сравнение Java и Groovy](./20.md) → [Основы языка Groovy](./11.md) (делегирование замыканий) → [Справочник по языку Groovy](./3.md) (Spock, Jenkins) |
...
2. [Сравнение Java и Groovy](./20.md) — таблица отличий, пример кода, `==`, GString, `def`, вызов из Java  
...
| Сравнивать `==` с Java | В Groovy `==` → `equals` — [Сравнение Java и Groovy](./20.md) |
| Писать приложение на Grails "с нуля" | Для нового веба чаще Spring/Ktor; Grails — [Синтаксические конструкции Groovy](./18.md), legacy |
| Забыть `def` в скрипте | Иначе `MissingPropertyException` — [Первая программа на Groovy](./2.md) |

---

### Что попробовать

1. Скрипт `hello.groovy` без IDE — [Первая программа на Groovy](./2.md).
2. В [Основы языка Groovy](./11.md) — мини-DSL с `delegate` как в Gradle.
3. Один тест Spock к Java-классу — раздел 19 в [Справочник по языку Groovy](./3.md).
