...
Перед стартом: [KotlinMobileApp](./22.md) (Compose) + [корутины](./222.md) + [Flow](./226.md). Обзор MVVM на Android — [Kotlin в мобильных приложениях](/encyclopedia/4-code-dev/4-12-mobilnye-prilozheniya/1135).
...
**DataStore vs SharedPreferences** — DataStore пишет на диск **асинхронно** и отдаёт `Flow`; это лучше стыкуется с корутинами ([Flow в Kotlin](./226.md)).
...
- **`data class`** — автоматические `equals`, `copy`; удобно для immutable state ([Объектно-ориентированное программирование в Kotlin](./15.md)).
...
`viewModelScope` отменяется в `onCleared()` — нет утечки после закрытия экрана ([Корутины в Kotlin](./222.md)).
...
- **`collectAsState()`** — подписка на Flow; при новом `_petState` — recomposition ([Flow в Kotlin](./226.md)).
...
2. Вибрация при `PetMood.Critical` — `Vibrator` из Android API ([Kotlin в мобильных приложениях](/encyclopedia/4-code-dev/4-12-mobilnye-prilozheniya/1135)).
3. Unit-тест `applyDecayStep` без эмулятора — [Тестирование на Kotlin](./223.md).
