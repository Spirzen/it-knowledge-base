...
title: Ключевые слова языка Kotlin
description: Kotlin использует набор зарезервированных слов для построения синтаксических конструкций языка. Все ключевые слова разделены на категории по назначению.
---
...
| Ключевое слово | Значение | Пример |
|----------------|----------|--------|
| `sealed` | Ограниченная иерархия типов (подклассы в том же модуле) | `sealed class Result { class Success : Result(); class Error : Result() }` |
