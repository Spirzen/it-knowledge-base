...
### Интерфейс и абстрактный класс — выбор

| | `interface` | `abstract class` |
|---|-------------|------------------|
| Состояние (поля) | только `abstract`/`open` свойства или скрытое хранение через геттеры | можно хранить состояние в теле |
| Конструктор | нет | да |
| Несколько контрактов | класс реализует несколько интерфейсов | один суперкласс |
| Реализация по умолчанию | да (`fun foo() { }`) | да (неабстрактные методы) |
| Когда выбирать | контракт поведения, роли (`Serializable`, `Comparable`) | общая логика + обязательные точки переопределения |
...
`render()` в интерфейсе — **default implementation** (как `default` в Java 8+). Подкласс обязан реализовать только `draw()`.
...
- Набор взаимоисключающих вариантов с данными → `sealed class` (часто вместо глубокой иерархии).
...
Kotlin-код компилируется в JVM-байткод, совместимый с Java 6+. `data class` и компаньоны генерируют методы, ожидаемые Java-библиотеками (`equals`, `hashCode`, `copy`, статические фабрики). Миграция модулей может идти постепенно: один JAR — смесь `.kt` и `.java`.
