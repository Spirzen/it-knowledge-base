...
Сравнение с Java — [асинхронность Java](/encyclopedia/5-languages/5-03-java/298.md) (`CompletableFuture`, virtual threads). Полное сравнение языков — [Java и Kotlin](./235.md).
...
<span id="praktikum-parallelnaya-zagruzka"></span>

## Практикум — параллельная загрузка

Сценарий из [практикума](/encyclopedia/4-code-dev/4-05-asinhronnost/3). [`suspend`](https://kotlinlang.org/api/kotlinx.coroutines/kotlinx-coroutines-core/kotlinx.coroutines/suspend.html)-функция `download` имитирует сеть через [`delay`](https://kotlinlang.org/api/kotlinx.coroutines/kotlinx-coroutines-core/kotlinx.coroutines/delay.html).
...
Внутри [`coroutineScope`](https://kotlinlang.org/api/kotlinx.coroutines/kotlinx-coroutines-core/kotlinx.coroutines/coroutine-scope.html) каждый URL получает свою [`async`](https://kotlinlang.org/api/kotlinx.coroutines/kotlinx-coroutines-core/kotlinx.coroutines/async.html)-задачу; [`awaitAll`](https://kotlinlang.org/api/kotlinx.coroutines/kotlinx-coroutines-core/kotlinx.coroutines/await-all.html) ждёт все сразу.
...
<ExternalCodeEmbed example="kotlin/code-405-3-007" title="Kotlin — sequential и async" minHeight={300} />

Сравнение с Java `CompletableFuture` — [таблица в статье](./222.md#сравнение-с-java). Общий практикум — [раздел "Асинхронность"](/encyclopedia/4-code-dev/4-05-asinhronnost/3).
