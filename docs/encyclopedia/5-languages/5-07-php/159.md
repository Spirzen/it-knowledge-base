...
<div class="callout callout--info">
  ...
  С PHP 7 любой перехватываемый сбой — объект <code>Throwable</code> — <code>Exception</code> для ожидаемых сбоев библиотек и домена, <code>Error</code> для бывших "фатальных" ошибок языка (<code>TypeError</code>, деление на ноль). Структурный блок <code>try</code> / <code>catch</code> / <code>finally</code> соответствует общей модели раскрутки стека. На границе веб-приложения уместен один <code>catch (Throwable)</code> с логированием и HTTP-ответом без утечки стека наружу.<br /><br />
  Сначала прочитайте <a href="/encyclopedia/4-code-dev/4-06-arhitektura-vypolneniya/111#oshibka-i-isklyuchenie">ошибка vs исключение</a> и <a href="/encyclopedia/4-code-dev/4-06-arhitektura-vypolneniya/111">общую теорию</a>. В PHP <code>Error</code> — фатальные сбои языка, <code>Exception</code> — перехватываемые отклонения.
</div>
  </div>
...
<ExternalCodeEmbed example="php/php-507-159-001" title="Базовая конструкция try / catch / finally" minHeight={534} />
...
<ExternalCodeEmbed example="php/php-507-159-002" title="Перехват Exception и Error" minHeight={318} />
