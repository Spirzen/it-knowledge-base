...
description: Справочник-шпаргалка по PHP — типы, синтаксис, стандартная библиотека, типовые паттерны. Не заменяет пошаговое обучение. Учебный курс — раздел.
...
**Учебные статьи:** [Переменные и типы](./15.md) · [Скаляры](./152.md) · [Составные](./153.md).
...
##### 1.3.1. Приоритет и ассоциативность
...
| `$_FILES` | Загруженные файлы | Чтение/запись (осторожно!) | `['avatar' => …]` — валидация: [Загрузка файлов и валидация в PHP](./162.md) |
...
| `hash(string $algo, string $data, bool $binary = false)` | 5.1.2 | Универсальный интерфейс (`md5`, `sha256`, `sha3-256`, `blake3`, `crc32`, `crc32b`, `adler32` и др.). Список: `hash_algos()`. |
| `hash_hmac(string $algo, string $data, string $key, bool $binary = false)` | 5.1.2 | HMAC — хеширование с ключом. |
...
**Сводка** (индексный массив `$arr`):

| Действие | Синтаксис |
|----------|-----------|
| Добавить в конец | `$arr[] = $val`, `array_push($arr, $val)` |
| Вставить / удалить сегмент | `array_splice($arr, $offset, $count, $replace)` |
| Прочитать / заменить | `$arr[$i]`, `$arr[$i] = $val` |
| Удалить с конца / начала | `array_pop`, `array_shift` |

Ассоциативный массив (словарь) — `$arr[$key] = $val`, `$arr[$key]`, `unset($arr[$key])`.
...
| `file_put_contents(string $filename, mixed $data, int $flags = 0, ?resource $context = null)` | 5.0 | Запись строки/массива в файл. `$data` приводится к строке (`implode('', $data)` для массива). **Флаги**: `FILE_USE_INCLUDE_PATH`, `FILE_APPEND`, `LOCK_EX`. |
...
| `http_build_query(array\|object $data, string $numeric_prefix = "", ?string $arg_separator = null, int $encoding_type = PHP_QUERY_RFC1738)` | 5.1.2 | Формирование query string. `$encoding_type`: `RFC1738`, `RFC3986`. |
...
| **Заголовки** | `header('X-Content-Type-Options: nosniff')`, `X-Frame-Options: DENY`, `Content-Security-Policy: default-src 'self'`. |

**Пример CSP:**
```php
header("Content-Security-Policy: default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' Данные:; frame-ancestors 'none';");
```
...
| `listen.owner`, `listen.group`, `listen.mode` | `www-Data`, `www-Data`, `0660` | Права на Unix-сокет. |
| `user`, `group` | `www-Data` | От чьего имени работают процессы. |
...
- ~~`FILTER_SANITIZE_STRING`~~ (удалён в PHP 8.1), `FILTER_SANITIZE_EMAIL`, `FILTER_SANITIZE_URL`, `FILTER_SANITIZE_NUMBER_INT`, `FILTER_SANITIZE_NUMBER_FLOAT`
...
<ExternalCodeEmbed example="php/php-507-211-001" title="18.1. Простой роутер (без зависимостей)" minHeight={696} />
...
<ExternalCodeEmbed example="php/php-507-211-002" title="18.2. Простой DI-контейнер (PSR-11-like)" minHeight={720} />
...
<ExternalCodeEmbed example="php/php-507-211-003" title="18.3. Middleware (PSR-15)" minHeight={720} />
...
**Справочники** — [Справочник по C++](/encyclopedia/5-languages/5-06-cpp/3), [Справочник по Smalltalk](/encyclopedia/5-languages/5-08-smalltalk/5), [Справочник по C#](/encyclopedia/5-languages/5-05-csharp/471), [Справочник по Kotlin](/encyclopedia/5-languages/5-09-kotlin/3), [Справочник по ASP.NET](/encyclopedia/5-languages/5-05-csharp/452), [Справочник по языку Go](/encyclopedia/5-languages/5-10-go/3).
