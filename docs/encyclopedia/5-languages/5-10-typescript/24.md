...
**Теория:** [Обобщения и обобщённое программирование](/encyclopedia/4-code-dev/4-07-paradigmy-i-urovni-abstraktsii/114) — определения, зачем нужны generics, отличия языков.

---

**Дженерики (generics)** — параметры **типа**: одна функция или класс работает с разными данными, компилятор сохраняет связь между аргументом и результатом. Без них пришлось бы дублировать код или опираться на `any`. Удобный порядок — [функции](./14.md), эта статья, [async `Promise<T>`](./17.md).
...
<ExternalPlayEmbed example="languages/generic-constraint-play" title="Generic constraints" minHeight={420} />
...
- Возврат `T[K]` — точный тип поля, без `any`.
...
- `Promise<User>` — тип **всей** async-функции; `await res.json()` даёт `unknown`, без гарантии `User`.
- `value is User` — type predicate: после `if (!isUser(raw))` в ветке `return raw` тип уже `User`.
- `as Promise<User>` на `json()` — частая ошибка: `json()` возвращает `Promise<unknown>`, без типизированного ответа API.
...
- Шаблон `` `${A}-${B}` `` строит **декартово произведение** union — осторожно с большими множествами.
- Часто сочетают с `as const` объектом и `keyof typeof` — [Типы §as const](./10.md#as-const).
- Практический пример путей во вложенном объекте — [§Path и PathType](#пути-во-вложенном-объекте-path-и-pathtype).
...
- В продакшене часто берут готовые библиотеки; ниже — упрощённая "чистая" версия для учёбы.
