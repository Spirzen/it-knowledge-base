...
JavaScript даёт **массивы**, **Map**, **Set** и обычные объекты-словари. TypeScript описывает **элементы и ключи** так, чтобы `map`, `filter` и доступ по индексу не превращались в `any`. Здесь — выбор структуры и типичные ошибки, без копирования всех таблиц из [Справочник по TypeScript](/encyclopedia/5-languages/5-01-javascript/301#2-составные-типы). Общая теория — [Коллекции](/encyclopedia/4-code-dev/4-08-oop/61). Удобный порядок — [типы и типизация](./10.md), эта статья, [дженерики](./24.md), [классы](./18.md).

> Runtime-поведение массивов и итераторов — [типы в JavaScript](/encyclopedia/5-languages/5-01-javascript/18). Циклы `for` — [Циклы в TypeScript](./13.md).

<ExternalPlayEmbed example="data-markup/collection-methods-play" title="Методы коллекции" minHeight={520} playProps={{ defaultLang: 'ts' }} />
...
**Массив** в TypeScript — упорядоченный список элементов одного типа `T`. Записать можно двумя эквивалентными формами:

| Запись | Читается как |
|--------|--------------|
| `string[]` | массив строк (синтаксис "массив of") |
| `Array<string>` | generic `Array` с параметром `string` |

Обе формы **одинаковы** для проверки типов. Стиль — вопрос команды; в документации Microsoft часто `T[]`, в generic-контексте удобнее `Array<T>`.
...
- Индекс за границей даёт `T | undefined` при `noUncheckedIndexedAccess` — [Рекомендации по разработке на TypeScript](./6.md).
- Пустой массив `[]` без аннотации выводится как `never[]` — укажите тип явно: `const ids: string[] = []`.
- Многомерные массивы: `number[][]` или `Array<Array<number>>`.
...
- Для вложенных структур нужен `Readonly` на уровне полей — [Типы данных и типизация в TypeScript](./10.md).
...
- Удобно для списков маршрутов, ключей — связка с `typeof` в [Типы данных и типизация в TypeScript](./10.md).
...
<ExternalCodeEmbed example="typescript/typescript-510-19-001" title="`Map<K, V>` и `Set<T>`" minHeight={354} />
...
- Для сложных сценариев чаще используют discriminated union — [Типы данных и типизация в TypeScript](./10.md), [Асинхронное программирование в TypeScript](./17.md).
...
Подробнее о параметрах типа — [Дженерики в TypeScript](./24.md).
...
Не утверждайте `as string[]` без проверки — [Рекомендации по разработке на TypeScript](./6.md).
...
| Кортеж и массив | путаница длины | `[A,B]` только для фикс. позиций |
...
- JS: [Типы данных в JavaScript](/encyclopedia/5-languages/5-01-javascript/18)
