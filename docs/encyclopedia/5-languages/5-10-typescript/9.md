...
Подробный разбор первого запуска — [4.md](./4.md).
...
Для Vue: `--template vue-ts`. Связка с React — [21.md](./21.md).
...
| `strict` | Строгие проверки — [6.md](./6.md) |
...
- В Node с `module: NodeNext` в импорте указывают **`.js`**, хотя исходник — `.ts` — [4.md](./4.md).
- `export type` / `import type` — только типы, не попадают в runtime emit — [8.md](./8.md).
...
Работает для **`interface`** и **`namespace`** — удобно дополнять типы библиотек. Для union и mapped types используйте `type`, не interface. Подробнее — [Справочник — декларации](/encyclopedia/5-languages/5-01-javascript/301#10-декларации), emit `.d.ts` — [15.md](./15.md).
...
| `strict` выключен | Наследие шаблона | Включить в новом коде — [6.md](./6.md) |
