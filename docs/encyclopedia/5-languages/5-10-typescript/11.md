...
**Маршрут:** [Типы и типизация](./10.md) → **переменные** → [ветвления](./12.md) → [функции](./14.md).
...
Аннотация при **присвоении** (`x = value as string`) — это уже **утверждение** (`as`), а не объявление типа; предпочитайте сужение через `if` — ниже и в [Операторы и условные ветвления в TypeScript](./12.md).
...
Предпочитайте аннотации и **сужение** (`if`, `typeof`) вместо `as` — [Операторы и условные ветвления в TypeScript](./12.md).
