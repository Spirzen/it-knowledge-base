...
**Дальше:** [Форматы и подключение](./9.md) · [Обработка ошибок](./27.md) · [Справочник — лучшие практики](/encyclopedia/5-languages/5-01-javascript/301#16-лучшие-практики)

---

TypeScript даёт максимум пользы, когда команда договорилась о **правилах** — strict-флаги, работа с внешними данными, слои типов, CI и постепенная миграция с JavaScript. Здесь — инженерные практики; синтаксис — в [Типы данных и типизация в TypeScript](./10.md) и [Функции в TypeScript](./14.md). Таблицы флагов — в [Справочник — лучшие практики](/encyclopedia/5-languages/5-01-javascript/301#16-лучшие-практики).
...
Полная таблица — [Справочник — флаги безопасности](/encyclopedia/5-languages/5-01-javascript/301#флаги-безопасности-compileroptions).
...
Ручные проверки масштабируются плохо. Часто используют **Zod** или **Valibot**: схема → parse → тип. Базовые виды проверок — [Проверка и валидация](/encyclopedia/4-code-dev/4-14-razrabotka-i-otladka/118):
...
- **Именование**: суффиксы `Dto`, `Request`, `Response` для сети; без суффикса — domain.
- **Barrel-файлы** (`types/index.ts` с реэкспортом) — по желанию команды; на больших проектах иногда замедляют сборку.
- **Коллокация**: тип, используемый только в одном модуле, можно держать в том же файле; общий контракт — в `types/`.

Структура каталогов в monorepo и CI — [Экосистема §структура пакета](./3.md#типичная-структура-одного-пакета).
...
Пример состояния UI — в [типы §discriminated union](./10.md) и [Асинхронность](./17.md).
...
Краткий указатель в JS-курсе — [JS-курс: поэтапная миграция](/encyclopedia/5-languages/5-01-javascript/30#поэтапная-миграция).
...
1. Перейдите на **TypeScript 6.0** и уберите `ignoreDeprecations` — снимите предупреждения о `moduleResolution: node10`, `baseUrl`, `target: es5` и т.д.
2. Проверьте зависимости: **typescript-eslint**, кастомные трансформеры, Volar — нужен ли `@typescript/typescript6` side-by-side ([подробнее](./15.md#typescript-7-native)).
3. На CI с малым RAM зафиксируйте `--checkers` / `--builders`, чтобы сборки не расходились по памяти.
4. Полный `build` + тесты — не ограничивайтесь только `tsc --noEmit`, если есть emit metadata или decorator-фреймворки.
...
1. Предпочитайте исправление типов, `unknown` + guard, корректные `@types` — не комментарии.
2. Если `@ts-expect-error` неизбежен — **одна строка**, пояснение *почему* и ссылка на issue / версию TS.
3. Запретите `@ts-ignore` в ESLint (`@typescript-eslint/ban-ts-comment`) или требуйте `ts-expect-error` с описанием.
4. После обновления TypeScript пересматривайте директивы: `@ts-expect-error` без ошибки сломает `tsc`.

Синтаксис утверждений `as` / `!` — [Операторы §утверждения](./12.md#утверждения-типов-type-assertions).
...
- [Экосистема](./3.md)
- [Справочник — лучшие практики](/encyclopedia/5-languages/5-01-javascript/301#16-лучшие-практики)
