...
**Цикл** — конструкция, которая повторяет блок кода, пока выполняется условие или пока не закончится коллекция. В TypeScript циклы **совпадают с JavaScript** — те же `for`, `while`, `for...of`. TS добавляет **типы элементов** при переборе и подсказки в IDE. Удобный порядок — [ветвления](./12.md), эта статья, [коллекции](./19.md). Подробная теория и edge cases — в [циклах JavaScript](/encyclopedia/5-languages/5-01-javascript/23); ниже — типизация и связь с [коллекциями](./19.md).
...
- `user` в теле цикла типизирован как `User`, без `unknown`.
