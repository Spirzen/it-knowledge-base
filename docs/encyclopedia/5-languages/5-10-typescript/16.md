...
> Краткий указатель в JS-курсе — [JS-курс: TypeScript Server](/encyclopedia/5-languages/5-01-javascript/30#tsserver-typescript-server). CLI `tsc` — [15.md](./15.md).
...
- В больших репозиториях нагрузка на CPU/RAM — нормальна; помогают project references — [3.md](./3.md).
...
Подсказка строится тем же анализом, что и проверка присваиваний. Запись `as const` сузит вывод — см. [10.md](./10.md).
...
Рекомендация: в CI всегда **`tsc --noEmit`** или `tsc -b` — [6.md](./6.md), [3.md](./3.md).

---

## `tsserver` и JavaScript

В `.js` файлах при `allowJs` / `checkJs` сервер тоже анализирует типы (JSDoc или вывод). Это помогает **постепенной миграции** — [6.md](./6.md).
...
Для прикладного backend/frontend этот уровень не обязателен — достаточно [15.md](./15.md) и workspace `tsserver`. Таблицы и ссылки — [Справочник — расширение TypeScript](/encyclopedia/5-languages/5-01-javascript/301#18-расширение-typescript).

---

## Практика

1. Откройте [4.md](./4.md) проект и посмотрите inferred type у `greet`.
...
- [Справочник](./2.md) — [30](/encyclopedia/5-languages/5-01-javascript/30), [Справочник — инструменты и CLI](/encyclopedia/5-languages/5-01-javascript/301#17-инструменты-и-cli)
