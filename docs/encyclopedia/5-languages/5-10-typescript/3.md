...
> Обзор без учебных деталей — [Обзор TypeScript в JS](/encyclopedia/5-languages/5-01-javascript/30). Таблицы `tsconfig` — [301](/encyclopedia/5-languages/5-01-javascript/301).
...
- Типы API выносят в `src/types` — контракт между слоями — [6.md](./6.md).
- `typecheck` в CI даже если bundle делает Vite — [15.md](./15.md).
...
Минимальный `tsconfig` — [9.md](./9.md); политика strict — [6.md](./6.md).
...
Прикладные статьи: [21.md](./21.md) (React), [22.md](./22.md) (Node).
...
- **tsserver** использует тот же граф — быстрее в IDE на больших репо — [16.md](./16.md).
...
Частый паттерн SPA: **Vite собирает**, **`tsc -b` или `tsc --noEmit`** проверяет типы (в шаблоне `react-ts` это шаг `build` перед `vite build`) — [15.md](./15.md). Для Vue-проектов аналог — `vue-tsc --noEmit`.
...
| Обзор в JS-курсе | [30](/encyclopedia/5-languages/5-01-javascript/30) |
| Таблицы | [301](/encyclopedia/5-languages/5-01-javascript/301) |
| Индекс | [2.md](./2.md) — не второй справочник |
...
| Версии TS расходятся | глобальный tsserver | workspace TS — [16.md](./16.md) |
...
- [Справочник](./2.md) — [30](/encyclopedia/5-languages/5-01-javascript/30), [301](/encyclopedia/5-languages/5-01-javascript/301)
