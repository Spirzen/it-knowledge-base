...
TypeScript **не** заменяет тесты и валидацию сети: данные с сервера по-прежнему нужно парсить. Он снимает класс ошибок **внутри** вашего кода до запуска. Исторический контекст — [История TypeScript](./7.md).
...
3. **Разделение слоёв**: DTO (сеть) ≠ domain (бизнес) — [Рекомендации по разработке на TypeScript](./6.md), [TypeScript и Node.js](./22.md).
