...
TypeScript — это **JavaScript плюс система типов**. Синтаксис выполнения (скобки, запятые, `function`) совпадает с JS; сверху идут аннотации `: T`, `interface`, `type`, дженерики `<T>`. В статье — базовая грамматика TS и выбор между `interface` и `type`; полные таблицы — в [301](/encyclopedia/5-languages/5-01-javascript/301).
...
В `.js` после `tsc` остаётся обычный JavaScript — [15.md](./15.md).
...
Ключевое слово `type` задаёт **алиас**, то есть новое имя для типа. Union литералов вроде `Status` — основа discriminated unions; подробнее в [12.md](./12.md).
...
- В [18.md](./18.md) класс может `implements interface`.
...
- В runtime-модулях путь часто с `.js` при `NodeNext` — [9.md](./9.md).
...
Подробно — [14.md](./14.md); перегрузки и `this` там же.
...
Оператор `satisfies` проверяет форму, **не расширяя** вывод полей — см. [10.md](./10.md). Аннотация `: Routes` проще, но теряет узкий тип вроде `home.path`.
...
В `.ts` предпочитайте нативные аннотации; JSDoc — для миграции — [6.md](./6.md).
...
| `!` вместо проверки | небезопасно | narrowing — [12.md](./12.md) |
...
- JS: [17](/encyclopedia/5-languages/5-01-javascript/17)
