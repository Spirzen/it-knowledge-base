...
| Задача | Команда |
|--------|---------|
| Новый проект | `dotnet new console -n App` |
| Сборка / запуск | `dotnet build` / `dotnet run` |
| Пакеты | `dotnet add package <Name>` |
| Тесты | `dotnet test` |
...
- [19. WCF-конфигурация (`Система.serviceModel`)](#19-wcf-конфигурация-системаservicemodel)
...
```xml
<Reference Include="Система.Данные" />
...
```json
{
  "runtimeOptions": {
    ...
    "configProperties": {
      "Система.GC.Server": true,
      "Система.Threading.ThreadPool.MinThreads": 50,
      "Система.Globalization.Invariant": false,
      "Система.Reflection.Metadata.MetadataUpdater.IsSupported": false
    }
  }
}
```
...
|------|-----|----------|-------------|
| `Система.GC.Server` | bool | Серверный GC. | `true` для ASP.NET, `false` для консольных приложений. |
| `Система.GC.Concurrent` | bool | Параллельная сборка мусора. | `true`. |
| `Система.GC.RetainVM` | bool | Удерживать память после GC (не возвращать ОС). | `false`. |
| `Система.Threading.ThreadPool.MinThreads` | int | Мин. число потоков в пуле. | Зависит от ядер. |
| `Система.Threading.ThreadPool.MaxThreads` | int | Макс. число потоков. | Очень большое (≈32767). |
| `Система.Globalization.Invariant` | bool | Отключить локализацию (меньше памяти, быстрее). | `false`. |
| `Система.Reflection.Metadata.MetadataUpdater.IsSupported` | bool | Разрешить hot reload. | `true` в Debug, `false` в Release. |
| `Система.Runtime.Loader.DefaultContext.TrustedPlatformAssemblies` | строка | Явный список `.dll` (для изолированных загрузчиков). | Редко используется. |
...
```json
{
  "configProperties": {
    "Система.GC.Server": true,
    "Система.Globalization.Invariant": true
  }
}
```
...
```xml
<PropertyGroup>
  <RuntimeHostConfigurationOption Include="Система.Globalization.Invariant" Value="true" />
</PropertyGroup>
```
    ...
    "net8.0": {
      "Newtonsoft.Json/13.0.3": {
        ...
        "dependencies": {
          ...
          "Система.ComponentModel.TypeConverter": "4.3.0"
        }
      }
    }
  }
...
<configuration>
  ...
  <startup>...</startup>
  <runtime>...</runtime>
  <Система.web>...</Система.web>
  <Система.serviceModel>...</Система.serviceModel>
  <Система.Данные>...</Система.Данные>
...
<section 
  name="mySection"
  type="Система.Configuration.NameValueSectionHandler"
  allowLocation="true"
...
|------|---------|--------------|
| `Система.Configuration.AppSettingsSection` | Для `<appSettings>` | `<add key="K" value="V" />` |
| `Система.Configuration.NameValueSectionHandler` | Простой key-value | `<add key="K" value="V" />` |
| `Система.Configuration.DictionarySectionHandler` | То же, но возвращает `IDictionary` | `<add key="K" value="V" />` |
| `Система.Configuration.SingleTagSectionHandler` | Один тег с атрибутами | `<my attr1="v1" attr2="v2" />` |
| `Система.Configuration.IgnoreSectionHandler` | Игнорировать секцию | Любое содержимое |

##### `<sectionGroup>` — группировка секций

```xml
<sectionGroup name="applicationSettings" 
              type="Система.Configuration.ApplicationSettingsGroup, Система">
  <section name="MyApp.Properties.Settings"
           type="Система.Configuration.ClientSettingsSection, Система" />
</sectionGroup>
```
...
<Система.Данные>
  <DbProviderFactories>
    <remove invariant="Система.Данные.SQLite" />
    <add name="SQLite Данные Provider"
         invariant="Система.Данные.SQLite"
         description=".NET Framework Данные Provider for SQLite"
         type="Система.Данные.SQLite.SQLiteFactory, Система.Данные.SQLite" />
  </DbProviderFactories>
</Система.Данные>
```
...
- `<Система.web>` для ASP.NET,
- `<Система.Данные>` с провайдерами,
...
### 19. WCF-конфигурация (`Система.serviceModel`)
