...
Справочник-шпаргалка по **ASP.NET**: типы, синтаксис, стандартная библиотека, типовые паттерны. Не заменяет пошаговое обучение. Учебный курс: [раздел](/encyclopedia/5-languages/5.05. CSharp/intro).
...
| User secrets | Только в `Разработка`: `dotnet user-secrets set "Key" "Value"` → `%APPDATA%\Microsoft\UserSecrets\{GUID}\secrets.json`. |
...
| `[FromForm]` | `application/x-www-form-urlencoded`, `multipart/form-Данные` | Для HTML-форм и загрузки файлов |
...
| **`<input>` (InputTagHelper)** | `asp-for="Model.Property"`, `asp-format`, `type` | Генерирует `<input>` с `id`, `name`, `value`, `type`, `Данные-val-*` (для валидации). |
...
| **`<partial>`** | `name`, `for`, `model`, `view-Данные` | `<partial name="_Product" model="product" />` — современная замена `@Html.Partial`. |

##### 4.2. Валидация и `Данные-val-*`
...
<input type="email" id="Email" name="Email" value=""
       Данные-val="true"
       Данные-val-required="The Email field is required."
       Данные-val-email="The Email field is not a valid e-mail address." />
```

> ✅ Валидация:  
> - `ModelState.IsValid` — проверка на сервере  
> - `Данные-val-*` + JS — проверка на клиенте  
> - `[Required]`, `[EmailAddress]`, `[Range]`, `[RegularExpression]` и др. — из `Система.ComponentModel.DataAnnotations`.
...
builder.Logging.ClearProviders()
       ...
       .AddFilter("Система", LogLevel.Warning)
