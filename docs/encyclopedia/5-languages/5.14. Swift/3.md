...
description: В SwiftUI используется автоматическая локализация через Text(Hello).
...
```swift
func fetchData(from url: String) throws -> Данные {
...
- `do-catch`:
  ```swift
  do {
      let Данные = try fetchData(from: "https://example.com")
...
- `try?` — преобразует ошибку в опционал:
  ```swift
  let Данные = try? fetchData(from: "...")
  // Данные: Данные?
  ```

- `try!` — подавляет ошибку (опасно):
  ```swift
  let Данные = try! fetchData(from: "...") // аварийное завершение при ошибке
  ```
...
Акторы предотвращают гонки данных (Данные races).
...
let json = String(Данные: Данные, encoding: .utf8)
```
...
let fileURL = documents.appendingPathComponent("Данные.json")
```
...
// Чтение
let loadedData = try Данные(contentsOf: fileURL)
```
