...
| `throws` | Указание возможности генерации ошибки в сигнатуре | `func load() throws -> Данные` |
| `rethrows` | Передача ошибки из параметра-замыкания | `func process(_ block: () throws -> Void) rethrows` |
| `try` | Вызов функции, которая может сгенерировать ошибку | `let Данные = try loadData()` |
