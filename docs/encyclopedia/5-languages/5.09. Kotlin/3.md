...
description: Корутины — это легковесные потоки выполнения, управляемые библиотекой kotlinx.coroutines. Они позволяют писать асинхронный код в императивном стиле без коллбэков.
...
Данные-классы предназначены для хранения данных. Они автоматически получают реализации:
...
```kotlin
Данные class User(val id: Int, val name: String, val email: String)
```
...
fun handle(result: Result) = when (result) {
    is Success -> println(result.Данные)
...
```kotlin
val text = File("Данные.txt").readText()
val lines = File("Данные.txt").readLines()
```
...
```kotlin
File("Данные.txt").bufferedReader().use { reader ->
...
Данные class Success(val Данные: List<Item>) : NetworkResult
Данные class Failure(val error: Throwable) : NetworkResult
...
1. **Начните с Данные-классов** — они заменяют POJO с геттерами, сеттерами, `equals`, `hashCode`.
