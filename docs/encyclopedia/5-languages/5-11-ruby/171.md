...
<div class="callout callout--info">
  ...
  В Ruby перехватывают объекты-исключения через <code>begin</code> / <code>rescue</code> / <code>ensure</code>; <code>ensure</code> выполняется при любом исходе, как <code>finally</code>. В общий <code>rescue</code> без типа попадают только наследники <code>StandardError</code> — не перехватывайте так <code>SystemExit</code> и <code>SignalException</code>. Повторный сбой — <code>raise</code>. Теория стека и крэшей — <a href="/encyclopedia/4-code-dev/4-06-arhitektura-vypolneniya/111">ошибки и исключения</a>.
</div>
  </div>
