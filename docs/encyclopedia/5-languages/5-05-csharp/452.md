...
Справочник-шпаргалка по **ASP.NET**: типы, синтаксис, стандартная библиотека, типовые паттерны. Не заменяет пошаговое обучение. Учебный курс: [раздел](/encyclopedia/5-languages/5-05-csharp/intro). Официальные учебники и хаб документации: [455 — Microsoft Learn](./455.md).
...
Middleware — компоненты, обрабатывающие каждый HTTP-запрос и ответ в порядке регистрации ("луковичная" модель). **Когда middleware, а когда endpoint filter или policy pipeline** — критерии и полная схема запроса: [451 — Middleware, Endpoint Filters, Policy Pipeline](./451.md#где-жить-логике-middleware-endpoint-filters-policy-pipeline).
...
Полный навигатор с серией MVC и Docker: [455.md](./455.md). Ниже — быстрые ссылки для работы со справочником.
...
Оглавление: [tutorials/razor-pages](https://learn.microsoft.com/ru-ru/aspnet/core/tutorials/razor-pages/?view=aspnetcore-10.0) — проект **RazorPagesMovie**, 8 частей (model, scaffold, SQL, search, validation). Рекомендуемый старт для новичков; таблица в [455](./455.md).

---

### Серия MVC MvcMovie (практика)

Старт: [first-mvc-app/start-mvc](https://learn.microsoft.com/ru-ru/aspnet/core/tutorials/first-mvc-app/start-mvc?view=aspnetcore-10.0&tabs=visual-studio) — проект **MvcMovie**, .NET 10, далее controller/views, EF, search, validation (таблица частей в [455](./455.md)).
