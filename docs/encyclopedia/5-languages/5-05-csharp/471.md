...
description: Примечание — global используется в global — —Система.Int32 для разрешения коллизий имён с помощью глобального пространства имён.
...
|--------------|----------------|--------------|------------------------|
| `bool`       | `Система.Boolean` | 8 | `true` / `false` |
| `sbyte`      | `Система.SByte`   | 8 | −128 … 127 |
| `byte`       | `Система.Byte`    | 8 | 0 … 255 |
| `short`      | `Система.Int16`   | 16 | −32 768 … 32 767 |
| `ushort`     | `Система.UInt16`  | 16 | 0 … 65 535 |
| `int`        | `Система.Int32`   | 32 | −2 147 483 648 … 2 147 483 647 |
| `uint`       | `Система.UInt32`  | 32 | 0 … 4 294 967 295 |
| `long`       | `Система.Int64`   | 64 | −2⁶³ … 2⁶³−1 |
| `ulong`      | `Система.UInt64`  | 64 | 0 … 2⁶⁴−1 |
| `nint`       | `Система.IntPtr`  | 32/64 | зависит от архитектуры (указательный размер) |
| `nuint`      | `Система.UIntPtr` | 32/64 | зависит от архитектуры |
| `char`       | `Система.Char`    | 16 | UTF-16 символ (один элемент кодировки) |
| `float`      | `Система.Single`  | 32 | ~±1.5 × 10⁻⁴⁵ … ±3.4 × 10³⁸, 7 цифр точности |
| `double`     | `Система.Double`  | 64 | ~±5.0 × 10⁻³²⁴ … ±1.7 × 10³⁰⁸, 15–16 цифр |
| `decimal`    | `Система.Decimal` | 128 | ±1.0 × 10⁻²⁸ … ±7.9 × 10²⁸, 28–29 цифр, для финансовых вычислений |
| `string`     | `Система.String`  | ref | ссылка на неизменяемую UTF-16 строку |
| `object`     | `Система.Object`  | ref | корневой тип всех ссылочных и упакованных значений |

##### Категории типов:
- **Значимые (value types)**: структуры (`struct`), перечисления (`enum`), простые типы (`int`, `DateTime`, `Guid`, `Span<T>`, `UnsafePointer`, `nint`).
- **Ссылочные (reference types)**: классы (`class`), интерфейсы (`interface`), делегаты (`delegate`), массивы, строки — все наследуются от `Система.Object`.
- **Указатели (`T*`)**: доступны только в `unsafe`-контексте.

##### Специальные типы / конструкции:
| Конструкция | Описание |
|-------------|----------|
| `dynamic` | Отложенное связывание: проверка членов происходит во время выполнения. Наследуется от `Система.Object`, но компилятор отключает статическую проверку. |
...
| `^` | Index from end: `arr[^1]` — последний элемент; `^n` = `Length - n` (требуется `using Система;`) |
...
| `in` | `void M(in ReadOnlySpan<byte> Данные)` | `ref readonly` — нельзя модифицировать, но избегается копирование |
...
```csharp
public static bool TryParseHeader(ReadOnlySpan<byte> Данные, out int length)
{
    if (Данные.Length < 4) { length = 0; return false; }
    length = BinaryPrimitives.ReadInt32LittleEndian(Данные);
    return length >= 0;
}
```
...
#### 5. `Система.Runtime.CompilerServices.Unsafe`

> Пакет: `Система.Runtime.CompilerServices.Unsafe` (входит в .NET Standard 2.1+)
...
ReadOnlySpan<byte> Данные = ...;
int value = Unsafe.ReadUnaligned<int>(ref MemoryMarshal.GetReference(Данные));
...
| `[MethodImpl(MethodImplOptions.NoInlining)]` | Метод | Запрет встраивания (для профилирования, Безопасность) |
...
public static void Process(scoped Span<byte> Данные)
{
    // Нельзя: Func<byte> f = () => Данные[0];
}
```
Поддерживаемые типы:
- `ref T`, `ref readonly T`  
...
| `[Conditional("TRACE")]` | — | То же для `Система.Diagnostics.Trace` |
...
##### Правила:
- Должен наследоваться от `Система.Attribute`.  
...
| `Система.Runtime.CompilerServices.RuntimeHelpers.GetUninitializedObject(type)` | Очень быстро | Не вызывает конструктор — **только для сериализаторов** |
...
- `Система.Text.Json.Serialization.Metadata` — метаданные для сериализации без рефлексии.
