...
Теория CQRS — [CQRS](/encyclopedia/7-project/7-06-proektirovanie-i-arhitektura/design/2122); сквозная структура solution — [Clean Architecture на ASP.NET Core](/encyclopedia/7-project/7-06-proektirovanie-i-arhitektura/design/2143). Web API и endpoints — [Первая программа на ASP.NET Core](./4511.md).
...
Минимальный Clean Architecture из [Чистая архитектура](/encyclopedia/7-project/7-06-proektirovanie-i-arhitektura/design/2132) **работает без MediatR**: `CreateTaskUseCase` инжектируется в контроллер напрямую.
...
Интеграционный тест — `WebApplicationFactory` + `POST` с телом JSON ([Тесты ASP.NET Core — юнит и интеграция](./4516.md)): проверяется вся цепочка endpoint → MediatR → EF.
