...
Справочник-шпаргалка по **LINQ**: типы, синтаксис, стандартная библиотека, типовые паттерны. Не заменяет пошаговое обучение. Учебная глава: [LINQ — язык интегрированных запросов](/encyclopedia/5-languages/5-05-csharp/29) (в т.ч. три части запроса, классификация выполнения операторов, поставщики `IQueryable`). Официальная документация: [LINQ в C#](https://learn.microsoft.com/ru-ru/dotnet/csharp/linq/). Раздел: [введение в C#](/encyclopedia/5-languages/5-05-csharp/intro).
...
- [1.5. Отложенное (deferred) и Немедленное (immediate) выполнение](#15-отложенное-deferred-и-немедленное-immediate-выполнение)
...
### 1.5. Отложенное (deferred) и Немедленное (immediate) выполнение
...
| **Отложенное, потоковое** | Отложенное; не буферизует весь источник перед выдачей элемента | `Where`, `Select`, `Take`, `Skip`, `TakeWhile`, `SkipWhile`, `Distinct` |  
| **Отложенное, непотоковое** | Отложенное; требует полного чтения/буфера для операции | `OrderBy`, `GroupBy`, `Join`, `Reverse` |  
...
- Избегайте без `Include` — приведёт к N+1 или ошибке.

```sql
Select(x => new { x, x.Relation })
```
- Для пагинации:  
  ```csharp
...
**Справочники** — [Справочник по конфигурациям в C#](/encyclopedia/5-languages/5-05-csharp/101), [Справочник по ASP.NET](/encyclopedia/5-languages/5-05-csharp/452), [Справочник по F#](/encyclopedia/5-languages/5-04-platforma-dotnet/181), [Справочник по C#](/encyclopedia/5-languages/5-05-csharp/471), [Документация и инструменты Java (Microsoft)](/encyclopedia/5-languages/5-03-java/294), [Справочник по C++](/encyclopedia/5-languages/5-06-cpp/3).
