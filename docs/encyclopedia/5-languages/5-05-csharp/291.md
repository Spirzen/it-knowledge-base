...
description: LINQ (Language Integrated Query) — это единая модель запросов к данным в C, реализованная через — Синтаксис выражений запросов (from ... where ... select), Методы расширения (Where, Select, GroupBy,…
...
- [📌 Методы запросов (`Система.Linq.Enumerable` / `Система.Linq.Queryable`)](#-методы-запросов-системаlinqenumerable-системаlinqqueryable)
...
- [📌 Свёртка, итерация, и расширенные методы из `Система.Interactive`](#-свёртка-итерация-и-расширенные-методы-из-системаinteractive)
...
- [8.3. Библиотека `Система.Linq.Async`](#83-библиотека-системаlinqasync)
- [📌 LINQ для нестандартных источников](#-linq-для-нестандартных-источников)
- [9.1. LINQ to XML (`Система.Xml.Linq`)](#91-linq-to-xml-системаxmllinq)
- [9.2. LINQ to JSON (через `Система.Text.Json` + `JsonDocument`)](#92-linq-to-json-через-системаtextjson-jsondocument)
- [9.3. LINQ к Parquet, CSV, Avro (через `Microsoft.Данные.Analysis`, `CsvHelper`, `Parquet.Net`)](#93-linq-к-parquet-csv-avro-через-microsoftданныеanalysis-csvhelper-parquetnet)
...
|-------|-----------|-------------------|------------|----------------|  
| **LINQ to Objects** | `IEnumerable<T>` | `Система.Linq.Enumerable` | Выполняется в .NET CLR. Сразу на материализованных данных. | `Func<T, ...>` |  
| **LINQ to Entities / IQueryable** | `IQueryable<T>` | `Система.Linq.Queryable` | Выражение анализируется провайдером (например, EF Core) и транслируется (в SQL, REST и т.п.). | `Expression<Func<T, ...>>` |
...
### 📌 Методы запросов (`Система.Linq.Enumerable` / `Система.Linq.Queryable`)
...
### 📌 Свёртка, итерация, и расширенные методы из `Система.Interactive`

> **Важно**: Методы из `Система.Interactive` (пакет `Система.Interactive`) не входят в стандартную поставку .NET, но широко используются в продвинутых сценариях (потоковая обработка, реактивное программирование, сложные трансформации). Ниже — только те, что имеют практическую ценность для справочника и не дублируют `Enumerable`.
...
|-------|----|-----------|----------|-------------|
| `Buffer<TSource>(int count)` | `Система.Interactive` | `Buffer(3)` | Группирует элементы по `count`, возвращает `IEnumerable<IList<TSource>>`. Последняя группа может быть неполной. | Эквивалент `Chunk`, но возвращает `IList<T>`, а не `T[]`. |
| `Buffer<TSource>(int count, int skip)` | `Система.Interactive` | `Buffer(3, 1)` | Скользящее окно: сдвиг на `skip`, размер окна `count`. | ```csharp [1,2,3,4].Buffer(2,1) → [[1,2],[2,3],[3,4]] ``` |
| `Pairwise<TSource>` | `Система.Interactive` | `Pairwise()` | Возвращает `(current, next)` для соседних элементов. | ```csharp [1,2,3].Pairwise() → [(1,2), (2,3)] ``` - Пустая или одиночная послед-ть → пустой результат. |
| `StartWith<TSource>(params TSource[])` | `Система.Interactive` | `StartWith(0)` | Добавляет элементы *в начало* последовательности. | Эквивалент `new[] { x }.Concat(source)`, но чище. |
| `EndWith<TSource>` | `Система.Interactive` | `EndWith(999)` | Добавляет в конец. | Эквивалент `source.Append(x)`, но принимает `params`. |

### 4.3. Условная и отложенная логика

| Метод | Из | Сигнатура | Описание |
|-------|----|-----------|----------|
| `If<TSource>(bool condition, Func<IEnumerable<TSource>> thenSource)` | `Система.Interactive` | — | Условное включение последовательности. |
| `Switch<TSource>(Func<int, IEnumerable<TSource>> selector)` | `Система.Interactive` | — | Выбор источника по индексу (редко). |
| `Defer<TSource>(Func<IEnumerable<TSource>> factory)` | `Система.Interactive` | — | Откладывает *создание* источника до первого `MoveNext()`. Полезно для отложенной инициализации (например, чтение из файла только при использовании). |
...
- Избегайте `async` внутри `Select`/`Where` — они не `await`ятся. Используйте `SelectAwait` из `Система.Linq.Async`, если нужна асинхронная проекция.
...
### 8.3. Библиотека `Система.Linq.Async`

Пакет `Система.Linq.Async` (от Reactive Extensions) добавляет асинхронные аналоги LINQ:
...
### 9.1. LINQ to XML (`Система.Xml.Linq`)
...
### 9.2. LINQ to JSON (через `Система.Text.Json` + `JsonDocument`)
...
### 9.3. LINQ к Parquet, CSV, Avro (через `Microsoft.Данные.Analysis`, `CsvHelper`, `Parquet.Net`)
...
2. Или загрузить в `DataFrame` (`Microsoft.Данные.Analysis`) и использовать `Filter`, `Select`, `GroupBy` — но это **не LINQ**, а методы DataFrame API.
...
3. Для Parquet:  
   ...
   var column = group.Columns[0].Данные.Cast<int>();
...
Пространство имён: `Система.Linq`. Требует `using Система.Linq;` (да, то же, но методы в `ParallelEnumerable`).
...
> Всего **56 стандартных методов** в `Система.Linq.Enumerable` (не считая перегрузок). Ниже — классификация.
