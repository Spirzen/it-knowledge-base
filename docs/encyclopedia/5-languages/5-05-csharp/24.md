...
```
Кортежи в C# 7+ основаны на структуре Система.ValueTuple. `ValueTuple<T1>` — для 1 элемента, `ValueTuple<T1, T2>` — для 2. До 8 элементов напрямую. Если больше — 8-й элемент может быть ValueTuple (вложенный).
```csharp
