...
description: Обобщения (generics) — это механизм языка C, позволяющий определять классы, методы, интерфейсы, делегаты и структуры без указания конкретного типа данных. Вместо этого используется параметр типа,…
---
# Обобщения (generics)
