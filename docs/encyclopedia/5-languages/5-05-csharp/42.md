...
description: В ранних версиях .NET Framework использовались первые низкоуровневые классы для HTTP-запросов - HttpWebRequest и HttpWebResponse. Сейчас в .NET уже используется - HttpClient, из пространства имён…
---
...
string response = await client.GetStringAsync("https://api.example.com/Данные");
...
```csharp
HttpResponseMessage response = await client.GetAsync("https://api.example.com/Данные");
if (response.IsSuccessStatusCode)
{
    string content = await response.Content.ReadAsStringAsync();
    var Данные = JsonSerializer.Deserialize<DataModel>(content);
}
```
...
```csharp
var Данные = new { Name = "Alice", Age = 25 };
string json = JsonSerializer.Serialize(Данные);
...
```
HttpUtility находится в Система.Web (для .NET Framework) или в пакете Система.Web.Extensions (для .NET Core+).


Для загрузки файлов используется multipart/form-Данные. Пример:
```csharp
