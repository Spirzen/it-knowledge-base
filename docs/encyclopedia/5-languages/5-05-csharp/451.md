...
Практика: [Minimal API — endpoint filters](./4517.md#endpoint-filters). Справочник по `AddEndpointFilter`: [452](./452.md).
...
**Policy pipeline** — не отдельный класс фреймворка, а **архитектурный приём**: цепочка **независимых бизнес-проверок**, каждая отвечает за **одно** правило и может **остановить** выполнение с понятной ошибкой (403, 422, доменное исключение).
...
| **MediatR** `IPipelineBehavior` | Clean Architecture: проверки вокруг `IRequest` в Application — см. [4518](./4518.md), [2143](/encyclopedia/7-project/7-06-proektirovanie-i-arhitektura/design/2143). |
...
Типы фильтров MVC (`IAuthorizationFilter`, `IActionFilter`, `IExceptionFilter`) — в [документации Microsoft](https://learn.microsoft.com/ru-ru/aspnet/core/mvc/controllers/filters?view=aspnetcore-10.0). Логику action удобно проверять юнит-тестами с Moq; весь HTTP-pipeline — [4516](./4516.md).
