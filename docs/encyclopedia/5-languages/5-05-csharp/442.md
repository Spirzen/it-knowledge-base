...
Обзор всех способов: [44](./44.md). Высокоуровневый путь: [EF Core — первая программа](./441.md). Историческая модель COM-ADO: [ADO.NET (платформа .NET)](../5-04-platforma-dotnet/171.md).
...
1. и `DELETE FROM Books WHERE Id = @id` с параметрами.
