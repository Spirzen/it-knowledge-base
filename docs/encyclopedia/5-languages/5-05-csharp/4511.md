...
### Плавный старт для первого запуска

Если вы только переходите из "обычного C#" в веб, держите в голове простую модель:

- приложение "слушает" порт;
- клиент отправляет HTTP-запрос;
- ваш обработчик возвращает статус и тело;
- браузер или `curl` показывает результат.

В первом проекте достаточно понять три файла и три шага:

| Что | Где | Зачем |
|-----|-----|-------|
| Конфигурация и маршруты | `Program.cs` | Определяет, какие URL существуют |
| Контракты данных | `record`/классы DTO | Формат JSON для входа и выхода |
| Точка проверки | `/health` | Быстро проверить, что сервер отвечает |
...
- `Middleware` выполняет инфраструктурные шаги: логирование, безопасность, обработку ошибок.
- `Handler` обозначает конечный обработчик (`MapGet` или метод контроллера), где выполняется бизнес-логика.
- `JSON 200` показывает успешный ответ API с JSON-телом и кодом `200 OK`.
...
4. Добавляют интеграционный тест на ключевой маршрут ([4516](./4516.md)).
