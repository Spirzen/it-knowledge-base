...
Blazor проще изучать, если идти маленькими итерациями:

1. Сначала один компонент с кнопкой и счётчиком.
2. Потом ввод текста и список (`List<string>`).
3. Дальше HTTP-запрос к API из [4511](./4511.md).
4. Затем разбиение на переиспользуемые компоненты.

Так вы сразу видите результат в браузере и понимаете, за что отвечает каждый слой.
