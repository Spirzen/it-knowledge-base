...
Два уровня дополняют друг друга в типичном MVC-проекте:

| Уровень | Что проверяет | С чего начать |
|---------|---------------|---------------|
| **Юнит** | Один контроллер, логика action, `ModelState` | [Moq + `HomeController`](#unit-mvc) |
| **Интеграция** | `Program.cs`, middleware, маршруты, JSON | `WebApplicationFactory` + `GET /health` |

Сначала юнит-тест на изолированный action — быстрая обратная связь. Затем один интеграционный smoke-тест на весь pipeline.
