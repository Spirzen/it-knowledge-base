...
В языке C# отсутствуют глобальные функции в стиле скриптовых языков. Все базовые операции реализованы через статические методы системных классов пространства имён `Система`. Ниже представлен справочник ключевых методов и свойств, выполняющих роль встроенных функций.
...
| `Array.IndexOf(array, value)` | Поиск индекса элемента | `int pos = Array.IndexOf(Данные, 42);` |
...
| `File.Exists(path)` | Проверка существования файла | `bool exists = File.Exists("Данные.bin");` |
...
| `Path.Combine(part1, part2)` | Безопасное объединение частей пути с учётом ОС | `string fullPath = Path.Combine("Данные", "users", "list.csv");` |
...
| `FileStream.Write(buffer, offset, count)` | Запись байтов из буфера в файл | `fs.Write(Данные, 0, Данные.Length);` |
...
| Метод или свойство | Назначение | Пример |
|-------------------|------------|--------|
| `HttpClient.GetAsync(uri)` | Асинхронный HTTP-запрос методом GET | `HttpResponseMessage response = await client.GetAsync("https://api.example.com/Данные");` |
...
| `Task.WaitAll(Задачи)` | Ожидание завершения всех переданных задач | `Task.WaitAll(task1, task2, task3);` |
...
| `using (resource)` | Автоматическое вызывание `Dispose()` при выходе из блока | `using (var file = File.OpenRead("Данные.bin")) { ... }` |
...
| `BinaryFormatter.Serialize(stream, graph)` | Бинарная сериализация объекта | `formatter.Serialize(stream, Данные);` |
...
| Метод или свойство | Назначение | Пример |
|-------------------|------------|--------|
| `SHA256.HashData(bytes)` | Вычисление хеша SHA-256 для массива байтов | `byte[] hash = SHA256.HashData(Данные);` |
| `MD5.HashData(bytes)` | Вычисление хеша MD5 для массива байтов | `byte[] hash = MD5.HashData(Данные);` |
| `Aes.Create()` | Создание экземпляра алгоритма AES для шифрования | `using Aes aes = Aes.Create();` |
| `RSACryptoServiceProvider.Encrypt(Данные, fOAEP)` | Шифрование данных открытым ключом RSA | `byte[] cipher = rsa.Encrypt(plain, true);` |
| `RSACryptoServiceProvider.Decrypt(Данные, fOAEP)` | Расшифровка данных закрытым ключом RSA | `byte[] plain = rsa.Decrypt(cipher, true);` |
| `ProtectedData.Protect(Данные, entropy, scope)` | Шифрование данных с использованием DPAPI Windows | `byte[] protected = ProtectedData.Protect(Данные, null, DataProtectionScope.CurrentUser);` |
| `ProtectedData.Unprotect(Данные, entropy, scope)` | Расшифровка данных, защищённых DPAPI | `byte[] plain = ProtectedData.Unprotect(protected, null, DataProtectionScope.CurrentUser);` |
...
| `Registry.LocalMachine` | Корневой раздел HKEY_LOCAL_MACHINE | `RegistryKey key = Registry.LocalMachine.OpenSubKey("СИСТЕМА");` |
