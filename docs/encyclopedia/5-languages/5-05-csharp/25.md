...
description: Класс - это тип ссылочного вида, наследуемый от System.Object.
---
...
Если ООП для вас новое или вы учите C# с нуля, сначала пройдите материалы **без привязки к синтаксису**: [парадигмы и уровни абстракции](/encyclopedia/4-code-dev/4-07-paradigmy-i-urovni-abstraktsii/intro), затем [ООП — о разделе](/encyclopedia/4-code-dev/4-08-oop/intro) — [зачем объекты](/encyclopedia/4-code-dev/4-08-oop/7), [введение](/encyclopedia/4-code-dev/4-08-oop/1), [абстракция](/encyclopedia/4-code-dev/4-08-oop/2), [инкапсуляция](/encyclopedia/4-code-dev/4-08-oop/3), [наследование](/encyclopedia/4-code-dev/4-08-oop/4), [полиморфизм](/encyclopedia/4-code-dev/4-08-oop/5). Ниже — как это устроено в **C#**.
:::

## ООП в C#

C# во многом похож на Java, но есть важные отличия:
1. Свойства (properties) — полноценная языковая конструкция, а не пара методов.
2. Структуры (struct) — значимые типы на стеке (легковесные объекты).
3. Настоящие свойства с авто-геттерами/сеттерами — меньше кода.
4. Именованные/опциональные параметры — не нужно перегружать конструкторы.
5. Оператор `nameof` — безопасная работа с именами.
...
<ArchiStylerPlay defaultLanguage="csharp" defaultPattern="mvp" title="Классы и связи в C#" subtitle="MVP на диаграмме: View, Presenter, Model и превью кода" />
...
- `internal` внутри сборки (`.dll`/`.exe`) — аналог `package-private`
- `public` везде.
...
**Флаги** — комбинация значений через побитовые операции. Атрибут `[Flags]` и степени двойки:
