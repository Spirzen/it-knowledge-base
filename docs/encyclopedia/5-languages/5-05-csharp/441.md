...
Мы соберём консольное приложение **"Книги"** на **SQLite** — один файл `books.db` на диске, без установки SQL Server. Тот же `DbContext` потом подключим к [ASP.NET Core](./4511.md). Обзор всех подходов: [44](./44.md). Язык запросов LINQ: [29](./29.md). Полный API операторов: [291](./291.md).
