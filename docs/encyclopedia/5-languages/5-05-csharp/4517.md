...
В [Первая программа на ASP.NET Core](./4511.md) вы видели `app.MapGet("/health", ...)`. Это и есть **Minimal API**: HTTP-обработчик объявляется рядом с настройкой приложения, без класса `XxxController`.
...
<ExternalCodeEmbed example="csharp/csharp-505-4517-001" title="Эндпоинты и DTO" minHeight={720} />
...
**Endpoint filter** — код **после routing**, вокруг конкретного handler: валидация входа маршрута, feature flag на группу, аудит POST. Это не middleware (оно для всех URL) и не цепочка доменных регуляций (её оформляют как **policy pipeline** в Application — см. [ASP.NET - фреймворк для веб-приложений](./451.md#где-жить-логике-middleware-endpoint-filters-policy-pipeline)).
...
См. [Identity — JWT, cookie и MVC](./4515.md).
