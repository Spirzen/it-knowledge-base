...
Внутри F#-проекта домен чаще описывают **записями** (`type Person = { Name: string }`) и **размеченными объединениями** (`type Payment = Cash of decimal | Card of string`). Это даёт `match`, исчерпывающие ветки и неизменяемые данные ([Сопоставление с образцом в F# — практикум](./186.md)).
...
- внутри доменной логики сначала используйте записи, DU и функции;
- объектные конструкции подключайте на границе интеграции с C#-экосистемой и фреймворками .NET.
...
Обработка — через `match` ([Сопоставление с образцом в F# — практикум](./186.md)). Enum — когда внешний API уже зафиксировал набор констант без полезной нагрузки.
