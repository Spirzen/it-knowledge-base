...
- пишете публичный API для ASP.NET Core или C#-клиентов -> `task` и `Task`;
- пишете внутреннюю функциональную композицию в F# -> `async`;
- нужен безопасный доступ к общему состоянию без гонок -> агент `MailboxProcessor`.
...
Исключения внутри `async`/`task` ловят **`try/with`** как в обычном коде. Для **ожидаемых** сбоев (файл не найден, неверный id) предпочтительнее `Result` и `match` ([Сопоставление с образцом в F# — практикум](./186.md)), а исключения — для действительно аварийных ситуаций.
...
<ExternalCodeEmbed example="fsharp/dotnet-504-189-001" title="MailboxProcessor — агент с очередью" minHeight={426} />
