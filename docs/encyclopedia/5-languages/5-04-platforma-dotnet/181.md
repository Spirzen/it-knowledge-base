...
Справочник-шпаргалка по **F#**: типы, синтаксис, стандартная библиотека, типовые паттерны. Не заменяет пошаговое обучение. Учебный курс: [раздел](/encyclopedia/5-languages/5-04-platforma-dotnet/intro).
...
```fsharp
let list = new Система.Collections.Generic.List<int>()
list.Add(42)
```
...
```fsharp
let dict = Система.Collections.Generic.Dictionary<string, int>()
dict.["age"] <- 30
```
...
```fsharp
let sb = Система.Text.StringBuilder()
...
try
    riskyOperation()
with
| :? Система.IO.FileNotFoundException as ex ->
...
let fetchUrlAsync url =
    async {
        ...
        let! html = webClient.AsyncDownloadString(Система.Uri(url))
        return html.Length
    }
```
...
```fsharp
let (|Int|_|) str =
    match Система.Int32.TryParse(str) with
    | (true, n) -> Some n
    | _ -> None
...
let sales = Sales.Load("Данные.csv")
...
module Email =
    ...
    let create (s: string) =
        if Система.Text.RegularExpressions.Regex.IsMatch(s, @"^.+@.+\..+$")
        then Ok (Email s)
...
##### JSON с Система.Text.Json

```fsharp
open Система.Text.Json
...
let Данные = Sales.Load("actual_sales.csv")
for row in Данные.Rows do
...
```bash
dotnet add package FSharp.Данные --version 6.4.0
```
...
- Используйте `Span<T>` и `Memory<T>` для работы с буферами без выделения памяти (через `Система.Memory`).
...
- Используйте property-based Тестирование (FsCheck) для инвариантов.
...
| JSON                | Newtonsoft.Json, Система.Text.Json | Сериализация |
| CSV/HTML/XML        | FSharp.Данные            | Типы-провайдеры |
