...
| Действие | Функция |
|----------|---------|
| Прочитать | `Array.get arr i` |
| Заменить | `Array.set arr i v` |
| Создать | `Array.create n v`, `Array.zeroCreate n` |

Плюс `Array.map`, `Array.filter`, `Array.iter`, `Array.length`.

---

##### Модуль `Map` (неизменяемый словарь)

`Map.add`, `Map.find`, `Map.remove` — возвращают новую карту. Для мутаций в F# чаще используют `Dictionary` из .NET или `mutable.Map`.
...
<ExternalCodeEmbed example="xml/dotnet-504-181-001" title="3. Настройки проекта (.fsproj)" minHeight={390} />
...
<ExternalCodeEmbed example="fsharp/dotnet-504-181-002" title="5. Продвинутые возможности языка" minHeight={372} />
...
<ExternalCodeEmbed example="fsharp/dotnet-504-181-003" title="4. Сетевые операции" minHeight={318} />
...
<ExternalCodeEmbed example="fsharp/dotnet-504-181-004" title="6. Параллелизм и конкурентность" minHeight={408} />
...
<ExternalCodeEmbed example="yaml/dotnet-504-181-005" title="2. Непрерывная интеграция и доставка (CI/CD)" minHeight={408} />
...
Multi-stage для ASP.NET Core с `ASPNETCORE_URLS` и healthcheck — [Dockerfile — 10 типовых образов](/lab/Примеры/11113#9-aspnet-core).
