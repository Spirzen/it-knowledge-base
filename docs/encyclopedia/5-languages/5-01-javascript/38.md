...
## Плавный старт — зачем это нужно до оптимизаций

На этапе прототипа `scroll`-обработчики выглядят простыми. Проблемы начинаются с ростом страницы: каталог становится длиннее, появляются вложенные контейнеры, интерфейс начинает "тормозить".

Observer API переводит логику в модель "сообщи, когда произошло изменение". Это проще масштабируется и даёт стабильное поведение даже при сложной вёрстке.
...
| Подход | Эффект |
|--------|--------|
| До | постоянные проверки координат на `scroll`, рост нагрузки на главный поток |
| После | браузер вызывает колбэк только в нужный момент, интерфейс остаётся плавным |
