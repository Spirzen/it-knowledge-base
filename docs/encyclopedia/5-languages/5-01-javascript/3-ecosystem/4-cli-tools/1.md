...
## Другие популярные CLI-утилиты (куда смотреть дальше)

Ниже — инструменты, которые часто встречаются в реальных проектах рядом с `npm`, `vite`, `eslint` и т. п. Их удобно держать в голове как «шортлист», чтобы быстро подобрать утилиту под задачу.

### Окружение и версии Node.js (чтобы «работало у всех одинаково»)

- **`corepack`**: менеджер менеджеров пакетов (поставляется с Node.js) — включает и закрепляет версии `yarn` / `pnpm` в проекте (`packageManager` в `package.json`).
- **`nvm` / `nvm-windows`**: переключение версий Node.js на машине (полезно, когда разные проекты требуют разные версии).
- **`fnm`**: быстрый менеджер версий Node (часто выбирают как «nvm, но быстрее»).
- **`volta`**: «закрепляет» версии `node`/`npm`/`pnpm` на уровне проекта и прозрачно подхватывает их при запуске команд.

### Создание проектов и генерация шаблонов (скаффолдинг)

- **`create-*` семейство через `npm create …`**: современный стандарт инициализации шаблонов (`npm create astro@latest`, `npm create svelte@latest`, `npm create nuxt@latest` и т. д.).
- **`degit`**: скачать шаблон из репозитория без истории git (быстро и удобно для boilerplate).
- **`plop`**: генератор файлов по шаблонам внутри команды (например, компонент + тест + story одним запуском).
- **`hygen`**: генерация кода и файлов, похожая по идее на Plop, часто встречается в монорепо.

### Сборка, бандлинг, транспиляция (кроме Webpack/Vite)

- **`esbuild`**: очень быстрый бандлер/транспилятор; часто используют как «движок» для других инструментов или в простых сборках.
- **`rollup`**: популярный бандлер для библиотек (особенно когда важны ESM/CJS выходы и tree-shaking).
- **`parcel`**: zero-config сборщик (минимум настроек, быстро «поднять демо»).
- **`swc`**: быстрый компилятор (Rust) — может заменять Babel/tsc для некоторых сценариев.
- **`rspack`**: сборщик в стиле Webpack, ориентированный на скорость (Rust).
- **`tsup`**: удобная CLI для сборки TypeScript-библиотек «из коробки» (обычно поверх esbuild).
- **`tsx`**: запуск TypeScript-файлов как скриптов/CLI без отдельного шага компиляции (часто вместо `ts-node`).

### Монорепо и оркестрация задач (кроме Turborepo)

- **`nx`**: мощный набор для монорепозиториев (граф зависимостей, генераторы, кэш, плагины под фреймворки).
- **`lerna`**: исторический инструмент для монорепо (сейчас чаще встречается в legacy; нередко вместе с workspaces).

### Качество, стиль, pre-commit и CI-«обвязка»

- **`stylelint`**: линтер для CSS/Sass/PostCSS (актуально для больших UI-проектов).
- **`husky`**: git-hooks в проекте (например, чтобы запускать линтеры перед коммитом).
- **`lint-staged`**: запускает линтеры/форматтеры только на изменённых файлах (ускоряет pre-commit).
- **`commitlint`**: проверка сообщений коммитов (обычно под Conventional Commits).
- **`editorconfig` (через плагины) + `prettier`**: единые отступы/концы строк между IDE и CLI.

### Тестирование и e2e

- **`vitest`**: тест-раннер, хорошо сочетается с Vite (часто вместо Jest в новых проектах).
- **`playwright`**: e2e/интеграционные тесты в браузере + мощные фикстуры.
- **`cypress`**: e2e тестирование с удобной отладкой (популярен в продуктовых командах).

### Деплой и «облако из терминала»

- **`vercel`**: деплой Next.js/статических проектов, предпросмотры PR, переменные окружения.
- **`netlify`**: деплой статики/SSR (в зависимости от сетапа), предпросмотры, формы и т. п.
- **`firebase`**: хостинг, функции, emulators и деплой (`firebase deploy`).
- **`wrangler`**: Cloudflare Workers/Pages — локальная разработка и деплой.
- **`serverless`**: фреймворк для AWS Lambda и serverless-инфраструктуры (встречается в backend-проектах на Node).

### Создание собственных CLI (поверх Node.js)

- **Аргументы и команды**: `commander`, `yargs`, `cac` (минималистичный), `oclif` (фреймворк для больших CLI).
- **Интерактивные промпты**: `inquirer`, `enquirer`, `prompts`, `clack` (современный UX).
- **Исполнение команд/процессов**: `execa` (удобнее, чем голый `child_process`), `zx` (скрипты в стиле «bash на JS»).
- **Красивый вывод**: `chalk` (цвет), `ora` (спиннер), `boxen` (рамки), `log-update` (обновление строк), `cli-table3` (таблицы).
