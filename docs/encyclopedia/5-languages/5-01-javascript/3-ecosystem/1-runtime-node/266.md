...
Команды npm — [npm — команды, зависимости и lock-файлы](./265.md). Runtime и модули — [Node.js - серверный JavaScript](./26.md). Express и Router — [Express — middleware, маршруты и ошибки](./263.md).
...
<ExternalCodeEmbed example="json/js-501-3-ecosystem-1-runtime-node-266-001" title="package.json как контракт проекта" minHeight={426} />
...
Полный разбор npm-полей и lock — [npm — команды, зависимости и lock-файлы](./265.md).
...
// graceful shutdown — см. CLI Node.js — запуск, отладка и деплой (267.md)
export { server };
```

Supertest подключается к `app` **без** реального порта — [Тестирование JavaScript — Vitest и Testing Library](../../33.md).
...
5. **Ошибки API** — единый JSON `{ error, code? }` — см. [Express — middleware, маршруты и ошибки](./263.md).
...
Связка React на `:5173` и API на `:3000` — [Fullstack на JavaScript — API и фронтенд](./264.md).
...
| Синхронный `readFileSync` на каждый запрос | `fs/promises` или потоки — [Встроенные модули Node.js — fs, потоки и http](./268.md) |
...
| Этап | Структура |
|------|-----------|
| 1 | Один `server.js`, массив в памяти — [Первая программа на Node.js](./262.md) |
| 2 | `routes/`, `middleware/errorHandler` — [Express — middleware, маршруты и ошибки](./263.md) |
