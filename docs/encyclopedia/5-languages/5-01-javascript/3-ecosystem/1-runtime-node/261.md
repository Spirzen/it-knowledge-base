...
<ExternalPlayEmbed example="code-dev/package-dependency-play" title="package.json и lockfile" minHeight={420} />
...
Компактные таблицы в [§0.5](#05-компактные-шпаргалки) частично восходят к шпаргалке [OverAPI Node.js](https://overapi.com/nodejs) (MIT, [исходники](https://github.com/code9media/overthecs.github.io)); здесь они переведены и приведены к **Node.js 20 LTS** (без удалённых API вроде `domain`, встроенного `punycode`, `new Buffer()`).
...
- [0.5. Компактные шпаргалки (OverAPI)](#05-компактные-шпаргалки)
...
1. Введение в Node.js — [Node.js - серверный JavaScript](./26.md)
2. Таймеры (`setTimeout`, `setInterval`, `setImmediate`)
3. Модули и `npm` — [npm — команды, зависимости и lock-файлы](./265.md)
4. Структура проекта — [Структура Node-проекта и правила разработки](./266.md)
5. Работа с файловой системой (`fs`) — [Встроенные модули Node.js — fs, потоки и http](./268.md)
   - введение в `fs`
   - синхронные операции
   ...
   - `pipe`
6. Сервер на Node.js — [Встроенные модули Node.js — fs, потоки и http](./268.md), [Первая программа на Node.js](./262.md)
...
Таблицы для быстрого поиска по API. Подробные разборы — в [§1](#1-глобальные-объекты-и-переменные)–[§2](#2-встроенные-модули); практика — [Встроенные модули — fs, потоки и http](./268.md).
...
ESM-аналоги: `import`, `import.meta.url`, проверка точки входа — [Точка входа — require.main и import.meta](./48.md).
...
<ExternalCodeEmbed example="json/js-501-3-ecosystem-1-runtime-node-261-001" title="Пример `package.json`" minHeight={552} />
...
<ExternalCodeEmbed example="javascript/js-501-3-ecosystem-1-runtime-node-261-002" title="Сервер на чистом Node.js" minHeight={336} />
