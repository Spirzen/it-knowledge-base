...
description: Справочник-шпаргалка по Angular — типы, синтаксис, стандартная библиотека, типовые паттерны. Не заменяет пошаговое обучение. Учебный курс — раздел.
slug: /encyclopedia/5-languages/5-01-javascript/291
...
Справочник-шпаргалка по **Angular** — модули, DI, шаблоны, RxJS. Не заменяет пошаговое обучение — курс: [раздел Angular](./intro.md). Таблицы синтаксиса и API — для быстрого поиска фактов.
...
Разбор:
- Команда извлекает строки, помеченные для локализации, из шаблонов и компонентов Angular.
- На выходе формируется файл переводов (`messages.xlf` и совместимые форматы), который передают локализаторам.
- Шаг нужен для организации централизованного процесса перевода в многоязычных приложениях.
- Обычно выполняется после изменений текстов в UI перед сборкой релиза.
...
- [1. Архитектурные блоки Angular](#1-архитектурные-блоки-angular)
- [2. Модули (NgModules)](#2-модули-ngmodules)
- [3. Сервисы и внедрение зависимостей (DI)](#3-сервисы-и-внедрение-зависимостей-di)
- [4. Маршрутизация (RouterModule)](#4-маршрутизация-routermodule)
- [5. Формы](#5-формы)
- [6. HttpClient](#6-httpclient)
- [7. RxJS и реактивное программирование](#7-rxjs-и-реактивное-программирование)
- [8. Жизненный цикл компонентов и директив](#8-жизненный-цикл-компонентов-и-директив)
- [9. Анимации](#9-анимации)
...
- [12. Конфигурация проекта](#12-конфигурация-проекта)
- [13. Тестирование](#13-тестирование)
- [14. Best Practices](#14-best-practices)
- [15. Сигналы (Signals) — Angular 16+](#15-сигналы-signals-angular-16)
- [16. Автономные компоненты, директивы и pipes (Standalone API)](#16-автономные-компоненты-директивы-и-pipes-standalone-api)
- [17. Обнаружение изменений (Change Detection)](#17-обнаружение-изменений-change-detection)
- [18. Работа с DOM и безопасность](#18-работа-с-dom-и-безопасность)
- [19. Кастомные элементы (Angular Elements)](#19-кастомные-элементы-angular-elements)
- [20. PWA (Progressive Web App)](#20-pwa-progressive-web-app)
- [21. Серверный рендеринг (SSR) и Hydration](#21-серверный-рендеринг-ssr-и-hydration)
- [22. Миграция и совместимость](#22-миграция-и-совместимость)
- [23. Angular CLI: Полный список команд и флагов](#23-angular-cli-полный-список-команд-и-флагов)
- [24. Структура проекта Angular](#24-структура-проекта-angular)
- [25. Конфигурационные файлы: полное описание параметров](#25-конфигурационные-файлы-полное-описание-параметров)
- [26. Практические шаблоны](#26-практические-шаблоны)
- [Структура файла](#структура-файла)
- [1. Глобальные настройки CLI](#1-глобальные-настройки-cli)
- [2. Проекты (`projects`)](#2-проекты-projects)
- [3. Мультипроектные рабочие области](#3-мультипроектные-рабочие-области)
- [4. Практические рекомендации](#4-практические-рекомендации)
...
Разбор:
- Параметры конструктора одновременно объявляют поля класса и внедряют зависимости через DI-контейнер Angular.
- `private http: HttpClient` даёт доступ к HTTP-клиенту без ручного создания экземпляра.
- `private logger: LoggerService` внедряет собственный сервис для логирования и трассировки.
- Паттерн снижает связанность: в тестах эти зависимости можно подменить моками.
...
Разбор:
- `Routes` описывает карту URL-путей и соответствующих экранов приложения.
- `path: ''` задаёт корневой маршрут, `path: 'user/:id'` показывает использование параметра маршрута.
- `loadChildren` подключает модуль лениво, что уменьшает первоначальный размер бандла.
- `path: '**'` служит fallback-маршрутом для неизвестных URL и обычно ведёт на страницу 404.
- Такая конфигурация помогает разделить приложение на функциональные зоны и ускорить загрузку.
...
Разбор:
- `#f="ngForm"` создаёт ссылку на объект формы, через который доступны валидность и состояние контролов.
- `ngModel` включает двустороннюю связь значения поля с моделью шаблонной формы.
- `required` и `email` подключают встроенные валидаторы Angular для обязательности и формата почты.
- Подход удобен для простых форм, где нужна быстрая реализация без программного объявления `FormGroup`.
...
Разбор:
- `FormGroup` создаёт реактивную структуру формы в TypeScript-коде компонента.
- Каждый `FormControl` хранит значение, валидность и состояние конкретного поля.
- `Validators.required` и `Validators.email` задают правила проверки данных до отправки формы.
- Реактивный подход облегчает unit-тестирование и сложные сценарии с динамическими полями.
...
Разбор:
- `[formGroup]="profileForm"` связывает DOM-форму с объектом `FormGroup`, объявленным в компоненте.
- `formControlName="name"` подключает конкретный input к контролу `name` внутри группы.
- Angular автоматически синхронизирует значение поля, статус валидации и состояние touched/dirty.
- Такой шаблон формирует понятный контракт между HTML и реактивной моделью данных.
...
Разбор:
- `HttpParams` создаёт объект query-параметров для безопасного формирования URL.
- Цепочка `.set(...)` возвращает новый экземпляр, поэтому параметры удобно собирать последовательно.
- `this.http.get(..., { params })` отправляет GET-запрос с параметрами `page` и `size`.
- Подход упрощает пагинацию, фильтрацию и повторное использование сетевой логики.
...
Разбор:
- `HttpHeaders` формирует набор HTTP-заголовков в типобезопасном формате Angular.
- Заголовок `Authorization: Bearer ...` обычно несёт access token для защищённых API.
- Передача `{ headers }` добавляет заголовки к конкретному запросу.
- Конструкция часто используется вместе с интерцепторами для централизованной авторизации.
...
Разбор:
- `trigger('fadeInOut', ...)` объявляет имя анимации, которое затем используется в шаблоне.
- `state('in', style({ opacity: 1 }))` задаёт стабильное состояние полностью видимого элемента.
- `transition(':enter', ...)` и `transition(':leave', ...)` описывают анимации появления и исчезновения.
- `animate(300)` определяет длительность перехода в миллисекундах, обеспечивая плавный UX.
...
Разбор:
- `[@fadeInOut]` связывает элемент с триггером анимации из метаданных компонента.
- Значение `'in'` активирует состояние, описанное в `state('in', ...)`.
- При добавлении/удалении элемента Angular применяет `:enter` и `:leave` переходы автоматически.
- Привязка остаётся декларативной: поведение анимации читается прямо в шаблоне.
...
<ExternalCodeEmbed example="typescript/js-501-3-ecosystem-2-frontend-frameworks-3-angular-291-001" title="26.2. Шаблон сервиса с HttpClient" minHeight={444} />
...
<ExternalCodeEmbed example="typescript/js-501-3-ecosystem-2-frontend-frameworks-3-angular-291-002" title="26.3. Шаблон guard" minHeight={354} />
...
<ExternalCodeEmbed example="json/js-501-3-ecosystem-2-frontend-frameworks-3-angular-291-003" title="2.2. Архитектор (`architect`)" minHeight={318} />
...
**Справочники** — [Справочник по Vue.js](/encyclopedia/5-languages/5-01-javascript/3-ecosystem/2-frontend-frameworks/2-vue/281), [Справочник по TypeScript](/encyclopedia/5-languages/5-01-javascript/301), [Справочник по React](/encyclopedia/5-languages/5-01-javascript/3-ecosystem/2-frontend-frameworks/1-react/271), [Справочник по Ext JS](/encyclopedia/5-languages/5-01-javascript/3-ecosystem/2-frontend-frameworks/4-ext-js/311), [Справочник по Node](/encyclopedia/5-languages/5-01-javascript/3-ecosystem/1-runtime-node/261), [Справочник по Django](/encyclopedia/5-languages/5-02-python/301).
