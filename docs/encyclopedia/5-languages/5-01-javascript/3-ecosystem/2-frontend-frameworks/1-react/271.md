...
description: React — это декларативная JavaScript-библиотека для построения пользовательских интерфейсов.
slug: /encyclopedia/5-languages/5-01-javascript/271
...
Справочник-шпаргалка по **React** — компоненты, хуки, JSX, экосистема. Не заменяет пошаговое обучение — курс: [раздел React](./intro.md). Таблицы синтаксиса и API — для быстрого поиска фактов.
...
- [Общие принципы React](#общие-принципы-react)
- [Основные понятия](#основные-понятия)
- [Хуки React](#хуки-react)
- [Встроенные компоненты и API](#встроенные-компоненты-и-api)
- [Атрибуты и пропсы стандартных HTML-элементов в JSX](#атрибуты-и-пропсы-стандартных-html-элементов-в-jsx)
- [Специальные пропсы](#специальные-пропсы)
- [Настройки и параметры сборки](#настройки-и-параметры-сборки)
- [Context API](#context-api)
- [Управление формами](#управление-формами)
- [Обработка событий](#обработка-событий)
- [Жизненный цикл компонентов](#жизненный-цикл-компонентов)
- [Производительность](#производительность)
- [Обработка ошибок](#обработка-ошибок)
- [Роутинг (на основе React Router v6)](#роутинг-на-основе-react-router-v6)
- [Concurrent Mode и современный рендеринг](#concurrent-mode-и-современный-рендеринг)
- [Suspense и ленивая загрузка](#suspense-и-ленивая-загрузка)
- [Server Components (экспериментальная модель)](#server-components-экспериментальная-модель)
- [Интеграция с внешними библиотеками управления состоянием](#интеграция-с-внешними-библиотеками-управления-состоянием)
- [Пользовательские хуки — шаблоны и примеры](#пользовательские-хуки--шаблоны-и-примеры)
- [Обмен данными между компонентами](#обмен-данными-между-компонентами)
- [Рендеринг — условия и списки](#рендеринг--условия-и-списки)
- [Стилизация — обзор подходов](#стилизация--обзор-подходов)
- [HOC — обзор](#hoc--обзор)
- [UI-библиотеки — обзор](#ui-библиотеки--обзор)
- [Типичные ошибки](#типичные-ошибки)
- [Идеи для практики](#идеи-для-практики)
...
| Переиспользование | пользовательские хуки |

Разбор с примерами подписок, мемоизации и `useHover` — [React — хуки, состояние и формы](./276.md).
...
`Profiler` активен только в Разработка-сборке.  
...
`StrictMode` запускает некоторые хуки дважды в Разработка для выявления побочных эффектов.  
...
<ExternalCodeEmbed example="javascript/js-501-3-ecosystem-2-frontend-frameworks-1-react-271-001" title="`ErrorBoundary`" minHeight={390} />
...
<ExternalCodeEmbed example="javascript/js-501-3-ecosystem-2-frontend-frameworks-1-react-271-002" title="`useTransition`" minHeight={318} />
...
<ExternalCodeEmbed example="javascript/js-501-3-ecosystem-2-frontend-frameworks-1-react-271-003" title="`useLocalStorage`" minHeight={372} />
...
Подписка на `resize` окна; удобно для адаптивной вёрстки без media-query в JS.
...
Синхронизация темы с `localStorage` и классом на `document.documentElement`.
...
| Паттерн | Суть | Когда |
|---------|------|--------|
| **Props** | Родитель передаёт данные и колбэки вниз | Близкие уровни дерева |
| **Подъём state (lifting state up)** | Общее состояние поднимают к ближайшему общему предку | Два соседа должны делить одни данные |
| **Props drilling** | Props проходят через промежуточные компоненты без использования | Признак того, что пора Context или store |
| **Context API** | `createContext` + `Provider` + `useContext` | Тема, язык, авторизация, редкие глобальные данные |
| **Обработчики событий** | `onClick`, `onSubmit` — колбэки из родителя | Действия пользователя меняют state наверху |

Учебный разбор — [React - библиотека для пользовательских интерфейсов](./27.md#карта-тем-react), пошаговый старт — [Первая программа на React](./272.md). **Готовые блоки UI** (счётчик, todo, форма, modal, Context, Router) с построчным разбором — [React — компоненты-рецепты](/lab/Примеры/1146).

---

### Рендеринг — условия и списки

```jsx
{isLoading && <p>Загрузка…</p>}
{items.length ? <List data={items} /> : <Empty />}
{items.map((item) => <Card key={item.id} item={item} />)}
```

- Условие через `&&`, тернарный оператор или ранний `return` в компоненте.
- Списки — только через `.map()` с уникальным **`key`** (ид из БД, slug).
- Индекс массива как `key` допустим для статичного списка без перестановок.

---

### Стилизация — обзор подходов

| Подход | Плюсы | Минусы |
|--------|-------|--------|
| **Обычный CSS / SCSS** | Привычно, глобальные темы | Имена классов, область видимости |
| **CSS Modules** (`*.module.css`) | Локальные классы, без коллизий | Отдельный файл на компонент |
| **Inline-стили** | Быстрые прототипы | Нет псевдоклассов, тяжело масштабировать |
| **CSS-in-JS** (styled-components, Emotion) | Стили рядом с компонентом, темы из props | Runtime или сборка |
| **Tailwind CSS** | Утилитарные классы, малый бандл после purge | Свой словарь классов |

Готовые HTML-блоки на Tailwind с разбором классов (до React и сборки) — [Tailwind — готовые блоки](/lab/Примеры/1117).

В [Next.js](../3-meta-frameworks/273.md) часто комбинируют CSS Modules и Tailwind. Подробный контекст — [экосистема React](./27.md#экосистема-react).

---

### HOC — обзор

**Higher-Order Component (HOC)** — функция, которая принимает компонент и возвращает новый с дополнительным поведением.
...
const Dashboard = withAuth(DashboardView);
```

Типичные задачи — проверка прав, логирование, подключение данных. В новых проектах ту же роль часто выполняют **пользовательские хуки** (`useAuth`) и композиция компонентов.

---

### UI-библиотеки — обзор

| Библиотека | Стиль | Заметка |
|------------|-------|---------|
| **Material UI (MUI)** | Material Design, много готовых виджетов | Популярен в корпоративных SPA |
| **Chakra UI** | Доступность, темизация из коробки | Удобные layout-примитивы |
| **Ant Design** | Плотные админ-панели, таблицы | Часто в B2B и внутренних порталах |
| **Tailwind CSS** | Utility-классы | Часто с [shadcn/ui](https://ui.shadcn.com/) — копируемые компоненты на Radix; учебные примеры — [Tailwind — готовые блоки](/lab/Примеры/1117) |
| **shadcn/ui** | Набор компонентов под Tailwind | Код в репозитории, не "чёрный ящик" npm |

React остаётся слоем логики; библиотека даёт кнопки, модалки, таблицы. Стили можно смешивать с CSS Modules в одном проекте.
...
|--------|-------------|-------------|
| `key=&#123;index&#125;` при сортировке/удалении | Сбой фокуса, "прыгающие" поля | `key=&#123;item.id&#125;` |
| `setState(obj.field = x)` / `arr.push()` | React не видит изменения | Иммутабельное обновление, новая ссылка |
| `useEffect` без `deps` или с нестабильным объектом в `deps` | Бесконечные запросы/рендеры | Явный массив зависимостей |
| `useEffect` без cleanup для таймера/subscribe | Утечки после ухода со страницы | `return () => clear…` |
| Прямой `document.querySelector` для UI | Обход декларативной модели | State + controlled components |
| `dangerouslySetInnerHTML` без санитизации | Риск XSS | Санитизация, доверенный контент |

Разбор для новичка — [Первая программа на React](./272.md#частые-ошибки) и [галерея типовых ошибок](/lab/Примеры/1146#частые-ошибки).

---

### Идеи для практики

**Начальный уровень** — Todo, калькулятор, блокнот, форма регистрации, поиск фильмов по API.

**Средний уровень** — счётчик на хуках с историей, переключатель темы, фильтруемый список, пагинация REST, navbar с `react-router-dom`.

Связанное задание с API — [Первая программа на React — "Заметки"](./272.md#задание-заметки). Готовые рецепты под лабораторную — [React — компоненты-рецепты](/lab/Примеры/1146) (todo, калькулятор, форма входа, список с `fetch`).
...
**Справочники** — [Справочник по Node](/encyclopedia/5-languages/5-01-javascript/3-ecosystem/1-runtime-node/261), [Справочник по Vue.js](/encyclopedia/5-languages/5-01-javascript/3-ecosystem/2-frontend-frameworks/2-vue/281), [Справочник по JavaScript](/encyclopedia/5-languages/5-01-javascript/251), [Справочник по Angular](/encyclopedia/5-languages/5-01-javascript/3-ecosystem/2-frontend-frameworks/3-angular/291), [Справочник-шпаргалка по Git](/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/115), [Справочник по TypeScript](/encyclopedia/5-languages/5-01-javascript/301).
