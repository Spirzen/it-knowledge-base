...
## Плавный старт — выбор хранилища по задаче

У большинства приложений есть одновременно несколько типов данных: тема интерфейса, токен сессии, черновик формы, офлайн-кэш. Один универсальный storage для всего почти всегда создаёт проблемы.

Правильный маршрут — распределить данные по назначению: что-то живёт только во вкладке, что-то отправляется в каждый HTTP-запрос, а что-то хранится как локальная база.
...
|--------|----------------|
| До | всё кладётся в `localStorage`, секреты и кэш смешиваются |
| После | сессия в `HttpOnly` cookie, настройки в `localStorage`, большие данные в IndexedDB |
