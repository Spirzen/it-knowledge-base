...
- **`Данные`** — статические данные, передаваемые в маршрут.
...
- `Данные` — Observable статических данных из конфигурации.
...
this.http.get('/api/Данные', { headers });
```
...
#### 18.2. Безопасность (Безопасность)
