...
## Плавный старт — зачем это нужно на практике

Когда проект растёт, один длинный `app.js` быстро становится неудобным. ES-модули позволяют разделить код на понятные части: сеть, UI, форматирование, маршруты.

Синтаксис остаётся стандартным и в браузере, и в большинстве современных сборок. Поэтому знания из этой статьи применяются и в "голом" JS, и в проектах на Vite/Webpack.

---

## Мини-кейс "до/после"

| Подход | Что получает команда |
|--------|----------------------|
| До | один общий файл, трудно находить и переиспользовать код |
| После | небольшие модули с явными `import/export`, проще тестировать и поддерживать |
