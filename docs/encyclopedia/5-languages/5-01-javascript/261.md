...
Справочник-шпаргалка по **Node**: типы, синтаксис, стандартная библиотека, типовые паттерны. Не заменяет пошаговое обучение. Учебный курс: [раздел](/encyclopedia/5-languages/5-01-javascript/intro).

---

## Краткое пояснение

Шпаргалка по **Node**: таблицы синтаксиса, API, команд и параметров — для быстрого поиска фактов.
...
- [5. `package.json` — полное руководство](#5-packagejson-полное-руководство)
...
3. Модули и `npm`
4. Работа с файловой системой (`fs`)
   - введение в `fs`
   - синхронные операции
