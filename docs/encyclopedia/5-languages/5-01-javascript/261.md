...
Примеры:
```js
const hash = crypto.createHash('sha256').update('Данные').digest('hex');
...
- `fs.writeFile(file, Данные[, options], callback)`
- `fs.appendFile(file, Данные[, options], callback)`
...
const Данные = await fs.readFile('file.txt', 'utf8');
```
...
#### Callback-стиль:
```js
fs.readFile('file.txt', (err, Данные) => {
  if (err) {
    console.error('Ошибка чтения:', err.message);
    return;
  }
  console.log(Данные.toString());
});
```
...
```js
try {
  const Данные = await fs.promises.readFile('file.txt', 'utf8');
  console.log(Данные);
} catch (err) {
...
- Python предпочтителен для CPU-интенсивных задач (ML, Данные Наука)
