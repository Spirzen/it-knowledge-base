...
>   writable?: boolean;     // можно ли перезаписать значение (только для Данные-свойств)
>   value?: any;            // значение (Данные-свойство)
...
>   }
>   increment() {
>     const Данные = privateData.get(this);
>     Данные.count++;
>   }
> }
> ```
...
>     if (Данные.items.length === 0) return;
>     yield Данные.items;
>     page++;
>   }
> }
> 
> (async () => {
>   for await (const items of fetchPages('/api/Данные')) {
>     console.log(items);
>   }
> })();
> ```
...
| `createTextNode(Данные)` | строка | `Text` | Текстовый узел |
...
| `dataset` | Объект для `Данные-*` атрибутов (`Данные-user-id` → `dataset.userId`) |
...
fetch(url, {
  ...
  body: JSON.stringify(Данные), // для POST/PUT
...
> // В Worker:
> self.onmessage = e => {
>   const ia = new Int32Array(e.Данные.sab);
