...
slug: /encyclopedia/5-languages/5-01-javascript/251
...
**Учебные статьи:** [Типы данных](./18.md) · [Объекты и прототипы](./22.md).
...
| `Object.groupBy(items, callbackFn)` | итерируемое, `(element, index) => key` | `Record<string, T[]>` | Группирует элементы по строковому (или symbol) ключу | ES2024; не мутирует `items`; см. [типы данных](./18.md#группировка-по-ключу) |
...
> 📌 **Иммутабельные пары** к мутирующим методам выше — сводная таблица в [типах данных JS](./18.md#иммутабельные-альтернативы-мутирующим-методам): `push` → `[...arr, x]`, `sort` → `toSorted`, `splice` → `toSpliced` и т.д.
...
> 📌 Группировка массива по полю — не метод `Array`, а **`Object.groupBy(iterable, fn)`** и **`Map.groupBy(iterable, fn)`** (ES2024). См. [группировка по ключу](./18.md#группировка-по-ключу).
...
> 📌 Учебные примеры и сводная таблица частых методов — в [типах данных JS](./18.md#методы-строк).
...
| Метод | Аргументы | Возвращает | Описание |
|-------|-----------|------------|----------|
| `Map.groupBy(items, callbackFn)` | итерируемое, `(element, index) => key` | `Map` с массивами по группам | Как `Object.groupBy`, но ключи любого типа (ES2024) |
...
<ExternalCodeEmbed example="javascript/js-501-251-001" title="Итерабельность и протоколы" minHeight={318} />
...
<ExternalCodeEmbed example="javascript/js-501-251-002" title="Генераторы (`function*`)" minHeight={354} />
...
> ✅ Пример `withResolvers` (deferred, ожидание внешнего события):
> ```js
> const { promise, resolve, reject } = Promise.withResolvers();
> channel.onmessage = () => resolve(payload);
> channel.onerror = () => reject(new Error("channel failed"));
> const data = await promise;
> ```
> Подробнее — [21.md § Управляемый Promise](./21#управляемый-promise-promisewithresolvers).
...
<ExternalCodeEmbed example="javascript/js-501-251-003" title="Асинхронность — `Promise`, `async`/`await`, очередь задач" minHeight={318} />
...
<ExternalCodeEmbed example="javascript/js-501-251-004" title="Web APIs (вне DOM, но в браузере)" minHeight={336} />



> ⚠️ `fetch` **не** отклоняется при HTTP-ошибках (4xx/5xx) — только при сетевых сбоях. Галерея GET, POST, query и axios с построчным разбором — [Fetch / axios — типовые запросы](/lab/Примеры/1145).
...
##### Reflect - API метапрограммирования
...
<ExternalCodeEmbed example="javascript/js-501-251-005" title="Системные типы и API (низкоуровневые)" minHeight={354} />
...
**Справочники** — [Справочник-шпаргалка по Git](/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/115), [Справочник по Node](/encyclopedia/5-languages/5-01-javascript/3-ecosystem/1-runtime-node/261), [Справочник по CSS](/encyclopedia/3-data-markup/3-10-css/71), [Справочник по React](/encyclopedia/5-languages/5-01-javascript/3-ecosystem/2-frontend-frameworks/1-react/271), [Справочник по HTML](/encyclopedia/3-data-markup/3-09-html/21), [Справочник по Vue.js](/encyclopedia/5-languages/5-01-javascript/3-ecosystem/2-frontend-frameworks/2-vue/281).
