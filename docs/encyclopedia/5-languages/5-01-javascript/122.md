...
description: Встроенные функции JavaScript - ключевые методы массивов, строк и объектов для повседневной разработки.
---
...
Встроенные функции в JavaScript — это функции, которые являются неотъемлемой частью самого языка и предоставляются движком JavaScript автоматически без необходимости их объявления или импорта, образуя фундаментальный набор инструментов для выполнения базовых операций с данными, управления объектами, работы со строками, числами, массивами и другими типами, а также для взаимодействия с окружением выполнения. Эти функции находятся в глобальной области видимости или доступны как методы встроенных объектов, таких как Object, Array, String, Number, Math, Date, JSON и других, и они составляют стандартную библиотеку языка, которая доступна в любой среде выполнения, будь то браузер, Node.js или любая другая реализация JavaScript. К встроенным функциям относятся как функции-конструкторы, используемые для создания объектов с ключевым словом new, например, Object, Array, Date, Error и Promise, так и функции, которые вызываются напрямую, такие как parseInt, parseFloat, isNaN, isFinite, encodeURI, decodeURI, и другие, которые выполняют полезные операции без необходимости создания экземпляров объектов. Встроенные функции также включают методы прототипов, которые доступны на экземплярах соответствующих объектов, например, методы массивов, такие как map, filter и reduce, методы строк, такие как split, replace и toUpperCase, методы чисел, такие как toFixed и toPrecision, а также методы объектов, такие как hasOwnProperty и toString, которые являются неотъемлемой частью повседневной разработки. Значительная часть встроенных функций реализована на уровне движка на высокопроизводительных языках, таких как C или C++, что обеспечивает им максимальную скорость выполнения по сравнению с пользовательскими функциями, написанными на JavaScript, поэтому их использование всегда предпочтительнее, чем написание собственных реализаций для стандартных задач. Встроенные функции являются основой, на которой строится любой JavaScript-код, и без них невозможно написать ни одной программы, поэтому глубокое понимание их возможностей, поведения и особенностей является обязательным условием для эффективной разработки на этом языке, позволяя писать более краткий, надежный и производительный код.
...
Глобальные функции — это особый подвид встроенных функций в JavaScript, которые доступны из любого места программы без необходимости обращения к какому-либо объекту или модулю, поскольку они являются прямыми свойствами глобального объекта, которым в браузерах является window, а в Node.js — global, и которые предоставляют набор универсальных утилит для выполнения наиболее общих и часто встречающихся задач, не требующих специализированного контекста. Классическими примерами глобальных функций являются функции преобразования типов, такие как parseInt для преобразования строки в целое число с указанием системы счисления, parseFloat для преобразования строки в число с плавающей точкой, isNaN для проверки, является ли значение нечисловым, и isFinite для проверки, является ли число конечным, а также функции кодирования и декодирования URL, такие как encodeURI, decodeURI, encodeURIComponent и decodeURIComponent, которые используются для безопасной передачи данных в HTTP-запросах. Глобальные функции также включают функции управления временем, такие как setTimeout и setInterval для отложенного выполнения кода и периодических операций, а также clearTimeout и clearInterval для отмены этих операций, которые, хотя и являются методами глобального объекта, по своей сути являются полноценными функциями и доступны из любого контекста выполнения. В современной практике разработки на JavaScript набор глобальных функций значительно расширился с введением новых возможностей языка, таких как функции atob и btoa для кодирования и декодирования данных в формат Base64, которые изначально были специфичны для браузеров, но затем были добавлены в Node.js, а также глобальные функции, связанные с промисами и асинхронным программированием, такие как Promise.resolve, Promise.reject и Promise.all, которые стали доступны глобально благодаря стандартизации промисов. Особенностью глобальных функций является то, что они могут быть переопределены пользовательским кодом, поскольку они являются обычными свойствами глобального объекта, и такая возможность иногда используется для полифиллов, когда разработчики добавляют реализации функций для старых окружений, где они отсутствуют, но в целом переопределение глобальных функций считается плохой практикой и может привести к трудноуловимым ошибкам и несовместимости. Глобальные функции образуют минимальный набор инструментов, который гарантированно доступен в любом JavaScript-окружении, и они часто являются первыми функциями, с которыми знакомится начинающий разработчик, поскольку они решают самые базовые задачи, такие как чтение пользовательского ввода, преобразование данных, работа со временем и взаимодействие с окружением.
...
Разбор с примерами, шпаргалкой по группам (поиск, срезы, замена, регистр, дополнение) и колонкой "Результат" на `"Hello"` — в [типах данных JavaScript — методы строк](./18.md#методы-строк). Ниже — компактный справочник всех имён.
...
| Функция | Значение | Пример |
|---------|----------|--------|
| `import()` | Динамический импорт модуля (основа code splitting в сборщиках) | `import("./module.js")` |
...
Подробнее про разбиение бандла на чанки и метрики **LCP** / **INP** — [оптимизация загрузки на клиенте](/encyclopedia/2-system-network/2-04-kak-rabotayut-sayty-i-veb-sayty/111#frontend-performance).
