...
Пример:
```js
const { Данные, isLoading, error } = useQuery({
  queryKey: ['todos'],
  queryFn: fetchTodos,
});
```
...
#### React Тестирование Library
React Тестирование Library — это инструмент для тестирования компонентов в условиях, приближенных к реальному использованию.  
Основные функции:
