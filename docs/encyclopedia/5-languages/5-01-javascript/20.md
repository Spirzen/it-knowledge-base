...
description: Так, это похоже на функцию, где есть три выражения — начало задаёт начальное состояние цикла. Обычно здесь объявляется и инициализируется счётчик, который выполняется один раз перед началом. В начале…
---
