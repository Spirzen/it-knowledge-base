...
|-----|---------|---------|
|getAttribute()	|`div.getAttribute('Данные-id')`	|Получить атрибут
|setAttribute()	|`div.setAttribute('Данные-test', '123')`	|Установить атрибут
...
В Node.js:
	...
	- File Система (fs): чтение/запись файлов – readFile, writeFile, promises;
...
// С табуляцией
console.log(JSON.stringify(Данные, null, "\t"));
```
...
.then(Данные => console.log("Данные получены"))
.catch(error => console.error("Ошибка:", error));
