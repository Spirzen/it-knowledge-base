...
Сервер слушает порт 3000 и обрабатывает каждый входящий запрос через колбэк-функцию. Метод `res.writeHead` устанавливает код ответа и заголовки. Клиент использует метод `http.get` для отправки запроса. data поступают частями, поэтому они накапливаются в переменной `data` внутри события `data`. После завершения передачи (`event end`) данные парсятся как JSON.
