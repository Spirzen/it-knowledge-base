...
import TavernChatPlay from '@site/src/components/TavernChatPlay';
...
Эти проекты можно давать как "закрепление после главы". Они развлекательные, но тренируют те же темы, что и прикладные скрипты — **прямо в статье**, без установки и запуска файлов.

<LangPracticeToysHub theme="javascript" toys={['quest', 'memelogs', 'lootbox']} />

### Жанры видеоигр — те же навыки, игровая тема

<GameGenreToysStack theme="javascript" genres={['arcade', 'puzzle', 'rpg']} />

### 1) Консольный квест "Побег из дата-центра"

- **Что тренирует:** `if/else`, циклы, функции, работу с состоянием.
- **Идея:** игрок проходит комнаты, выбирает действия, собирает ключи, избегает "перегрева сервера".
- **Расширение:** добавьте систему предметов и несколько концовок.

### 2) Генератор "мемных логов"

- **Что тренирует:** строки, шаблонные литералы, массивы, `Math.random`.
- **Идея:** скрипт генерирует случайные dev-логи в стиле "03:17 ERROR: прод упал от лишней запятой".
- **Расширение:** подключите JSON-файл с шаблонами и уровнями "критичности".

### 3) Лутбокс для инвентаря

- **Что тренирует:** объекты, массивы, вероятности, сериализацию в JSON.
- **Идея:** игрок открывает лутбоксы, получает предметы разной редкости, сохраняет инвентарь в файл.
- **Расширение:** добавьте "гарантированный эпик" после N открытий.

### 4) Мини-чат таверны (Node.js)

<TavernChatPlay theme="javascript" />

- **Что тренирует:** события, асинхронные ответы, маршрутизация по комнатам.
- **Идея:** клиент-серверный чат с командами — без установки Node, прямо в статье.
