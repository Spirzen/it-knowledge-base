...
Перед скриптами соберите страницу, с которой будете работать: [HTML-страницы целиком](/lab/Примеры/1153) (каркас, форма, семантика). Дальше — [JavaScript DOM — 30 приёмов](/lab/Примеры/1144). Стили «только на CSS» из каталога [Uiverse / Galaxy](/lab/Примеры/1155) ниже дополняются скриптом, когда нужно состояние между сессиями или сложное поведение.

---

## Когда достаточно CSS, когда нужен JavaScript

Многие элементы интерфейса в учебных сниппетах (архив [Galaxy](https://github.com/uiverse-io/galaxy)) работают **без JavaScript**. Скрипт подключают, когда одних псевдоклассов и `:has()` недостаточно.

| Задача | Часто хватает CSS | Обычно нужен JS |
|--------|-------------------|-----------------|
| Hover-tooltip с `data-tooltip` | ✅ | Панель на touch, позиционирование у края экрана |
| Switch / checkbox / radio (вид) | ✅ `appearance: none`, `:checked` | Отправка формы, валидация, связь с API |
| Раскрывающийся поиск | ✅ `:focus`, `:valid` | Подсказки, debounce, запрос к серверу |
| Тёмная тема на странице | ✅ `:has()` + checkbox | Запомнить выбор в `localStorage` |
| Skeleton / spinner | ✅ `@keyframes`, `aria-busy` | Подмена на реальный контент после `fetch` |
| Toast / alert | ✅ статичный блок | Очередь, таймер закрытия, focus |
| Modal | Частично `:target` | Focus trap, `Escape`, portal, scroll lock |
| Таблица с сортировкой | — | ✅ клик по заголовку, перестройка DOM |
| Вкладки (tabs) | ✅ `:target` или radio | Сложная логика, lazy load панелей |

**Прогрессивное улучшение:** сначала семантичный HTML ([формы](/encyclopedia/3-data-markup/3-09-html/2)), затем CSS ([113.md](/encyclopedia/3-data-markup/3-10-css/113)), затем JS для поведения и данных. Типизация UI в React — [TypeScript и React](/encyclopedia/5-languages/5-10-typescript/21#типизация-ui-компонентов).
