...
Сравнение: [Vue](./282.md) · [Angular](./292.md) · [Next.js](./2731.md). Склейка с API: [264](./264.md). Обзор: [27.md](./27.md). **Галерея готовых компонентов с разбором** — [React — компоненты-рецепты](/lab/Примеры/1146). Аналог для Vue и Svelte — [галерея компонентов](/lab/Примеры/1147).
...
**`key`** — стабильный идентификатор элемента (`id` из API, не индекс массива при удалении и сортировке). React по `key` сопоставляет старые и новые узлы и не «ломает» фокус и внутреннее состояние строки.
...
Для SEO и серверного HTML чаще берут [Next.js](./2731.md) — там маршруты из структуры папок `app/`.
...
| Список «ломается» при удалении | Нет `key` или `key=&#123;index&#125;` при перестановке | Стабильный `key` из данных (`id`) |
...
| `fetch` failed / CORS | API выключен или нет CORS | [264](./264.md), прокси в Vite |
| State «не обновился» | Мутация массива/объекта | Новая ссылка: `setItems([...items, newItem])` |
...
Главное задание раздела — **«Заметки»** ниже: оно закрывает форму, CRUD и работу с API.
