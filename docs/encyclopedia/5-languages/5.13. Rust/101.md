...
// Внешний код использует: my_crate::Сеть::TcpConnection
```
...
fn process_data(Данные: Vec<i32>) { /* ... */ }

// Хорошо: обобщение с трейтом
fn process_data<T: Display + Clone>(Данные: Vec<T>) { /* ... */ }
```
...
pub enum ProcessingError {
    ...
    #[error("Сеть timeout after {0} seconds")]
    Timeout(u64),
}
```
...
fn process_file(path: &Path) -> Result<(), ProcessingError> {
    ...
    let Данные = parse_content(&content)?;
    validate_data(&Данные)?;
    save_processed_data(&Данные)?;
    Ok(())
}
```
...
fn benchmark_processing(c: &mut Criterion) {
    let Данные = generate_test_data();
    c.bench_function("process_large_dataset", |b| {
        b.iter(|| process_data(&Данные))
    });
}
