...
description: Функции и продвинутые параметры в PowerShell - переиспользование кода, валидация аргументов и удобство CLI.
slug: /encyclopedia/5-languages/5-26-powershell/118
---
...
> **Интерактивное демо** — вызов функции и стек на примере **JavaScript**. В PowerShell объявление другое, но вызов, параметры и возврат устроены так же. Обобщённо: [функции в коде](/encyclopedia/4-code-dev/4-02-chto-takoe-kod-i-kak-on-rabotaet/4).
...
```powershell
Get-MultipleUsers -UserNames "Alice", "Bob", "Charlie"
```
...
```powershell
Set-Config -Settings @{Timeout=30; Retry=3}
```
...
```powershell
Invoke-WithTransform -Transformation { param($inputData) Write-Host "Преобразовано: $inputData" }
```
...
Проверка входных данных перед обработкой предотвращает ошибки и обеспечивает стабильную работу функции. Использование валидационных атрибутов и ручных проверок повышает надежность. См. [Проверка и валидация](/encyclopedia/4-code-dev/4-14-razrabotka-i-otladka/118).
...
## Стандартные блоки и модули

В [125](/encyclopedia/5-languages/5-26-powershell/125) разобран поэтапный путь: функция с `[CmdletBinding()]` и `[OutputType]` → скрипт-оркестратор → модуль `.psm1`. Здесь — краткое резюме для функций из этой статьи.

| Приём | Зачем в автоматизации |
|-------|------------------------|
| `[CmdletBinding()]` | `-Verbose` в планировщике, `-ErrorAction Stop` в блоках |
| `[OutputType([string])]` | Понятно, что вернёт функция в pipeline |
| `ValidateRange`, `ValidateSet` | Защита от «−30 дней» и опечаток в параметрах |
| `throw` при невосстановимой ошибке | Планировщик увидит `exit 1` у вызывающего скрипта |
| `Export-ModuleMember` | Явный публичный API модуля |

Функцию, которую планируете вызывать из двух и более `.ps1`, оформляйте сразу как блок по правилам [125](/encyclopedia/5-languages/5-26-powershell/125), а не копируйте тело между файлами.

---

## Практика проектирования функций

Чтобы функции были удобны в реальной автоматизации, проверяйте три вещи:
- "контракт": параметры и валидация (`ValidateSet`, `ValidatePattern`);
- "безопасность": `SupportsShouldProcess` для изменений состояния;
- "наблюдаемость": `Write-Verbose` и понятные ошибки через `throw` или `Write-Error`.

Так функции легко переиспользовать в скриптах, модулях и pipeline-задачах.

См. также:
- [125 — стандартные блоки и модули](/encyclopedia/5-languages/5-26-powershell/125)
- [126 — триггеры](/encyclopedia/5-languages/5-26-powershell/126)
