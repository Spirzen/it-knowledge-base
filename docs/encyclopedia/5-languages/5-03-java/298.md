...
<ExternalCodeEmbed example="java/java-503-298-001" title="CompletableFuture" minHeight={336} />
...
- Для базового понимания сначала изучите [Virtual Threads](./308.md), затем вернитесь к этому блоку.
...
<span id="praktikum-parallelizm-v-java"></span>

## Практикум — параллелизм в Java

Демо **ParallelismDemo** проходит по цепочке тем из раздела [Практикум](/encyclopedia/4-code-dev/4-05-asinhronnost/3): потоки, [гонка данных](/encyclopedia/4-code-dev/4-05-asinhronnost/11), синхронизация, `Runnable`, пул.

<ExternalCodeEmbed example="java/code-405-3-002" title="Java — ParallelismDemo" minHeight={480} />
...
Исправление — `synchronized`, `AtomicInteger` или потокобезопасные структуры — см. [JVM и потоки](./23.md#гонка-данных-race-condition).

### Пул потоков ExecutorService

`new Thread` на каждую задачу дорог по памяти. [`ExecutorService`](https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/util/concurrent/ExecutorService.html) держит фиксированное число рабочих потоков:
...
Сравнение с Python, C#, Go — [практикум раздела "Асинхронность"](/encyclopedia/4-code-dev/4-05-asinhronnost/3).
