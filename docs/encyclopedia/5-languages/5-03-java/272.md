...
description: "После первого Spring Boot API возникает вопрос: кто может вызывать эндпоинты? Публичный URL без защиты подходит для учебы; в реальном API нужны логин, роли, токены."
  публичные эндпоинты без устаревшего WebSecurityConfigurerAdapter.
---
...
### Зачем не проверять пароль в каждом методе контроллера
