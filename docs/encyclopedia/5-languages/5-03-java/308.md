...
<div class="callout callout--warning">
  ...
  Долгий блокирующий код внутри `synchronized` на virtual thread может **закрепить** поток на carrier (*pinning*) и снизить выигрыш.

  Для нового кода предпочтительны `ReentrantLock` и короткие критические секции.

  Подробности — в JEP и release notes Java 21.
  </div>
</div>
