...
2. **Run Configuration в IDEA** с VM options на `$PROJECT_DIR$/target/javafx-lib` после `mvn compile` и копирования `*-win.jar` (см. [3111.md](./3111.md#запуск-maven-idea-и-module-path)).
...
Так отрабатываются **компоновка** (контейнеры вместо координат), **события** (`setOnAction`, привязки свойств) и **стили** (inline CSS и классы на сцене). Каркас с `Application.launch(HelloApp.class, args)` в отдельном `Main` описан в [3111.md](./3111.md#две-точки-входа-main-и-класс-application).
...
Таблица «ожидание → решение» для новичка — в [3111.md](./3111.md#позиционирование-и-компоновка--что-путает-новичка).
...
`DialogPane` у `Alert` — обычный узел: можно задать тёмный фон, красную шапку и hover на кнопке OK. Иконка типа alert меняется ограниченно; для произвольной вёрстки и анимаций используют `Dialog<T>` с кастомным `content`. Пример с красной кнопкой и стилизованным диалогом — [3111.md](./3111.md#стилизация--окно-кнопка-и-alert).
...
Пошаговый разбор «Maven vs module-path» — [3111.md](./3111.md#частые-ошибки).
