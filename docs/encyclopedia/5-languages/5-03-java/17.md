...
Аналог `for…of` в JavaScript — цикл **for-each** (`for (int value : values)`). Обобщённо — [циклы в коде](/encyclopedia/4-code-dev/4-02-chto-takoe-kod-i-kak-on-rabotaet/5), по JavaScript для сравнения синтаксиса — [циклы в JavaScript](/encyclopedia/5-languages/5-01-javascript/20).
...
<div class="callout callout--tip">
  ...
  Сначала пишите вариант, который проще всего читать через месяц. Для большинства задач это `if-else` и обычный `for`, а не "умные" сокращения.
</div>
  </div>
