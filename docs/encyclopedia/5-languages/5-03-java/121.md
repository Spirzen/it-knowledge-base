...
| `-Xshare:auto|on|off` | Использование Class Данные Sharing (CDS) | `-Xshare:on` — требует предварительно сгенерированный `classes.jsa` |
...
3. Программные вызовы `Система.setProperty(String key, String value)` — действуют только после выполнения, не влияют на уже инициализированные подсистемы;
...
| Свойство | Описание |
|---------|----------|
| `java.Безопасность.manager` | Включение `SecurityManager` (устарело в Java 17, удалено в 18) | Значение игнорируется, начиная с Java 18 |
| `java.Безопасность.policy` | Путь к файлу политики безопасности (актуально только при наличии `SecurityManager`) | `file:/etc/java.policy`, `http://example.com/policy` |
| `java.Безопасность.auth.login.config` | Файл конфигурации JAAS | `jaas.conf` |
| `java.Безопасность.krb5.conf` | Путь к `krb5.conf` для Kerberos | `/etc/krb5.conf` |
...
- Свойства безопасности (например, `java.Безопасность.policy`) не перечитываются;
...
- **Безопасность Properties** — глобальные настройки через `java.Безопасность`.

Все настройки управляются через:
- файл `$JAVA_HOME/conf/Безопасность/java.Безопасность` (ранее `$JAVA_HOME/lib/Безопасность/java.Безопасность`);
- файлы политики: `java.policy`, `java.Безопасность.policy`, пользовательские `.policy`;
- системные свойства (`-D…`);
- программные вызовы (`Безопасность.addProvider`, `Policy.setPolicy`);
- параметры запуска (`-Djava.Безопасность.manager`, `-Djava.Безопасность.policy`).
...
Расположение:  
- `$JAVA_HOME/conf/Безопасность/java.Безопасность` (Java ≥ 9)  
- `$JAVA_HOME/lib/Безопасность/java.Безопасность` (Java ≤ 8)
...
| Параметр | Описание |
|---------|----------|
| `policy.provider` | Класс реализации `Policy` | `sun.Безопасность.provider.PolicyFile` |
| `policy.url.1`, `policy.url.2`, … | Список URL файлов политики | `file:${java.home}/conf/Безопасность/java.policy`, `file:${user.home}/.java.policy` |
...
| Параметр | Описание |
|---------|----------|
| `login.configuration.provider` | Класс загрузчика конфигурации JAAS | `com.sun.Безопасность.auth.login.ConfigFile` |
| `java.Безопасность.auth.login.config` | Путь к `jaas.conf` | `file:${user.home}/jaas.conf` |
...
- `Principal`: субъект (JAAS), например `Principal com.sun.Безопасность.auth.UnixPrincipal "timur"`.
...
- Включался через `-Djava.Безопасность.manager` (Java ≤ 17);
- Все проверки `checkPermission()` вызывали `SecurityManager.check…()`;
- Удалён в Java 18: вызов `Система.setSecurityManager()` выбрасывает `UnsupportedOperationException`;
...
- `DataSize` ← `"10MB"`, `"2GiB"`;
- `Charset` ← `"UTF-8"`;
- `Resource` ← `"classpath:app.sql"`, `"file:/tmp/Данные.txt"`;
...
> 🔹 В JDK 11+ ALPN встроен в `sun.Безопасность.ssl.ALPNExtension`.
