---
tags: [engineer, developer, architect, required, beginner]
title: JVM в проде — jcmd, дамп памяти и JFR
description: >-
  JVM в проде: jcmd, heap dump и JFR — практика при тормозах и
  OutOfMemoryError без перегруза теорией GC.
---

import ExternalCodeEmbed from '@site/src/components/ExternalCodeEmbed';


# JVM в проде — jcmd, дамп памяти и JFR

<div class="article-tags">
  <span class="tag tag-required">ОБЯЗАТЕЛЬНО</span>
  <span class="tag tag-beginner">ДЛЯ НОВИЧКОВ</span>
</div>

<span class="complexity-badge">Разработчику</span>
<span class="complexity-badge">Архитектору</span>

---

## JVM в проде — jcmd, дамп памяти и JFR

Теория GC и потоков — в [JVM, память и потоки](./23.md). Здесь — **практика** — какие команды ввести, когда приложение тормозит, "ест" память или упало с `OutOfMemoryError`.

Инструменты входят в **JDK** (каталог `bin` рядом с `java`), не в урезанную JRE — `jcmd`, `jmap`, `jfr`.

---

### Контекст перед началом

Эта тема часто кажется "слишком низкоуровневой", но на практике она полезна даже junior-разработчику. Когда сервис тормозит, команда ждёт не теорию, а набор артефактов — thread dump, heap dump, JFR и короткое описание симптомов.

Простое правило: сначала собрать факты, потом перезапускать сервис.

---

### Словарь перед командами

| Термин | Объяснение |
|--------|------------|
| **JVM** | Процесс, в котором крутится ваш `.jar` (Spring Boot, Tomcat внутри) |
| **PID** | Номер процесса в ОС; подставляете вместо `<pid>` в командах |
| **Heap (куча)** | Память для объектов (`new`, коллекции); OOM `Java heap space` — про неё |
| **Metaspace** | Память под метаданные классов; отдельный OOM `Metaspace` |
| **Heap dump** | Снимок всех объектов в куче в файл `.hprof` для анализа |
| **Thread dump** | Снимок стеков всех потоков в момент времени — ищут deadlock и "горячий" код |
| **JFR** | Java Flight Recorder — короткая запись профиля: CPU, GC, аллокации |
| **GC** | Сборщик мусора освобождает объекты без ссылок |

**Когда что делать (дерево решений):**

```
Приложение "висит" или CPU 100%  →  Thread.print (jcmd)
OutOfMemoryError / память растёт →  heap dump + Eclipse MAT / IDEA
Нужно "где тормозит метод"       →  JFR 60 сек + JDK Mission Control
```

---

### Когда открывать эту статью

| Симптом | Первый шаг |
|---------|------------|
| "Висит", CPU 100% | `jcmd <pid> Thread.print` |
| `OutOfMemoryError: Java heap space` | heap dump + анализ |
| Нужно понять, куда ушло время | короткая запись **JFR** |

---

## 1. Найти PID процесса

**Linux / macOS:**

```bash
jcmd
# или
ps aux | grep java
```

**Windows (PowerShell):**

```powershell
jcmd
# или диспетчер задач → подробности → java.exe
```

Запомните число **PID** — подставляйте вместо `<pid>` ниже. Если процессов Java несколько, смотрите колонку с main-классом или аргументами (`com.example.demo.DemoApplication`).

**Проверка JDK:** `java -version` и `jcmd -h` должны работать из той же установки, которой запущено приложение.

---

## 2. Снимок потоков (зависания, высокий CPU)

```bash
jcmd <pid> Thread.print > threads.txt
```

Откройте `threads.txt`: ищите повторяющийся стек в состоянии `RUNNABLE` — часто там цикл или блокировка.

**Разбор файла `threads.txt`:**

| Что видите | Как читать |
|------------|------------|
| `"main" #1 prio=5 os_prio=0 tid=...` | Имя потока и номер |
| `java.lang.Thread.State: RUNNABLE` | Поток выполняется или ждёт CPU |
| `BLOCKED` / `WAITING` | Ждёт lock или `wait()` |
| `at com.example.service.Foo.bar(Foo.java:42)` | Стек: сверху вниз — от текущего кадра к `main` |
| Один и тот же `at ...` десятки раз в RUNNABLE | Частый признак бесконечного цикла или горячего метода |

**Deadlock:** в конце дампа JVM иногда печатает `Found one Java-level deadlock` — два потока держат lock друг друга.

---

## 3. Heap dump (утечки памяти, OOM)

```bash
jcmd <pid> GC.heap_dump heap.hprof
```

Альтернатива (устаревающая, но встречается):

```bash
jmap -dump:live,format=b,file=heap.hprof <pid>
```

Файл `heap.hprof` откройте в **Eclipse MAT**, **VisualVM** или IntelliJ IDEA (*Analyze Heap Dump*).

**На что смотреть в анализаторе:**

- Dominator tree — кто держит больше всего памяти;
- Leak Suspects — эвристики утечек;
- сравнение двух дампов "до/после" нагрузки.

<div class="callout callout--tip">
  <div class="callout-title">OOM в логах</div>

  <div class="callout-body">
  Запустите JVM с `-XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=./dumps` — дамп появится автоматически при падении.
</div>
  </div>


---

## 4. Краткая запись JFR (профилирование)

**Java Flight Recorder** — встроенный профайлер Oracle JDK / Temurin 11+.

Запись 60 секунд:

```bash
jcmd <pid> JFR.start name=debug settings=profile duration=60s filename=recording.jfr
# подождать минуту
jcmd <pid> JFR.dump name=debug filename=recording.jfr
jcmd <pid> JFR.stop name=debug
```

Откройте `recording.jfr` в **JDK Mission Control** (JMC) или в IDEA (*Analyze JFR Snapshot*).

**Разбор:** в JMC смотрите *Method Profiling*, *GC*, *Memory* — где CPU и аллокации.

---

## 5. Минимальный чек-лист инцидента

1. Зафиксировать время и версию (`java -version`).
2. `jcmd <pid> VM.flags` — флаги JVM.
3. `jcmd <pid> GC.heap_info` — размер кучи.
4. `Thread.print` при подозрении на deadlock/CPU.
5. Heap dump или JFR — по типу проблемы.
6. Перезапуск только после сбора артефактов (если политика позволяет).

Подробнее о памяти и GC — [JVM, память и потоки](./23.md). Полная шпаргалка по флагам запуска (`-D`, `-X`, `-XX`) — [Справочник по Java, §24](./3.md#24-jvm--параметры-запуска-и-настройка).

---

## Пример — учебная утечка



<ExternalCodeEmbed example="java/java-503-302-001" title="Пример — учебная утечка" minHeight={318} />



Запуск с дампом при OOM:

```bash
java -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=. LeakDemo
```

В MAT список `byte[]` и статическая `LEAK` будут доминировать — хорошая тренировка чтения дампа.

---

### Частые ошибки

| Ошибка | Что делать |
|--------|------------|
| `jcmd` не найден | В PATH должен быть каталог `bin` JDK, не JRE |
| Отказ в доступе к PID | Права пользователя / sudo (осторожно в проде) |
| Огромный `.hprof` | Дамп на диск с местом; `live` уменьшает размер |
| Путать heap и metaspace | OOM бывает разный — читайте текст исключения |

---

### Что попробовать

1. Снять `Thread.print` для своего [Spring Boot](./271.md) под нагрузкой `ab` или `curl` в цикле.
2. Включить `HeapDumpOnOutOfMemoryError` в dev-профиле.
3. Вернуться к [JVM, память и потоки](./23.md) и сопоставить GC-логи с JFR.

---

### Дальше

[JVM, память и потоки](./23.md) · [Отладка в IDEA](./132.md) · [Testcontainers](./273.md)

---

## Быстрый runbook для дежурства

Ниже шаблон, который удобно хранить в internal wiki и использовать прямо во время инцидента:

1. Зафиксировать время инцидента и PID.
2. Снять `Thread.print` два раза с интервалом 10-20 секунд.
3. Снять `GC.heap_info` и при необходимости `heap_dump`.
4. Запустить JFR на 60-120 секунд под реальной нагрузкой.
5. Приложить артефакты к тикету и только потом перезапустить процесс.

Если пропустить шаги 2-4, корневая причина часто теряется сразу после рестарта.

---

## Как интерпретировать результат без перегруза

### Признаки CPU-проблемы

- несколько потоков в `RUNNABLE`;
- похожие вершины стека в горячих потоках;
- в JFR высокий self-time у небольшого числа методов.

---

### Признаки memory-проблемы

- рост heap после Full GC без возврата к прежнему уровню;
- крупные доминаторы в MAT (`byte[]`, `HashMap`, кэши);
- повторяющийся `OutOfMemoryError` под одинаковой нагрузкой.

---

### Признаки lock contention проблемы

- много потоков в `BLOCKED` на одном и том же мониторе;
- длинные цепочки ожиданий в `ReentrantLock$NonfairSync` или похожих кадрах;
- в JFR растут события monitor enter и lock stall.

---

## Runbook при гонке или deadlock

1. Снять `Thread.print` минимум два раза с интервалом 10-20 секунд.
2. Сравнить верхние кадры стеков. Одинаковые `RUNNABLE` стеки обычно указывают на горячий цикл, массовый `BLOCKED` часто указывает на lock contention.
3. Проверить конец дампа на `Found one Java-level deadlock`.
4. Запустить JFR на 60 секунд и открыть вкладки *Threads* и *Locks*.
5. Зафиксировать список проблемных классов/методов и только после этого менять таймауты или размер пулов.

Эта последовательность помогает не перепутать CPU saturation, deadlock и обычное ожидание внешнего I/O.

Полезные ссылки для расшифровки терминов:

- [JVM, память и потоки](./23.md)
- [Асинхронность в Java](./298.md)
- [Справочник по Java](./3.md)

---

## Частый сценарий в Spring Boot

Сервис получает всплеск трафика, кэш не ограничен, память растёт, latency ухудшается. В таком случае действуют так:

1. `jcmd <pid> GC.heap_info` — зафиксировать текущий размер heap.
2. `jcmd <pid> JFR.start ... duration=60s` — снять профиль.
3. `jcmd <pid> GC.heap_dump heap.hprof` — сделать дамп.
4. Проверить кэш-стратегию и TTL в коде, затем повторить нагрузочный тест.

Связанные материалы:
- [Spring Boot — первый API](./271.md)
- [Современные конструкции Java](./300.md)
- [Системное администрирование](/encyclopedia/2-system-network/2-06-sistemnoe-administrirovanie/1)

---
