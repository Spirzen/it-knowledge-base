...
related:
  ...
  - title: "Ввод и вывод в Java"
    doc: encyclopedia/5-languages/5-03-java/102
---
...
<ExternalPlayEmbed example="lab/first-program-play" title="Первая программа" minHeight={420} playProps={{ language: 'java' }} />
...
**Java** — "напиши один раз — запускай везде" на **JVM**: корпоративный бэкенд, большие кодовые базы, Android (исторически). Старт: **JDK**, IDE, класс с `public static void main(String[] args)`. Подробнее — [точка входа JVM](./40.md).

В этой статье два равноправных пути:

| Вариант | IDE | Кому удобнее |
|---------|-----|--------------|
| **A** | [IntelliJ IDEA](#вариант-a-intellij-idea-рекомендуется) | Большинство команд, Spring, Kotlin |
| **B** | [NetBeans](#вариант-b-netbeans--maven) | Учебные курсы, привычный NetBeans |

Оба создают **Maven-проект** с одной и той же структурой `src/main/java`. После Hello World логичный шаг — [ввод и вывод](./102.md) (консоль, `Scanner`), затем [отладка](./132.md); Spring — после Core ([Первая программа на Spring Framework](./271.md)).
...
Выберите один вариант IDE и дойдите до вывода <code>Hello World!</code> в консоли. На каждом шаге смотрите дерево проекта слева — так проще связать папки на диске с тем, что видите в IDE.
</div>
  </div>
...
Полный обзор среды (установка, интерфейс, навигация, отладка, Maven) — в статье [IntelliJ IDEA — IDE для разработки на Java](./103.md). Кратко: **IntelliJ IDEA** бесплатна в базовом ядре (с 2025.3 — единый продукт вместо отдельной Community Edition) и полностью подходит для Java + Maven. Скачивайте с [страницы загрузки для Windows](https://www.jetbrains.com/idea/download/?section=windows) или [jetbrains.com/idea](https://www.jetbrains.com/idea/).

1. Установите **JDK 17+** (Temurin, Oracle или [Microsoft Build of OpenJDK](/encyclopedia/5-languages/5-03-java/294.md)). Проверка: `java -version` в терминале.
...
6. Зелёная стрелка слева от `main` или **Shift+F10** — запуск. Вывод — во вкладке **Run** внизу.
7. **ПКМ по проекту → Build** — IDEA вызовет `mvn compile`; артефакты в `target/`.

**Разбор:** `package` совпадает с путём папок; `public class Main` — имя файла `Main.java`; `main` — точка входа JVM. Подробнее о Maven — [структура и сборки](./12.md).
...
Дальше по IDE: [отладка Java-кода](./132.md) (тот же проект, breakpoint на `println`).
...
![image-1.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-1.png)

Если мы перейдём к папке с проектом, увидим следующее:

![image-2.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-2.png)
...
![image-3.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-3.png)
![image-4.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-4.png)
...
![image-5.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-5.png)

10. Если переименовать проект или изменить свойства без рефакторинга, может быть ошибка в панели **Output** – и если ошибка говорит о том, что не может найти main class (который и нужен для запуска), то нужно исправить **pom.xml** и пересобрать проект. Как задать или сменить класс с `main` в манифесте — в разделе [Точка входа в Maven](./12.md#точка-входа-в-maven-main-class) главы про сборки; в NetBeans также помогает свойство `exec.mainClass` (пункт 13 ниже) и `<mainClass>` в shade-плагине (раздел "Запуск вне IDE").
...
![image-6.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-6.png)
![image-7.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-7.png)
...
<ExternalCodeEmbed example="xml/java-503-13-001" title="Добавление юнит-теста" minHeight={426} />
...
![image-8.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-8.png)
![image-9.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-9.png)
![image-10.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-10.png)
![image-11.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-11.png)
![image-12.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-12.png)

Мы получим в структуре **"Test Packages"**:

![image-13.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-13.png)
...
![image-14.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-14.png)
![image-15.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-15.png)

20. В окне **Output** мы увидим результаты тестов:

![image-16.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-16.png)
...
![image-17.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-17.png)

Внимательно с названием папки и файла – строго как указано выше:

![image-18.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-18.png)
...
![image-19.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-19.png)
...
Этого пока достаточно. Если вы успешно проделали это шаги, вы:
	...
	- подключили пакеты и добавили JUnit (подробнее — [JUnit 5](./291.md));
...
<ExternalCodeEmbed example="xml/java-503-13-002" title="Запуск вне IDE" minHeight={498} />
...
![image-20.png](https://assets.spirzen.ru/encyclopedia/5-languages/5-03-java/image-20.png)
...
<ExternalPlayEmbed example="basics/installer-wizard-play" title="Мастер установки" minHeight={480} playProps={{ appName: 'Установка JDK', productName: 'OpenJDK / Temurin', welcomeText: 'Мастер установит Java Development Kit. Запомните путь — его укажете в JAVA_HOME и в IDE.', defaultPath: 'C:\\\\Program Files\\\\Eclipse Adoptium\\\\jdk-21', extraOptionsLabel: 'Установить переменные JAVA_HOME и обновить PATH', cardSubtitle: 'Шаг 5 в инструкции NetBeans — далее mvn и сборка' }} />

5. Устанавливаем **JDK** (Java Development Kit). Варианты:
   - [Oracle JDK](https://www.oracle.com/java/technologies/downloads/) или дистрибутив на базе OpenJDK;
   - **[Microsoft Build of OpenJDK](https://learn.microsoft.com/ru-ru/java/openjdk/download)** — [установка](https://learn.microsoft.com/ru-ru/java/openjdk/install?tabs=exe%2Chomebrew%2Cubuntu) (см. [навигатор Microsoft](./294.md)).
   При установке запоминаем путь к JDK — например, `C:\Program Files\Java\jdk-21\` или каталог Microsoft OpenJDK.

6. Проверяем, что переменная **JAVA_HOME** указывает на каталог JDK (например, `C:\Program Files\Java\jdk-21\` или путь Microsoft OpenJDK) — так система и Maven находят компилятор.
...
<span id="classpath-где-jvm-ищет-классы"></span>
...
<div class="callout callout--info">
  <div class="callout-title">Maven и IDE скрывают classpath</div>

  <div class="callout-body">
  В IntelliJ IDEA и NetBeans classpath собирается автоматически; Maven кладёт зависимости в локальный репозиторий и подставляет их при <code>mvn exec:java</code> или запуске JAR. Ручной <code>-cp</code> нужен, когда вы компилируете без сборщика или отлаживаете «Could not find or load main class». Подробнее о структуре проекта — в <a href="./12.md">Maven и сборках</a>, о пути от исходника — в <a href="./1.md#put-isxodnika-do-zapuska">основах Java</a>.
  </div>
</div>

После classpath логично перейти к [вводу и выводу](./102.md) — три короткие консольные программы с разбором.
...
3. Консольные задачи: [Lab — шаблоны и примеры](/lab/Примеры/1131).
4. Окно с кнопкой (лабораторная по GUI): [Lab — Java Swing](/lab/Примеры/1143) после [основных конструкций](./16.md).
5. Веб: [Spring Boot](./271.md) после базового `main`.
...
**Первые шаги** ([маршрут подборки](/about/collections/pervye-shagi)) — [Первая программа на Python](/encyclopedia/5-languages/5-02-python/16), [Первая программа на PHP](/encyclopedia/5-languages/5-07-php/13), [Первая программа на Go](/encyclopedia/5-languages/5-10-go/24), [Первая программа на C#](/encyclopedia/5-languages/5-05-csharp/16), [Первая программа на C++](/encyclopedia/5-languages/5-06-cpp/1002), [Первая программа на TypeScript](/encyclopedia/5-languages/5-10-typescript/4).
