...
Анимация выполняется в **JavaFX Application Thread**; для длительной работы без блокировки UI используйте `Task` + `Platform.runLater`. Пример в контексте `ProgressBar` — [3111.md — вкладка «Прогресс»](./3111.md#вкладка-прогресс--progressbar-slider-и-timeline).
