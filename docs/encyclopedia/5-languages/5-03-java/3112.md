...
Анимация выполняется в **JavaFX Application Thread**; для длительной работы без блокировки UI используйте `Task` + `Platform.runLater`. Пример в контексте `ProgressBar` — [3111.md — вкладка "Прогресс"](./3111.md#вкладка-прогресс).
...
Каркас — [131.md — Swing](./131.md). Полные программы с разбором строк — [Lab — Java Swing](/lab/Примеры/1143). Все изменения UI — в **EDT**: `SwingUtilities.invokeLater(() -> { ... })`.
...
4. Долгие операции не кладите в `ActionListener` — см. [112.md](/encyclopedia/4-code-dev/4-11-desktopnye-prilozheniya/112.md) и `SwingWorker` в [311.md](./311.md#swing-и-потоки).
...
|---------|--------|-------|
| `package javafx… does not exist` | Нет `javafx.version` в `pom` — [3111.md](./3111.md#чеклист-pomxml-перед-первым-run) | — |
| `runtime components are missing` | Запуск без `--module-path` — [3111.md](./3111.md#запуск-maven-idea-и-module-path) | — |
...
- [Архитектура десктопа](/encyclopedia/4-code-dev/4-11-desktopnye-prilozheniya/1) · [раздел «Десктопные приложения»](/encyclopedia/4-code-dev/4-11-desktopnye-prilozheniya/intro) · [112.md](/encyclopedia/4-code-dev/4-11-desktopnye-prilozheniya/112.md)
