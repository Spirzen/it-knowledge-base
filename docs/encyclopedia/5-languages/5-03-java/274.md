...
## Жизненный цикл токена в реальном API

Учебная схема с одним `access_token` удобна для старта. В production обычно применяют пару `access_token` + `refresh_token`:

1. Пользователь логинится по паролю.
2. Сервер выдаёт короткий `access_token` (`5-15` минут) и более длинный `refresh_token`.
3. Клиент использует `access_token` в `Authorization: Bearer ...`.
4. При истечении срока клиент получает новый `access_token` через refresh-механизм.
5. При выходе из системы refresh-токен отзывается на сервере.

### Что класть в claims

Добавляйте только технически нужные поля:

- `sub` (идентификатор пользователя),
- `scope` или `roles`,
- `iat`, `exp`, при необходимости `jti`.

Избегайте в claims секретов и лишних персональных данных. JWT легко декодируется на клиенте, его защищает подпись, а не шифрование payload.
