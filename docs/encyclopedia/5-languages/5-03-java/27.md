...
- **Spring Boot**, **Spring Безопасность**, **Spring Данные** и другие — это *проекты*, *построенные на основе* Spring Framework, решающие конкретные задачи (ускорение старта, безопасность, работа с данными и пр.). Они не являются частью ядра, но тесно с ним интегрированы и часто используются совместно.
...
- **`spring-expression` (SpEL)** — мощный язык выражений, встроенный во время выполнения. Позволяет динамически вычислять значения в конфигурации: `@Value("#{systemProperties['user.region']}")`, `@Value("#{T(java.lang.Math).random() * 100}")`, вызов методов, доступ к коллекциям. Используется в XML/Java-конфигурации, Spring Безопасность, Spring Данные и др.

#### 2. Данные Access / Integration (Доступ к данным и интеграция)
...
Одна из самых успешных абстракций Spring — **Spring Данные**. Его задача — устранить рутину CRUD-операций и повторяющегося SQL/JPA-кода, сохранив при этом гибкость.
...
Spring Данные — это *фасад*. За ним скрываются специализированные модули:

- `spring-Данные-jpa` — для реляционных БД через Hibernate, EclipseLink;
- `spring-Данные-mongodb` — для документных БД;
- `spring-Данные-redis` — для in-memory хранилищ;
- `spring-Данные-elasticsearch` — для поисковых движков;
- `spring-Данные-jdbc` — упрощённая альтернатива JPA без lazy loading и кэширования.
...
Любой метод репозитория по умолчанию выполняется в транзакции (благодаря `@Transactional`, который Spring Данные добавляет в реализацию). Пагинация поддерживается через типы `Page<T>` и `Pageable`:
...
Spring Безопасность — это *независимый фреймворк*, но настолько тесно интегрированный с Spring, что воспринимается как его часть. Он решает две задачи:
...
Все запросы к защищённому приложению проходят через цепочку сервлет-фильтров (`FilterChain`). Spring Безопасность регистрирует свой главный фильтр — `FilterChainProxy`, который делегирует работу конкретным `SecurityFilter`’ам:
...
Помимо URL, Spring Безопасность позволяет ограничивать доступ *к методам* с помощью аннотаций:
...
Spring Безопасность не изобретает протоколы — он *реализует* их:
...
1. **Auto-configuration** — анализ classpath и автоматическое создание бинов. Наличие `spring-boot-starter-Данные-jpa` и `HikariCP` → Spring Boot создаёт `DataSource`, `EntityManagerFactory`, `JpaTransactionManager`.
2. **Starter-зависимости** — предопределённые наборы библиотек для конкретных задач:
   ...
   - `spring-boot-starter-Данные-jpa` — JPA + Hibernate + транзакции;
   - `spring-boot-starter-Безопасность` — Spring Безопасность.
...
По умолчанию многие endpoint’ы отключены в production (`/env`, `/heapdump`) из соображений безопасности. Их можно включить выборочно, настроив права доступа через Spring Безопасность.
...
Spring Безопасность предоставляет базовые механизмы, но реальные системы требуют глубокой настройки.

#### Валидация JWT: собственный `JwtDecoder`

По умолчанию Spring Безопасность использует `NimbusJwtDecoder`, но его можно кастомизировать для:
...
Spring Безопасность позволяет регистрировать *события безопасности*:
...
Spring Данные JPA поддерживает `EntityGraph`:
```java
...
public class LegacyService implements ApplicationContextAware {
    ...
    public void doWork() {
        DataService Данные = context.getBean(DataService.class);
    }
}
```
...
public class ModernService {
    ...
    public void doWork() {
        DataService Данные = dataServiceProvider.getIfAvailable();
    }
}
```
...
### Spring и Java Platform Module Система (JPMS)
...
Spring Данные позволяет легко поддерживать обе модели: `JpaRepository` для записи, `ElasticsearchRepository` для чтения.
