...
| `Math.random()` | Случайное `double` от 0.0 (вкл.) до 1.0 (искл.) | `double rand = Math.random();` |
| `Math.round(float a)` | Округление до ближайшего целого | `long r = Math.round(3.7f);` |

Подробнее про диапазоны целых, `ThreadLocalRandom` и отличие от Python `randint` — [Основные конструкции — случайные числа](./16.md#случайные-числа).
