...
description: "Ключевые классы и интерфейсы Java — обзор стандартной библиотеки для строк, файлов, коллекций, дат и базовых операций."
---
...
Set — коллекция без дубликатов. Реализации — `HashSet` (O(1) в среднем, порядок не гарантирован), `LinkedHashSet` (порядок вставки), `TreeSet` (сортировка, O(log n)), `EnumSet` (для `enum`). Полная шпаргалка — [Коллекции в Java](./24.md).

Map — ассоциативный массив. Реализации — `HashMap`, `LinkedHashMap`, `TreeMap`, `ConcurrentHashMap` (многопоточность). Для очередей — `ArrayDeque`, `PriorityQueue`, `BlockingQueue`.
...
Этот блок полезен как "быстрый пульт" при ежедневной разработке. Подробная теория по коллекциям есть в отдельной статье — [Коллекции Java](./24.md).
...
| Действие | API | Пример |
|---|---|---|
| Добавить в конец | `list.add(value)` | `users.add("Ann")` |
| Вставить по индексу | `list.add(index, value)` | `users.add(0, "Admin")` |
| Прочитать | `list.get(index)` | `users.get(1)` |
| Заменить | `list.set(index, value)` | `users.set(1, "Bob")` |
| Удалить | `list.remove(index)` / `list.remove(value)` | `users.remove("Ann")` |
...
| Действие | API | Пример |
|---|---|---|
| Добавить или заменить | `map.put(key, value)` | `ages.put("Ann", 30)` |
| Прочитать | `map.get(key)` / `map.getOrDefault(key, def)` | `ages.getOrDefault("Tom", 0)` |
| Удалить | `map.remove(key)` | `ages.remove("Ann")` |
...
| Действие | API | Пример |
|---|---|---|
| Добавить | `set.add(value)` | `tags.add("java")` |
| Удалить | `set.remove(value)` | `tags.remove("legacy")` |
| Проверить наличие | `set.contains(value)` | `tags.contains("spring")` |

---

#### Queue / Deque / Stack

| Тип | Операция | API | Пример |
|---|---|---|---|
| Queue (FIFO) | Добавить в хвост | `offer(value)` | `queue.offer(task)` |
| Queue (FIFO) | Извлечь с головы | `poll()` | `queue.poll()` |
| Queue (FIFO) | Посмотреть голову | `peek()` | `queue.peek()` |
| Stack (LIFO) | Положить | `push(value)` | `stack.push("A")` |
| Stack (LIFO) | Снять верхний | `pop()` | `stack.pop()` |
| Stack (LIFO) | Посмотреть верхний | `peek()` | `stack.peek()` |
| Deque | Работа с двумя концами | `addFirst/addLast`, `pollFirst/pollLast` | `deque.addFirst(1)` |

<div class="callout callout--tip">
  <div class="callout-title">Практический маршрут</div>

  <div class="callout-body">
  После этого справочника переходите к [Stream API](./23.md), [исключениям](./17.md) и [потокам](./19.md), чтобы связать коллекции с обработкой данных и ошибками.
</div>
  </div>
