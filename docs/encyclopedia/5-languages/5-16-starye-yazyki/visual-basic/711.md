...
- `StringBuilder`: изменяемая строка (из пространства имён `Система.Text`).
...
- Добавление сборок .NET (`Система.IO`, `System.Net.Http` и др.)
- NuGet-пакеты
...
#### JSON (через Newtonsoft.Json или Система.Text.Json)
...
#### JsonSerializer (Система.Text.Json)
...
- Используйте `Using` для автоматического освобождения ресурсов:
  ```vb
  Using файл As New StreamReader("Данные.txt")
