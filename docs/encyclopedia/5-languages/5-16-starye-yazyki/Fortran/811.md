...
description: Справочник Fortran - синтаксис, программные единицы, модули и практические конструкции языка.
...
Справочник-шпаргалка по **Fortran**: типы, синтаксис, стандартная библиотека, типовые паттерны. Не заменяет пошаговое обучение. Учебный курс: [раздел](/encyclopedia/5-languages/5-16-starye-yazyki/Fortran/intro).
...
- [1. Общие характеристики языка](#1-общие-характеристики-языка)
...
- [7. Массивы](#7-массивы)
- [8. Подпрограммы](#8-подпрограммы)
- [9. Модули (`module`)](#9-модули-module)
- [10. Объектно-ориентированное программирование](#10-объектно-ориентированное-программирование)
- [11. Ввод и вывод (I/O)](#11-ввод-и-вывод-io)
- [12. Обработка ошибок](#12-обработка-ошибок)
- [13. Параллелизм и coarray (ко-массивы)](#13-параллелизм-и-coarray-ко-массивы)
- [14. Встроенные процедуры (intrinsics)](#14-встроенные-процедуры-intrinsics)
- [15. Компиляция и сборка проектов](#15-компиляция-и-сборка-проектов)
- [16. Отладка и профилирование](#16-отладка-и-профилирование)
- [17. Лучшие практики и стиль кода](#17-лучшие-практики-и-стиль-кода)
- [18. Пример структуры крупного проекта](#18-пример-структуры-крупного-проекта)
- [19. Совместимость и переносимость](#19-совместимость-и-переносимость)
...
#### 7.3. Операции с элементами массива

| Действие | Синтаксис |
|----------|-----------|
| Прочитать | `a(i)` или `matrix(i, j)` |
| Заменить | `a(i) = value` |
| Диапазон индексов | `a(2:5)`, `matrix(:, 3)` — срезы |
| Динамический размер | `allocate(a(n))`, `deallocate(a)` |

Индексация по умолчанию с **1**, не с 0. Вставка "в середину" с сдвигом — вручную или через `pack` / `unpack`; для частых вставок лучше `allocatable` + `allocate` нужного размера.

---

#### 7.3.1. Векторизованные операции
...
Переменные объявляют как `class(base_type), allocatable`, чтобы хранить объекты производных типов. Доступ к полям конкретного типа — через `select type`:
...
Вызов type-bound procedure: `call s%area()` или `x = s%area()` — если `area` объявлена как функция.
...
### 13. Параллелизм и coarray (ко-массивы)
...
```fortran
real, allocatable :: shared_data[:]
```

Каждый "образ" (image) программы имеет свою копию. Обмен данными:

```fortran
shared_data[this_image()] = local_value
sync all
...
- `merge(tsource, fsource, mask)` — условный выбор элемента/массива

```fortran
logical :: mask
real :: out
mask = (x > 0.0)
out = merge(1.0, 0.0, mask)
```
...
- Используйте `-g -fcheck=all` (gfortran) для проверки границ массивов, `allocate` и согласованности аргументов.
- Типичные runtime-ошибки: выход за границы массива, неассоциированный `pointer`, повторный `allocate` без `deallocate`, несовпадение формы массивов при вызове без явного интерфейса.
- Запуск под `gdb`:
...
- Для работы с путями и файлами учитывайте различия ОС (Unix и Windows), особенно в строках имён файлов.
...
**Справочники** — [Справочник по COBOL](/encyclopedia/5-languages/5-16-starye-yazyki/Cobol/711), [Справочник по Lisp](/encyclopedia/5-languages/5-16-starye-yazyki/Lisp/811), [Справочник по Lua](/encyclopedia/5-languages/5-15-lua-i-luau/3), [Справочник по Pascal](/encyclopedia/5-languages/5-16-starye-yazyki/Pascal/711), [Справочник по языку Swift](/encyclopedia/5-languages/5-14-swift/3), [Справочник по visual-basic](/encyclopedia/5-languages/5-16-starye-yazyki/visual-basic/711).
