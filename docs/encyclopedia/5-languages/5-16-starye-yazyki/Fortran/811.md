...
description: Программа на Fortran состоит из одной или нескольких программных единиц — Основная программа (program), Внешние подпрограммы (subroutine, function), Модули (module), Блоки внутренних подпрограмм…
...
- [13. Параллелизм и коарсенная многопоточность](#13-параллелизм-и-коарсенная-многопоточность)
...
```fortran
open(unit=10, file='Данные.txt', status='old', action='read')
```
