...
- `Тестирование` — проверка состояния (возвращают булевы значения)
...
"Требует улучшения"
processor process: Данные withOptions: options andContext: context andLogger: logger.
```
...
options := ProcessingOptions new
	...
	logLevel: #debug;
	yourself.
processor process: Данные with: options.
```
