...
description: Порядок вычисления определяется строго — Унарные сообщения (слева направо), Бинарные сообщения (слева направо), Ключевые сообщения (слева направо)
...
```smalltalk
file := 'Данные.txt' asFileReference writeStream.
file nextPutAll: 'Hello'.
file close.

content := 'Данные.txt' asFileReference contents.
```
