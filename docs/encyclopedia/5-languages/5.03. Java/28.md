...
description: Класс java.io.File (или современный java.nio.file.Path) используется для работы с файловой системой. Позволяет проверять существование файла или каталога, создавать и удалять файлы, получать…
---
...
Класс java.lang.Система предоставляет доступ к системным ресурсам и окружению. Содержит стандартные потоки ввода-вывода (in, out, err), методы для работы со временем (currentTimeMillis(), nanoTime()), управления памятью (gc()) и завершения приложения (exit()). Также позволяет получать переменные окружения и системные свойства, такие как имя ОС или версия JVM.
...
| `Система.getProperties()` | Получает системные свойства JVM | `Properties props = Система.getProperties();` |
| `Система.getProperty(key)` | Получает значение системного свойства | `Система.getProperty("os.name")` |
| `Система.lineSeparator()` | Возвращает разделитель строк | `Система.lineSeparator()` |
...
| `Система.currentTimeMillis()` | Возвращает время в миллисекундах | `long ms = Система.currentTimeMillis();` |
| `Система.nanoTime()` | Возвращает время в наносекундах | `long ns = Система.nanoTime();` |
...
// ... код
System.out.println("Время выполнения: " + (Система.currentTimeMillis() - start) + " мс");
```
...
| `file.isFile()` | Является ли файлом | `new File("Данные.txt").isFile()` |
| `file.getName()` | Возвращает имя файла или папки | `new File("Данные.txt").getName()` → "Данные.txt" |
| `file.getAbsolutePath()` | Возвращает полный путь к файлу | `new File("Данные.txt").getAbsolutePath()` |
