...
CompletableFuture.supplyAsync(() -> fetchData())
    .thenAccept(Данные -> processData(Данные));
// Если fetchДанные выбросит исключение, оно не будет обработано

// Хорошо: обработка ошибок
CompletableFuture.supplyAsync(() -> fetchData())
    .thenAccept(Данные -> processData(Данные))
    .exceptionally(ex -> {
        logger.error("Failed to process Данные", ex);
        return null;
    });
```
...
// Хорошо: декомпозиция
public void generateReport() {
    ReportДанные Данные = prepareReportData();
    String content = formatReport(Данные);
