...
| `Система.gc()` | Запрос запуска сборщика мусора | `Система.gc();` |
| `Система.currentTimeMillis()` | Получение текущего времени в миллисекундах с эпохи Unix | `long now = Система.currentTimeMillis();` |
| `Система.nanoTime()` | Получение высокоточного времени для измерения интервалов | `long start = Система.nanoTime();` |
...
| `Arrays.toString(Object[] a)` | Преобразование массива в строковое представление | `String view = Arrays.toString(Данные);` |
