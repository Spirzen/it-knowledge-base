...
- [9. Безопасность: WS-Безопасность (WSS)](#9-безопасность-ws-безопасность-wss)
...
- [WS-I Безопасность Profile 1.1](#ws-i-безопасность-profile-11)
...
### 9. Безопасность: WS-Безопасность (WSS)

SOAP не содержит встроенной безопасности. **WS-Безопасность** (OASIS Standard) — основной механизм защиты сообщений.
...
> 🔸 Поддержка:
> - **.NET**: `Система.Transactions` + `TransactionScope` + `WS-AtomicTransaction` binding (только в enterprise-средах, напр. с MSDTC).
...
|-----------|-------------|
| **SoapUI (ReadyAPI)** | Генерация из WSDL, mocking, load Тестирование, Безопасность Тестирование (WS-Безопасность), MTOM, WS-A, WS-RM. |
...
|-----------|--------|----------------|-------------|
| **ASMX (Web Services)** | .NET Framework 1.0–4.8 | SOAP 1.1, `document/literal`, базовый WS-Безопасность | Устарело, не поддерживает MTOM без костылей, нет WS-A/WS-RM. |
| **WCF** | .NET Framework 3.0–4.8 | Полная: SOAP 1.1/1.2, MTOM, WS-Безопасность (WSS 1.0/1.1), WS-A, WS-RM, WS-AT | Требует `customBinding` для тонкой настройки. Не поддерживается в .NET Core 1–2. |
| **CoreWCF** | .NET 5+ (community) | Порт WCF: `basicHttpBinding`, `wsHttpBinding`, MTOM, частично WS-Безопасность | Активно развивается; WS-RM и WS-AT — в Планы развития. |
...
|-----------|------|-----------|
| **JAX-WS (Metro)** | Java EE / Jakarta EE | SOAP 1.1/1.2, MTOM, WS-Безопасность (через `@Policy`), WS-A (частично) | Встроен в GlassFish, WebLogic; требует `wsservlet` для Безопасность. |
| **Apache CXF** | Spring, standalone | Полная: WS-Безопасность (WSS4J), WS-A, WS-RM (Sandesha2), WS-AT (Narayana), MTOM/XOP | Наиболее гибкий open-source стек. |
| **Spring-WS** | Spring Framework | «Contract-first» (только `document/literal`), MTOM, WS-Безопасность (через WSS4J), но **без WSDL generation** | Фокус на безопасности и контроле над XML. |

### Python

| Библиотека | Поддержка | Комментарий |
|-----------|-----------|-------------|
| **zeep** | SOAP 1.1/1.2, WSDL 1.1/2.0, MTOM (частично), WS-Безопасность (UsernameToken, Timestamp, BinarySecurityToken) | Активно поддерживается, типизация через `xsd`. Лучший выбор. |
| **pysimplesoap** | SOAP 1.1, базовый WS-Безопасность | Устаревшая, минимальная поддержка. |
| **suds-jurko** | SOAP 1.1, WSDL, нет MTOM/WS-Безопасность | Не поддерживается с 2014 г. |
...
|--------|-------------|---------------|
| **`<Данные><![CDATA[...]]></Данные>` внутри `xsd:string`** | Некорректный XML при парсинге: CДанные не сохраняется в объектной модели; нарушает XSD-валидацию (`xs:string` не допускает CДанные как особую конструкцию). | Использовать `&lt;`, `&gt;`, `&amp;` — XML-парсер сам декодирует. |
...
| **Интеграция с гос/банковскими системами (ЕПГУ, Банк России, СМЭВ 3.0)** | ✅ SOAP | Требования ГОСТ Р 7.0.97-2016, СМЭВ — строго SOAP/WSDL/WS-Безопасность. |
