...
description: SOAP-сообщение — это XML-документ с фиксированной структурой, определяемой схемами http — //schemas.xmlsoap.org/soap/envelope/ (SOAP 1.1) или http — //www.w3.org/2003/05/soap-envelope (SOAP 1.2).
referenceType: tool
related:
  - title: "Протокол SOAP"
...
Структура SOAP-сообщений, WSDL и типовые сценарии в табличном виде. Учебный курс: [Протокол SOAP](/encyclopedia/2-system-network/2-09-osnovy-integratsionnogo-vzaimodeystviya/126), раздел [микросервисов](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/intro).
...
| Задача | Подсказка |
|--------|-----------|
| Установка | официальная документация **SOAP** |
...
- [1. Общая структура SOAP-сообщения](#1-общая-структура-soap-сообщения)
- [2. Элемент `<soap:Envelope>`](#2-элемент-soapenvelope)
- [Атрибуты:](#атрибуты)
- [3. Элемент `<soap:Header>`](#3-элемент-soapheader)
- [Структура подзаголовка:](#структура-подзаголовка)
- [Атрибуты подзаголовка (на **каждом дочернем элементе** `<Header>`):](#атрибуты-подзаголовка-на-каждом-дочернем-элементе-header)
- [4. Элемент `<soap:Body>`](#4-элемент-soapbody)
- [Возможные дочерние элементы:](#возможные-дочерние-элементы)
- [5. Элемент `<soap:Fault>` (ошибки)](#5-элемент-soapfault-ошибки)
- [Структура SOAP 1.1:](#структура-soap-11)
- [Структура SOAP 1.2:](#структура-soap-12)
- [Поля `faultcode` (SOAP 1.1) и `<Code>` (SOAP 1.2)](#поля-faultcode-soap-11-и-code-soap-12)
- [`<detail>` / `<Detail>`](#detail-detail)
- [6. Привязка к HTTP — как SOAP передаётся по сети](#6-привязка-к-http-как-soap-передаётся-по-сети)
- [HTTP-заголовки (обязательные и рекомендуемые)](#http-заголовки-обязательные-и-рекомендуемые)
- [HTTP-статусы и их интерпретация](#http-статусы-и-их-интерпретация)
- [7. Вложения и двоичные данные (Binary Attachments)](#7-вложения-и-двоичные-данные-binary-attachments)
- [Способы:](#способы)
- [Пример MTOM/XOP (в SOAP 1.2)](#пример-mtomxop-в-soap-12)
- [8. WSDL и SOAP: как описывается интерфейс](#8-wsdl-и-soap-как-описывается-интерфейс)
- [Структура WSDL 1.1 (кратко):](#структура-wsdl-11-кратко)
- [Ключевые атрибуты в `<soap:binding>` и `<soap:operation>`](#ключевые-атрибуты-в-soapbinding-и-soapoperation)
- [9. Безопасность: WS-Security (WSS)](#9-безопасность-ws-безопасность-wss)
- [Элементы в `<soap:Header>`:](#элементы-в-soapheader)
- [Пример `<wsse:UsernameToken>` (plain text — **не для production**):](#пример-wsseusernametoken-plain-text-не-для-production)
- [Рекомендуемые `Type` для `<wsse:Password>`:](#рекомендуемые-type-для-wssepassword)
- [`<wsu:Timestamp>` (обязателен при `PasswordDigest` и для защиты от replay):](#wsutimestamp-обязателен-при-passworddigest-и-для-защиты-от-replay)
- [10. WS-Addressing (W3C Recommendation, 2006)](#10-ws-addressing-w3c-recommendation-2006)
- [Основные элементы в `<soap:Header>`:](#основные-элементы-в-soapheader)
- [Пространства имён:](#пространства-имён)
- [Специальные URI-значения:](#специальные-uri-значения)
- [Пример асинхронного вызова:](#пример-асинхронного-вызова)
- [11. WS-ReliableMessaging (WS-RM, OASIS Standard)](#11-ws-reliablemessaging-ws-rm-oasis-standard)
- [Основные концепции:](#основные-концепции)
- [Элементы в `<soap:Header>`:](#элементы-в-soapheader)
- [Пример `<wsrm:Sequence>`:](#пример-wsrmsequence)
- [Поведение:](#поведение)
- [12. WS-Coordination и WS-AtomicTransaction](#12-ws-coordination-и-ws-atomictransaction)
- [Архитектура:](#архитектура)
- [Пространства имён:](#пространства-имён)
- [Элемент в `<soap:Header>`:](#элемент-в-soapheader)
- [Операции в теле:](#операции-в-теле)
- [13. Профили совместимости и ограничения](#13-профили-совместимости-и-ограничения)
- [WS-I Basic Profile 1.1 (и 2.0) — де-факто стандарт для совместимости](#ws-i-basic-profile-11-и-20-де-факто-стандарт-для-совместимости)
- [WS-I Attachments Profile 1.0](#ws-i-attachments-profile-10)
- [WS-I Security Profile 1.1](#ws-i-безопасность-profile-11)
- [14. Отладка и инструменты](#14-отладка-и-инструменты)
- [Пример: подпись `wsse:UsernameToken` с `PasswordDigest` в Postman (Pre-request Script):](#пример-подпись-wsseusernametoken-с-passworddigest-в-postman-pre-request-script)
- [15. Сравнение SOAP 1.1 и SOAP 1.2: таблица различий](#15-сравнение-soap-11-и-soap-12-таблица-различий)
- [16. Поддержка в фреймворках и стеках](#16-поддержка-в-фреймворках-и-стеках)
- [.NET](#net)
- [Java](#java)
- [Python](#python)
- [Node.js / JS/TS](#nodejs-jsts)
- [17. Анти-паттерны и типичные ошибки](#17-анти-паттерны-и-типичные-ошибки)
- [18. Когда оставаться на SOAP, а когда мигрировать](#18-когда-оставаться-на-soap-а-когда-мигрировать)
...
<ExternalCodeEmbed example="xml/infra-security-8-8-05-mikroservisy-i-integratsiya-1201-001" title="1. Общая структура SOAP-сообщения" minHeight={336} />
...
```xml
<ns:Безопасность xmlns:ns="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd"
...
<ExternalCodeEmbed example="xml/infra-security-8-8-05-mikroservisy-i-integratsiya-1201-002" title="Структура SOAP 1.2" minHeight={408} />
...
|--------|----------|----------|
| **Base64-вложение в XML** | Нестандартный, но распространённый | `<Данные>YWJj...</Данные>` — просто, но +33% объём, высокий overhead парсинга. Подходит для небольших данных (`<`100 КБ). |
...
Разбор:
- Заголовок `multipart/related` (MTOM/XOP) отделяет SOAP-конверт от бинарного вложения в одном HTTP-сообщении.
...
<ExternalCodeEmbed example="xml/infra-security-8-8-05-mikroservisy-i-integratsiya-1201-003" title="Структура WSDL 1.1" minHeight={696} />
...
|--------|-------------------|------------|
| `<wsse:Безопасность>` | `http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd` | Контейнер для всех элементов безопасности. |
...
```xml
<wsse:Безопасность soap:mustUnderstand="1"
...
- Запрещает подпись/шифрование без политики (`wsse:Безопасность` без `wsu:Id` — недопустимо)
...
|-----------|-------------|
| **SoapUI (ReadyAPI)** | Генерация из WSDL, mocking, load testing, Безопасность Тестирование (WS-Security), MTOM, WS-A, WS-RM. |
...
<ExternalCodeEmbed example="javascript/infra-security-8-8-05-mikroservisy-i-integratsiya-1201-004" title="Пример — подпись `wsse:UsernameToken` с `PasswordDigest` в Postman (Pre-request Script):" minHeight={408} />
...
| **CoreWCF** | .NET 5+ (community) | Порт WCF: `basicHttpBinding`, `wsHttpBinding`, MTOM, частично WS-Security | Активно развивается; WS-RM и WS-AT — в Планы развития. |
...
|-----------|------|-----------|
| **JAX-WS (Metro)** | Java EE / Jakarta EE | SOAP 1.1/1.2, MTOM, WS-Security (через `@Policy`), WS-A (частично) | Встроен в GlassFish, WebLogic; требует `wsservlet` для Безопасность. |
...
|--------|-------------|---------------|
| **`<Данные><![CDATA[...]]></Данные>` внутри `xsd:string`** | Некорректный XML при парсинге: CData не сохраняется в объектной модели; нарушает XSD-валидацию (`xs:string` не допускает CData как особую конструкцию). | Использовать `&lt;`, `&gt;`, `&amp;` — XML-парсер сам декодирует. |
...
| **Отсутствие `mustUnderstand="1"` у критичных заголовков** | Промежуточный узел проигнорирует заголовок → нарушение безопасности/транзакции. | Всегда указывать `soap:mustUnderstand="1"` для `wsse:Безопасность`, `wsrm:Sequence`, `wscoor:CoordinationContext`. |
...
| Область | Примечание |
|---------|------------|
| Версии | актуальные LTS/стабильные релизы **SOAP** |
...
**Справочники** — [Справочник по Azure Repos Git](/encyclopedia/8-infra-security/8-04-devops-ci-cd/3124), [Справочник по gRPC](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/1202), [E2E-тесты и CI с Playwright и GitHub Actions](/encyclopedia/8-infra-security/8-04-devops-ci-cd/3123), [Справочник по GraphQL](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/1203), [Справочник по Tempo](/encyclopedia/8-infra-security/8-04-devops-ci-cd/3122), [Справочник по RabbitMQ](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/1204).
