...
description: Заголовок Sec-WebSocket-Key используется для предотвращения кэширования и проверки подлинности.
---
...
ws.addEventListener('message', (event) => {
    const Данные = JSON.parse(event.Данные);
    console.log('Получено сообщение:', Данные);
    
    switch (Данные.type) {
        case 'message':
            displayMessage(Данные.username, Данные.text);
            break;
        case 'user_joined':
            updateUserList(Данные.users);
            break;
    }
});
...
wss.on('connection', (ws, req) => {
    ...
    ws.on('message', (Данные) => {
        try {
            const message = JSON.parse(Данные);
...
function broadcastToRoom(roomName, message, excludeClientId = null) {
    ...
    const Данные = JSON.stringify(message);
    ...
    async def broadcast(self, message: dict, room: str = None):
        ...
        for client_id in recipients:
            if client_id in self.active_connections:
                websocket = self.active_connections[client_id]
                try:
                    await websocket.send_text(Данные)
                except Exception:
...
async def websocket_endpoint(websocket: WebSocket, client_id: str):
    ...
    try:
        while True:
            Данные = await websocket.receive_text()
            message = json.loads(Данные)
    ...
    <body>
        ...
        <script>
            ...
            ws.onmessage = (event) => {
                ...
                message.textContent = JSON.stringify(Данные);
...
```
event: message
Данные: {"type":"notification","text":"Новое сообщение"}
id: 12345
retry: 30000

Данные: Простое сообщение без типа
...
- `Данные` — полезная нагрузка (может занимать несколько строк).
...
```
Данные: Первая строка
Данные: Вторая строка
Данные: Третья строка
```
...
eventSource.onmessage = (event) => {
    console.log('Данные:', event.Данные);
};
...
eventSource.onmessage = (event) => {
    console.log('Получено сообщение:', event.Данные);
};

// Обработчик события с типом 'notification'
eventSource.addEventListener('notification', (event) => {
    const Данные = JSON.parse(event.Данные);
    console.log('Уведомление:', Данные);
    showNotification(Данные.title, Данные.body);
});

// Обработчик события с типом 'progress'
eventSource.addEventListener('progress', (event) => {
    const Данные = JSON.parse(event.Данные);
    updateProgressBar(Данные.percent);
});
...
const handlers = {
    'message': (Данные) => console.log('Сообщение:', Данные),
    'update': (Данные) => updateUI(Данные),
    'error': (Данные) => showError(Данные),
    'complete': (Данные) => {
        console.log('Завершено:', Данные);
        eventSource.close();
    }
};
...
public class EventsController : ControllerBase
{
    ...
    /// </summary>
    private async Task SendEventAsync(string eventType, object Данные, string id = null)
    {
        ...
        sb.AppendLine($"Данные: {Система.Text.Json.JsonSerializer.Serialize(Данные)}");
        sb.AppendLine();
...
def generate_sse_event(
    ...
    if Данные is not None:
        if isinstance(Данные, dict):
            data_str = json.dumps(Данные)
        else:
            data_str = str(Данные)
        
        # Многострочные данные
        for line in data_str.split('\n'):
            lines.append(f"Данные: {line}")
...
async def event_stream(request: Request, start_id: int = 0):
    ...
    yield generate_sse_event(
        ...
        Данные={"message": "Подключено к потоку событий", "start_id": start_id},
    ...
    try:
        ...
        yield generate_sse_event(
            event_type="error",
            Данные={"message": str(e)}
        )
    finally:
        # Финальное событие
        yield generate_sse_event(
            event_type="disconnected",
            Данные={"message": "Поток завершён"}
        )
...
async def notification_stream(request: Request):
    ...
    try:
        while True:
            ...
            new_notifications = [
                (nid, Данные) for nid, Данные in notifications_store.items()
                if nid > last_id
            ]
            
            for nid, Данные in sorted(new_notifications):
                yield generate_sse_event(
...
public class SseController {
    ...
    public Flux<ServerSentEvent<String>> getEvents() {
        return Flux.interval(Duration.ofSeconds(2))
            .map(sequence -> ServerSentEvent.<String>builder()
                ...
                .event("tick")
                .Данные(String.format("{\"sequence\":%d,\"timestamp\":%d}",
                    sequence, Система.currentTimeMillis()))
                .build());
    }
            ...
            .map(sequence -> {
                if (sequence == 0) {
                    ...
                    return ServerSentEvent.<String>builder()
                        .id(String.valueOf(sequence))
                        .event("message")
                        .Данные(String.format("{\"count\":%d,\"time\":%d}",
                            sequence, Система.currentTimeMillis()))
                        .build();
                }
            });
    ...
    public Flux<ServerSentEvent<String>> notificationsStream() {
        ...
        return sink.asFlux()
            .map(Данные -> ServerSentEvent.<String>builder()
                .Данные(Данные)
                .build())
    ...
    public void broadcastNotification(String type, String title, String body) {
        String message = String.format(
            ...
            type, title, body, Система.currentTimeMillis()
        );
            ...
            while (true) {
                try {
                    ...
                    broadcastNotification(
                        "info",
                        "Уведомление #" + count,
                        "Тестовое уведомление в " + Система.currentTimeMillis()
                    );
...
public class SseEmitterController {
    ...
    public SseEmitter handleSse() {
        ...
        String emitterId = "emitter-" + Система.currentTimeMillis();
        ...
        try {
            emitter.send(SseEmitter.event()
                .name("connected")
                .Данные("{\"status\":\"connected\",\"id\":\"" + emitterId + "\"}"));
        } catch (IOException e) {
            e.printStackTrace();
        }
            ...
            try {
                while (true) {
                    ...
                    emitter.send(SseEmitter.event()
                        ...
                        .name("tick")
                        .Данные("{\"count\":" + count + ",\"time\":" + Система.currentTimeMillis() + "}"));
                }
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
            } catch (IOException e) {
     ...
     */
    public void broadcast(String eventName, Object Данные) {
...
app.get('/events', (req, res) => {
    ...
    const interval = setInterval(() => {
        ...
        sendEvent(res, 'tick', Данные, eventId.toString());
...
app.get('/notifications', (req, res) => {
    ...
    notifications
        ...
        .forEach(notification => {
            sendEvent(res, 'notification', notification.Данные, notification.id.toString());
        });
...
app.post('/notify', express.json(), (req, res) => {
    ...
    console.log(`Новое уведомление #${notificationId}:`, notification.Данные);
 ...
 */
function sendEvent(res, eventType, Данные, id = null) {
    if (id) {
        res.write(`id: ${id}\n`);
    }
    ...
    const dataStr = typeof Данные === 'string' ? Данные : JSON.stringify(Данные);
    dataStr.split('\n').forEach(line => {
        res.write(`Данные: ${line}\n`);
    });
