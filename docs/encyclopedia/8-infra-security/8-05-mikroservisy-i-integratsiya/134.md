...
Сквозной сценарий с webhook и outbox — [mTLS, JWS-подпись webhooks и AsyncAPI с outbox](/encyclopedia/7-project/7-06-proektirovanie-i-arhitektura/design/1172).
...
Приложение читает `X-Client-Cert-Subject`, сопоставляет с таблицей `partnerId` (как в [mTLS, JWS-подпись webhooks и AsyncAPI с outbox](/encyclopedia/7-project/7-06-proektirovanie-i-arhitektura/design/1172#регистрация-клиента)). PEM в handler **не разбирают** — это уже сделал ingress.
...
**FastAPI** (сервер) — `OAuth2PasswordBearer`, `Depends(get_current_user)`; см. [Первая программа на FastAPI](/encyclopedia/5-languages/5-02-python/3432).
