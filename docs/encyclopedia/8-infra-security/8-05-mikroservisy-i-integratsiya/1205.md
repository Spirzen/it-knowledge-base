...
Разбор:
- Утилиты `bin/kafka-*.sh` (или `.bat` на Windows) управляют топиками и тестовой публикацией из каталога установки Kafka.
- `docker run -d` стартует контейнер в фоне; `-p host:container` пробрасывает порты AMQP и веб-UI на localhost.
- Команды Kafka создают топик с числом партиций и фактором репликации; переменные `KAFKA_*` настраивают single-node KRaft в Docker.
...
| `kafka-acls.sh --add` | `--allow-principal User:x --operation Read --topic T` | ACL |
| `kafka-mirror-maker.sh` | MM2 properties | Зеркалирование между кластерами |
...
| `delivery.timeout.ms` | | Общий дедлайн доставки (retry + ack) |
...
| `max.poll.interval.ms` | | Таймаут обработки batch; превышение → исключение из группы |
| `fetch.min.bytes` | | Минимум байт для ответа fetch (батчинг на broker) |
| `fetch.max.wait.ms` | | Макс. ожидание набора fetch.min.bytes |
| `fetch.max.bytes` | | Верхняя граница размера fetch-ответа |
| `max.partition.fetch.bytes` | | Лимит байт на партицию в одном fetch |
| `session.timeout.ms` | | Падение consumer → rebalance |
| `group.instance.id` | | Static membership — меньше rebalance при рестарте |
...
**Security (prod):** `listeners=SASL_SSL://...`, `authorizer.class.name=kafka.security.authorizer.AclAuthorizer`, `super.users=User:admin`.
...
Schema Registry (Confluent): Avro/Protobuf/JSON Schema для эволюции контрактов событий. Подробнее — [Kafka, раздел Schema Registry](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/119#kafka-schema-registry).

---

#### Schema Registry и сериализаторы

| Компонент | Назначение |
|-----------|------------|
| **Subject** | Имя схемы, обычно `<topic>-key` / `<topic>-value` |
| **Schema id** | Числовой id версии в реестре; в начале каждого Avro-сообщения |
| **BACKWARD / FORWARD / FULL** | Политика совместимости при регистрации новой версии |
| `KafkaAvroSerializer` | Java: регистрация схемы + сериализация |
| `schema.registry.url` | URL реестра в config producer/consumer |
| Apicurio / AWS Glue | Альтернативные реестры |

Wire format (Confluent): `0x00` + 4-byte schema id + Avro bytes.
...
**Метрики для алертов (prod):**

| Метрика | Порог / действие |
|---------|------------------|
| `UnderReplicatedPartitions` | > 0 дольше 5 мин → проверить брокер/диск/сеть |
| `OfflinePartitionsCount` | > 0 → критично, запись недоступна |
| Consumer lag (max по группе) | SLO команды (например, > 60 сек) |
| `RequestHandlerAvgIdlePercent` | Устойчиво низкий → CPU bottleneck |
| Producer `record-error-rate` | > 0 → retry, auth, quota |
| `unclean.leader.election.rate` | Должен быть 0 при `unclean.leader.election.enable=false` |
...
| Kafka как job queue без DLQ | Отдельный топик ошибок (`*.dlq`), retry с backoff |
| JSON без схемы в multi-team prod | Avro + Schema Registry, CI-проверка совместимости |
