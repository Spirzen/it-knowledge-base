...
| **Производительность** | `map` эффективнее `repeated KeyValue`, но менее гибок (нельзя добавить third-party metaДанные к записи). |
...
| **ALTS** | Application Layer Transport Безопасность (Google Cloud only) | `grpc++_alts` (C++), `io.grpc.alts` (Java). Не для публичных API. |
