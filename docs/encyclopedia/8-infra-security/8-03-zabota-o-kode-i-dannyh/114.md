...
Эта страница — **справочник подкоманд** с короткими пояснениями «когда вызывать». Сжатый набор из 12 команд — [4.13/115#12-komand](/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/115#12-komand). Логика трёх зон (рабочий каталог → индекс → коммит) — в [111](./111.md); флаги `--hard`, `--force`, `--staged` — в [115](./115.md); восстановление после ошибок — [101](./101.md) и [113](./113.md).
...
|--------|---------|------------|
| Начать проект локально | `git init`, `.gitignore`, `git add`, `git commit` | Затем `remote add` + push ([1](./1.md)) |
| Подключиться к чужому репо | `git clone <url>` | `--depth 1` для CI, полный clone для истории |
| Увидеть, что изменилось | `git status`, `git diff`, `git diff --staged` | Три зоны — [111](./111.md) |
...
| Откатить последний коммит | `git reset --soft HEAD~1` | `--hard` — [101](./101.md) |
| Пометить релиз | `git tag -a v1.0.0 -m "..."`, `git push origin v1.0.0` | Теги не push'ятся по умолчанию |
| Ошибка — найти коммит | `git reflog`, `git branch rescue HEAD@&#123;n&#125;` | [113](./113.md) |
...
**Когда:** новый проект с нуля, ещё нет remote. Инициализация создаёт `.git` в текущей папке ([112](./112.md)).
...
**Когда:** «отменить» коммиты или сбросить индекс. Режимы — [115](./115.md).
...
После `--hard` — сразу `git reflog`, не `git gc` ([101](./101.md)).
...
|--------|-------------|------------|
| `git add .` без просмотра | Секреты в коммите | `git status`, `.gitignore`, [117](./117.md) |
| `commit` на `main` напрямую | Нет review | feature-ветка + PR ([1111](./1111.md) или GitHub Flow) |
...
Подробнее про объекты и `.git` — [112](./112.md); флаги — [115](./115.md).
