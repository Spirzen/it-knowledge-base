...
<div class="callout callout--danger">
  ...
  2. **Никогда** `curl … | bash`, `wget -O - | bash`, `IEX (DownloadString …)` из интернета — сначала скачайте файл (`curl -o`), откройте в редакторе, потом запускайте. Безопасные паттерны CLI — [утилита curl](/encyclopedia/2-system-network/2-05-terminal/1133), [curl / fetch — примеры](/lab/Примеры/1133).
...
| 10 | "Удобный" алиас вроде `git undo` → `reset HEAD~1` | Привычка жать откат без понимания ([Внутреннее устройство Git](./112.md)) |
...
Не запускать `git gc --prune=now`. Подробнее — [Особенности работы с репозиториями в Git](./113.md).
...
4. Сменить пароли и токены, если скрипт мог их прочитать ([Методы защиты пользовательских и корпоративных данных](./117.md)).
...
Подробные сценарии и сроки хранения reflog — в статье [Особенности работы с репозиториями в Git](./113.md) и [Восстановление данных в Git](/encyclopedia/2-system-network/2-06-sistemnoe-administrirovanie/data-restoring/112). Краткий сценарий `reflog` + `reset --hard` с разбором — [лаборатория 1123](/lab/Примеры/1123#восстановление-через-reflog); карточки по симптомам — [4.13/1141](/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/1141).

<div class="callout callout--caution">
  <div class="callout-title">Когда reflog не спасёт</div>

  <div class="callout-body">
  Запись уже вышла из срока хранения reflog

  на remote сделали `push --force`, а у вас нет старого коммита

  удалили папку `.git` без бэкапа. Тогда остаются клон с другой машины, бэкап ([Методы защиты пользовательских и корпоративных данных](./117.md)) или обращение к коллегам.
</div>
  </div>
...
Резервные копии и шифрование — [Методы защиты пользовательских и корпоративных данных](./117.md); флаги Git — [Настройка и параметры Git](./115.md).
