...
description: Справочник по Grafana Mimir - долговременное хранение метрик, совместимость с Prometheus и горизонтальное масштабирование.
referenceType: tool
---
# Справочник по Mimir
...
```bash
# Prometheus-совместимый API (порт по конфигу, часто 9009)
curl -sf http://localhost:9009/prometheus/api/v1/status/buildinfo
```
