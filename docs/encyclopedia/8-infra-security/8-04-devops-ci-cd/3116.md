...
description: Справочник по Grafana — дашборды, источники данных, алертинг и provisioning для observability в CI/CD.
referenceType: tool
related:
...
CLI, конфигурация и типовые сценарии **Grafana** (DevOps, CI/CD, инфраструктура). Учебный курс: [раздел](/encyclopedia/8-infra-security/8-04-devops-ci-cd/intro). Пошаговый стенд — [Практикум Prometheus и Grafana](/encyclopedia/2-system-network/2-06-sistemnoe-administrirovanie/prometheus-grafana-praktikum/intro).

---

## Краткое пояснение

**Grafana** — платформа визуализации и алертинга поверх метрик, логов и трассировок (Prometheus, Loki, Tempo, Elasticsearch и др.). В DevOps её используют для дашбордов после деплоя, SLO/SLI, on-call и проверки релизов; конфигурацию часто выносят в Git (provisioning) и прогоняют через CI.
...
- [Источники данных (Data Sources)](#источники-данных-data-sources)
- [Дашборды](#дашборды)
- [Панели](#панели)
- [Переменные (Variables)](#переменные-variables)
- [Запросы (Queries)](#запросы-queries)
- [Аннотации](#аннотации)
- [Оповещения (Alerting)](#оповещения-alerting)
- [Конфигурация Grafana (grafana.ini)](#конфигурация-grafana-grafanaini)
...
- [Темы оформления (Themes)](#темы-оформления-themes)
- [Плагины](#плагины)
- [Совместная работа и обмен](#совместная-работа-и-обмен)
- [Шаблонизация и повторное использование](#шаблонизация-и-повторное-использование)
- [Продвинутые техники запросов](#продвинутые-техники-запросов)
- [Экспорт и внедрение](#экспорт-и-внедрение)
...
- **Данные source** — выбор источника данных из списка.
...
- **Type** — тип переменной.
- **Данные source** — источник данных (для Query-переменных).
...
- **Max Данные points** — ограничение количества точек на графике.
...
- **Данные source annotations** — автоматические события из источника (например, деплои из CI/CD).

---

#### Свойства аннотации
- **Name** — имя аннотации.
- **Данные source** — источник данных.
...
grafana-cli plugins install <plugin-id>                      # установить
grafana-cli plugins update <plugin-id>                       # обновить
grafana-cli plugins ls                                       # установленные
...
- **Данные source plugins** — подключение новых источников данных.
- **Panel plugins** — новые типы визуализаций.

---

#### Установка плагинов
- Через CLI — см. блок `grafana-cli` выше
...
- **Filter Данные by values** — фильтрация строк.
...
| Область | Примечание |
|---------|------------|
| Версии | актуальные LTS/стабильные релизы **Grafana** |
...
**Справочники** — [Справочник по Prometheus](/encyclopedia/8-infra-security/8-04-devops-ci-cd/3115), [Справочник по Elasticsearch](/encyclopedia/8-infra-security/8-04-devops-ci-cd/3117), [Справочник по Jenkins](/encyclopedia/8-infra-security/8-04-devops-ci-cd/3114), [Справочник по AWS](/encyclopedia/8-infra-security/8-04-devops-ci-cd/3118), [Справочник по GitHub Actions](/encyclopedia/8-infra-security/8-04-devops-ci-cd/3113), [Справочник по Logstash](/encyclopedia/8-infra-security/8-04-devops-ci-cd/3119).
