...
referenceType: tool
description: Terraform — инструмент для декларативного управления инфраструктурой как кодом (Infrastructure as Code, IaC). Он позволяет описывать целевую инфраструктуру в текстовых файлах, применять изменения безопасно и воспроизводи
...
Результат доступен как `Данные.aws_ami.ubuntu.id`.
...
resource "aws_rds_cluster" "main" {
  master_password = jsondecode(Данные.aws_secretsmanager_secret_version.db_password.secret_string)["password"]
}
```
...
resource "aws_instance" "web" {
  subnet_id = Данные.terraform_remote_state.Сеть.outputs.public_subnet_id
}
```
...
check "database_reachable" {
  ...
  assert {
    condition     = Данные.http.db_health.status_code == 200
