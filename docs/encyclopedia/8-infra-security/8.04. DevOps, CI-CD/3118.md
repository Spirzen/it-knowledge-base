...
referenceType: tool
description: AWS (Amazon Web Services) — это обширная облачная платформа, предоставляющая сотни сервисов и тысяч настроек, параметров, свойств и значений. Полный справочник по AWS невозможно уместить в один ответ, но можно создать ст
...
| Задача | Подсказка |
|--------|-----------|
| Установка | официальная документация **AWS** |
...
- [Данные Exchange](#данные-exchange)
...
- **Безопасность Group** — stateful firewall
- **Сеть ACL** — stateless firewall на уровне подсети

#### Свойства Безопасность Group
...
- Генерация Данные Key через `GenerateDataKey`
...
#### Trusted Access
- Разрешает AWS-сервисам управлять ресурсами в дочерних аккаунтах (например, AWS Backup, Безопасность Hub)
...
- **Данные Sharing** — совместное использование данных между кластерами
...
#### Основные компоненты
- **Crawlers** — сканируют источники и обновляют Данные Catalog
- **Данные Catalog** — метаданные таблиц и баз
...
- Безопасность Hub → централизованная панель
...
- **Данные Events** — S3 object-level, Lambda invocations
...
- **Migration Type**:
  - Full load
  - Full load + CDC (Change Данные Capture)
...
- **Данные Repository Association** — связь с S3 bucket
...
- Service Catalog
- IAM Identity Center
- CloudTrail, Config, Безопасность Hub
...
- **Channels** → **Pipelines** → **Данные Stores** → **Datasets**.
