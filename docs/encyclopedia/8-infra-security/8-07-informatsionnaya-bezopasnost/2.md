...
Иерархическая база данных конфигурационных параметров операционной системы, приложений и оборудования. Структура представлена в виде ветвей (hives), ключей и значений. Физически хранится в системных файлах (например, `%SystemRoot%\System32\config\` — `СИСТЕМА`, `SOFTWARE`, `SAM`, `БЕЗОПАСНОСТЬ`, `DEFAULT`) и в `%UserProfile%\NTUSER.DAT`. Реестр загружается в память при старте системы диспетчером конфигурации (`Cm`), компонентом ядра `ntoskrnl.exe`.
...
Windows использует модель **Безопасность identifiers (SID)** и **access tokens** для управления доступом. Привилегии (`Privileges`) — это права, назначаемые токенам (например, `SeDebugPrivilege`, `SeBackupPrivilege`). Контексты выполнения процессов отличаются по уровням доверия и возможностям:

| Контекст | SID | Примечания |
|---------|-----|------------|
| **СИСТЕМА** (`NT AUTHORITY\СИСТЕМА`) | `S-1-5-18` | Максимальные привилегии в локальной системе; запускает критические службы (`lsass`, `winlogon`, драйверы). Имеет `SeTcbPrivilege`, `SeDebugPrivilege`, `SeTakeOwnershipPrivilege` и др. |
...
- **Integrity Levels** (Mandatory Integrity Control): `Low`, `Medium`, `High`, `Система`. Ограничивают доступ даже при наличии DACL-разрешений (например, IE в режиме защищённого режима — `Low IL`).
...
:: Службы, запущенные с привилегиями СИСТЕМА или LocalService с автозапуском
...
| **Система** | `Система`, `HardwareEvents` | События драйверов, служб, ядра ОС |
| **Безопасность** | `Безопасность` | Аудит безопасности — входы, доступ к объектам, изменения политик и пр. (требует включения аудита через групповые политики или `auditpol`) |
...
- **Командная строка**: `wevtutil.exe` (например, `wevtutil qe Система /f:text /c:5`)
- **PowerShell**: `Get-WinEvent -LogName Система -MaxEvents 5`
...
Ключевые журналы: `Безопасность`, `Система`, `Application`. Особое внимание — событиям с ID:
...
```powershell
Get-WinEvent -LogName Безопасность -FilterXPath "*[Система[EventID=4688]]" -MaxEvents 1000 | 
...
- Расширенные ACL (`setfacl`):
  ...
  setfacl -d -m g:dev:rw- /srv/Данные      # установить умолч. ACL для новых файлов
...
$ace = New-Object Система.Безопасность.AccessControl.FileSystemAccessRule("Timur","Read","Allow")
...
ETALON_DIR="/opt/Безопасность/etalon"
REPORT="/var/log/Безопасность/audit_$(date +%F_%H%M).log"
...
Безопасность — повторяющийся цикл. Его этапы не всегда строго последовательны (например, реагирование может инициировать новый сбор данных), но логически связаны. В практике он реализуется как *конвейер обработки событий безопасности* (Безопасность event pipeline).
...
| **Аудиторские журналы** | `auth.log`, `Безопасность.evtx`, `auditd`-логи | Единственный объективный источник о действиях; их удаление/подмена — признак скрытой компрометации. |
...
- `HKLM\СИСТЕМА\CurrentControlSet\Services` — службы (проверка `ImagePath`, `Start`),  
- `HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\RunMRU` — недавние запуски,  
- `HKLM\БЕЗОПАСНОСТЬ\Policy\Secrets` — учётные данные (доступны только СИСТЕМА).
...
- **XML**: `xmllint`, `xmlstarlet`.  
  ...
  xmlstarlet sel -t -v "//Event/Система/EventID" events.xml
  ```
...
- **RCE (Remote Code Execution)**: `; cat /etc/passwd`, `` `id` ``, `${IFS}`, `Система(`  
  ```bash
...
IOC_FILE="/opt/Безопасность/iocs.txt"  # список IP, хешей, сигнатур
...
- затем запуск в `strace -f -e trace=Сеть ./malware.bin` → подтверждение сетевого вызова.
...
for key in range(256):
    if Данные[0] ^ key == 0x4d and Данные[1] ^ key == 0x5a:
        print(f"Possible XOR key: 0x{key:02x}")
        # Расшифровываем и сохраняем
        decrypted = bytes(b ^ key for b in Данные)
...
Тестирование на проникновение (penetration Тестирование, пентест) — это *контролируемая имитация атаки*, проводимая с целью верификации эффективности защитных мер. Это *проверка гипотез угроз*.
...
ALERTS="/var/log/Безопасность/alerts.log"
...
comm -23 <(sort /tmp/alert_candidates.txt | cut -f2) <(sort /etc/Безопасность/trusted_ips.txt) > /tmp/malicious_ips.txt
...
- **Linux**: `cron`  
  ```bash
  # Каждые 5 минут
  */5 * * * * /opt/Безопасность/ssh_bruteforce_pipeline.sh
  ```
- **Windows**: `schtasks`  
  ```cmd
...
- **Централизованное управление конфигурацией**: Ansible, SaltStack, Puppet.  
  ...
  ```yaml
  - name: Deploy Безопасность audit script
    hosts: all
    Задачи:
      - copy:
          src: validateconfig.sh
          dest: /opt/Безопасность/validateconfig.sh
          mode: '0700'
      - cron:
          name: "Безопасность audit"
          job: "/opt/Безопасность/validateconfig.sh >> /var/log/Безопасность/audit.log 2>&1"
          minute: "0"
          hour: "2"
  ```
