...
- **WebSocket** от `orders-api` к клиенту — **push** без опроса: статус заказа меняется редко, но UI должен узнать сразу. Подробнее о выборе транспорта — в [Реактивная коммуникация](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/116).
...
Код появится в шагах [Практикум — сервис каталога на Python](./4.md) и [Практикум — сервис заказов на C#](./5.md). Следующий шаг — **контракт API** без привязки к языку: [Проектирование контракта](./2.md).
