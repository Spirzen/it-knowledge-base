...
tags: [reference, advanced, notrequired, engineer, developer, architector, docker, kubernetes]
---
...
| Задача | Команда |
|--------|---------|
| Кластер (minikube) | `minikube start` |
| Применить манифест | `kubectl apply -f deploy.yaml` |
| Поды / логи | `kubectl get pods` / `kubectl logs <pod>` |
| Порт | `kubectl port-forward svc/app 8080:80` |
...
#### Компоненты Данные Plane (рабочей плоскости)

Данные Plane состоит из узлов (Node), на которых выполняются рабочие нагрузки.
...
- `kube-Система` — системные компоненты
...
Запускает **по одному Pod’у на каждом подходящем Node**. Используется для системных агентов: log collectors, monitoring agents, Сеть plugins.
...
- `Данные` — текстовые значения (UTF-8)
...
- `pod-Безопасность.kubernetes.io/enforce: baseline`
- `pod-Безопасность.kubernetes.io/warn: restricted`
- `pod-Безопасность.kubernetes.io/audit: privileged`
...
meta
  name: secure-app
  labels:
    pod-Безопасность.kubernetes.io/enforce: restricted
    pod-Безопасность.kubernetes.io/enforce-version: latest
```
...
Устанавливается как Deployment в `kube-Система`.
...
Kubernetes требует реализации **Container Сеть Interface (CNI)** для обеспечения сетевой связи между Pod’ами.
