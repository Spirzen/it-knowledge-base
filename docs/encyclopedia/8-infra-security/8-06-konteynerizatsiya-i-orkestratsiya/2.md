...
referenceType: tool
description: Docker — это платформа для создания, запуска и управления контейнеризированными приложениями. Контейнеры изолируют процессы и зависимости, обеспечивая единообразие выполнения в разных средах. Docker использует образы как
tags: [reference, advanced, notrequired, engineer, developer, architector, docker]
---
...
**Сеть**
- `--Сеть`: указывает сеть (`bridge`, `host`, имя пользовательской сети)
...
- `--Безопасность-opt`: настройки безопасности (`seccomp=profile.json`, `apparmor=unconfined`)
...
Пример:
```bash
docker Сеть create --driver bridge --subnet=172.20.0.0/16 mynet
```

##### `docker Сеть connect` / `disconnect`
...
services:
  ...
  db:
    ...
    volumes:
      - db_data:/var/lib/postgresql/Данные
...
##### Хранилище
- `Данные-root`: директория для хранения образов, контейнеров, томов (по умолчанию `/var/lib/docker`)
...
#### Типы плагинов
- **Сеть plugins**: Calico, Weave, Flannel
...
- Команда: `docker Система df`
- Очистка: `docker Система prune -a --volumes`
...
- Временное решение: `--Безопасность-opt label=disable`
