...
Метка `app: my-app` в `selector` **обязана совпадать** с `template.metadata.labels` в Deployment. Service получает стабильный DNS: `my-app-service` (в namespace `default` — `my-app-service.default.svc.cluster.local`). **kube-proxy** (или аналог в CNI) направляет трафик на IP подов; список адресов хранится в **Endpoints** / **EndpointSlice** (`kubectl get endpoints my-app-service`).
...
★ **Цикл согласования (reconcile).** Вы задаёте *желаемое* состояние (`replicas: 2`). Control plane сравнивает его с *фактическим* и через контроллеры создаёт или удаляет Pod'ы, пока расхождение не исчезнет. Поэтому после `apply` достаточно смотреть `kubectl get pods`, а не «запускать контейнер вручную».
