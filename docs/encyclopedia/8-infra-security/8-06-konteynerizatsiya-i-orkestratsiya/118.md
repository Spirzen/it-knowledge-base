...
| Установка Docker | Engine + Compose plugin (см. [официальную установку](https://docs.docker.com/engine/install/)). | `sudo apt install docker-ce docker-compose-plugin` Группа: `sudo usermod -aG docker $USER` → `newgrp docker` |
...
| Сборка образа | Соберите образ из Dockerfile или через Compose. | `docker build -t <имя>:<тег> .` или `docker compose up --build` |
