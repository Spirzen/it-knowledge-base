...
<ExternalPlayEmbed example="infra-security/infra-lifecycle-play" title="Путь кода к пользователю" minHeight={400} />

<ExternalPlayEmbed example="infra-security/dev-staging-prod-play" title="dev / staging / prod" minHeight={420} />

<ExternalPlayEmbed example="about/infra-diagram-studio" title="Infra Diagram Studio" minHeight={560} />

Рано или поздно, в IT-сфере, вам придётся столкнуться с организацией инфраструктуры. Проект уйдёт далеко за пределы "Hello World" и одной небольшой папочки. Это будет множество серверов, программ, систем (и конечно же людей), и всё будет взаимодействовать различными способами.

Данный раздел задуман как раз для обучения самой продвинутой и сложной части - она выходит за пределы проектирования и администрирования, включает современные технологии, о которых и будем говорить далее.
...
Перед углублением в балансировку, репликацию и другие архитектурные паттерны полезно прочитать [Основы развития информационных систем](./2.md): там объясняется эволюция от MVP до масштабирования и поэтапных миграций.

---

## Что важно понять в самом начале

Инфраструктура это операционная среда всего продукта.

Любая функция в коде получает ценность только тогда, когда:
- она доступна пользователю в нужный момент;
- данные сохраняются и восстанавливаются;
- релиз можно безопасно доставить и, при необходимости, откатить;
- инциденты быстро диагностируются и устраняются;
- доступы и секреты не превращаются в уязвимости.

Получается, что инфраструктура как раз и представляет собой совокупность этих условий, при которых ваш код вообще работает и приносит при этом пользу:
- доступность;
- память;
- поставка;
- здоровье;
...
Для всего этого выделяются колоссальные средства, силы и конечно человеческие ресурсы.

Поэтому инфраструктура нужна на старте разработки и в ежедневной эксплуатации.

---

## Базовые принципы инфраструктурного мышления

В отличие от простых приложений вроде "Hello World", которые мы пишем локально на своих рабочих устройствах, когда речь заходит о крупном приложении, требуется его не просто развернуть и запустить, но также заботиться о нём, развивать и масштабировать.

Представьте, что вы развернули приложение, и...захотели обновить. Что будем делать?
- вы развернули приложение на мощном сервере;
- подключили базу данных, настроили и убедились, что всё работает;
- но для обновления вам придётся выключать сервер, ставить новую версию - и всё это время система не будет работать, а пользователи вынуждены ждать.
- и если обновление получилось кривым, вам придётся вручную останавливать всё и откатывать на старую версию.

А ещё хуже - когда этот единственный сервер...умер. Просто сломался физически. У вас нет ни запасных копий, ни возможности переноса...

И всё это решает грамотный подход к планированию инфраструктуры с использованием современных технологий - масштабирование, репликация, шардирование, контейнеризация, оркестрация и прочее.

Давайте для начала определим основные базовые принципы.

### 1. Проектировать путь к масштабированию заранее

Даже если система маленькая, закладывайте:

- разделение сред (`dev/staging/prod`);
- конфигурацию через environment;
- статeless-подход там, где это возможно;
- наблюдаемость с первых релизов;
- автоматизацию повторяемых операций.

Однажды ваше приложение вырастет. Пользователей, данных и нагрузки станет больше.

### 2. Усложнять стек по сигналам, а не "по моде"

Переход к шардированию, сервисной сетке и сложной оркестрации оправдан только при измеренном узком месте, а не "на будущее". Преждевременное усложнение делает систему дорогой и хрупкой.

Да, это получается в противовес предыдущему - если вы как раз-таки слишком далеко смотрите и думаете, что рост будет так скоро - можно переборщить и сделать хуже.

### 3. Эксплуатация важнее красивой схемы

Хорошая архитектура на диаграмме не спасает без runbook, мониторинга, резервного копирования, ротации секретов и четкой модели доступов.

> **Runbook** - это простая шпаргалка (инструкция) для дежурного инженера, которая говорит "Если случилась беда - делай строго вот это, чтобы починить".

Это частая проблема, когда всё висит на одном администраторе, и всё падает именно, когда он уходит в отпуск.

### 4. Надежность - командная ответственность

За устойчивость отвечают не только DevOps или системные администраторы. Разработчики, QA, аналитики, архитектор и безопасность совместно формируют операционную зрелость продукта.

Поэтому и ошибки в архитектуре могут быть критичными, и стоить много, очень много денег в потерях.

---

## Инфраструктура и системное администрирование где пересечение

В практике инфраструктуры есть слой, который напрямую опирается на системное администрирование:

| Область | Что делает администратор/инженер | Что получает продукт |
|---------|----------------------------------|----------------------|
| ОС и хосты | hardening, патчи, управление сервисами | Стабильная и безопасная среда |
| Сеть | DNS, маршрутизация, firewall, VPN | Предсказуемая доступность |
| Доступы | роли, SSH-ключи, аудит | Контроль и трассируемость действий |
| Носители и данные | снапшоты, backup, restore drill | Защита от потери данных |
| Наблюдаемость | агенты логов/метрик, алертинг | Быстрое обнаружение деградации |
| Инциденты | triage, mitigation, postmortem | Быстрое восстановление сервиса |

Никто не запрещает администратору обладать DevOps-навыками, знать языки программирования, а программисту-разработчику уметь администрировать системы.

Но разница здесь принципиальная - администраторам, как правило, платят сильно меньше. И на практике, к сожалению, на системного администратора часто ложится хозяйственная часть - закупки оборудования, инвентаризация, физическое оборудование, включение, поддержка, включая не только компьютеры и сервера, но и принтеры и прочую технику.

Если хотите глубже разобрать админский слой, переходите в [Системное администрирование — о разделе](/encyclopedia/2-system-network/2-06-sistemnoe-administrirovanie/intro).

---

## Инфраструктурная зрелость по этапам роста

| Этап | Минимум, который должен быть | Что будет болеть, если не сделать |
|------|-------------------------------|------------------------------------|
| MVP | `dev/staging/prod`, CI, backup, базовый мониторинг | Случайные простои и риск потери данных |
| Первые пользователи | кэш, балансировка, централизованные логи | Деградация на пиках и длинные инциденты |
| Масштабирование | SLO, autoscaling, репликация, runbook | Нестабильные релизы и непредсказуемый SLA |
| Зрелый контур | IaC, GitOps, DR-практика, security automation | Высокая стоимость ошибок и ручной труд |

---

## Как связать инфраструктуру с архитектурой раздела 7

Эта глава дает операционный фундамент, а архитектурные решения раскрываются в 7-й главе:

- системная рамка и NFR: [Проектирование и архитектура — о разделе](/encyclopedia/7-project/7-06-proektirovanie-i-arhitektura/intro);
- карта масштабирования и типовые паттерны: [System Design — карта тем](/encyclopedia/7-project/7-06-proektirovanie-i-arhitektura/143);
- шпаргалка по кэшу, CDN, балансировке, шардированию: [12 концепций распределённой архитектуры](/encyclopedia/7-project/7-06-proektirovanie-i-arhitektura/141);
- инфраструктурная перспектива роста от MVP: [Основы развития информационных систем](./2.md).

Практический порядок чтения для целостной картины:

1. [Эта глава](./1.md) — "что такое инфраструктура и как она работает";
2. [Основы развития ИС](./2.md) — "почему система усложняется и как это контролировать";
3. [7.06 intro](/encyclopedia/7-project/7-06-proektirovanie-i-arhitektura/intro) + [143](/encyclopedia/7-project/7-06-proektirovanie-i-arhitektura/143);
4. профильные блоки 8.04/8.05/8.06/8.07 по вашей роли.

---

## От кода до пользователя

Когда вы запускаете `npm start` на ноутбуке, вы одновременно играете роли разработчика, системного администратора и сетевого инженера. В продакшене роли распределены по команде. Каждый шаг документируется и автоматизируется.
...
- Добавьте `.gitignore` для секретов и служебных файлов.
- Подключите проверку секретов в CI.
- Храните секреты в [Vault](/encyclopedia/8-infra-security/8-14-praktikum-vault/intro) или в cloud secrets manager.
- Посмотрите отчеты GitGuardian, чтобы увидеть реальные сценарии утечек.

### 2. Деплой сразу в prod

Без staging, без отката, без canary. Один баг — простой для всех пользователей.

- Используйте [стратегии развертывания](/encyclopedia/8-infra-security/8-04-devops-ci-cd/111).
- Для начала выбирайте rolling update или blue-green.
- Держите готовый rollback и проверяйте его на staging.
...
- Используйте [практикум DR](/encyclopedia/8-infra-security/8-15-praktikum-dr/intro) для тренировки сценариев аварии.
...
- Стартуйте с монолита или модульного монолита.
- Переходите к микросервисам только при подтвержденной нагрузке и зрелой команде.
- Сверьтесь с материалом [Первые шаги к микросервисам](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/101).

### 5. Устаревшие зависимости

Уязвимые пакеты из npm/PyPI. Log4Shell показал, что одна библиотека затрагивает весь стек.

- Включите контроль зависимостей и автообновления.
- Введите [Supply chain практики](/encyclopedia/8-infra-security/8-12-aktualnye-praktiki/1).
- Формируйте SBOM для важных сервисов.

### 6. Нет мониторинга

Команда узнает о простое от пользователей в Twitter вместо системного алерта.

- Добавьте endpoint health-check.
- Настройте uptime monitoring.
- Следите за базовыми метриками `5xx` и latency.

### 7. SSH по паролю на весь интернет

Брутфорс и ботнеты сканируют порт 22 постоянно.

- Используйте вход по SSH-ключам.
- Ограничьте доступ через VPN или bastion host.
- Добавьте fail2ban и аудит входов.
- Углубление в [8.07](/encyclopedia/8-infra-security/8-07-informatsionnaya-bezopasnost/intro).
...
Инфраструктура микросервисов требует дополнительных компонентов.

- service mesh или API Gateway;
- распределенный tracing;
- централизованные логи.

Подробно смотрите [8.05](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/intro) и [8.12/9 API Gateway](/encyclopedia/8-infra-security/8-12-aktualnye-praktiki/9).
...
- [Основы развития информационных систем](./2.md)
