...
В [обзорной статье](/encyclopedia/8-infra-security/8-02-low-code-no-code/1) вы уже видели демо-блок с кнопками и полями. Здесь — **тот же принцип в разборе**: один HTML-файл имитирует no-code конструктор (панель блоков, холст, модальное редактирование, экспорт готовой страницы). Это не коммерческая платформа вроде Bubble, а **учебная модель**, чтобы понять, из чего состоят настоящие NCP: состояние (массив элементов), рендер в DOM, события, локальное хранилище.
...
<div class="callout callout--info">
  ...
  <p>Сначала пробегите <strong>Архитектуру</strong> и <strong>структуру данных</strong>, затем откройте полный код и найдите в нём класс <code>PageBuilder</code>. Пошаговые заголовки ниже соответствуют методам класса.</p>
</div>
