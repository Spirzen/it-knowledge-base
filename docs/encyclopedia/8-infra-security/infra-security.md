---
title: 8. Инфраструктура и безопасность - о разделе
description: Инфраструктура и безопасность ПО - облачные платформы, DevOps практики, контейнеризация, микросервисная архитектура и методы защиты промышленных систем.
sidebar_label: 8. Инфраструктура и безопасность - о разделе
slug: /encyclopedia/8-infra-security/infra-Безопасность
id: infra-Безопасность
---

<div class="article-tags">
  <span class="tag tag-required">ОБЯЗАТЕЛЬНО</span>
  <span class="tag tag-beginner">ДЛЯ НОВИЧКОВ</span>
  <span class="tag tag-inprogress">В РАЗРАБОТКЕ</span>
</div>

import DocCardList from '@theme/DocCardList';

---

## О разделе

<DocCardList />

Сквозная схема **bare metal → ВМ → контейнеры → контейнеры в облачных ВМ** — [четыре модели развёртывания](/encyclopedia/2-system-network/2-02-platformy/21#chetiryre-modeli-razvertyvaniya) (базовая информатика и софт power user).

Текущая часть не для каждого. Не каждому аналитику нужно знать контейнеризацию, не каждому разработчику надо углубляться в облачные технологии.

```mermaid
mindmap
  root((Инфраструктура и безопасность))
    Облачные и визуальные решения
      8.01 Облачные технологии
      8.02 Low-code, No-code
    Управление кодом и данными
      8.03 Забота о коде и данных
      8.04 DevOps, CI-CD
    Архитектура систем
      8.05 Микросервисы и интеграция
      8.06 Контейнеризация и оркестрация
    Безопасность
      8.07 Информационная безопасность
```
