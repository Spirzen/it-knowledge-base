...
Цель юнит-тестирования — не обнаружение всех возможных ошибок (это невозможно в принципе), а *подтверждение корректности реализации ожидаемого поведения* для заданного набора входных условий. Тест не проверяет, «всё ли работает», а проверяет: «*при таких-то входных данных и внутреннем состоянии модуль возвращает именно такой результат, как было заявлено в его спецификации*». Таким образом, юнит-тест — это форма *исполняемой спецификации*: он фиксирует не только *что* должно происходить, но и *как именно* это реализовано и проверяется.
...
3. **Проверка (Assert)**  
   Сравнивается фактический результат (возвращённое значение, изменённое состояние объекта, выброшенное исключение) с ожидаемым. Утверждения должны быть точными: не «не равно нулю», а «равно 42»; не «строка не пустая», а «строка равна "OK"». Использование неточных проверок снижает ценность теста.
...
JUnit остаётся стандартом де-факто в экосистеме Java не из-за отсутствия альтернатив, а благодаря глубокой интеграции в инструментальную цепочку и устойчивому сообществу. JUnit 5 (Jupiter) представляет собой не просто обновление, а перепроектирование с нуля. Архитектура разделена на три независимых модуля:
...
PyTest — не форк `unittest`, а независимая реализация, ставшая фактическим стандартом благодаря своей выразительности и расширяемости. Главное отличие — отказ от наследования от `TestCase`. Тесты пишутся как обычные функции, что упрощает композицию и повторное использование.
...
Встроенная поддержка *assert rewriting* позволяет использовать стандартный оператор `assert` вместо специфичных методов (`self.assertEqual`). При падении теста PyTest показывает не просто «assertion failed», а детальное сравнение структур, включая diff для строк и деревьев.
...
*Snapshot testing* — не замена юнит-тестам, а дополнение. Он фиксирует сериализованное представление вывода (например, React-компонента) и при последующих запусках сравнивает его с сохранённой «снимковой» версией. Это эффективно для контроля неожиданных изменений в UI-логике, но требует ручного аудита при обновлении снимков.
...
Структура теста в RSpec — иерархия `describe` (контекст) → `context` (подконтекст) → `it` (пример поведения). Это позволяет выразить не просто «метод возвращает 5», а «при пустом входном списке метод возвращает 0, а при наличии элементов — их сумму». Такой подход превращает тесты в исполняемую документацию.
