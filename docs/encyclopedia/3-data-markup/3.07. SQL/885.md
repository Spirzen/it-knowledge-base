...
description: categories → categories — иерархическая связь через parent_id (1 ко многим)
---
...
-- ALTER СИСТЕМА SET shared_buffers = '4GB';
...
```sql
UPDATE users
SET Данные = jsonb_set(Данные, '{last_login}', to_jsonb(CURRENT_TIMESTAMP))
WHERE id = 42;
```
...
```sql
UPDATE users
SET Данные = Данные - 'temporary_token'
WHERE id = 42;
```
...
```sql
ALTER СИСТЕМА SET work_mem = '256MB';
SELECT pg_reload_conf();
```
