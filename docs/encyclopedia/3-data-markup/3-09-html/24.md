...
Для локализации плеера применяйте глобальные атрибуты:
- `lang` — язык контейнера;
- `dir` — направление текста интерфейса;
- `translate="no"` — метки и термины, которые нельзя переводить автоматически.
...
<ExternalCodeEmbed example="javascript/html-24-001" title="Базовый API элемента" minHeight={570} />
