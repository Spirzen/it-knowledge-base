...
description: Streams Pool — используется Oracle Streams (устаревшая технология).
---
...
- **Память (Система Global Area, SGA)**  
- **Процессы (Background и Server Processes)**

#### Система Global Area (SGA)
...
- **Shared Pool** — содержит:
  ...
  - Данные Dictionary Cache (метаданные о структуре БД)
...
- **SMON (Система Monitor)** — выполняет восстановление экземпляра, коалесценцию свободного пространства.
...
| `DB_UNIQUE_NAME` | Уникальное имя в Данные Guard конфигурации. |
...
## 6. Системные представления (Данные Dictionary Views)
...
### 15.3. Transparent Данные Encryption (TDE)
...
### 17.1. Oracle Данные Guard

**Данные Guard** обеспечивает защиту от сбоев через поддержание одной или нескольких резервных копий базы данных.
...
```sql
ALTER СИСТЕМА KILL SESSION 'sid,serial#' IMMEDIATE;
```
...
```sql
ALTER СИСТЕМА SET RESOURCE_MANAGER_PLAN = 'DAY_PLAN';
```
...
- **Данные Pump migration** — экспорт/импорт через `expdp`/`impdp`.
...
### 25.3. Данные Pump (expdp/impdp)

Экспорт схемы:

```bash
expdp Система/password DIRECTORY=dp_dir DUMPFILE=hr.dmp SCHEMAS=hr
```

Импорт с переносом:

```bash
impdp Система/password DIRECTORY=dp_dir DUMPFILE=hr.dmp REMAP_SCHEMA=hr:hr_new
```
