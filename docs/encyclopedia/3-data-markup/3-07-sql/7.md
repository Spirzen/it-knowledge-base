...
**Логический порядок в запросе** (упрощённо): `FROM` → `WHERE` → `GROUP BY` → `HAVING` → **оконные функции (`OVER`)** → `SELECT` → `ORDER BY` → `LIMIT`. Поэтому оконные выражения нельзя использовать в `WHERE` того же уровня — только в `SELECT` или внешнем запросе.
...
```
**`frame_clause`** — какие строки окна участвуют в расчёте:

| Рамка | Смысл |
| :--- | :--- |
| `ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW` | Накопительный итог |
| `ROWS BETWEEN 2 PRECEDING AND CURRENT ROW` | Текущая и две предыдущие |
| `RANGE …` | По значению столбца сортировки |
