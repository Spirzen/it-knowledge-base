...
Для **сравнения одного и того же SQL на SQLite, Oracle, MySQL и PostgreSQL** (схема TABLE1–3, `ALTER`, JOIN) — отдельная компактная страница [892](./892.md).
...
"Битые" ссылки — заказы без пользователя, позиции без заказа. **`NOT IN`** опасен при `NULL` в подзапросе ([109](./109.md)). Надёжнее `NOT EXISTS`:
