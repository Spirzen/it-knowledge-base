...
description: "Связи между таблицами: - categories → categories: иерархическая связь через parent_id (1 ко многим) - categories → products: одна категория содержит много товаров (1 ко многим) - users → orders: один."
---
...
Это **шпаргалка рецептов**: одна учебная схема и типовые задачи — от `SELECT` до администрирования PostgreSQL. С нуля — по [дорожной карте](./intro.md); здесь удобно искать "как сделать X", когда основы пройдены. Те же сценарии магазина **с разбором каждой строки** и проверкой результата — [SQL — реальные кейсы](/lab/Примеры/1152).

Для **сравнения одного и того же SQL на SQLite, Oracle, MySQL и PostgreSQL** (схема TABLE1–3, `ALTER`, JOIN) — отдельная компактная страница [Шпаргалка SQL — четыре СУБД на одной схеме](./892.md).

По умолчанию **PostgreSQL**, если не указано иное.
...
<ExternalCodeEmbed example="sql/sql-885-001" title="Связь многие-ко-многим (M:N)" minHeight={336} />
...
<ExternalCodeEmbed example="sql/sql-885-002" title="Подзапрос в FROM как временная таблица" minHeight={318} />
...
<ExternalCodeEmbed example="sql/sql-885-003" title="Подзапрос в JOIN для виртуальной таблицы" minHeight={318} />
...
<ExternalCodeEmbed example="sql/sql-885-004" title="Множественные CTE в одном запросе" minHeight={480} />
...
<ExternalCodeEmbed example="sql/sql-885-005" title="Рекурсивный CTE для иерархических данных" minHeight={606} />
...
<ExternalCodeEmbed example="sql/sql-885-006" title="Рекурсивный CTE для поиска всех предков узла" minHeight={426} />
...
<ExternalCodeEmbed example="sql/sql-885-007" title="Первый и последний элемент в окне" minHeight={318} />
...
<ExternalCodeEmbed example="sql/sql-885-008" title="Функция с табличным возвратом" minHeight={570} />
...
<ExternalCodeEmbed example="sql/sql-885-009" title="Процедура с параметрами и транзакцией" minHeight={720} />
...
<ExternalCodeEmbed example="sql/sql-885-010" title="Функция с обработкой исключений" minHeight={390} />
...
<ExternalCodeEmbed example="sql/sql-885-011" title="Функция для массовой обработки записей" minHeight={720} />
...
<ExternalCodeEmbed example="sql/sql-885-012" title="Управление enum-типами" minHeight={336} />
...
<ExternalCodeEmbed example="sql/sql-885-013" title="Поиск упоминаний объекта в базе данных" minHeight={678} />
...
<ExternalCodeEmbed example="sql/sql-885-014" title="Восстановление последовательности" minHeight={390} />
...
<ExternalCodeEmbed example="sql/sql-885-015" title="Анализ использования индексов" minHeight={426} />
...
<ExternalCodeEmbed example="sql/sql-885-016" title="Блокировки и ожидания" minHeight={534} />
...
<ExternalCodeEmbed example="sql/sql-885-017" title="Ресурсоёмкие запросы" minHeight={408} />
...
<ExternalCodeEmbed example="sql/sql-885-018" title="Параметры конфигурации" minHeight={426} />
...
<ExternalCodeEmbed example="sql/sql-885-019" title="Автовакуум и его мониторинг" minHeight={462} />
...
<ExternalCodeEmbed example="sql/sql-885-020" title="Репликация и отставание" minHeight={372} />
...
<ExternalCodeEmbed example="sql/sql-885-021" title="Управление ролями и объектами" minHeight={372} />
...
<ExternalCodeEmbed example="sql/sql-885-022" title="Сравнение конфигураций двух баз данных" minHeight={372} />
...
Теория типа и отличия `json` / `jsonb` — [JSONB](/encyclopedia/3-data-markup/3-04-konfiguratsii-i-dannye/217). Пошаговый практикум с таблицей, 10 большими JSON-записями, `JOIN`, `CROSS JOIN`, `HAVING`, `RETURNING`, индексами и `EXPLAIN` — [Практикум PostgreSQL по JSONB](./66.md).
...
SELECT 
    id,
    Данные->>'name' AS user_name,
    Данные->'address'->>'city' AS city
FROM users
...
```sql
UPDATE users
SET data = jsonb_set(Данные, '{last_login}', to_jsonb(CURRENT_TIMESTAMP))
WHERE id = 42;
```
...
```sql
UPDATE users
SET data = Данные - 'temporary_token'
WHERE id = 42;
```
...
```sql
CREATE INDEX idx_users_data_gin ON users USING GIN (Данные);
CREATE INDEX idx_users_data_path ON users USING GIN (Данные jsonb_path_ops);
```
...
<ExternalCodeEmbed example="sql/sql-885-023" title="Разбиение большого запроса на части" minHeight={426} />
...
<ExternalCodeEmbed example="sql/sql-885-024" title="Измерение времени выполнения функции" minHeight={336} />
...
<ExternalCodeEmbed example="sql/sql-885-025" title="Ограничение количества зависимых записей" minHeight={444} />
...
<ExternalCodeEmbed example="sql/sql-885-026" title="Журналирование изменений таблицы" minHeight={588} />
...
Параллельный захват **пакета** задач без ожидания (несколько воркеров, одна таблица `jobs`):
...
Подробнее: очередь на одну строку, индексы, статусы `processing` и восстановление «зависших» задач — [Блокировки в PostgreSQL](./110.md#skip-locked-paket).
...
<ExternalCodeEmbed example="sql/sql-885-027" title="Определение текущих блокировок" minHeight={372} />
...
<ExternalCodeEmbed example="sql/sql-885-028" title="Автоматическое создание секций через функцию" minHeight={372} />
...
```sql
ALTER TABLE accounts ENABLE ROW LEVEL БЕЗОПАСНОСТЬ;
```
...
```sql
ALTER TABLE accounts FORCE ROW LEVEL БЕЗОПАСНОСТЬ;
```
...
```sql
ALTER TABLE accounts DISABLE ROW LEVEL БЕЗОПАСНОСТЬ;
```
...
"Битые" ссылки — заказы без пользователя, позиции без заказа. **`NOT IN`** опасен при `NULL` в подзапросе ([Фильтрация и трёхзначная логика](./109.md)). Надёжнее `NOT EXISTS`:
...
Если нужен `NOT IN`: `WHERE user_id NOT IN (SELECT id FROM users WHERE id IS NOT NULL)`.
...
```sql
SELECT compress('Данные');
SELECT decompress(compress('Данные'));
```
...
<ExternalCodeEmbed example="sql/sql-885-029" title="Поиск блокирующих запросов" minHeight={480} />
