...
<div class="callout callout--tip">
  ...
  Рецепты на схеме интернет-магазина (categories, orders, …) — [885](./885.md). Тот же магазин с построчным разбором для учёбы — [SQL — реальные кейсы](/lab/Примеры/1152). Подключение, API и углубление по каждой СУБД — [SQLite](./887.md), [PostgreSQL](./888.md), [MySQL](./889.md), [Microsoft SQL Server](./890.md). Теория JOIN — [55](./55.md); VIEW — [8](./8.md); порядок выполнения SELECT — [107](./107.md) и [2](./2.md).
  </div>
</div>
...
Сформулированное предложение затем переводят в каркас из [107](./107.md#краткая-шпаргалка-конструкции-чтения): `SELECT` → `FROM` → `JOIN` → `WHERE` → `GROUP BY` → `HAVING` → `ORDER BY` → `LIMIT`.
...
Связь «центральная» таблица **TABLE1** с **TABLE2** и **TABLE3** через `IDFROMTABLE2` и `IDFROMTABLE3` повторяет типовой учебный JOIN из [55](./55.md#chetyre-osnovnyh-join-na-odnom-primere).
...
Подробнее про агрегаты — [6](./6.md#шпаргалка-агрегаты-и-группировка).
...
Теория и ограничения VIEW — [8](./8.md).

---

## Microsoft SQL Server

В исходном docx раздела T-SQL нет; для SQL Server те же идеи, но другой синтаксис типов (`INT`, `NVARCHAR`), `GETDATE()`, `ALTER COLUMN`, `TOP` вместо `LIMIT`. Сводка — [890](./890.md) и [T-SQL](./882.md).
...
| Задача | Куда идти |
| :--- | :--- |
| Первый запрос в psql / установка | [101](./101.md) |
| Тренажёр shop_data | [1](./1.md), [111](./111.md) |
| Рецепты на схеме магазина | [885](./885.md) |
| Четыре СУБД на TABLE1–3 | эта страница |
| Категории команд DDL/DML | [22](./22.md) |
| Оптимизация и индексы | [881](./881.md), [884](./884.md) |
