...
**Контекст:** учебная база **`demo`** — открытый логический дамп для практики SQL. Предметная область — воображаемая авиакомпания — бронирования, билеты, рейсы, аэропорты. После [Первые шаги с SQL](./101.md) и [PostgreSQL — практическая работа и API](./888.md) — лучший следующий шаг для **JOIN**, агрегатов и оконных функций на осмысленных данных.
...
Оператор `<@>` даёт расстояние в **милях**; множитель `1.609344` переводит в километры. Для продакшена с геодезической точностью смотрите **PostGIS** — в [PostgreSQL — практическая работа и API](./888.md) и [справочнике](/encyclopedia/3-data-markup/3-08-upravlenie-rsubd/2.md).
...
<div class="callout callout--tip">
  ...
  <div class="callout-body">
  Сложные маршруты с пересадками решаются через <strong>рекурсивный CTE</strong> — см. [Общие табличные выражения (CTE)](./551.md). Оптимизацию планов — [Оптимизация SQL-запросов](./881.md). Администрирование сервера — <a href="/encyclopedia/3-data-markup/3-08-upravlenie-rsubd/2.md">справочник PostgreSQL</a>.
  </div>
</div>
