---
title: 3. Данные и разметка - о разделе
description: Основы работы с данными и разметкой - классификация типов данных, алгоритмы обработки, SQL базы, конфигурационные форматы и принципы веб-разметки
sidebar_label: 3. Данные и разметка - о разделе
slug: /encyclopedia/3-data-markup/data-markup
id: Данные-markup
---

<div class="article-tags">
  <span class="tag tag-required">ОБЯЗАТЕЛЬНО</span>
  <span class="tag tag-beginner">ДЛЯ НОВИЧКОВ</span>
  <span class="tag tag-inprogress">В РАЗРАБОТКЕ</span>
</div>

import DocCardList from '@theme/DocCardList';

---

## О разделе

<DocCardList />

Вы ещё не забыли, что такое информация и данные? А типы данных? Если забыли, непременно вернитесь и повторите. Сейчас нам придётся заняться ими вплотную.

```mermaid
mindmap
  root((Данные и разметка))
    Основы данных
      3.01 Продвинутые операции
      3.02 Структуры данных
      3.03 Мыслительная база
    Конфигурация и БД
      3.04 Конфигурации
      3.05 Основы баз данных
      3.06 NoSQL
      3.07 SQL
      3.08 Управление РСУБД
    Веб-разметка
      3.09 HTML
      3.10 CSS
    Анализ данных
      3.11 Анализ данных
      3.11 Пакетная работа с данными
```

---
