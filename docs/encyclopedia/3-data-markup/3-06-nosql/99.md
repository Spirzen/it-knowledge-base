---
title: NoSQL — чек-лист
description: "Чек-лист раздела «NoSQL» — вопросы для самопроверки."
---
...
- **Schema patterns:** bucket (IoT), extended reference (заказ + дубли клиента), outlier (overflow). [MongoDB — проектирование документной схемы](/encyclopedia/3-data-markup/3-06-nosql/412).
- **Majority / rollback:** partition без majority → read-only; rollback сохраняет `.bson` в `dbPath/rollback/`. [§10 справочника](/encyclopedia/3-data-markup/3-06-nosql/41#10-репликация-replica-sets).
- **Shard key firehose:** монотонный ключ → один hot shard; hashed или compound prefix. [§11](/encyclopedia/3-data-markup/3-06-nosql/41#11-шардирование-sharding).
