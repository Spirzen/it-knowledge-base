...
description: Метки — это теги; не гарантируют схемы, но позволяют фильтровать (MATCH (n — User)).
referenceType: data
related:
...
Синтаксис, операторы, команды клиента и ограничения **Cypher**. Учебный курс: [раздел](/encyclopedia/3-data-markup/3-06-nosql/intro).
...
- [1. Общая структура запроса](#1-общая-структура-запроса)
- [2. Сущности графа и их свойства](#2-сущности-графа-и-их-свойства)
- [3. Клаузы и их параметры](#3-клаузы-и-их-параметры)
- [4. Типы данных в Cypher](#4-типы-данных-в-cypher)
- [5. Встроенные функции Cypher](#5-встроенные-функции-cypher)
- [6. Параметры запросов](#6-параметры-запросов)
- [7. Индексы и ограничения в контексте Cypher](#7-индексы-и-ограничения-в-контексте-cypher)
- [8. Производительность — `EXPLAIN`, `PROFILE`, планы](#8-производительность-explain-profile-планы)
- [9. Совместимость с GQL (ISO/IEC 39075:2022)](#9-совместимость-с-gql-isoiec-390752022)
- [10. Практические рекомендации для документации и преподавания](#10-практические-рекомендации-для-документации-и-преподавания)
- [11. Грамматика Cypher (упрощённая EBNF)](#11-грамматика-cypher-упрощённая-ebnf)
- [12. Сравнение с другими графовыми языками](#12-сравнение-с-другими-графовыми-языками)
- [13. Сложные сценарии: примеры с пояснением](#13-сложные-сценарии-примеры-с-пояснением)
- [14. Подзапросы и композиция](#14-подзапросы-и-композиция)
- [15. Особенности реализаций](#15-особенности-реализаций)
...
> ⚠️ В Cypher **нет** `BEGIN`/`COMMIT` в тексте запроса. Транзакции — на уровне Bolt/драйвера или в **cypher-shell**:

```bash
:begin
CREATE (n:User {id: 1})
:commit
# по умолчанию в shell включён режим :auto (один запрос = одна транзакция)
```
...
- удаляет **только ребро**.

```sql
DELETE r
```
- **ошибка**, если у узла остались рёбра (ограничение целостности).
...
> **Сначала:** [функции в коде](/encyclopedia/4-code-dev/4-02-chto-takoe-kod-i-kak-on-rabotaet/4), если вы ещё не разбирали, что такое функция в программе в целом. Ниже — встроенные функции Cypher в запросах к графу.
...
Примеры запросов (Neo4j Cypher):

```cypher
MATCH (n:User) WHERE n.email CONTAINS '@corp'
RETURN n;
...
|-------------|----------|---------|
| `CONTAINS` без full-text индекса | Полный скан + фильтр | Создать full-text индекс. |
| `MATCH (a)-[*]->(b)` без ограничений | Экспоненциальный взрыв путей | `shortestPath`, лимит длины, `WHERE` на свойства. |
...
> **Решение** — предварительно собрать id купленных товаров:

```cypher
MATCH (u:User {id: $user_id})-[:BOUGHT]->(bp:Product)
WITH u, collect(id(bp)) AS bought_ids
MATCH (u)-[:FRIENDS*1..2]-(f:User)-[:BOUGHT]->(p:Product)
WHERE NOT id(p) IN bought_ids
...
<ExternalCodeEmbed example="javascript/nosql-71-001" title="13.3 Выявление сообществ (Community Detection через Label Propagation — упрощённо)" minHeight={390} />



> 🔸 Это учебная реализация. В production — GDS:

```cypher
CALL gds.graph.project('friends', 'User', 'FRIENDS');
CALL gds.labelPropagation.stream('friends')
...
| Область | Примечание |
|---------|------------|
| Версии | актуальные LTS/стабильные релизы **Cypher** |
...
**Справочники** — [Справочник по Cassandra](/encyclopedia/3-data-markup/3-06-nosql/61), [Справочник по Memcached](/encyclopedia/3-data-markup/3-06-nosql/81), [Справочник по Redis](/encyclopedia/3-data-markup/3-06-nosql/51), [Справочник по SQL](/encyclopedia/3-data-markup/3-07-sql/883), [Справочник по MongoDB](/encyclopedia/3-data-markup/3-06-nosql/41), [Справочник по HTML](/encyclopedia/3-data-markup/3-09-html/21).
