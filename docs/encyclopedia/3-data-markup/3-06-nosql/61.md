...
description: hints_directory — путь для хранения hinted handoff-файлов. Используется при временной недоступности узла — координатор сохраняет изменения и позже отправляет их, когда узел вернётся. По умолчанию —…
referenceType: data
---
...
- `auto_snapshot` — автоматически создаёт снапшот перед `DROP TABLE` или `TRUNCATE`. Булево. По умолчанию: `true`. Снапшоты хранятся в `Данные/keyspace/table/snapshots/`. Не отменяет необходимости регулярного бэкапа.
...
- `TokenAwarePolicy` — оборачивает другую политику, добавляя предпочтение репликам (минимизирует Сеть hops).  
...
- `nodetool info` — информация об узле: uptime, heap usage, Данные size, cache stats, commit log size.
...
- `Система.log` — основной лог (`INFO` по умолчанию). Уровень меняется в `logback.xml`.  
...
> Рекомендация: отправлять `Система.log` в ELK/Graylog с парсингом структуры (например, через `cassandra-logback-encoder`).
...
| Область | Примечание |
|---------|------------|
| Версии | актуальные LTS/стабильные релизы **Cassandra** |
