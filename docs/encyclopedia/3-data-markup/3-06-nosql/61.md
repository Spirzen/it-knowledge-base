...
description: Синтаксис CQL, конфигурация cassandra.yaml, nodetool, consistency levels и ограничения Apache Cassandra.
referenceType: data
related:
  - title: "Справочник по Redis"
    doc: encyclopedia/3-data-markup/3-06-nosql/51
...
Синтаксис, операторы, команды клиента и ограничения **Cassandra**. Учебный курс: [раздел](/encyclopedia/3-data-markup/3-06-nosql/intro).

---

## Краткое пояснение

CQL похож на SQL синтаксически, но не семантически — нет JOIN, нет multi-row транзакций, partition key обязателен в `WHERE`. Ниже — конфигурация узла, свойства keyspace/таблиц, уровни консистентности и администрирование.
...
- [I. Настройки `cassandra.yaml` — основной конфигурационный файл Cassandra](#i-настройки-cassandrayaml-основной-конфигурационный-файл-cassandra)
- [II. Свойства ключевых пространств (Keyspaces)](#ii-свойства-ключевых-пространств-keyspaces)
- [III. Свойства таблиц (CQL `CREATE TABLE` / `ALTER TABLE`)](#iii-свойства-таблиц-cql-create-table-alter-table)
- [IV. Свойства и параметры CQL-запросов](#iv-свойства-и-параметры-cql-запросов)
- [V. Вторичные индексы (Secondary Indexes)](#v-вторичные-индексы-secondary-indexes)
- [VI. Материализованные представления (Materialized Views) — устаревшая функция](#vi-материализованные-представления-materialized-views-устаревшая-функция)
- [VII. Параметры клиентских драйверов (на примере DataStax Java Driver 4.x)](#vii-параметры-клиентских-драйверов-на-примере-datastax-java-driver-4x)
- [VIII. JMX-интерфейс Cassandra](#viii-jmx-интерфейс-cassandra)
- [IX. Утилита `nodetool`](#ix-утилита-nodetool)
- [X. Метрики и мониторинг в production](#x-метрики-и-мониторинг-в-production)
- [XI. Типовые конфигурации под сценарии](#xi-типовые-конфигурации-под-сценарии)
...
- `max_hint_window_in_ms` — максимальное окно (в миллисекундах), в течение которого hinted handoff накапливается для недоступного узла. По умолчанию: `10800000` (3 часа). После этого hinted handoff для этого узла прекращается — нужен repair:
...
Ключевое пространство создаётся командой **CREATE KEYSPACE** и определяет логическую группу таблиц с единой политикой репликации и устойчивости к сбоям.
...
> Важно: репликация в дополнительный DC не происходит автоматически — только после **ALTER KEYSPACE** и **nodetool rebuild** на узлах целевого DC:
...
```sql
ALTER KEYSPACE cache_ks WITH durable_writes = false;
```
...
- Редко меняется; изменение требует перезаписи SSTable:
...
- `QUORUM` — большинство реплик: `⌊RF/2⌋ + 1`. При `W + R > RF` (одинаковый CL на запись и чтение) чтение с высокой вероятностью увидит последнюю запись — это **tunable consistency**, не linearizability.
...
Пример полного цикла LWT:

```cql
INSERT INTO emails_lookup (email, user_id)
VALUES ('anna@example.com', a1b2c3d4-e89b-12d3-a456-426614174000)
IF NOT EXISTS;
-- Ответ: [applied] true  или  [applied] false + текущие значения

UPDATE accounts SET balance = balance - 100
WHERE user_id = a1b2c3d4-e89b-12d3-a456-426614174000
IF balance >= 100;
-- Ответ: [applied] true/false
```
...
- безопасный `TRUNCATE` (с снапшотом, если `auto_snapshot=true`).
...
nodetool status
nodetool info
nodetool repair -pr app_ks users
nodetool rebuild -- dc2
```
...
- Ключевые панели:  
  ...
  — *Compaction & Flush* — pending Задачи, bytes compacted.  
...
**Модель данных**:  
- Partition key — `(metric_name, bucket_time)` — например, `('cpu_usage', '2025-11-21')`; bucketing снижает риск hot partition.  
...
<ExternalCodeEmbed example="sql/nosql-61-001" title="1. Time-Series (логи, метрики, IoT)" minHeight={354} />
...
|----------|----------------|
| `InvalidRequest: Undefined column name` | Столбец не объявлен — `ALTER TABLE ... ADD` |
| `InvalidRequest: Cannot execute this query as it might involve data filtering` | Нет partition key в `WHERE` — перепроектировать таблицу или lookup |
| `Unauthorized` / `Unauthenticated` | `PasswordAuthenticator`, GRANT на keyspace |
| `TombstoneOverwhelmingException` | Много DELETE в одной партиции — TTL, repair, `gc_grace_seconds` |
| `WriteTimeout` / `ReadTimeout` | CL слишком строгий, down-узлы, нужен repair |
| Команда не найдена (`cqlsh`, `nodetool`) | PATH, установка пакета, `CASSANDRA_HOME` |
...
| Область | Примечание |
|---------|------------|
| Версии | 4.x / 5.x — актуальные; MV и UDF удалены (были в 3.x) |
| JDK | 11 или 17 для Cassandra 4.x+ (см. матрицу Apache) |
| CQL | Совместим между Apache Cassandra, ScyllaDB, Keyspaces (с оговорками) |
| Thrift | Удалён в 4.0; только Native Protocol (порт 9042) |
...
**Справочники** — [Справочник по Redis](/encyclopedia/3-data-markup/3-06-nosql/51), [Справочник по Cypher](/encyclopedia/3-data-markup/3-06-nosql/71), [Справочник по MongoDB](/encyclopedia/3-data-markup/3-06-nosql/41), [Справочник по Memcached](/encyclopedia/3-data-markup/3-06-nosql/81), [Справочник по XSLT](/encyclopedia/3-data-markup/3-04-konfiguratsii-i-dannye/212), [Справочник по SQL](/encyclopedia/3-data-markup/3-07-sql/883).
