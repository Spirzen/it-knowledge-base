...
- Регистр команд: **верхний** (`SET`, не `set`) — так принято в документации; сервер нечувствителен к регистру.
- **Нет** SQL-подобных `SELECT … WHERE` по значению — только по ключу (поиск по содержимому — RediSearch и др. модули).
- Точка с запятой **не** завершает команду.
- Пайплайн и транзакции: несколько команд подряд; атомарная группа — `MULTI` … `EXEC` с опциональным `WATCH key`.
