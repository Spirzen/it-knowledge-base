...
Единицы **`em`** и **`rem`** привязаны к размеру шрифта. **`em`** (от *element*) берёт базу от **родительского** элемента, **`rem`** (*root em*) — от **`font-size` корня** (`<html>`). Обе единицы работают в любых свойствах длины — `font-size`, `padding`, `margin`, `width`, `gap` и других. *Подробнее о шрифтах здесь* — [/encyclopedia/1-basics/1-15-tekst/](/encyclopedia/1-basics/1-15-tekst/intro).
