...
related:
  ...
  - title: "Справочник по SQL"
    doc: encyclopedia/3-data-markup/3-07-sql/883
...
Синтаксис, селекторы, свойства и ограничения **CSS**. Учебный курс: [раздел](/encyclopedia/3-data-markup/3-10-css/intro).

Компактные таблицы в [§ Компактные шпаргалки](#компактные-шпаргалки) частично восходят к шпаргалке [OverAPI CSS](https://overapi.com/css) (MIT, [исходники](https://github.com/code9media/overthecs.github.io)); здесь они переведены на русский, дополнены пометками об устаревших свойствах и сопоставлены с развёрнутым справочником ниже.
...
| Быстрый поиск | [компактные шпаргалки](#компактные-шпаргалки) |

---

## Компактные шпаргалки

Таблицы для быстрого поиска по синтаксису. Структура разделов ориентирована на [OverAPI CSS](https://overapi.com/css) (MIT, [исходники](https://github.com/code9media/overthecs.github.io/blob/master/_posts/detail/2012-07-01-css.html)); переведены на русский и помечены устаревшие/нереализованные свойства. Подробные описания — в [разделах ниже](#справочные-таблицы); актуальная поддержка — [MDN: CSS](https://developer.mozilla.org/ru/docs/Web/CSS).

- [Блочная модель](#блочная-модель-box-model)
- [Селекторы](#селекторы)
- [Стили](#стили)
- [Элементы](#элементы)
- [CSS3 — блочная модель](#css3--блочная-модель)
- [Анимация](#анимация)
- [Страница и печать](#страница-и-печать)
- [Прочее](#прочее)
...
| Свойство / селектор | Кратко |
|---|---|
| `margin` | Все свойства margin одной декларацией |
...
| Свойство / селектор | Кратко |
|---|---|
| `padding` | Все свойства padding одной декларацией |
...
| Свойство / селектор | Кратко |
|---|---|
| `border` | Все свойства border одной декларацией |
| `border-bottom` | Все свойства нижней границы одной декларацией |
| `border-bottom-color` | Цвет границы (bottom) |
| `border-bottom-style` | Стиль границы (bottom) |
| `border-bottom-width` | Толщина границы (bottom) |
| `border-color` | Цвет всех четырёх границ |
| `border-left` | Все свойства левой границы одной декларацией |
| `border-left-color` | Цвет границы (left) |
| `border-left-style` | Стиль границы (left) |
| `border-left-width` | Толщина границы (left) |
| `border-right` | Все свойства правой границы одной декларацией |
| `border-right-color` | Цвет границы (right) |
| `border-right-style` | Стиль границы (right) |
| `border-right-width` | Толщина границы (right) |
| `border-style` | Стиль всех четырёх границ |
| `border-top` | Все свойства верхней границы одной декларацией |
| `border-top-color` | Цвет границы (top) |
| `border-top-style` | Стиль границы (top) |
| `border-top-width` | Толщина границы (top) |
| `border-width` | Толщина всех четырёх границ |
| `outline` | Все свойства outline одной декларацией |
| `outline-color` | Цвет outline |
| `outline-style` | Стиль outline |
| `outline-width` | Толщина outline |
| `border-bottom-left-radius` | Скругление угла (bottom-left) |
| `border-bottom-right-radius` | Скругление угла (bottom-right) |
| `border-image` | Сокращённая запись |
| `border-image-outset` | Вынос border-image за пределы border-box |
| `border-image-repeat` | Повтор/растяжение border-image |
| `border-image-slice` | Срез border-image (slice) |
| `border-image-source` | Источник border-image |
| `border-image-width` | Толщина border-image |
| `border-radius` | Сокращённая запись |
| `border-top-left-radius` | Скругление угла (top-left) |
| `border-top-right-radius` | Скругление угла (top-right) |
| `box-decoration-break` | Скругление угла (top-right) |
...
|---|---|
| `.class` | По классу (`.имя`) |
| `#id` | По идентификатору (`#id`) |
| `*` | Универсальный селектор — все элементы |
| `element` | По типу элемента (`p`, `div`, …) |
| `element,element` | Группа селекторов (перечисление через запятую) |
| `element element` | Потомок (пробел между селекторами) |
| `element>element` | Прямой дочерний (`>`) |
| `element+element` | Соседний следующий (`+`) |
| `[attribute]` | По наличию атрибута `[attr]` |
| `[attribute=value]` | Атрибут равен значению `[attr=val]` |
| `[attribute~=value]` | Атрибут содержит слово в списке `[attr~=word]` |
| `[attribute|=value]` | Атрибут равен или начинается с `[attr|=val]` |
| `element1~element2` | Общий сосед (`~`) — после того же родителя |
| `[attribute^=value]` | Значение атрибута начинается с `[attr^=pre]` |
| `[attribute$=value]` | Значение атрибута заканчивается на `[attr$=suf]` |
| `[attribute*=value]` | Значение атрибута содержит подстроку `[attr*=sub]` |
...
| `:active` | Активная ссылка (нажатие) |
| `:hover` | При наведении курсора |
| `:focus` | Элемент в фокусе |
| `:first-letter` | Первая буква текста (`::first-letter`) |
| `:first-line` | Первая строка текста (`::first-line`) |
| `:first-child` | Первый дочерний элемент |
| `:before` | Псевдоэлемент перед содержимым (`::before`) |
| `:after` | Псевдоэлемент после содержимого (`::after`) |
| `:lang(language)` | Элемент с языком `lang` |
| `:first-of-type` | Первый элемент своего типа у родителя |
| `:last-of-type` | Последний элемент своего типа у родителя |
| `:only-of-type` | Единственный элемент своего типа |
| `:only-child` | Единственный дочерний элемент |
| `:nth-child(n)` | N-й дочерний (`:nth-child`) |
| `:nth-last-child(n)` | N-й с конца (`:nth-last-child`) |
| `:nth-of-type(n)` | N-й элемент типа (`:nth-of-type`) |
| `:last-child` | Последний дочерний элемент |
| `:not` | Отрицание (`:not(селектор)`) |
| `:root` | Корневой элемент документа |
| `:empty` | Пустой элемент (без детей и текста) |
| `:target` | Якорь из URL (`#id`) |
| `:enabled` | Включённый элемент формы |
...
| Свойство / селектор | Кратко |
|---|---|
| `background` | Все свойства background одной декларацией |
| `background-attachment` | Фиксация фона при прокрутке |
| `background-color` | Цвет фона |
| `background-image` | Фоновое изображение |
| `background-position` | Позиция фонового изображения |
| `background-repeat` | Повтор фонового изображения |
| `background-clip` | Область отрисовки фона (background-clip) |
| `background-origin` | Исходная точка фона (background-origin) |
...
| Свойство / селектор | Кратко |
|---|---|
| `color-profile` | Разрешает the specification of a source color profile other than the default *(устар./не реализ.)* |
| `opacity` | Прозрачность элемента |
| `rendering-intent` | Разрешает the specification of a color profile rendering intent other than the default *(устар./не реализ.)* |

#### Шрифт

| Свойство / селектор | Кратко |
|---|---|
| `font` | Все свойства font одной декларацией |
| `font-family` | Семейство шрифтов |
| `font-size` | Размер шрифта |
| `font-style` | Стиль шрифта |
| `font-variant` | Вариант начертания (капитель) |
| `font-weight` | Насыщенность шрифта |
| `@font-face` | Подключение веб-шрифта |
| `font-size-adjust` | Коррекция метрик при подстановке шрифта |
| `font-stretch` | Плотность гарнитуры (font-stretch) |
...
| `clear` | Запрет обтекания с указанной стороны |
| `clip` | Обрезка (устарело, используйте clip-path) *(устар./не реализ.)* |
| `cursor` | Тип курсора |
| `display` | Тип отображения (display) |
| `float` | Обтекание блока (float) |
| `left` | Смещение слева |
...
| `z-index` | Порядок наложения (z-index) |
...
| Свойство / селектор | Кратко |
|---|---|
| `target` | Сокращение для target-name, target-new, target-position (устар.) |
| `target-name` | Задаёт where to open links (target destination) *(устар./не реализ.)* |
| `target-new` | Задаёт whether new destination links should open in a new window or in a new tab of an existing window *(устар./не реализ.)* |
| `target-position` | Задаёт where new destination links should be placed *(устар./не реализ.)* |
...
| `text-transform` | Регистр текста (uppercase и др.) |
| `vertical-align` | Вертикальное выравнивание |
| `white-space` | Пробелы и переносы строк |
| `word-spacing` | Межсловный интервал |
| `text-outline` | Задаёт a text outline *(устар./не реализ.)* |
| `text-overflow` | Обрезка переполненного текста (ellipsis) |
| `text-shadow` | Тень текста |
| `text-wrap` | Правила переноса строк (text-wrap) |
| `word-break` | Правила переноса строк |
| `word-wrap` | Перенос длинных слов (устар. имя word-wrap) |

#### Списки

| Свойство / селектор | Кратко |
|---|---|
| `list-style` | Все свойства list-style |
| `list-style-image` | Изображение вместо маркера |
| `list-style-position` | Позиция маркера списка |
| `list-style-type` | Маркер списка |

#### Таблицы

| Свойство / селектор | Кратко |
|---|---|
| `border-collapse` | Схлопывание границ таблицы |
| `border-spacing` | Расстояние между ячейками |
| `caption-side` | Позиция подписи таблицы (caption) |
| `empty-cells` | Оформление пустых ячеек таблицы |
| `table-layout` | Алгоритм раскладки таблицы |

#### Marquee (устарело)

| Свойство / селектор | Кратко |
|---|---|
| `marquee-direction` | Устанавливает the direction of the moving content *(устар./не реализ.)* |
| `marquee-play-count` | Устанавливает how many times the content move *(устар./не реализ.)* |
| `marquee-speed` | Устанавливает how fast the content scrolls *(устар./не реализ.)* |
| `marquee-style` | Устанавливает the style of the moving content *(устар./не реализ.)* |
...
| Свойство / селектор | Кратко |
|---|---|
| `overflow-x` | Переполнение по оси (overflow-x/y) |
| `overflow-y` | Переполнение по оси (overflow-x/y) |
| `overflow-style` | Способ прокрутки при overflow (устар.) *(устар./не реализ.)* |
| `rotation` | Поворот (устар., см. transform) *(устар./не реализ.)* |

#### Старый Flexbox (box-*, устарело)

| Свойство / селектор | Кратко |
|---|---|
| `box-align` | Задаёт how to align the child elements of a box *(устар./не реализ.)* |
| `box-direction` | Задаёт in which direction the children of a box are displayed *(устар./не реализ.)* |
| `box-flex` | Задаёт whether the children of a box is flexible or inflexible in size *(устар./не реализ.)* |
| `box-flex-group` | Assigns flexible elements to flex groups *(устар./не реализ.)* |
| `box-lines` | Задаёт whether columns will go onto a new line whenever it runs out of space in the parent box *(устар./не реализ.)* |
| `box-ordinal-group` | Задаёт display order of the child elements of a box *(устар./не реализ.)* |
| `box-orient` | Задаёт whether the children of a box should be laid out horizontally	or vertically *(устар./не реализ.)* |
| `box-pack` | Задаёт horizontal position in horizontal boxes and the vertical position in vertical boxes *(устар./не реализ.)* |

#### Linebox (черновик)

| Свойство / селектор | Кратко |
|---|---|
| `alignment-adjust` | Позволяет more precise alignment of elements *(устар./не реализ.)* |
| `alignment-baseline` | Задаёт how an inline-level element is aligned with respect to its parent *(устар./не реализ.)* |
| `baseline-shift` | Позволяет repositioning of the dominant-baseline relative to the dominant-baseline *(устар./не реализ.)* |
| `dominant-baseline` | Задаёт a scaled-baseline-table *(устар./не реализ.)* |
| `drop-initial-after-adjust` | Устанавливает the alignment point of the drop initial for the primary connection point *(устар./не реализ.)* |
| `drop-initial-after-align` | Устанавливает which alignment line within the initial line box is used at the primary connection point with the initial letter box *(устар./не реализ.)* |
| `drop-initial-before-adjust` | Устанавливает the alignment point of the drop initial for the secondary connection point *(устар./не реализ.)* |
| `drop-initial-before-align` | Устанавливает which alignment line within the initial line box is used at the secondary connection point with the initial letter box *(устар./не реализ.)* |
| `drop-initial-size` | Управляет the partial sinking of the initial letter *(устар./не реализ.)* |
| `drop-initial-value` | Включает a drop-initial effect *(устар./не реализ.)* |
| `inline-box-align` | Устанавливает which line of a multi-line inline block align with the previous and next inline elements within a line *(устар./не реализ.)* |
| `line-stacking` | A shorthand property for setting the line-stacking-strategy, line-stacking-ruby, and line-stacking-shift properties *(устар./не реализ.)* |
| `line-stacking-ruby` | Устанавливает the line stacking method for block elements containing ruby annotation elements *(устар./не реализ.)* |
| `line-stacking-shift` | Устанавливает the line stacking method for block elements containing elements with base-shift *(устар./не реализ.)* |
| `line-stacking-strategy` | Устанавливает the line stacking strategy for stacked line boxes within a containing block element *(устар./не реализ.)* |
| `text-height` | Устанавливает the block-progression dimension of the text content area of an inline box *(устар./не реализ.)* |
...
| `column-fill` | Заполнение колонок по высоте |
| `column-gap` | Зазор между колонками |
| `column-rule` | Сокращённая запись |
| `column-rule-color` | Цвет разделителя колонок |
| `column-rule-style` | Стиль разделителя колонок |
| `column-rule-width` | Толщина разделителя колонок |
| `column-span` | Объединение колонок (column-span) |
| `column-width` | Ширина колонки |
| `columns` | Сокращённая запись |
...
| Свойство / селектор | Кратко |
|---|---|
| `@keyframes` | Ключевые кадры (@keyframes) |
| `animation` | Сокращение для animation-* (кроме play-state) |
| `animation-name` | Имя анимации (@keyframes) |
| `animation-duration` | Длительность цикла анимации |
| `animation-timing-function` | Кривая скорости (timing-function) |
| `animation-delay` | Задержка перед стартом |
| `animation-fill-mode` | Состояние до/после анимации (fill-mode) |
| `animation-iteration-count` | Число повторов анимации |
| `animation-direction` | Направление чередования циклов |
| `animation-play-state` | Запуск или пауза анимации |

#### 2D/3D-трансформации

| Свойство / селектор | Кратко |
|---|---|
| `transform` | 2D/3D-трансформация |
| `transform-origin` | Точка трансформации (transform-origin) |
| `transform-style` | 3D-режим вложенных элементов |
| `perspective` | Глубина перспективы |
| `perspective-origin` | Точка перспективы (perspective-origin) |
| `backface-visibility` | Видимость обратной стороны в 3D |

#### Переходы (transition)

| Свойство / селектор | Кратко |
|---|---|
| `transition` | Сокращённая запись |
| `transition-property` | Свойство для transition |
| `transition-duration` | Длительность перехода |
| `transition-timing-function` | Кривая скорости (timing-function) |
...
| Свойство / селектор | Кратко |
|---|---|
| `bookmark-label` | Задаёт label of the bookmark *(устар./не реализ.)* |
| `bookmark-level` | Задаёт level of the bookmark *(устар./не реализ.)* |
| `bookmark-target` | Задаёт target of the bookmark link *(устар./не реализ.)* |
| `float-offset` | Сдвигает floated elements in the opposite direction of the where they have been floated with float *(устар./не реализ.)* |
| `hyphenate-after` | Задаёт minimum number of characters in a hyphenated word after the hyphenation character *(устар./не реализ.)* |
| `hyphenate-before` | Задаёт minimum number of characters in a hyphenated word before the hyphenation character *(устар./не реализ.)* |
| `hyphenate-character` | Задаёт a string that is shown when a hyphenate-break occurs *(устар./не реализ.)* |
| `hyphenate-lines` | Указывает the maximum number of successive hyphenated lines in an element *(устар./не реализ.)* |
| `hyphenate-resource` | Задаёт a comma-separated list of external resources that can help the browser determine hyphenation points *(устар./не реализ.)* |
| `hyphens` | Перенос по слогам (hyphens) |
| `image-resolution` | Задаёт correct resolution of images |
| `marks` | Добавляет crop and/or cross marks to the document |

#### Генерируемый контент

| Свойство / селектор | Кратко |
|---|---|
| `content` | Генерируемый контент (::before/::after) |
| `counter-increment` | Увеличение CSS-счётчика |
| `counter-reset` | Сброс/создание CSS-счётчика |
| `quotes` | Тип кавычек для цитат |
| `crop` | Позволяет a replaced element to be just a rectangular area of an object, instead of the whole object *(устар./не реализ.)* |
| `move-to` | Переносит an element to be removed from the flow and reinserted at a later point in the document *(устар./не реализ.)* |
| `page-policy` | Определяет which page-based occurance of a given element is applied to a counter or string value *(устар./не реализ.)* |

#### Медиа страницы

| Свойство / селектор | Кратко |
|---|---|
| `fit` | Подсказка: a hint for how to scale a replaced element if neither its width nor its height property is auto *(устар./не реализ.)* |
| `fit-position` | Определяет the alignment of the object inside the box *(устар./не реализ.)* |
| `image-orientation` | Ориентация изображения |
| `page` | Задаёт a particular type of page where an element SHOULD be displayed |
| `size` | Задаёт size and orientation of the containing box for page content |
...
| Свойство / селектор | Кратко |
|---|---|
| `orphans` | Минимум строк внизу страницы при разрыве |
| `page-break-after` | Разрыв страницы после элемента |
| `page-break-before` | Разрыв страницы перед элемента |
| `page-break-inside` | Разрыв страницы внутри элемента |
| `widows` | Минимум строк вверху страницы при разрыве |

#### Речь (CSS Speech)

| Свойство / селектор | Кратко |
|---|---|
| `mark` | Сокращённая запись *(устар./не реализ.)* |
| `mark-after` | Позволяет named markers to be attached to the audio stream *(устар./не реализ.)* |
| `mark-before` | Позволяет named markers to be attached to the audio stream *(устар./не реализ.)* |
| `phonemes` | Задаёт a phonetic pronunciation for the text contained by the corresponding element *(устар./не реализ.)* |
| `rest` | Сокращённая запись *(устар./не реализ.)* |
| `rest-after` | Задаёт a rest or prosodic boundary to be observed after speaking an element's content *(устар./не реализ.)* |
| `rest-before` | Задаёт a rest or prosodic boundary to be observed before speaking an element's content *(устар./не реализ.)* |
| `voice-balance` | Задаёт balance between left and right channels *(устар./не реализ.)* |
| `voice-duration` | Задаёт how long it should take to render the selected element's content *(устар./не реализ.)* |
| `voice-pitch` | Задаёт average pitch (a frequency) of the speaking voice *(устар./не реализ.)* |
| `voice-pitch-range` | Задаёт variation in average pitch *(устар./не реализ.)* |
| `voice-rate` | Управляет the speaking rate *(устар./не реализ.)* |
| `voice-stress` | Указывает the strength of emphasis to be applied *(устар./не реализ.)* |
| `voice-volume` | Относится к to the amplitude of the waveform output by the speech synthesises *(устар./не реализ.)* |

#### Пользовательский интерфейс

| Свойство / селектор | Кратко |
|---|---|
| `appearance` | Внешний вид нативного UI-элемента |
| `box-sizing` | Модель расчёта размеров (box-sizing) |
| `icon` | Provides the author the ability to style an element with an iconic equivalent *(устар./не реализ.)* |
| `nav-down` | Задаёт where to navigate when using the arrow-down navigation key *(устар./не реализ.)* |
| `nav-index` | Задаёт tabbing order for an element *(устар./не реализ.)* |
| `nav-left` | Задаёт where to navigate when using the arrow-left navigation key *(устар./не реализ.)* |
| `nav-right` | Задаёт where to navigate when using the arrow-right navigation key *(устар./не реализ.)* |
| `nav-up` | Задаёт where to navigate when using the arrow-up navigation key *(устар./не реализ.)* |
| `outline-offset` | Смещение outline от границы |
| `resize` | Изменение размера пользователем (resize) |
...
- [Компактные шпаргалки](#компактные-шпаргалки)
...
- [`container`](#container)
- [`@container`](#container)
- [`scroll-state()` в `@container`](#scroll-state-в-container)
...
- **Единицы измерения**  
  ...
  - *Относительные*:  
    - *к шрифту* — `em`, `rem`, `ex`, `ch`, `cap`, `ic` — для `em`/`rem` с примером расчёта см. [Основные стили](./3.md#em-i-rem)  
...
— Примечания:  
  ...
  - Обзор всех пяти значений с примерами — [Основные стили, раздел position](./3.md#position).
...
— Примечания:  
  ...
  - `color-mix(in lch, red 30%, blue)` — смесь в заданном пространстве (CSS Color 5), заменяет `blend()`.

---

### `color-mix()`

— Синтаксис: `color-mix(in <colorspace>, <color> [<percentage>]#)`  
— Цветовые пространства — `srgb`, `hsl`, `hwb`, `lab`, `lch`, `oklab`, `oklch`, `xyz` и др.  
— Примеры:
  - `color-mix(in srgb, plum, #123456)` — 50/50 в sRGB;
  - `color-mix(in oklch, var(--brand) 20%, white)` — осветление токена;
  - `color-mix(in lch longer hue, red 44%, blue 16%)` — интерполяция оттенка по длинной дуге.  
— Примечания:
  - Палитру из одного `--brand` можно строить без препроцессора;
  - для UI чаще `oklch`/`oklab`, для совместимости с hex — `srgb`;
  - развёрнутый разбор — [Функции в CSS](./119.md#colormix--смешивание-без-препроцессора).
...
*Подробнее о шрифтах здесь* — [/encyclopedia/1-basics/1-15-tekst/](/encyclopedia/1-basics/1-15-tekst/intro).
...
— Initial: `none`  
— Примечания:  
  - ≠ `box-shadow` — учитывает альфа-канал (например, для SVG).
...
— Примечания:  
  ...
  - `<overflow-position>` = `safe | unsafe` — при `safe` и риске вылезания за край по оси выравнивания позиция сбрасывается к `start`; `unsafe` сохраняет заданную позицию при переполнении (см. [CSS Box Alignment](https://www.w3.org/TR/css-align-3/)).
...
— Синтаксис: `normal | size | inline-size | scroll-state`  
— Наследуется: нет  
— Анимируемое: нет  
— Initial: `normal`  
— Примечания:  
  ...
  - `scroll-state` — отслеживает состояния прокрутки (`stuck`, `snapped`, `scrollable`).
...
### `scroll-state()` в `@container`

— Назначение: стили по состоянию прокрутки контейнера вместо JS-обработчиков `scroll`.
— Типичные состояния:
  - `scroll-state(stuck: top)` / `scroll-state(stuck: bottom)` — элемент со `sticky` "прилип" к краю;
  - `scroll-state(snapped: y)` — элемент находится в точке scroll snap;
  - `scroll-state(scrollable: y)` — контейнер действительно прокручивается по оси.
— Пример:
```css
.section-header {
  position: sticky;
...
```
— Примечание: фича новая, проверяйте поддержку браузеров и добавляйте fallback через `@supports`.
...
### Функции CSS
> **Общая база** именованных вычислений в коде — [функции в коде](/encyclopedia/4-code-dev/4-02-chto-takoe-kod-i-kak-on-rabotaet/4). Ниже — функции значений CSS (`calc()`, `var()`, `rgb()` и др.).
...
**Справочники** — [Справочник по HTML](/encyclopedia/3-data-markup/3-09-html/21), [Справочник-шпаргалка по Git](/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/115), [Справочник по SQL](/encyclopedia/3-data-markup/3-07-sql/883), [Справочник по JavaScript](/encyclopedia/5-languages/5-01-javascript/251), [Справочник по Memcached](/encyclopedia/3-data-markup/3-06-nosql/81), [Справочник по Node](/encyclopedia/5-languages/5-01-javascript/3-ecosystem/1-runtime-node/261).
