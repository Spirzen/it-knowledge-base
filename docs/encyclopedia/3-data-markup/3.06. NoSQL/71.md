...
| `NODE KEY` | `CREATE CONSTRAINT user_ext_id FOR (n:User) REQUIRE (n.Система, n.external_id) IS NODE KEY` | Составной уникальный ключ (несколько свойств). |
...
Neo4j Graph Данные Наука Library (GDS) рекомендуется для production, но можно эмулировать:
