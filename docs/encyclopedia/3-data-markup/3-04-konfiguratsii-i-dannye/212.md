...
Синтаксис, операторы, команды клиента и ограничения **XSLT**. Учебный курс: [XSLT](./214.md), основы [XML](./2.md) и [XPath](./213.md); оглавление: [intro](./intro.md).
...
> XSLT (Extensible Stylesheet Language Transformations) — это язык программирования, разработанный специально для преобразования XML-документов в другие форматы. С его помощью исходные XML-данные можно автоматически перевести в HTML-страницу, обычный текстовый файл (TXT), PDF или в другой XML-документ с измененной структурой
...
XSLT-таблица — это XML-документ (обычно с пространством имён `http://www.w3.org/1999/XSL/Transform`, сокращённо `xsl:`). 

Корневой элемент — `<xsl:stylesheet>` или `<xsl:transform>` (синонимы).
...
> ✅ XSLT 3.0 смягчает конфликт шаблонов через атрибут режима:
...
- `fail` — ошибка.
- применить следующий по приоритету шаблон (рекурсивно).
...
**Справочники** — [Справочник по XML](/encyclopedia/3-data-markup/3-04-konfiguratsii-i-dannye/211), [Справочник по MongoDB](/encyclopedia/3-data-markup/3-06-nosql/41), [Справочник по эмодзи](/encyclopedia/3-data-markup/3-04-konfiguratsii-i-dannye/112), [Справочник по Redis](/encyclopedia/3-data-markup/3-06-nosql/51), [Основные команды Windows](/encyclopedia/2-system-network/2-05-terminal/102), [Справочник по Cassandra](/encyclopedia/3-data-markup/3-06-nosql/61).
