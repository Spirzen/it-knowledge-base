...
| `doctype-public` / `doctype-Система` | `"..."` | Для DTD. |
...
> 🚫 **Не рекомендуется** для генерации XML/HTML — нарушает well-formedness. Использовать только для legacy-совместимости (например, вставки CДанные или pre-сериализованного HTML).
...
<xsl:for-each select="book">
  ...
  <xsl:sort select="year" Данные-type="number" order="descending"/>
...
| `Данные-type` | `"text"`, `"number"`, `"qname"` | `"text"` — лексикографически (по `lang`/`collation`), `"number"` — численно. |
...
| `case-order` | `"upper-first"`, `"lower-first"` | Влияет при `Данные-type="text"`. |
