...
## XPointer — адрес фрагмента в URI

**XPath** выбирает узлы **внутри** уже загруженного документа. **XPointer** (XML Pointer Language) решает другую задачу: указать **фрагмент** XML в URL — «возьми не весь `chapter.xml`, а элемент с `id="intro"`».

Схема:

```text
https://example.com/doc.xml#xpointer(id('intro'))
https://example.com/doc.xml#xpointer(/book/chapter[3])
```

XPointer опирается на XPath и расширяет его **термами** (в ранних спецификациях W3C):

| Терм | Назначение |
|------|------------|
| `id('name')` | Элемент с атрибутом типа `ID` или `xml:id="name"` |
| `element(/1/2/3)` | Путь по номерам дочерних элементов от корня |
| `xpointer(expr)` | Обёртка над XPath-выражением |
| `xmlns(...)` | Объявление префикса для XPath внутри URI |

Пример для документа с отделами:

```xml
<company>
  <department id="sales">
    <employee><name>Sarah Bellum</name></employee>
  </department>
  <department id="marketing">...</department>
</company>
```

| XPointer | Что выберет |
|----------|-------------|
| `id('sales')` | `<department id="sales">` |
| `xpointer(//employee[1]/name)` | Первый `<name>` среди сотрудников |
| `xpointer(/company/department[2])` | Второй отдел |

**XInclude** использует XPointer в атрибуте `xpointer`, когда нужен не весь включаемый файл:

```xml
<xi:include href="chapter1.xml" xpointer="xpointer(//*[@id='summary'])"/>
```

Современный стандарт — [XPointer Framework](https://www.w3.org/TR/xptr-framework/) и элементные схемы (`element()`); поддержка в инструментах неравномерна. Для новых проектов чаще включают целые файлы через XInclude или собирают документ в коде. Подробнее — [справочник по XML](./211.md#xinclude-xml-inclusions), [XLink](./211.md#xlink-xml-linking-language).

**ID и IDREF** (DTD) и **`xml:id`** — простой способ пометить якорь без полного XPointer: `href="doc.xml#section-3"` работает, если у элемента `id="section-3"`.
...
- **XPointer / XInclude** — фрагмент документа в URI;
