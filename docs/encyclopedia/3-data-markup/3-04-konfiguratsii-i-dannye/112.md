...
Эмодзи (emoji) — это стандартизированный графический язык пиктограмм и смайликов, который используется в электронных сообщениях, мобильных клавиатурах и на веб-страницах для передачи смыслов, предметов или эмоций. Само слово пришло из японского языка и буквально переводится как «картинка» (e) + «знак/символ» (moji). В отличие от обычных текстовых смайликов (эмотиконов типа :-)), которые набираются знаками препинания, эмодзи — это полноценные готовые символы, встроенные в единую мировую кодировку Юникод.

Они нужны для:
- передачи интонации и эмоций;
- экономии времени и места;
- универсальности общения между разными языками;
...
Разница будет именно в интонации текста.
...
| **Linux (Google)** | Noto Color Emoji (open-source) | CBDT, позже COLRv1 | 2016 | Полная (при установке v2.035+, см. команду ниже) |
...
Установка Noto Color Emoji на Debian/Ubuntu:
...
- **PostgreSQL с ICU**: — сортировка по порядку в `emoji-test.txt` (группы → подгруппы).

```sql
CREATE COLLATION "emoji" (provider = icu, locale = 'und-u-kr-emoji');
```
...
| Многострочные ZWJ в minified JSON | `\u200D` после минификации может слиться | Не минифицируйте JSON как строку — используйте AST-ориентированные инструменты (см. **jq** ниже) |

```bash
jq --compact-output . < data.json > data.min.json
```
...
> 📊 Вывод: сохраняйте UTF-8 в JSON (без `\uXXXX`-экранирования эмодзи):
...
```javascript
JSON.stringify({ emoji: "😀" }); // UTF-8 по умолчанию
```
...
|---------|-------------------------------|-----------|
| **Windows NTFS** | ✅ | В CMD до PS 5.1 для UTF-8: `chcp 65001` |
...
```bash
pg_dump --encoding=UTF8 --no-owner --no-privileges -Fc app_db > app_db.dump
```
...
✅ Санитизация (корректно обрабатывает ZWJ):
...
```
✅ Валидация URL (ошибка при недекодируемой последовательности):
...
**Справочники** — [Основные команды Windows](/encyclopedia/2-system-network/2-05-terminal/102), [Справочник по XML](/encyclopedia/3-data-markup/3-04-konfiguratsii-i-dannye/211), [Основные команды в Linux](/encyclopedia/2-system-network/2-05-terminal/101), [Справочник по XSLT](/encyclopedia/3-data-markup/3-04-konfiguratsii-i-dannye/212), [Справочник по HTTP-протоколу](/encyclopedia/2-system-network/2-03-set-i-internet/611), [Справочник по MongoDB](/encyclopedia/3-data-markup/3-06-nosql/41).
