...
★ **XAML (eXtensible Application Markup Language)** — декларативный язык разметки на базе XML для описания пользовательского интерфейса в экосистеме [.NET](/encyclopedia/5-languages/5-04-platforma-dotnet/intro). Вместо того чтобы в коде создавать каждую кнопку и подписывать обработчики вручную, вы описываете **дерево объектов**: окно, панели, контролы, стили и привязки данных. Компилятор или загрузчик разметки превращает XML в экземпляры классов UI-фреймворка.

XAML читается и человеком, и машиной. Его типичные сценарии:

- **WPF** — классические desktop-приложения Windows ([обзор WPF](https://learn.microsoft.com/ru-ru/dotnet/desktop/wpf/overview/));
...
Режимы `OneWay`, `TwoWay`, `OneTime`; конвертеры — через `{Binding ..., Converter=...}`. В production UI на XAML обычно строят на **MVVM** ([desktop-стеки](/encyclopedia/4-code-dev/4-11-desktopnye-prilozheniya/112.md)).
