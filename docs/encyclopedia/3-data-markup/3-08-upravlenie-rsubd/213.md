...
- **Системные триггеры**: `LOGON`, `LOGOFF`, `STARTUP`, `SHUTDOWN` (уровень экземпляра, не таблицы)

```sql
-- пример системного триггера (Oracle)
CREATE OR REPLACE TRIGGER audit_logon
AFTER LOGON ON DATABASE
BEGIN
  INSERT INTO audit_log (event_time, os_user, db_user)
  VALUES (SYSTIMESTAMP, SYS_CONTEXT('USERENV','OS_USER'), USER);
END;
/
```
...
Включение аудита (параметр экземпляра, затем перезапуск):
