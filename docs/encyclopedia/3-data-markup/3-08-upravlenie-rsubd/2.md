...
description: "Параметры, VACUUM, FDW, конфигурация для 1С, мониторинг, резервное копирование и новинки PostgreSQL 16 — справочник администратора."
---
...
<ExternalPlayEmbed example="data-markup/backup-restore-timeline-play" title="Backup и PITR" minHeight={480} />


<ExternalPlayEmbed example="data-markup/pg-hba-rule-play" title="pg_hba.conf" minHeight={480} />


<ExternalPlayEmbed example="data-markup/vacuum-bloat-play" title="VACUUM и bloat" minHeight={480} />


<ExternalPlayEmbed example="data-markup/postgres-tuning-sandbox-play" title="PostgreSQL — tuning" minHeight={480} />
...
- Поддержка SSL/TLS в `postgresql.conf`:
...
- **`n_tup_newpage_upd`** (с PG16) — оценка эффективности **HOT**-обновлений.
...
- Внешние — `pgBadger` (анализ логов), `Prometheus + postgres_exporter`, [Zabbix — практикум](/encyclopedia/2-system-network/2-06-sistemnoe-administrirovanie/zabbix-praktikum/intro), `Datadog`.

---

<span id="novinki-postgresql-16"></span>

### 11.4. Новинки PostgreSQL 16 (краткий обзор)

Полный список — [release notes](https://postgrespro.ru/docs/postgresql/16/release-16). Ниже — то, что чаще всего встречается в эксплуатации и разработке.
...
| **JSON** | `json_array`, `json_object`, `json_*agg`; предикаты [**`IS JSON`**](/encyclopedia/3-data-markup/3-07-sql/66.md#predikaty-is-json-postgresql-16), `IS JSON ARRAY`, `IS JSON OBJECT`, `IS JSON SCALAR` (функция `json_table` — в планах стандарта) |
...
Установка и первый `psql` — [Первые шаги с SQL](/encyclopedia/3-data-markup/3-07-sql/101.md); обзор для разработчика — [PostgreSQL — практическая работа и API](/encyclopedia/3-data-markup/3-07-sql/888.md).
...
<span id="foreign-data-wrapper-fdw"></span>
...
| **oracle_fdw**, **mysql_fdw**, **tds_fdw** | Сторонние СУБД (устанавливаются отдельно, см. [pgxn.org/tag/fdw](https://pgxn.org/tag/fdw/)) |
...
<div class="callout callout--caution">
  <div class="callout-title">Секции и шарды</div>

  <div class="callout-body">
  Секции на одном сервере и шарды на разных — разные уровни масштабирования. Секция из <strong>foreign table</strong> не даёт распределённой транзакции между узлами; для промышленного шардирования смотрите Citus, Shardman и архитектуру в <a href="/encyclopedia/3-data-markup/3-08-upravlenie-rsubd/1.md#шардинг">обзоре DBA</a>.
  </div>
</div>
...
<span id="postgresql-i-1s"></span>

### 13.5. PostgreSQL и 1С

1С:Предприятие **официально поддерживает** PostgreSQL как СУБД для клиент–серверного режима — см. [работу с БД в 1С](/encyclopedia/5-languages/5-27-1s/118.md). Для production нужна сборка **с патчами 1С** (дистрибутив на [releases.1c.ru](https://releases.1c.ru)) или **Postgres Pro Standard / Enterprise**, где патчи уже включены. "Vanilla" PostgreSQL с PGDG подходит для обучения SQL, но не для сертифицированной связки с 1С.
...
<div class="callout callout--warning">
  <div class="callout-title">Не для учебного vanilla PostgreSQL</div>

  <div class="callout-body">
  Параметры <code>plantuner</code> и <code>online_analyze</code> относятся к сборкам с патчами 1С / Postgres Pro. На обычном PostgreSQL 16 из PGDG их не включайте — достаточно базовой настройки памяти и autovacuum из §13.2.
  </div>
</div>
