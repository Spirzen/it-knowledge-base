...
description: PostgreSQL не освобождает место сразу после DELETE. Используйте — VACUUM — помечает место как свободное, VACUUM FULL — перезаписывает таблицу, освобождая место на диске (блокирует таблицу), pg_repack…
---
...
- TDE (Transparent Данные Encryption) не встроен, но реализуется через расширения (например, `pg_tde` в разработке).
...
-- Обновление
UPDATE products
SET Данные = jsonb_set(Данные, '{price}', '199.99')
WHERE id = 1;
```
