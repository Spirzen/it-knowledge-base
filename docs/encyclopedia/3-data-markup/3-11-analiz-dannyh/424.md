...
<ExternalPlayEmbed example="data-markup/time-series-decompose-play" title="Декомпозиция ряда" minHeight={480} />
...
<div class="callout callout--tip">
  <div class="callout-title">Связанные главы Python-стека</div>

  <div class="callout-body">
  <strong>NumPy</strong> — <a href="/encyclopedia/5-languages/5-02-python/337">337 — массивы и LA</a>, <a href="/lab/Примеры/1129">Lab 1129</a>.<br />
  <strong>Pandas</strong> — <a href="/encyclopedia/5-languages/5-02-python/33">33 — обзор</a>, <a href="/encyclopedia/5-languages/5-02-python/331">331 — practicum</a>.<br />
  <strong>Matplotlib</strong> — <a href="/encyclopedia/5-languages/5-02-python/319">319 — графики</a>.
  </div>
</div>
...
**Подробно:** [337 — NumPy: массивы, векторы и матрицы](/encyclopedia/5-languages/5-02-python/337) (теория и [справочник функций](/encyclopedia/5-languages/5-02-python/337#numpy-chasto-funktsii)). **Практика:** [Lab 1129](/lab/Примеры/1129). **Математика:** [Векторы](/encyclopedia/3-data-markup/3-03-myslitelnaya-baza/342), [Матрицы](/encyclopedia/3-data-markup/3-03-myslitelnaya-baza/343).
...
<span id="tekst-kak-priznaki"></span>

### Текст как признаки

В таблице pandas каждый столбец — число или категория. **Текст** (отзыв, тикет, комментарий) модель принимает только после **кодирования** — превращения строки в вектор чисел фиксированной длины. Это тот же принцип, что [кодирование категориальных признаков](/encyclopedia/6-ai/6-02-mashinnoe-obuchenie/4) для столбца "город", только единицами становятся **слова** или **токены**.

Общая картина NLP — в [NLP и работа с текстом](/encyclopedia/6-ai/6-09-transformery-i-nlp/1). Ниже — минимум для аналитика, который уже работает с pandas и sklearn.
...
| Способ | Суть | Типичное применение |
|--------|------|---------------------|
| Bag-of-words | 1 в ячейке, если слово есть в документе | Быстрая проверка идеи |
| TF | Счётчик вхождений по словарю | Короткие тексты, частотный анализ |
| TF-IDF | TF с поправкой на редкость в корпусе | Классификация, поиск похожих документов |
| Индекс + embedding | Номер слова → обучаемый вектор | [PyTorch](/encyclopedia/5-languages/5-02-python/333), трансформеры |
...
- `fit_transform` обучает словарь **на переданных текстах**. Для test и production — только `transform`, иначе [утечка данных](/encyclopedia/6-ai/6-02-mashinnoe-obuchenie/6) (модель "видит" статистику будущих документов).
...
**Тональность** — метка настроения текста (например, positive / negative). Задача сводится к [классификации](/encyclopedia/3-data-markup/3-11-analiz-dannyh/424) после кодирования.
...
- `classification_report` показывает precision, recall, F1 по классам — см. [метрики ML](/encyclopedia/6-ai/6-02-mashinnoe-obuchenie/1).

Прикладной контекст — [распознавание текста и отзывов](/encyclopedia/6-ai/6-06-primenenie-ii/120).

#### Путь к PyTorch

Разреженный TF-IDF (тысячи столбцов, много нулей) хорошо сочетается с линейными моделями sklearn. В **глубоком обучении** текст кодируют **целыми индексами токенов**, а смысл слова несёт слой `nn.Embedding` — таблица обучаемых векторов. Сквозной код — [практикум по тональности](/encyclopedia/5-languages/5-02-python/336); основы тензоров и цикла обучения — [PyTorch для разработчика](/encyclopedia/5-languages/5-02-python/333).

**Маршрут после таблиц**

- [pandas и NumPy](/encyclopedia/5-languages/5-02-python/33) — подготовка данных
- TF-IDF baseline — этот раздел
- [scikit-learn](/encyclopedia/5-languages/5-02-python/332) — классические модели
- [PyTorch](/encyclopedia/5-languages/5-02-python/333) — нейросети
- [тональность на тексте](/encyclopedia/5-languages/5-02-python/336) или [MNIST](/encyclopedia/5-languages/5-02-python/335)
