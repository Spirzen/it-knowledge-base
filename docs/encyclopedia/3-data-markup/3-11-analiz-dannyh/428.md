...
Сводка **типовых вызовов pandas**, которые закрывают большинство задач при исследовании таблицы в ноутбуке или скрипте: от `read_csv()` до экспорта отчёта. Команды сгруппированы по смыслу, как в учебных шпаргалках по DataFrame. **NumPy** (массивы, `mean`, `axis`) — [1129 — примеры с разбором](/lab/Примеры/1129). **Готовые скрипты Pandas** с построчным разбором — [Pandas — типовые операции](/lab/Примеры/1113). **Те же задачи в Excel** (СУММ, СУММЕСЛИ, ВПР) — [1139 — формулы в Lab](/lab/Примеры/1139). Базовое чтение `.txt` и CSV через stdlib — [Python — файлы и текст](/lab/Примеры/1126).
