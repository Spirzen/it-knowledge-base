...
description: "Batch, bulk, chunk и поток — теория пакетной обработки, массовые CRUD в одном запросе, разбиение тяжёлых операций, транзакции, идемпотентность, ETL, очереди, MapReduce и инструменты."
...
- **Поштучно:** \(T_{\text{row}} \approx N \cdot (L + r)\)
- **Пакетом (одна транзакция, один round-trip):** \(T_{\text{bulk}} \approx L + N \cdot r'\), где \(r' \le r\) за счёт буферизации СУБД и бинарных протоколов
...
Семантика доставки и идемпотентность потребителя — [133](/encyclopedia/2-system-network/2-09-osnovy-integratsionnogo-vzaimodeystviya/133). Батчинг продюсера Kafka — [119](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/119).
...
Hub по доставке и exactly-once на практике — [133](/encyclopedia/2-system-network/2-09-osnovy-integratsionnogo-vzaimodeystviya/133). Импорт с checkpoint в C# — [45](/encyclopedia/5-languages/5-05-csharp/45).
...
|------|------------|-----------|
| **Задержка волной** | Шаг 3 стартует до готовности шага 2 | Оркестратор с зависимостями (Airflow, Dagster) — [425](/encyclopedia/3-data-markup/3-11-analiz-dannyh/425) |
...
Это **не** замена ночного ETL: объём до тысяч–десятков тысяч записей в синхронном запросе, ответ за секунды–минуты. Миллионы строк — [чанки](#chunk-tuning), staging и фоновый job ([425](/encyclopedia/3-data-markup/3-11-analiz-dannyh/425)).
...
| **Update** | `PUT` / `PATCH /resources/{id}` | `PATCH /resources/bulk`, массив `{ id, patch }` | Нужен стабильный ключ строки |
...
Связка с [идемпотентностью](#idempotency) и заголовком `Idempotency-Key` — [213](/encyclopedia/7-project/7-06-proektirovanie-i-arhitektura/design/213), [HTTP-интеграции](/encyclopedia/2-system-network/2-09-osnovy-integratsionnogo-vzaimodeystviya/118).
...
Теория очередей: at-least-once, DLQ, повтор — [121](/encyclopedia/2-system-network/2-09-osnovy-integratsionnogo-vzaimodeystviya/121), [122](/encyclopedia/2-system-network/2-09-osnovy-integratsionnogo-vzaimodeystviya/122).
...
Spark: чтение JDBC с **`numPartitions`** и диапазоном по numeric key — тот же **range split**, что выше. PySpark в аналитике — [426](/encyclopedia/3-data-markup/3-11-analiz-dannyh/426).
...
9. Сверить с **потоком**: что нельзя batch — вынести в [423](/encyclopedia/3-data-markup/3-11-analiz-dannyh/423).
...
| Тема | Статья |
|------|--------|
| Поток vs пакет | [423 — потоковая аналитика](/encyclopedia/3-data-markup/3-11-analiz-dannyh/423) |
| ETL, оркестрация | [425](/encyclopedia/3-data-markup/3-11-analiz-dannyh/425) |
| Big Data, DWH | [11](/encyclopedia/3-data-markup/3-11-analiz-dannyh/11) |
...
| REST batch API | [117](/encyclopedia/2-system-network/2-09-osnovy-integratsionnogo-vzaimodeystviya/117#rest-api-design-principles) |
...
| Идемпотентность доставки | [133](/encyclopedia/2-system-network/2-09-osnovy-integratsionnogo-vzaimodeystviya/133) |
| Проектирование БД, интеграции | [116 — БД](/encyclopedia/7-project/7-06-proektirovanie-i-arhitektura/design/116), [117 — API и интеграции](/encyclopedia/7-project/7-06-proektirovanie-i-arhitektura/design/117), [213 — идемпотентность](/encyclopedia/7-project/7-06-proektirovanie-i-arhitektura/design/213) |
