...
description: Константа Описание API+ Примечания -------------------------------------- ACTION_MAIN Главная точка входа (без данных) 1+ Требует CATEGORY_LAUNCHER для иконки в лаунчере. ACTION_VIEW Открыть данные…
...
| `moduleName` | Имя библиотеки (`Система.loadLibrary("native-lib")`). |
...
val userNameFlow: Flow<String> = dataStore.Данные
    .map { preferences ->
...
// Из DataStore:
val userName by dataStore.Данные.collectAsState(initial = "Guest")
...
| `adb logcat -b main,Система,crash -v threadtime` | Фильтрация по буферам и формату | `-b all` — все буферы (`radio`, `events`, `kernel` и др.) |
...
- `pkgFlags=[ СИСТЕМА DEBUGGABLE ]` — флаги пакета
...
- `Система`: `screen_brightness`, `screen_off_timeout`, `volume_ring`, `volume_music`
...
| `<certificates>` | `src` (`Система`, `user`, `@raw/...`, `@xml/...`) | Источник: `Система` — встроенные CA; `user` — добавленные пользователем; `@raw/ca_cert` — PEM-файл; `@xml/trust_anchors` — `<certificates>` в XML. |
| `<debug-overrides>` | — | Применяется **только** при `android:debuggable="true"` (игнорируется в release-сборке). |

> ⚠️ **Важно**:  
> - Если `<pin-set>` задан, но **не указан `<trust-anchors>`**, используются `Система` + `user` по умолчанию.  
