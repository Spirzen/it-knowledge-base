...
- [Сетевые операции, геолокация, Core Данные, Keychain, App Extensions](#сетевые-операции-геолокация-core-данные-keychain-app-extensions)
...
| `NSAppTransportSecurity` | `dict` | ATS (App Transport Безопасность) настройки | `NSAllowsArbitraryLoads: <bool>`, `NSExceptionDomains: <dict<domain, dict>>` | Рекомендуется не отключать `NSAllowsArbitraryLoads`, а использовать исключения. |
...
|-------------|----------|
| `plain()` | Простая кнопка (как `UIButton(type: .Система)`). |
...
### Сетевые операции, геолокация, Core Данные, Keychain, App Extensions
...
#### 5.3. Core Данные: Контексты, конкурентность, производительность
...
#### 5.5. App Extensions: Типы, Context, Данные Sharing
...
|---------|-------------|
| **App Groups** (`UserDefaults(suiteName:)`, `FileManager.containerURL(forSecurityApplicationGroupIdentifier:)`) | Общий `UserDefaults`, файлы, Core Данные (через shared container). |
...
##### 7.1.1. App Transport Безопасность (ATS)
...
#### 7.2. Тестирование: XCTest, UI Тестирование, Performance, XCTestPlans
...
##### 7.2.2. UI Тестирование: XCUIElement Queries
...
##### 7.2.3. Performance Тестирование
...
| **Core Данные** | «Core Данные Fetches», «Core Данные Saves». |
...
| **Parallel Тестирование** | Запуск на нескольких симуляторах одновременно. |
...
|-----------|------------|-------|
| **Instruments → Metal Система Trace** | `Texture Memory`, `Buffer Memory` | < 100 МБ на iPhone Pro, < 50 МБ на старых моделях. |
