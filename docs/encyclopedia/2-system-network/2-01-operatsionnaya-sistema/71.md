...
description: ⚠️ Начиная с iOS 13, если используется SceneDelegate — launch options приходят в scene(_ — willConnectTo — options — ) через UIScene.ConnectionOptions.
...
Команды, пути, службы и параметры **iOS** — для администрирования и работы в терминале. Учебный курс: [раздел](/encyclopedia/2-system-network/2-01-operatsionnaya-sistema/intro).

## Краткое пояснение

Настройки и возможности iOS.
...
- [Core App Metadata, Launch Configuration & Runtime Environment](#core-app-metadata-launch-configuration-runtime-environment)
- [UI/UIKit Core](#uiuikit-core)
- [UI-компоненты](#ui-компоненты)
- [Модальные представления, Scene Lifecycle, Фоновые режимы, Privacy & Permissions](#модальные-представления-scene-lifecycle-фоновые-режимы-privacy-permissions)
- [Сетевые операции, геолокация, Core Data, Keychain, App Extensions](#сетевые-операции-геолокация-core-данные-keychain-app-extensions)
- [SwiftUI, Accessibility в SwiftUI, WidgetKit, Push Notifications, App Clips](#swiftui-accessibility-в-swiftui-widgetkit-push-notifications-app-clips)
- [Безопасность, Тестирование, Localization, Distribution, Debugging](#безопасность-тестирование-localization-distribution-debugging)
- [Производительность, Crash Reporting, CI/CD, Документация, Deprecation](#производительность-crash-reporting-cicd-документация-deprecation)
...
Все ключи в `Info.plist` являются **строками**, но значения — строго типизированы: `<string>`, `<integer>`, `<array>`, `<dict>`, `<bool>`, `<Данные>`.
...
Фокус — на **публичных API, runtime-состояниях, диагностируемых атрибутах и поведениях, критичных для корректной работы приложения в реальных условиях** (включая edge Кейсы: отклонение разрешений, переходы между foreground/background, adaptive interfaces).
...
| `networkServiceType` | `URLSessionTask.NetworkServiceType` | `.default`, `.voip`, `.video`, `.background`, `.voice`, `.networkServiceTypeResponsiveData` | Влияет на QoS и Энергетика usage. |
...
- Не поддерживает `uploadTask(with:from:)` с `Данные` — только с `FileURL`.  
...
| `.Сеть` | Нет сети/GPS | Повторить позже, использовать кэш |
...
| Entitlement | `com.apple.Безопасность.application-groups` |
...
|-----|----------|--------|
| `kSecUseAuthenticationUI` | `kSecUseAuthenticationUIFail`, `kSecUseAuthenticationUIAllow`, `kSecUseAuthenticationUISkip` | Показывать ли Система prompt. |
...
Фокус — на **публичных, стабильных API, runtime-диагностике, совместимости**, а также на **ограничениях и edge Кейсы**, критичных для production-развёртывания.
...
> - Нет Сеть requests в `getTimeline(...)`. Только кэш/фоновая загрузка.  
...
### Безопасность, Тестирование, Localization, Distribution, Debugging
...
#### 7.1. Безопасность: ATS, Certificate Pinning, SSL, Biometry, Secure Enclave
...
#### 7.2. Тестирование: XCTest, UI testing, Performance, XCTestPlans
...
| **Энергетика Log** | Рейтинг энергоэффективности (App Store требует ≥3/5). |
...
##### 8.1.4. Энергетика Impact

| Показатель | Диагностика | Причины высокого потребления |
|-----------|-------------|-----------------------------|
| **CPU Wakeups** | Энергетика Log → «CPU wakeups / sec» | Таймеры (`Timer`), polling, `CADisplayLink` без паузы. |
| **Location Accuracy** | `kCLLocationAccuracyBest` в фоне | Использовать `significantLocationChanges` или `reduced`. |
| **Сеть Calls** | Battery Usage → «Сеть» | Частые мелкие запросы → объединять в batch. |
| **Background Activity** | «Background Энергетика use» | `beginBackgroundTask` без `endBackgroundTask`. |
...
| `-exportOptionsPlist export.plist` | Конфигурация экспорта (ad-hoc, app-store, Разработка). |
...
| Область | Примечание |
|---------|------------|
| Версии | актуальные LTS/стабильные релизы **iOS** |
...
**Справочники** — [Справочник по Linux](/encyclopedia/2-system-network/2-01-operatsionnaya-sistema/51), [Справочник по Android](/encyclopedia/2-system-network/2-01-operatsionnaya-sistema/81), [Справочник по Windows 11](/encyclopedia/2-system-network/2-01-operatsionnaya-sistema/41), [Основы IP-адресации](/encyclopedia/2-system-network/2-03-set-i-internet/41), [Справочник по Microsoft Excel](/encyclopedia/1-basics/1-15-tekst/41), [Справочник по HTTP-протоколу](/encyclopedia/2-system-network/2-03-set-i-internet/611).

**Мобильная разработка** — [Мобильные игры](/encyclopedia/9-spinoff/9-04-razrabotka-igr/122), [Справочник по Android](/encyclopedia/2-system-network/2-01-operatsionnaya-sistema/81), [Особенности тестирования мобильных приложений](/encyclopedia/7-project/7-05-testirovanie/124), [Kotlin — о разделе](/encyclopedia/5-languages/5-09-kotlin/intro), [Беспроводные технологии - Bluetooth, Zigbee, NFC](/encyclopedia/2-system-network/2-10-zhelezo/119), [Swift — о разделе](/encyclopedia/5-languages/5-14-swift/intro).
