...
| Задача | Подсказка |
|--------|-----------|
| Справка | `man`, `--help`, документация **Windows 11** |
| Конфигурация | см. таблицы в разделе «Справочные таблицы» |
| Права | пользователь, группа, `sudo` при необходимости |
...
| **Virtualization-Based Безопасность (VBS)** | `Enabled`, `Disabled`, `Not Supported` | Зависит от поддержки CPU (SLAT, NX), BIOS и TPM. Состояние: `Get-CimInstance -ClassName Win32_DeviceGuard -Namespace root\Microsoft\Windows\DeviceGuard` → `VirtualizationBasedSecurityStatus`. |
...
| **SerialNumber (Система)** | Строка | BIOS/UEFI Serial. Может быть `To be filled by O.E.M.` на сборных ПК. |
...
| `DisableCAD` | `REG_DWORD` | `1` / `0` | Отключает Ctrl+Alt+Del на экране входа. Требуется политика `Безопасность Options → Interactive logon: Do not require CTRL+ALT+DEL`. |
...
`HKLM\СИСТЕМА\CurrentControlSet\Control\DeviceGuard`
...
##### 2.3.3. Аудит событий (`HKLM\БЕЗОПАСНОСТЬ\Policy\PolAdtEv` — **защищён**, доступ через `auditpol.exe`)
...
`HKLM\СИСТЕМА\CurrentControlSet\Services\Tcpip\Parameters`
...
##### 2.5.2. DNS и DoH  
`HKLM\СИСТЕМА\CurrentControlSet\Services\Dnscache\Parameters`
...
| **Конфигурация компьютера → Административные шаблоны → Система → Учётные записи устройств** → `Настройка параметров Device Guard` | `HKLM\СИСТЕМА\CurrentControlSet\Control\DeviceGuard\*` | Включить/Отключить HVCI, VBS | Только на Enterprise/Образование. |
...
| `wevtutil` | `wevtutil qe Система /f:text /c:10` | `Get-WinEvent -LogName Система -MaxEvents 10` |
| `fsutil` | `fsutil behavior query DisableDeleteNotify` | `Get-ItemProperty HKLM:\СИСТЕМА\CurrentControlSet\Control\FileSystem -Name "DisableDeleteNotify"` |
...
- Тип запуска (`Boot`, `Система`, `Automatic`, `Manual`, `Disabled`),  
- Зависимости (через `sc.exe qc <имя>` или `Get-Service | Select-Object Name, DependentServices`),  
- Реестровый путь: `HKLM\СИСТЕМА\CurrentControlSet\Services\<ServiceName>`.
...
| **Audit Mode** | Рабочий стол без учётной записи, доступны все административные инструменты | `HKLM\СИСТЕМА\Setup\AuditBoot = 1`, `HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Setup\State\IMAGE_STATE = IMAGE_STATE_AUDIT_MODE` | Вход: `sysprep /audit /reboot`. Выход: `sysprep /oobe /shutdown` или `sysprep /generalize /shutdown`. |
...
| **Windows 365 Boot** | Загрузка в облако (Cloud PC) вместо локальной ОС | BIOS/UEFI загружает `CloudPCBoot.wim`, `HKLM\СИСТЕМА\CurrentControlSet\Control\CloudPC` существует | Диагностика: `Get-CimInstance -Namespace root\cimv2\CloudPC -ClassName CloudPC_ConnectionStatus`. |
...
> Description: Windows(R) Operating Система, OEM_DM channel
...
| **Политики групповой безопасности (локальные)** | `secedit /export /cfg Безопасность.cfg` | Текстовый файл с `UserRightsAssignment`, `RegistryValues`, `PrivilegeRights`. |
...
| Применение политик безопасности | `secedit /configure /db secedit.sdb /cfg Безопасность.cfg /areas SECURITYPOLICY` |
...
| **4. Безопасность** | - Включить HVCI (`Безопасность → Device Безопасность`) - Установить VS Code, Git, PowerShell 7+, Node.js, .NET 8 SDK - Отключить Defender в доверенных папках (через `Add-MpPreference -ExclusionPath`) |
...
| **Документирование** | Экспорт конфигурации в Markdown: `Get-ComputerInfo | ConvertTo-Markdown > Система.md` (через модуль `MarkdownPS`). |
