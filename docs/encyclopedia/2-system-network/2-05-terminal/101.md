...
| `curl URL` | HTTP-запрос, заголовки, API |
...
`grep` сопоставляет строки с **регулярным выражением** (по умолчанию базовый синтаксис BRE; расширенный — флаг `-E`, как в `egrep`). Таблица `.`, `*`, `^`, `$`, классов `[...]` — в [справочнике RegEx](/encyclopedia/4-code-dev/4-02-chto-takoe-kod-i-kak-on-rabotaet/615).
...
Полезно для тестирования API, скачивания файлов, отправки запросов. Подробный разбор флагов, POST, аутентификации и скриптов — [утилита curl](/encyclopedia/2-system-network/2-05-terminal/1133).
