...
Справочник по **повседневным командам Linux** в интерактивном терминале: файлы, текст, процессы, сеть, пакеты, права, systemd. Такой набор закрывает большинство задач в shell на сервере и в WSL. Предполагается базовое знакомство с [терминалом и оболочкой](/encyclopedia/2-system-network/2-05-terminal/1); синтаксис `|`, `>`, `&&` — в [статье про знаки препинания](/encyclopedia/2-system-network/2-05-terminal/11). Примеры ориентированы на Debian/Ubuntu (`apt`); для Fedora/Arch смотрите таблицу менеджеров пакетов ниже.
...
<div class="callout callout--warning">
  ...
  <div class="callout-body">
  <p><code>rm -rf</code> необратима; в WSL путь <code>/mnt/c/...</code> бьёт по диску Windows. Полный список ловушек (pipe в bash, Git, агент в IDE) — <a href="/encyclopedia/8-infra-security/8-03-zabota-o-kode-i-dannyh/101">Опасные скрипты</a>.</p>
</div>
  </div>
...
`grep` сопоставляет строки с **регулярным выражением** (по умолчанию базовый синтаксис BRE; расширенный — флаг `-E`, как в `egrep`). Таблица `.`, `*`, `^`, `$`, классов `[...]` — в [справочнике RegEx](/encyclopedia/4-code-dev/4-02-chto-takoe-kod-i-kak-on-rabotaet/615); готовые команды для логов и фильтров — [Regex — готовые паттерны](/lab/Примеры/615); однострочник `grep -RIn` и фильтр ERROR — [Bash — галерея Lab](/lab/Примеры/1151#bash-grep).
