...
| Задача | Подсказка |
|--------|-----------|
| Справка | `man`, `--help`, документация **Основные команды Windows** |
...
|eventvwr	|Get-EventLog -LogName Система	|Просмотр логов системы
...
```powershell
PS C:\> Get-EventLog -LogName Система -Newest 10
```
...
C:\secure\file.txt NT AUTHORITY\СИСТЕМА:(F)
...
**PowerShell**
```powershell
PS C:\> $env:Path = [Система.Environment]::GetEnvironmentVariable("Path","Machine") + ";" + [Система.Environment]::GetEnvironmentVariable("Path","User")
```
