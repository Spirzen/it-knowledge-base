...
description: Apache Kafka - потоковая обработка событий в реальном времени, масштабирование потребителей и построение event-driven архитектур.
---
...
> **Справочник:** CLI `kafka-*`, настройки producer/consumer/broker и consumer groups — [Справочник по Apache Kafka](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/1205).
>
> **Углубление:** [Kafka](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/119) в разделе "Микросервисы и интеграция".
...
<ExternalPlayEmbed example="system-network/kafka-simulation" title="Симуляция Kafka" minHeight={480} />
...
Kafka изначально создавалась в LinkedIn как **распределённый журнал событий** (commit log): данные только добавляются, хранятся на диске и доступны многим подписчикам одновременно. Сегодня платформа лежит в основе event-driven архитектур, потоковой аналитики и интеграции микросервисов.

![image-14.png](https://assets.spirzen.ru/encyclopedia/2-system-network/2-09-osnovy-integratsionnogo-vzaimodeystviya/image-14.png)
...
Kafka решает задачу **управления данными в движении** — когда сотни сервисов, баз и SaaS-систем нужно связать в единый поток событий в реальном времени. По сравнению с классическими брокерами очередей у неё несколько сильных сторон.

**Несколько производителей.** Множество сервисов могут писать в один топик в едином формате. Например, все микросервисы сайта отправляют просмотры страниц в топик `page-views`, а аналитика читает один поток вместо десятка отдельных очередей.

**Несколько потребителей.** В модели pub/sub каждый подписчик получает копию потока. Сообщение, прочитанное одним consumer group, остаётся доступным для других групп. Внутри одной группы каждая партиция назначается ровно одному consumer — это балансировка нагрузки без "съедания" сообщения другими клиентами.

**Долговременное хранение на диске.** Сообщения сохраняются по настраиваемым правилам (`retention.ms`, `retention.bytes`). Consumer может отставать, перезапускаться или читать историю с начала — данные в топике сохраняются. Это принципиально отличает Kafka от очереди, где сообщение исчезает после ack.

**Масштабируемость и производительность.** Горизонтальное масштабирование через партиции и брокеры; последовательная запись на диск и батчинг дают миллионы сообщений в секунду при низкой задержке.

**Экосистема.** Поверх ядра строятся Kafka Connect (интеграция с БД и внешними системами), Kafka Streams и ksqlDB (потоковая обработка), Schema Registry (эволюция схем Avro/Protobuf). Подробнее — [Kafka в разделе 8](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/119).

<div class="callout callout--info">
  <div class="callout-title">Kafka и RabbitMQ</div>

  <div class="callout-body">
  RabbitMQ — классическая **очередь** (point-to-point, routing, ack → удаление). Kafka — **журнал событий** с retention и повторным чтением. Для задач "обработать и забыть" часто достаточно RabbitMQ; для event sourcing, аналитики потоков и нескольких независимых подписчиков — Kafka. Подробнее — [RabbitMQ](./122.md).
  </div>
</div>
...
| Сценарий | Как используется Kafka |
|----------|------------------------|
| **Activity tracking** | Клиентские приложения пишут события (клики, просмотры) в топики; downstream — отчёты, ML, поиск |
| **Обмен сообщениями** | Сервисы публикуют "сырые" уведомления; отдельный consumer форматирует и отправляет email/SMS/push |
| **Метрики и логи** | Приложения шлют метрики и лог-строки в топики; потребители — Prometheus/Grafana, Elasticsearch, Hadoop |
| **Event-driven микросервисы** | Доменные события (`OrderCreated`, `PaymentCompleted`) связывают сервисы без синхронных цепочек HTTP |
| **CDC и интеграция** | Debezium/Kafka Connect транслирует изменения из PostgreSQL, MySQL в другие системы |
| **Stream processing** | Агрегации, окна, join потоков в реальном времени через Kafka Streams |
...
**Топики** — именованные каналы, куда producer **добавляет** сообщения в конец журнала. Топик только растёт; потребители читают с нужного offset.
...
![image-15.png](https://assets.spirzen.ru/encyclopedia/2-system-network/2-09-osnovy-integratsionnogo-vzaimodeystviya/image-15.png)
...
С **Kafka 4.0** единственный режим координации — **KRaft** (встроенный Raft-консенсус). ZooKeeper удалён из дистрибутива. Для локальной разработки удобнее Docker или пошаговый практикум — [Java-приложение с Apache Kafka и PostgreSQL](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/1191).

#### Установка Java

Kafka работает поверх Java (JDK 17+ для актуальных версий). Проверка:
...
#### Быстрый старт через Docker (KRaft)
...
Полный набор переменных и CLI — в [Справочнике по Apache Kafka](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/1205).

---

#### Установка из архива (KRaft)

1. Скачайте бинарный дистрибутив с [kafka.apache.org](https://kafka.apache.org/community/downloads/).
2. Сгенерируйте cluster id и отформатируйте хранилище (один раз):
...
Ключевые параметры KRaft в `server.properties`:
...
<div class="callout callout--warning">
  <div class="callout-title">Legacy-кластеры с ZooKeeper</div>

  <div class="callout-body">
  Kafka 3.x в режиме ZooKeeper ещё встречается в старых инсталляциях. Новые проекты и миграции — только KRaft. Подробная настройка single-node на Windows — в [практикуме 1191](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/1191).
  </div>
</div>
...
Kafka требует Java (OpenJDK 17+ для Kafka 4.x). Запуск возможен вручную, через Docker или в управляемых сервисах (Confluent Cloud, AWS MSK, Azure Event Hubs for Kafka).

Пример KRaft через Docker Compose (без ZooKeeper):
...
Kafka даёт предсказуемые гарантии на уровне платформы; итоговая семантика зависит от настроек producer, broker и consumer.

**Что гарантирует Kafka:**

- **Порядок внутри партиции** — если producer A записал сообщение B после A в одну партицию, consumer прочитает B после A.
- **Durability** — зафиксированное сообщение (записано во все in-sync реплики при `acks=all`) сохраняется, пока жива хотя бы одна реплика.
- **Consumer видит только committed** — записи из незавершённых транзакций скрыты при `isolation.level=read_committed`.

**Семантики на уровне приложения:**

| Семантика | Суть | Типичная настройка |
|-----------|------|-------------------|
| At-most-once | Сообщение может потеряться | `acks=0`, commit offset до обработки |
| At-least-once | Дубликаты возможны, потерь нет | `acks=all`, commit после обработки, идемпотентный consumer |
| Exactly-once | Однократная обработка | `enable.idempotence=true`, транзакционный producer, `read_committed` |

Область действия EOS, связка с outbox и **effectively exactly-once** — [Идемпотентность и семантика доставки](/encyclopedia/2-system-network/2-09-osnovy-integratsionnogo-vzaimodeystviya/133). Подробнее — [Kafka в разделе 8](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/119) и [Справочник по Apache Kafka](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/1205).

---

### Механизмы обработки

- **Exactly-once semantics** — идемпотентный producer (`enable.idempotence`) плюс транзакции для связки "запись в Kafka + запись в БД" или stream processing.
- **Stream processing** — Kafka Streams или ksqlDB для агрегаций, окон и join потоков без отдельного кластера.
- **Connect** — фреймворк для интеграции с внешними системами (БД, S3, Elasticsearch) через готовые коннекторы.
...
- **Headers** — метаданные записи (trace-id, тип события); задаются в `ProducerRecord`.
- **Interceptors** — hooks producer/consumer для метрик и логирования без дублирования кода.

<div class="callout callout--info">
  <div class="callout-title">Формат payload</div>

  <div class="callout-body">
  String и JSON подходят для прототипов. В prod с несколькими командами — Avro или Protobuf и Schema Registry. Подробнее — [Kafka, Schema Registry](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/119#kafka-schema-registry).
  </div>
</div>
...
- **Сериализация** — гибкая поддержка форматов через интерфейсы `Serializer`/`Deserializer`; в prod — Avro/Protobuf + [Schema Registry](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/119#kafka-schema-registry).
...
Продолжение темы в разделе "Инфраструктура и безопасность" — [Kafka](/encyclopedia/8-infra-security/8-05-mikroservisy-i-integratsiya/119).
