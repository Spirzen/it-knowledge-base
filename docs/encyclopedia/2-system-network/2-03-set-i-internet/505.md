...
description: >-
  Кроссплатформенный справочник сетевой диагностики (ping, ss, ipconfig, dig),
  curl, Wireshark, PuTTY и инструменты для Home Lab.
related:
  - title: Как работать с сетью
    doc: encyclopedia/2-system-network/2-03-set-i-internet/81
  - title: Ускорение интернета
    doc: encyclopedia/2-system-network/2-03-set-i-internet/220
  - title: Приватность и self-hosting
    doc: encyclopedia/2-system-network/2-03-set-i-internet/404
---
...
<div class="callout callout--tip">
  <div class="callout-title">Сеть для power user</div>

  <div class="callout-body">
  Домашний сервер, WireGuard и "почему не открывается сайт" требуют тех же утилит, что и у разработчика.

  Маршрут раздела — [Как работать с сетью](/encyclopedia/2-system-network/2-03-set-i-internet/81); теория протоколов — в [оглавлении](/encyclopedia/2-system-network/2-03-set-i-internet/intro). **Дальше:** [приватность и self-hosting](/encyclopedia/2-system-network/2-03-set-i-internet/404).
</div>
  </div>
...
При сбоях подключения, "сайт не открывается" или нестабильном канале сначала смотрят **доступность** (`ping`), **маршрут** (`tracert` / `mtr`), **адреса и DNS** (`ipconfig` / `ip`, `dig` / `nslookup`), затем **порты и HTTP** (`netstat` / `ss`, `curl -I`). Полная кроссплатформенная таблица с примерами — в [Сетевых подключениях и диагностике](/encyclopedia/2-system-network/2-06-sistemnoe-administrirovanie/6#setevye-komandy-diagnostika).
...
**Когда что использовать:** `ping` / `Test-NetConnection` — "жив ли хост"; `tracert` — где рвётся маршрут; `nslookup` / `Resolve-DnsName` — проблема DNS; **Wireshark** — только если нужно увидеть пакеты (TLS внутри шифрован, но SYN/RST и DNS видны). Теория протоколов — в [разделе "Сеть"](/encyclopedia/2-system-network/2-03-set-i-internet/intro).

Установка CLI-утилит на Windows: `winget` (см. [1.035 / 501](/encyclopedia/1-basics/1-035-bazovaya-informatika/201)), на Linux — `apt` / `dnf`.
...
Для главы про **диагностику** достаточно знать, откуда ставить CLI. Подробнее про `winget` и воспроизводимый стек — в [обзоре раздела](/encyclopedia/1-basics/1-035-bazovaya-informatika/201).
