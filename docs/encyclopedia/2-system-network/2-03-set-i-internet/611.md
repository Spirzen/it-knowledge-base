...
description: Справочник по HTTP — методы, коды ответов, заголовки, кэширование, cookies и отличия HTTP/2 и HTTP/3 (RFC 9110–9114).
...
Методы, коды ответов, заголовки и форматы **HTTP** в табличном виде. Учебный курс: [раздел](/encyclopedia/2-system-network/2-03-set-i-internet/intro).

---

## Краткое пояснение

Таблицы ниже — для быстрого поиска по RFC 9110–9114. Кратко о свойствах методов:

- **Безопасный метод** — не меняет состояние сервера (*safe*, RFC 9110).
- **Идемпотентный** — повтор с теми же параметрами даёт тот же эффект на ресурсе (ответ может отличаться: `200` vs `204`).
- **Кэшируемость** — по умолчанию кэшируются ответы `200`, `203`, `204`, `206`, `300`, `301`, `308`, `404`, `405`, `410`, `414`, `501` (RFC 9111 §4.2.2); для остальных нужны явные директивы `Cache-Control`.
...
Подробнее про флаги, методы и отладку — [утилита curl](/encyclopedia/2-system-network/2-05-terminal/1133), [curl / fetch — примеры](/lab/Примеры/1133). Методы, коды и заголовки — в таблицах ниже; в браузере — DevTools → Network.
...
- [Тело сообщения (Message Body)](#тело-сообщения-message-body)
- [Кодировки и сжатие (`Content-Encoding`)](#кодировки-и-сжатие-content-encoding)
- [Аутентификация: схемы и безопасность](#аутентификация-схемы-и-безопасность)
- [Сессии: углублённый разбор `Set-Cookie`](#сессии-углублённый-разбор-set-cookie)
- [Кэширование: продвинутые сценарии](#кэширование-продвинутые-сценарии)
- [Соединения и транспорт](#соединения-и-транспорт)
- [HTTP-методы (RFC 9110)](#http-methods-rfc)
- [HTTP/2 (RFC 9113) и HTTP/3 (RFC 9114) — ключевые отличия](#http2-rfc-9113-и-http3-rfc-9114-ключевые-отличия)
- [HTTP-методы — практические примеры](#http-methods-practical)
- [Шпаргалка по статус-кодам](#status-codes-cheatsheet)
- [Статус-коды — подробно](#status-codes-detail)
- [Трассировка и мониторинг](#трассировка-и-мониторинг)
- [Безопасность: углублённый разбор](#безопасность-углублённый-разбор)
- [Инструменты для работы с HTTP](#инструменты-для-работы-с-http)

---

<span id="http-methods-rfc"></span>
...
- Сравнение **POST / PUT / PATCH** на одном примере — [POST, PUT и PATCH](/encyclopedia/2-system-network/2-09-osnovy-integratsionnogo-vzaimodeystviya/118#post-put-patch).

<span id="http-methods-practical"></span>

#### HTTP-методы — практические примеры

Краткая шпаргалка по девяти основным методам с типичными запросами и ответами. Развёрнутые пояснения — в [HTTP как основа веб-интеграций](/encyclopedia/2-system-network/2-09-osnovy-integratsionnogo-vzaimodeystviya/118#http-methods-practical). Отличия **POST, PUT и PATCH** — [сравнительная таблица](/encyclopedia/2-system-network/2-09-osnovy-integratsionnogo-vzaimodeystviya/118#post-put-patch).

| Метод | Пример запроса | Типичный успешный ответ |
|-------|----------------|-------------------------|
| `GET` | `GET /v1/products/iphone` | `200 OK` + JSON или HTML в теле |
| `POST` | `POST /v1/users` + JSON в теле | `201 Created`, заголовок `Location` |
| `PUT` | `PUT /v1/users/123` + полный JSON объекта | `200 OK` или `204 No Content` |
| `PATCH` | `PATCH /v1/users/123` + `&#123; "email": "..." &#125;` | `200 OK` |
| `DELETE` | `DELETE /v1/users/123` | `200 OK` или `204 No Content` |
| `HEAD` | `HEAD /v1/products/iphone` | `200 OK`, тело пустое, заголовки как у `GET` |
| `OPTIONS` | `OPTIONS /v1/users` | `200 OK`, заголовок `Allow: GET, POST, …` |
| `CONNECT` | `CONNECT host:443` через прокси | `200 Connection Established` |
| `TRACE` | `TRACE /index.html` | `200 OK`, эхо заголовков запроса |

```bash
# GET — получить ресурс
curl -s https://api.example.com/v1/products/iphone
...
# DELETE — удаление
curl -s -X DELETE https://api.example.com/v1/users/123

# HEAD — только заголовки
curl -s -I https://api.example.com/v1/products/iphone

# OPTIONS — поддерживаемые методы
curl -s -X OPTIONS -I https://api.example.com/v1/users
```

---

<span id="status-codes-cheatsheet"></span>

### Шпаргалка по статус-кодам

Браузер или API-клиент получает **числовой код** в первой строке ответа. Первая цифра задаёт класс: `1` — промежуточный ответ, `2` — успех, `3` — перенаправление, `4` — ошибка запроса или клиента, `5` — ошибка сервера. Ниже — основные коды по [RFC 9110](https://www.rfc-editor.org/rfc/rfc9110) и распространённые расширения; пояснения, RFC и практика — в [подробных таблицах](#status-codes-detail).

**1xx — информационные** (запрос ещё не завершён)
...
| `418` | I'm a teapot (шутка RFC 2324) |
...
| `499` | Client Closed Request (Nginx, не RFC) |
...
| `599` | Network Connect Timeout Error (не RFC, прокси/CDN) |

<div class="callout callout--tip">
  <div class="callout-title">Что запомнить на практике</div>

  <div class="callout-body">
  - **`404`** — нет ресурса по URI (или скрытие существования под `403`).
  - **`418`** — шуточный код из RFC 2324; иногда Easter egg на CDN.
  - **`429`** — rate limit; смотрите `Retry-After`.
  - **`500`** — сбой приложения; **`502`/`504`** — чаще прокси и upstream.
  - Коды **`520`–`530`** (Cloudflare) и похожие — **нестандартные**; смотрите документацию CDN.
  </div>
</div>

---

<span id="status-codes-detail"></span>
...
| `429` | `Too Many Requests` | RFC 6585 | Превышен лимит запросов (rate limiting). | Рекомендуется `Retry-After`. Может сопровождаться `X-RateLimit-*` заголовками (не стандартизированы). Пример: `X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`. Алгоритмы лимита (token bucket, leaky bucket, sliding window) — [§10](/encyclopedia/7-project/7-06-proektirovanie-i-arhitektura/141#10-rate-limiting). |
...
| `499` | `Client Closed Request` | Nginx (не RFC 9110) | Клиент разорвал соединение до получения ответа. | Полезен в access-логах Nginx и мониторинге "обрывов"; в [шпаргалке](#status-codes-cheatsheet). |
...
| `599` | `Network Connect Timeout Error` | Не RFC 9110 | Таймаут установления соединения к origin (часто на прокси/CDN). | Отличается от `504`: сбой на этапе connect, а не ожидания ответа upstream. |
| `520–598` | — | Cloudflare, AWS, Fastly | Нестандартные коды операторов CDN/облаков. | Примеры:  
...
> — Отправляется при `POST`, `PUT`, `DELETE`, `PATCH`, `TRACE`, `CONNECT`, и всех CORS-предварительных запросах. — Для `GET`/`HEAD` — только при кросс-сайтовом CORS. — `null` для `Данные:`, `file:`, sandbox’ed iframe’ов. |
...
> - Для HTTP/2 и HTTP/3 тело передаётся в `ДАННЫЕ` фреймах; `Transfer-Encoding: chunked` **не используется**.
...
Цепочка от HTTP Basic и session cookie до JWT, SSO и OAuth 2.0 grant flows — в [аутентификации и авторизации](/encyclopedia/2-system-network/2-08-osnovy-informatsionnoy-bezopasnosti/111#evolyutsiya-sposobov-vhoda).
...
| Потеря пакета (транспорт) | блокирует одно TCP-соединение из нескольких | блокирует **все** HTTP/2-потоки в соединении (TCP HOL) | задерживает один QUIC-поток, остальные идут |

Подробное объяснение потоков и TCP HOL — в [HTTP как основа веб-интеграций](/encyclopedia/2-system-network/2-09-osnovy-integratsionnogo-vzaimodeystviya/118#http2-http3-streams).
...
|------------|------------|------------------------|
| **`curl`** | Отправка HTTP-запросов, отладка | см. блок "Быстрый старт" и `curl --compressed -H "Accept-Encoding: br,gzip" …` |
...
**Справочники** — [Основы IP-адресации](/encyclopedia/2-system-network/2-03-set-i-internet/41), [Основные команды в Linux](/encyclopedia/2-system-network/2-05-terminal/101), [Справочник по Android](/encyclopedia/2-system-network/2-01-operatsionnaya-sistema/81), [Основные команды Windows](/encyclopedia/2-system-network/2-05-terminal/102), [Справочник по iOS](/encyclopedia/2-system-network/2-01-operatsionnaya-sistema/71), [Справочник по эмодзи](/encyclopedia/3-data-markup/3-04-konfiguratsii-i-dannye/112).
