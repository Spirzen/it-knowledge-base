...
description: Администрирование баз данных - резервное копирование, обслуживание, контроль производительности и безопасность эксплуатационной среды.
---
...
<ExternalPlayEmbed example="about/database-schema-viewer-play" title="Параметры подключения и схема" minHeight={480} playProps={{ title: 'Параметры подключения и схема', subtitle: 'Эмуляция сеанса с СУБД: хост, порт, база — затем ER-диаграмма таблиц и внешних ключей' }} />
...
- разрешения подключения в списке контроля доступа (ACL) СУБД (например, `pg_hba.conf`, `mysqld.cnf` + `GRANT`, `mongod.conf` + `Безопасность.authorization`);
...
- для DMZ и production — разделять трафик по VLAN/VPC и применять Сеть segmentation.
...
3. **Привязку к таблице пространств (tablespaces)**, если используется внешнее хранилище (SSD для hot Данные, HDD для cold archive).
...
<ExternalPlayEmbed example="system-network/backup-demo" title="Резервное копирование" />
...
- логические бэкапы удобны для миграций, частичного восстановления и проверок структуры;
- физические бэкапы удобны для скорости и восстановления крупных инсталляций;
- снапшоты удобны как быстрый операционный слой, но сами по себе не заменяют хранение копий вне хоста.

Зрелая схема обычно сочетает все три подхода, потому что у каждого свой сильный сценарий.
...
Такой подход закрывает разные сценарии отказа. Локальная копия сокращает время восстановления при обычной аварии, удалённая защищает от потери площадки, а логическая копия упрощает частичное восстановление объектов и миграции между версиями.
...
Метрики важно читать в связке. Например, рост latency при стабильном CPU чаще указывает на диск или сеть, а рост подключений без роста throughput часто говорит о блокировках или неверной работе пула. Такой контекстный анализ обычно быстрее выводит на корневую причину, чем просмотр одного графика изолированно.
...
- **Внешние системы**:
  - **Prometheus + Grafana** — стандарт де-факто. Экспортеры:
    ...
    - `mssql_exporter`,
    - `mongodb_exporter`.
  - **[Zabbix / Nagios](/encyclopedia/2-system-network/2-06-sistemnoe-administrirovanie/zabbix-praktikum/intro)** — классические системы для enterprise-сред; поддерживают шаблоны мониторинга СУБД.
...
Дифференциация важна для устойчивости дежурства. Когда предупреждения и критичные аварии смешаны в одном канале с одинаковой срочностью, команда быстро теряет чувствительность к сигналам. Хорошая схема алертинга помогает реагировать быстрее и с меньшим числом ложных эскалаций.
...
Именно валидация отделяет "обновление завершилось" от "обновление безопасно введено в прод". На практике многие регрессии проявляются не в момент запуска сервиса, а в первые минуты под реальной нагрузкой — на специфичных запросах, репликации, пуле соединений или фоновых задачах обслуживания.
...
Безопасность СУБД зависит от окружения.
...
```yaml
  livenessProbe:
    ...
    failureThreshold: 3
```
- **Zabbix UserParameter** (см. [Практикум Zabbix](/encyclopedia/2-system-network/2-06-sistemnoe-administrirovanie/zabbix-praktikum/5)):
```
...
## Минимальный операционный стандарт для СУБД

Чтобы БД оставалась управляемой, фиксируют четыре обязательных процесса:

- Ежедневные бэкапы и регулярный тест восстановления.
- Контроль свободного места и скорости роста данных.
- Ротация ключей и паролей сервисных учёток.
- Регламент обновлений с тестовым контуром.

---

### Типовой набор ежедневных проверок

- Состояние репликации и lag.
- Количество активных подключений к пределу.
- Длительные блокировки и мёртвые транзакции.
- Ошибки ввода-вывода и состояние дисковой подсистемы.

Полезные связи: [Планирование и автоматизация задач](./8.md), [Диагностика и обработка системных ошибок](./9.md), [Настройка и обслуживание серверов](./4.md).
