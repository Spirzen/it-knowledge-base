...
related:
  ...
  - title: Инструменты виртуализации
...
Подробно про гипервизоры и настройку BIOS — в [Инструменты виртуализации](/encyclopedia/2-system-network/2-02-platformy/21). Теория ОС — [раздел 2.01](/encyclopedia/2-system-network/2-01-operatsionnaya-sistema/1).
...
Для [WireGuard](/encyclopedia/2-system-network/2-03-set-i-internet/404) и Nextcloud обычно нужен **статический IP в LAN** (bridged или отдельный физический сервер).
...
Дальше — Docker по официальной инструкции или один compose-файл из [приватности](/encyclopedia/2-system-network/2-03-set-i-internet/404).
...
Роутер: DHCP reservation для NAS (всегда один IP). Проброс портов 80/443 — **только** если понимаете риски; лучше [VPN](/encyclopedia/2-system-network/2-03-set-i-internet/404).
...
Порт только на localhost — снаружи пускать **reverse proxy** (Caddy/Nginx) с TLS или доступ через WireGuard. Пароли и токены — в `.env` (см. [скрипты](/encyclopedia/4-code-dev/4-011-instrumenty-razrabotki/402)).
...
4. Одна ВМ или Docker: **WireGuard** или **Vaultwarden** (см. [приватность](/encyclopedia/2-system-network/2-03-set-i-internet/404)).
5. Бэкап конфигов на второй диск / облако по правилу 3-2-1.

Дальше: [self-hosting и сеть](/encyclopedia/2-system-network/2-03-set-i-internet/404).
