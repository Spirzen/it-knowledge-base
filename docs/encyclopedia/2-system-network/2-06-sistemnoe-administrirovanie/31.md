...
Перед разбором "странной" аутентификации в домене проверьте синхронизацию (допуск Kerberos — обычно **±5 минут**):

```bash
timedatectl status          # Linux: System clock synchronized: yes
chronyc tracking            # источник NTP и offset в секундах
```

```cmd
w32tm /query /status        :: Windows: источник, Stratum, Last Successful Sync Time
w32tm /resync               :: принудительная синхронизация (нужны права администратора)
```

<ExternalPlayEmbed example="system-network/ntp-clock-drift-simulator-play" title="Дрейф часов и NTP" minHeight={480} />

<ExternalPlayEmbed example="code-dev/process-thread-visualizer" title="Процессы и потоки" minHeight={480} />
...
## Диагностика в проде - короткий алгоритм

Когда проявилась аномалия, держите темп:

1. Зафиксировать время, симптом и затронутые сервисы.
2. Проверить NTP, DNS и диск - это самые частые инфраструктурные причины.
3. Ограничить влияние инцидента (rate limit, failover, временное отключение фона).
4. После стабилизации перейти к root-cause диагностике.

---

### Что добавить в мониторинг

- Метрики длительности фоновых задач и долю неуспешных запусков.
- Отдельные алерты на рост lag, queue backlog и disk pressure.
- Корреляцию событий по хосту, сервису и correlation ID.

См. также: [Планирование и автоматизация задач](./8.md), [Диагностика и обработка системных ошибок](./9.md).
