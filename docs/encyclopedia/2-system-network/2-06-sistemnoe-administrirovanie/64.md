...
Лишние службы можно отключать прямо из терминала — без `services.msc`. Через **PowerShell** (от имени администратора) меняют тип запуска:

```powershell
Set-Service -Name YandexBrowserService -StartupType Disabled
Set-Service -Name YandexBrowserService -StartupType Manual
```
...
Так можно **выключить** службу (`Disabled`), **разрешить ручной запуск** (`Manual`), **вернуть автозапуск** (`Automatic`), а также **запустить или остановить** её сейчас — `Start-Service` и `Stop-Service`. Имя службы смотрят в `Get-Service` или в колонке **Name** в `services.msc` (не путать с отображаемым именем).
