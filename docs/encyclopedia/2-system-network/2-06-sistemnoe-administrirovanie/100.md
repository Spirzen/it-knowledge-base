...
Показывает стеки в реальном времени. Нужны права на трассировку (`perf_event_paranoid`, часто root). Для записи сессии — `perf record -g -p <pid>` и `perf report` (кратко также в [Администрирование Linux-систем](./93.md)).
...
**`MemAvailable`** — ориентир «сколько можно выделить без агрессивного свопа»; для быстрой оценки информативнее, чем только `MemFree`. **`Dirty`** — страницы, ещё не сброшенные на диск; рост при тяжёлой записи. Полный справочник полей — [Справочник по Linux — `/proc`](/encyclopedia/2-system-network/2-01-operatsionnaya-sistema/51).
...
Высокий `%util` при малом throughput иногда указывает на мелкий random I/O или проблемный диск — сверка с `dmesg` и SMART (см. [Диагностика и обработка системных ошибок — аппаратная диагностика](./9.md)).
...
Аудит «что слушает хост», поиск лишних сервисов. Таблица сетевых команд — [Сетевые подключения и диагностика — справочник при сбоях](./6.md#setevye-komandy-diagnostika).
...
Пропускная способность и задержки канала — `iftop`, `nethogs`, `iperf3` (в [Сетевые подключения и диагностика](./6.md) и [Диагностика и обработка системных ошибок — сценарий медленного сервиса](./9.md)).
...
<div class="callout callout--info">
  ...
  `dmesg` фиксирует то, что сообщило **ядро**. Ошибки уровня HTTP, SQL или бизнес-логики ищите в `journalctl -u …` и файлах в `/var/log/` — см. [Диагностика и обработка системных ошибок](./9.md).
  </div>
</div>
...
См. раздел [CPU](#cpu). Для Java/.NET есть свои инструменты (`jstat`, `dotnet-counters`) — в [Диагностика и обработка системных ошибок — диагностика по приложению](./9.md).
