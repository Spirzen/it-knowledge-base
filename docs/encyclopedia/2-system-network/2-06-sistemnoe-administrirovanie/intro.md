...
| 9 | [Linux-сервер с нуля](./93.md#linux-server-s-nulya-bystryy-marshrut) | SSH, firewall, службы, бэкап, мониторинг |
...
- **Клиентская Windows** на ПК — жизненный цикл, переустановка, активация — [Windows на рабочей станции — жизненный цикл](./95.md); пошаговая установка — [Установка и первоначальная настройка ОС](./2.md).
- **Корпоративный офис (СНГ)** в примерах часто описан через **Windows Server** и **Active Directory** — см. [Настройка и обслуживание серверов](./4.md), [Конфигурация рабочих станций](./5.md), [Групповые политики в Windows](./411.md).
- **Серверы и облако** нередко на **Linux** — [Администрирование Linux-систем](./93.md) (в т.ч. [дерево каталогов FHS](./93.md#fhs-napominalka), [rwx и chmod](./93.md#linux-prava-rwx)); **GNOME/KDE и споры DE** — [GNU/Linux — рабочие столы и споры окружений](./96.md); сеть и диагностика — [Сетевые подключения и диагностика](./6.md); тормоза на хосте — [диагностика производительности](./100.md); шпаргалка shell-команд — [справочник Linux](/encyclopedia/2-system-network/2-05-terminal/101#napominalka).

---

## Linux-сервер с нуля (что читать по порядку)

Если вы поднимаете новый Linux-хост и хотите довести его до рабочего состояния, читайте в такой последовательности:

- [Установка и первоначальная настройка ОС](./2.md) — загрузочный образ, разметка диска, первый запуск.
- [Администрирование Linux-систем](./93.md#linux-server-s-nulya-bystryy-marshrut) — базовый чек-лист после первого входа на сервер.
- [Сетевые подключения и диагностика](./6.md) — DNS, порты, маршрутизация, проверка доступности.
- [Диагностика и обработка системных ошибок](./9.md) — логи и типовые сбои.
- [Мониторинг](./92.md) — метрики, алерты и контроль состояния хоста.
- [Восстановление данных](./data-restoring/intro.md) — аварийные сценарии и восстановление.

Маршрут закрывает полный эксплуатационный цикл:
