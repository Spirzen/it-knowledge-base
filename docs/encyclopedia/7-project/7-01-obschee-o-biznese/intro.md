---
title: Общее о бизнесе — о разделе
description: >-
  Бизнес-контекст для IT: ценность, процессы, заказчик, продукт — зачем это
  разработчику и аналитику; маршрут раздела Вселенная IT.
sidebar_label: Общее о бизнесе — о разделе
related:
  - title: "Аналитика — о разделе"
    doc: encyclopedia/7-project/7-04-analitika/intro
  - title: "Методология и ЖЦ ПО — о разделе"
    doc: encyclopedia/7-project/7-03-metodologiya-i-zhiznennyy-tsikl-po/intro
  - title: "ITSM и ИТ-услуги — о разделе"
    doc: encyclopedia/7-project/7-16-itsm-i-it-uslugi/intro
  - title: "Команда и управление — о разделе"
    doc: encyclopedia/7-project/7-02-komanda-i-upravlenie/intro
  - title: "Scrum — о разделе"
    doc: encyclopedia/7-project/7-14-scrum/intro
  - title: "Базы знаний и задачники — о разделе"
    doc: encyclopedia/7-project/7-09-bazy-znaniy-i-zadachniki/intro
---

import DocCardList from '@theme/DocCardList';

# О разделе

IT-проекты существуют внутри **организаций с целями, процессами и ограничениями**. Раздел даёт общий словарь — что называют "бизнесом" в формулировках "бизнес-логика" и "бизнес-процесс", как устроен заказчик, продукт и цифровые каналы (маркетплейсы, госуслуги). Это мост к [аналитике](/encyclopedia/7-project/7-04-analitika/intro) и [методологиям](/encyclopedia/7-project/7-03-metodologiya-i-zhiznennyy-tsikl-po/intro), без MBA на сотню страниц.

---

## С чего начать

| Шаг | Материал | Содержание |
|-----|----------|--------|
| 1 | [Основы бизнеса в IT-проектах](./1) | Определения, IT в компании |
| 2 | [Модели IT-бизнеса](./2) | Продукт, интегратор, аутстафф, аутсорс |
| 3 | [Договор и приёмка](./3) | Глазами разработчика и аналитика |
| 4 | [Бизнес-логика](./111) | Углубление в требования |

Остальные статьи — по оглавлению, если роль требует (руководитель, аналитик, техпис).

<DocCardList />

{/* sidebar-collections */}

---

## В подборках

Статья входит в [тематические подборки](/about/collections) и блок "С чего начать?" на [главной](/). Соседние шаги того же маршрута:

**Управление проектами и Agile** — [Команда и управление — о разделе](/encyclopedia/7-project/7-02-komanda-i-upravlenie/intro), [Методология и жизненный цикл ПО — о разделе](/encyclopedia/7-project/7-03-metodologiya-i-zhiznennyy-tsikl-po/intro), [Scrum — о разделе](/encyclopedia/7-project/7-14-scrum/intro), [Базы знаний и задачники — о разделе](/encyclopedia/7-project/7-09-bazy-znaniy-i-zadachniki/intro), [Культура кода — о разделе](/encyclopedia/7-project/7-10-kultura-koda/intro), [Экономика производства ПО — о разделе](/encyclopedia/7-project/7-13-ekonomika-proizvodstva-po/intro).

{/* /sidebar-collections */}

---
