...
Полный обзор — [Методы рефакторинга](/encyclopedia/4-code-dev/4-02-chto-takoe-kod-i-kak-on-rabotaet/612). Схема БД меняется отдельно — *рефакторинг баз данных*, не путать с переносом только классов.
...
2. **Один PR — одна цель** — рефакторинг структуры отдельно от новой функциональности ([YAGNI и техдолг](/encyclopedia/7-project/7-10-kultura-koda/10)).
