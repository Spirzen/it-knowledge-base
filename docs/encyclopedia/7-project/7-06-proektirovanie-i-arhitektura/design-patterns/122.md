...
<div class="callout callout--warning">
  ...
  <p>Добавляйте терминальное звено, которое либо выполняет работу, либо явно бросает исключение / пишет в лог "запрос не обработан". Иначе ошибки теряются молча.</p>
</div>
  </div>
...
<div class="callout callout--tip">
  ...
  <div class="callout-body">
  <p>В новом Spring Boot REST-сервисе часто достаточно встроенных фильтров и <code>HandlerInterceptor</code>, без собственной иерархии <code>RequestHandler</code>. Порядок этапов (фильтры → <code>DispatcherServlet</code> → <code>preHandle</code> → контроллер → <code>afterCompletion</code>) — в <a href="/encyclopedia/5-languages/5-03-java/27#spring-mvc-request-lifecycle">жизненном цикле запроса Spring Boot</a>. Собственная цепочка оправдана, когда правила домена не укладываются в Security и нужен явный, тестируемый порядок шагов.</p>
</div>
  </div>
