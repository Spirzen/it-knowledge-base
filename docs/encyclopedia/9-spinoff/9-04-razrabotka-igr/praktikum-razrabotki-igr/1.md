...
<div class="callout callout--tip">
  ...
  Идите этапы по порядку. Пропускать можно только блок «Архитектура», если уже делали игры на Pygame. После этапа 8 имеет смысл открыть <a href="#milestone-8">промежуточную сборку</a> — там один связный <code>main.py</code> без класса <code>Game</code>.
</div>
...
5. [Промежуточная сборка (этап 8)](#milestone-8) — цельный `main.py` для проверки.
6. [Полный листинг `Game`](#full-game-class) — финальная архитектура.
...
<div class="callout callout--info">
  <div class="callout-title">Зачем отдельный файл</div>
  На этапах 9–11 легко потерять порядок вызовов в цикле. Эталон фиксирует рабочую последовательность до рефакторинга в класс <code>Game</code>.
</div>

Структура файла (скелет — заполните методами из этапов 1–8):
...
Полный эталонный репозиторий можно собрать, последовательно подставив тела методов из этапов 1–8 — или перейти сразу к [классу `Game`](#full-game-class) на этапе 14.
