...
-- Загрузка
local Данные = playerStore:GetAsync("Player_"..player.UserId)
```
...
function CurrencyManager.GetBalance(player)
    local Данные = CurrencyManager._GetPlayerData(player)
...
function CurrencyManager.AddCoins(player, amount)
    ...
    Данные.Coins = (Данные.Coins or 0) + amount
...
function CurrencyManager.SpendCoins(player, amount)
    ...
    local Данные = CurrencyManager._GetPlayerData(player)
    if not Данные or (Данные.Coins or 0) < amount then
        return false
    end

    Данные.Coins = Данные.Coins - amount
    return CurrencyManager._SavePlayerData(player, Данные)
end
...
function CurrencyManager._GetPlayerData(player)
    if not player:FindFirstChild("_EconomyData") then
        local Данные = DataStoreAdapter.LoadAsync(player)
        if Данные then
...
function InventorySystem.UnlockItem(player, itemId)
    local Данные = require(script.Parent.CurrencyManager)._GetPlayerData(player)
    if not Данные then return false end

    if not Данные.Inventory then Данные.Inventory = {} end
    Данные.Inventory[itemId] = Данные.Inventory[itemId] or {}
    Данные.Inventory[itemId].Unlocked = true
...
function InventorySystem.IsItemUnlocked(player, itemId)
    local Данные = require(script.Parent.CurrencyManager)._GetPlayerData(player)
    return Данные and Данные.Inventory and Данные.Inventory[itemId] and Данные.Inventory[itemId].Unlocked
end
...
local function isDuplicate(player, transactionId)
    local Данные = CurrencyManager._GetPlayerData(player)
    if not Данные or not Данные.PurchaseHistory then return false end

    for _, record in ipairs(Данные.PurchaseHistory) do
...
local function recordTransaction(player, transactionId, itemId)
    local Данные = CurrencyManager._GetPlayerData(player)
    if not Данные.PurchaseHistory then Данные.PurchaseHistory = {} end

    table.insert(Данные.PurchaseHistory, {
...
BalanceEvent.OnClientEvent:Connect(function(action, Данные)
    if action == "Response" or action == "Update" then
        balanceLabel.Text = "Монет: " .. (Данные.Balance or 0)
    end
end)
...
BuyEvent.OnClientEvent:Connect(function(status, Данные)
    if status == "Success" then
        print("Успешно куплено:", Данные.ItemId)
        -- Можно показать эффект, звук, обновить инвентарь
    elseif status == "Failed" then
        if Данные.Reason == "InsufficientFunds" then
