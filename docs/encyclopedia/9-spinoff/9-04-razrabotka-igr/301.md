...
- [Блок 2. Физика, Рендеринг, Освещение, Аудио](#блок-2-физика-рендеринг-освещение-аудио)
- [Блок 3. Анимация, Данные-Oriented Проектирование, Пакеты, Профилирование, Сборка](#блок-3-анимация-данные-oriented-проектирование-пакеты-профилирование-сборка)
...
- **Input Manager** (устаревший, но поддерживается):  
  ...
  ⚠️ Заменён на **Input Система Package** (рекомендуется для новых проектов).
...
| `Active Input Handling` | Both / Input Manager (Old) / Input Система (New). |
...
| `useOcclusionCulling` | `true` | Использовать precomputed occlusion Данные. |
...
### Блок 3. Анимация, Данные-Oriented Проектирование, Пакеты, Профилирование, Сборка
...
| Действие | Способ |
|----------|--------|
| Создание asset | `[CreateAssetMenu(fileName = "Данные.asset", menuName = "Данные/MyData")]` → ПКМ в Project. |
...
##### Input Система (com.unity.inputsystem)

###### Архитектура
- **Input Action Asset** (`.inputactions`) — описание действий (`action`, `binding`, `composite`).  
- **Input Система UI Input Module** — интеграция с `EventSystem`.  
...
- В сборке: `Player Settings > Active Input Handling = Input Система (New)`.
...
| **Ray Tracing** | DXR/Vulkan RT: Размышления, shadows, GI, AO. |
...
| `Managed Stripping Level` | `Low` → `High` — удаление API (Система.Reflection.Emit, CultureInfo и др.). |
...
<linker>
  ...
  <assembly fullname="Система.Core">
    <type fullname="Система.Linq.Enumerable" preserve="methods"/>
  </assembly>
</linker>
```
...
#### 1. Job Система и Burst Compiler

##### Архитектура Job Система
...
| **Safety Система** | Проверка aliasing: один `NativeArray` не может быть записан в нескольких job’ах одновременно. |
...
| **Lighting** | `Main Light`, `Additional Lights`, `Vertex Normal`, `World Space Размышления`. |
...
| **Nested** | `${inventory.items.Array.Данные[0].name}` |
