...
<div class="callout callout--info">
  ...
  Для учебного прототипа достаточно <strong>One-Page</strong> — одностраничного описания концепта, механик и win/lose. Шаблон и типы документов (GDD, TDD, One-Page) — в <a href="/encyclopedia/9-spinoff/9-04-razrabotka-igr/117">гейм-дизайне</a>.
</div>
</div>
...
При `Time.timeScale = 0` игра на паузе: `Update` может вызываться, но `Time.deltaTime` равен нулю, физика не продвигается. Подробная таблица — в [справочнике](/encyclopedia/9-spinoff/9-04-razrabotka-igr/301#блок-1-основа-lifecycle-gameobject-transform-component-project-settings).
...
На капсуле игрока: **Rigidbody**, **Capsule Collider**, **Freeze Rotation** X/Z. Пол и платформы — слой `Ground`, его укажите в `groundMask`.
...
**Без Cinemachine:** камеру на отдельном объекте можно двигать в **`LateUpdate`**, копируя позицию игрока + смещение `(0, 2, -5)` — так же, как в пошаговых учебниках по Unity; Cinemachine заменяет этот шаблонный код.
...
<div class="callout callout--tip">
  ...
  <div class="callout-body">
  Singleton (<code>GameManager</code>), Object Pool (снаряды), Observer (<code>StateChanged</code>) — частые приёмы в Unity-проектах. Обзор паттернов проектирования — <a href="/encyclopedia/7-project/7-12-konstruirovanie-po/intro">Конструирование ПО</a>.
</div>
</div>
