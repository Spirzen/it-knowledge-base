...
Unity вызывает методы `MonoBehaviour` в предсказуемом порядке:
...
*   **Счёт и сбор предметов** — простой синглтон и триггер на монете:
...
Повесьте `GameScore` на пустой объект в сцене (один экземпляр). На монете — `Pickup`:
...
public class Pickup : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (GameScore.Instance != null)
            GameScore.Instance.AddPoints(points);
        Destroy(gameObject);
    }
}
```
