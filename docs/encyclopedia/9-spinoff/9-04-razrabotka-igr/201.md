---
tags: [reference, notrequired, advanced]
...
Горячие клавиши, команды и форматы **Roblox** для ежедневной работы. Учебный курс: [раздел](/encyclopedia/9-spinoff/9-04-razrabotka-igr/intro).
...
GetProfile.OnServerInvoke = function(player)
    local success, Данные = pcall(function()
        return {
            level = 5,
            gold = 1250,
...
Players.PlayerAdded:Connect(function(player)
    ...
    if not success then
        ...
        Данные = { level = 1, gold = 0, inventory = {} }
    end
...
Players.PlayerRemoving:Connect(function(player)
    local Данные = player:GetAttribute("PlayerData")
    if Данные then
        spawn(function()
            local retries = 3
            repeat
...
local function migrate(Данные)
    ...
    if Данные.version == 1 then
        -- v1 → v2: gold → resources.gold
        Данные.resources = { gold = Данные.gold }
        Данные.gold = nil
        Данные.version = 2
    end
    return Данные
...
- Храните копию в `leaderstats` (для отладки):  
  ```lua
  player.leaderstats.Gold.Value = Данные.gold
  ```
...
MarketplaceService.ProcessReceipt = function(receiptInfo)
    ...
    if player then
        if productId == 987654321 then -- "1000 Gold"
            local Данные = player:GetAttribute("PlayerData") or {}
            Данные.gold = (Данные.gold or 0) + 1000
            player:SetAttribute("PlayerData", Данные)
        end
    else
...
-- Сервер B: приём
Players.PlayerAdded:Connect(function(player)
    local Данные = TeleportService:GetPlayerTeleportData(player)
    if Данные then
        print("Телепортирован из другого сервера:", Данные.checkpoint)
    end
end)
```
...
local Данные = DS:GetAsync("Player_" .. player.UserId)
Debug.profileend()
