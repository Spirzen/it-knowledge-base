...
Под «развёртыванием базы данных» понимается не установка СУБД, а создание *экземпляра логической базы* внутри работающего сервера и подготовка его к использованию приложением.
...
Резервное копирование — это не опция, а обязательная процедура эксплуатации. Отказ от неё делает систему критически уязвимой к аппаратным сбоям, человеческим ошибкам и атакам.
...
Мониторинг СУБД — неотъемлемая часть системного администрирования. Цель — не только обнаружить сбой, но и *предотвратить* его за счёт выявления деградирующих параметров.
...
**Важно**: автоматический failover требует:
  ...
  — проверки health-состояния (не только «порт открыт», но и `SELECT 1`);
