...
title: Типизация
description: Обзор систем типизации в программировании, основные архитектурные модели, преобразования типов, упаковка/распаковка, ковариантность/контравариантность и прочее.
---
...
В системах с **единым корневым типом** (например, `Object` в Java, `Система.Object` в C#, `Any` в Swift/Kotlin), все значения — даже скаляры — могут быть представлены как объекты. Однако скаляры (int, bool) хранятся непосредственно в стеке или в полях структур, а объекты — в куче, со служебной информацией (vtable, sync word и пр.).
...
> - Проверка безопасности памяти (Rust ownership Система),  
