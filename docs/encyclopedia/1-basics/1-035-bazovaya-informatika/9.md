...
На уроках информатики, в кружках и дома часто встречаются **конкретные программы и библиотеки** — BeautifulSoup, Blender, Godot и другие. Ниже — что каждый инструмент делает технически и с чего начать первый рабочий результат.
...
**BeautifulSoup** — парсер HTML/XML на Python. Не качает страницу сам: сначала `requests.get(url)`, потом `BeautifulSoup(html, 'html.parser')`.

Минимальный скрипт — вывести все ссылки со страницы:
...
Сайты, где контент рисуется только JavaScript'ом в браузере, BeautifulSoup **не увидит** без Selenium/Playwright. Подробнее — [BeautifulSoup](/encyclopedia/5-languages/5-02-python/316).

**pyTelegramBot** (`import telebot`) — HTTP-обёртка над [Bot API](https://core.telegram.org/bots/api). Токен выдаёт [@BotFather](https://t.me/BotFather).
...
Бот должен **постоянно работать** (на ПК, VPS или хостинге), иначе не ответит. Состояние пользователей хранят в `dict`, JSON или SQLite. Подробнее — [Telegram-боты](/encyclopedia/5-languages/5-02-python/317).
...
- фото и цифровая ретушь — **растр** (Paint.NET и аналоги);
- логотипы, схемы и макеты под печать — **вектор** (CorelDRAW и аналоги), чтобы масштабировать без потери качества.
...
**Godot 4** — движок с узлами (Node) и сценами. Скрипт на **GDScript** на объекте `CharacterBody2D`:
...
Экспорт в `.exe` / APK — Project → Export. [Godot](/encyclopedia/9-spinoff/9-04-razrabotka-igr/113).

**Construct 3** — события вида "Keyboard → On Space pressed → Player → Jump"; код не обязателен; проект в браузере, экспорт HTML5.

**Flutter** — виджеты на **Dart**; `flutter create myapp`, UI декларативный (`Scaffold`, `ListView`). Одна кодовая база → Android/iOS. Маршрут: [Dart](/encyclopedia/5-languages/5-22-dart/intro) → [Flutter](/encyclopedia/5-languages/5-22-dart/311).
...
- **Цель на 2–4 недели** — что должно заработать в конце (игра, бот, приложение, макет)?
- **Уровень команды** — есть ли опыт кода или нужен no-code/low-code старт?
- **Ограничения среды** — школьные ПК, лицензии, интернет, мощность компьютеров.
- **Формат результата** — демо на уроке, публикация в магазине, печать, портфолио.
