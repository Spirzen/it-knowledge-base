...
related:
  - title: Графика и схемы (обзор)
    doc: /encyclopedia/1-basics/1-035-bazovaya-informatika/504
...
Diagrams.net (более известный как Draw.io) — это бесплатный кроссплатформенный инструмент с открытым исходным кодом для создания схем, блок-схем и диаграмм. Сервис работает непосредственно в веб-браузере, не требует регистрации и является одной из самых популярных альтернатив платной программе Microsoft Visio.

Что можно создавать в Diagrams.net:
- блок-схемы алгоритмов программ, бизнес-процессов;
- сетевые структуры, карты компьютерных сетей, серверные архитектуры;
- UML-диаграммы, модели баз данных, структуры классов;
- интеллект-карты для мозговых штурмов;
- макеты, простые интерфейсы сайтов и мобильных приложений.

Порой в работе приходится реализовывать диаграмму для планирования или проектирования, допустим, когда мы хотим предварительно смоделировать процесс оказания какой-то услуги, шаг за шагом, или показать алгоритм работы программы для новичка.

> Блок-схема — это графическое описание алгоритма или процесса, где каждый шаг изображается в виде геометрической фигуры, а порядок выполнения задается стрелками.

> Диаграмма — это более широкое понятие, обозначающее любой графический рисунок, который наглядно показывает соотношение между какими-либо данными, элементами или структурами.

Основные виды диаграмм:
- Статистические (столбчатые, круговые) показывают пропорции или изменения чисел (например, график продаж по месяцам).
- Структурные (иерархические) отражают устройство системы (например, схема отделов компании и подчинения сотрудников).
- Диаграммы связей (UML, концепт-карты) показывают, как объекты взаимодействуют друг с другом (например, схема базы данных).

<div class="callout callout--tip">
  <div class="callout-title">Обзор в другой главе</div>

  <div class="callout-body">
  diagrams.net упоминается в контексте графики и Visio в главе <a href="/encyclopedia/1-basics/1-035-bazovaya-informatika/504">"Графика, дизайн и 3D"</a>. Здесь — архитектура, практика работы и XML-схема файлов.
</div>
  </div>
...
> Редактор диаграмм — это программа или веб-сервис для создания, редактирования и публикации визуальных схем. Он заменяет рисование от руки на бумаге или доске, предлагая готовые цифровые инструменты. Вместо того чтобы чертить линии и фигуры вручную, пользователь просто перетаскивает их мышкой на холст.

Главное отличие от "рисования в Paint" — диаграмма — это **модель графа** (вершины, рёбра, слои), а не набор пикселей. 

Файл `.drawio` — текстовый XML, который удобно хранить в Git, сравнивать в diff и генерировать скриптами. По сути, это обычный текстовый файл, внутри которого записан код в формате XML. Этот код содержит информацию обо всех фигурах, их координатах, цветах, тексте и соединительных линиях вашей схемы. Если вы сохраните диаграмму как картинку в формате PNG с галочкой «Включить копию диаграммы» (Include a copy of my diagram), то этот файл можно будет использовать и как обычное изображение в презентации, и одновременно открывать в Draw.io для дальнейшего редактирования.
...
- [Графика и 3D — обзор Visio и diagrams.net](/encyclopedia/1-basics/1-035-bazovaya-informatika/504)
