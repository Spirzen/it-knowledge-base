...
description: >-
  ping, curl, Wireshark, PuTTY, WinMTR и инструменты диагностики для Home Lab и
  повседневной работы power user.
related:
...
| Утилита | Задача |
| :--- | :--- |
| `ping` / `pathping` | Жив ли хост, потери |
| `tracert` / `mtr` | Где обрывается маршрут |
| `ipconfig` / `Get-NetIPConfiguration` | IP, шлюз, DNS |
| `nslookup` / `dig` | Ответ DNS |
| **PuTTY** / Windows Terminal SSH | Home Lab |
| **WinMTR** | Нестабильный ping до игры/сервера |
| **Wireshark** | Пакеты (когда "магия" не помогла) |
