...
В репозитории [it-knowledge-base](https://github.com/Spirzen/it-knowledge-base) лежит сводная схема экосистемы: три домена (spirzen.ru, code, play), интеграция iframe/postMessage, сборка и runtime. Исходник — `info/it-universe-architecture.drawio`; для статей экспортируется PNG (`static/img/it-universe-architecture.png`). Пересборка drawio: `node scripts/generate-architecture-drawio.mjs`. Легенда — [О проекте → архитектура](/about/project#arhitektura-proekta); те же потоки в Mermaid — [экосистема](/about/project#it-universe-c4-mermaid).
