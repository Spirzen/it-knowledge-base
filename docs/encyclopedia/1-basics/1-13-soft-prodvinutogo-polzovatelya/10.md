...
| Файл | Назначение |
| :--- | :--- |
| `info/it-universe-three-tier.drawio` | **Обзор** — три домена и слой интеграции (iframe + postMessage) |
| `info/it-universe-architecture.drawio` | **Полная** — репозитории, сборка, деплой, runtime |

Для статей обе экспортируются в PNG (`static/img/`). Пересборка полной схемы: `node scripts/generate-architecture-drawio.mjs`. Легенда — [О проекте → архитектура](/about/project#arhitektura-proekta); те же потоки в Mermaid — [экосистема](/about/project#it-universe-c4-mermaid).
