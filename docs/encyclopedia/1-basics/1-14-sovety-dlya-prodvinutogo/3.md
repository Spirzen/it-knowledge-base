...
## Зачем продвинутому пользователю виртуальная машина
...
| :--- | :--- | :--- |
| «Поставлю Linux и сломаю» | Риск загрузчика и разделов | Снапшот → откат за минуту |
| Сервис Nextcloud / Pi-hole | Порты, Docker, фаервол | Отдельная сеть NAT |
| Подозрительный .exe / кряк | Заражение | Одноразовая ВМ без общих папок |
...
### Hyper-V vs VirtualBox vs VMware Player
...
4. Снапшот `clean-install`.
5. С хоста: `ssh user@IP_ВМ` (IP — `ip a` в гостевой системе).
6. `sudo apt update && sudo apt upgrade` → снапшот `updated`.
...
### Минимальный docker-compose (идея)
...
Порт только на localhost — снаружи пускать **reverse proxy** (Caddy/Nginx) с TLS или доступ через WireGuard. Пароли и токены — не в compose, а в `.env` (см. [скрипты](/encyclopedia/1-basics/1-14-sovety-dlya-prodvinutogo/2)).

### WSL 2: что умеет и чего не умеет

| Умеет | Не заменяет |
| :--- | :--- |
| Bash, Git, Node, Python dev | Полноценный systemd на старых сборках (ограничения) |
| Docker Desktop integration | Аппаратный USB для некоторых сценариев |
| Доступ к файлам Windows `/mnt/c` | Сервер 24/7 при выключенном ПК |
