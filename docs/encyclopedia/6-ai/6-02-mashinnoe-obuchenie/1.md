...
description: Машинное обучение (Machine Learning, ML) — это область искусственного интеллекта, которая разрабатывает алгоритмы, способные извлекать знания из данных. Вместо явного программирования правил…
---
...
using Microsoft.ML;
using Microsoft.ML.Данные;

var mlContext = new MLContext();
var Данные = mlContext.Данные.LoadFromTextFile<HouseData>("houses.csv", separatorChar: ',');
var pipeline = mlContext.Transforms.Concatenate("Features", "Size", "Rooms")
    .Append(mlContext.Regression.Тренажеры.Sdca());
var model = pipeline.Fit(Данные);
```
...
Instances Данные = source.getDataSet();
Данные.setClassIndex(Данные.numAttributes() - 1);

LinearRegression model = new LinearRegression();
model.buildClassifier(Данные);
```
...
# Загружаем данные
iris = load_iris()
X_train, X_test, y_train, y_test = train_test_split(iris.Данные, iris.target, test_size=0.3)
...
var mlContext = new MLContext();
var Данные = mlContext.Данные.LoadFromTextFile<IrisData>("iris.csv", separatorChar: ',');
var pipeline = mlContext.Transforms.Concatenate("Features", "SepalLength", "SepalWidth", "PetalLength", "PetalWidth")
    .Append(mlContext.BinaryClassification.Тренажеры.SdcaLogisticRegression());
var model = pipeline.Fit(Данные);
```
...
X_train, X_test, y_train, y_test = train_test_split(cancer.Данные, cancer.target, test_size=0.3)
...
var mlContext = new MLContext();
var Данные = mlContext.Данные.LoadFromTextFile<CancerData>("cancer.csv", separatorChar: ',');
var pipeline = mlContext.Transforms.Concatenate("Features", "Feature1", "Feature2", "Feature3")
    .Append(mlContext.BinaryClassification.Тренажеры.LightGbm());
var model = pipeline.Fit(Данные);
```
...
# Загружаем данные
wine = load_wine()
X_train, X_test, y_train, y_test = train_test_split(wine.Данные, wine.target, test_size=0.3)
...
import smile.Данные.DataFrame;
import smile.io.Read;

DataFrame Данные = Read.csv("wine.csv");
RandomForest model = RandomForest.fit(Данные.x(), Данные.factor("target").array(), 100);
```
...
# Загружаем данные
digits = load_digits()
X_train, X_test, y_train, y_test = train_test_split(digits.Данные, digits.target, test_size=0.3)
...
# Загружаем данные
cancer = load_breast_cancer()
X_train, X_test, y_train, y_test = train_test_split(cancer.Данные, cancer.target, test_size=0.2)
...
# Применяем конвейер
Данные = pd.read_csv('dataset.csv')
X_processed = preprocessor.fit_transform(Данные)
```
...
**Точность** — доля правильно классифицированных объектов от общего количества. Простая и интуитивно понятная метрика, но может быть вводящей в заблуждение при несбалансированных классах.

**Точность** — доля истинно положительных предсказаний от всех предсказанных положительных. Показывает, насколько можно доверять положительному предсказанию модели.

**Полнота** — доля истинно положительных предсказаний от всех фактически положительных объектов. Показывает, насколько хорошо модель находит все положительные случаи.
...
from torch.utils.Данные import DataLoader, TensorDataset
import numpy as np
...
**NumPy** — это фундаментальная библиотека для научных вычислений на Python. NumPy предоставляет поддержку многомерных массивов и матриц, а также широкий набор математических функций для работы с ними.
...
axes[0, 1].hist(Данные, bins=30, alpha=0.7, color='green')
...
sns.boxplot(x='category', y='value1', Данные=Данные)
...
correlation = Данные[['value1', 'value2', 'value3']].corr()
sns.heatmap(correlation, annot=True, cmap='coolwarm', center=0)
plt.title('Матрица корреляции')
plt.show()

# Pairplot
sns.pairplot(Данные, hue='category', palette='Set2')
plt.suptitle('Pairplot данных', y=1.02)
plt.show()

# Violin plot
plt.figure(figsize=(10, 6))
sns.violinplot(x='category', y='value2', Данные=Данные, palette='muted')
...
def create_time_series_data(n_samples=1000):
    ...
    Данные = trend + seasonality + noise
    return Данные

# Подготовка данных для обучения
def create_sequences(Данные, seq_length):
    X, y = [], []
    for i in range(len(Данные) - seq_length):
        X.append(Данные[i:i + seq_length])
        y.append(Данные[i + seq_length])
    return np.array(X), np.array(y)

# Генерируем данные
Данные = create_time_series_data(2000)

# Нормализуем данные
scaler = MinMaxScaler()
data_scaled = scaler.fit_transform(Данные.reshape(-1, 1)).flatten()
