...
Категориальные `Suburb`, `CouncilArea`, `Type` переводим в числа **one-hot** через `pd.get_dummies` (подробнее — [Кодирование категориальных признаков](/encyclopedia/6-ai/6-02-mashinnoe-obuchenie/4)):

```python
df = pd.get_dummies(df, columns=["Suburb", "CouncilArea", "Type"])
```

Подробнее о выборе кодировщика — [Кодирование категориальных признаков](/encyclopedia/6-ai/6-02-mashinnoe-obuchenie/4).
...
<ExternalCodeEmbed example="python/ai-6-6-02-mashinnoe-obuchenie-7-001" title="GridSearchCV" minHeight={372} />
...
<ExternalCodeEmbed example="python/ai-6-6-02-mashinnoe-obuchenie-7-002" title="Production-вариант — Pipeline" minHeight={462} />
...
- Перейти к классификации — [Lab 1157 — Titanic](/lab/Примеры/1157) и [Titanic на Kaggle](/encyclopedia/6-ai/6-02-mashinnoe-obuchenie/21).
- Углубить метрики регрессии (RMSE, R²) — [Машинное обучение](/encyclopedia/6-ai/6-02-mashinnoe-obuchenie/1#для-задач-регрессии).
