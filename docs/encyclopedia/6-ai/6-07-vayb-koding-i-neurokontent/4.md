...
**Claude Code** — агентный инструмент Anthropic для разработки: читает репозиторий, правит файлы, запускает shell и Git, подключает внешние системы через [MCP](/encyclopedia/6-ai/6-04-modeli-i-instrumenty/114). Доступен в **терминале**, **VS Code / Cursor**, **Desktop**, **JetBrains** и **браузере** ([claude.ai/code](https://claude.ai/code)). Ниже — установка (macOS/Windows), терминал и Git, сессии, plan mode, MCP, безопасность и практикум lead-webhook. Сравнение с Cursor — [4.14 / 1113](/encyclopedia/4-code-dev/4-14-razrabotka-i-otladka/1113); риски слепого merge — [вайб-кодинг](./1).
...
**Синхронизация правил:** один источник истины — `ARCHITECTURE.md` или `AGENTS.md`; в `CLAUDE.md` и `.cursorrules` — ссылки на него ([1113](/encyclopedia/4-code-dev/4-14-razrabotka-i-otladka/1113)). **Git** — общий синхронизатор: Claude Code коммитит, Cursor открывает тот же diff.
...
**Контекстная инженерия** — осознанная сборка того, что попадает в окно модели: файлы, правила, diff, результаты tools. Качество ответа определяется контекстом сильнее, чем «красотой» формулировки ([117](/encyclopedia/6-ai/6-04-modeli-i-instrumenty/117)).
...
Параметры генерации (temperature, structured output) — [118](/encyclopedia/6-ai/6-04-modeli-i-instrumenty/118).
...
Изоляция — [devcontainer](https://code.claude.com/docs/en/devcontainer), [sandboxed Bash](https://code.claude.com/docs/en/sandboxing). Политика опасных команд — [101](/encyclopedia/8-infra-security/8-03-zabota-o-kode-i-dannyh/101).
...
Эксплуатация агентов в prod — [AgentOps](/encyclopedia/6-ai/6-08-agentops/intro). Опасные команды — [101](/encyclopedia/8-infra-security/8-03-zabota-o-kode-i-dannyh/101).
