...
3. **Structured output** — библиотека **[Instructor](https://github.com/jxnl/instructor)** + Pydantic-модель: LLM возвращает JSON, схема валидируется до записи в БД.

```mermaid
flowchart LR
  IMG[Скан / фото] --> L[Layout parser]
  L --> V[Vision-LLM / OCR]
  V --> S[Instructor + Pydantic]
  S --> DB[(JSON в БД / API)]
```
...
Runnable-примеры — [Практикум — проекты по ИИ](/encyclopedia/6-ai/6-05-razrabotka-ii/122) (ветка OCR). Связка OCR → текст → [RAG](/encyclopedia/6-ai/6-05-razrabotka-ii/113) — типичный pipeline для чата по корпоративным сканам.
