...
При локальной разработке сервер слушает **localhost** (`127.0.0.1`), браузер открывает `http://localhost:3000`. Запуск dev-сервера — [советы новичку](/encyclopedia/1-basics/1-12-sovety-dlya-novichka/13). DNS, HTTPS, nginx — [2.04 Сайты](/encyclopedia/2-system-network/2-04-kak-rabotayut-sayty-i-veb-sayty/intro).
