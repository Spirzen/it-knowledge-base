...
2. Запустил (Windows может спросить [права администратора](/encyclopedia/1-basics/1-12-sovety-dlya-novichka/11)).
...
Публикация в Microsoft Store — отдельный путь через **MSIX**, без классического `setup.exe`: [117](./117.md).
...
WinForms-мастер: форма с `Button` «Далее», `TextBox` для пути (`FolderBrowserDialog`), `ProgressBar` + фоновый `Task.Run` для копирования (не блокировать UI — см. [112](./112.md)).
...
| **MSI / MSIX** | Средняя–высокая | Стандарт Windows | Windows | Корпорации, Store ([117](./117.md)) |
| **electron-builder** | Средняя | Зависит от tool | Win/mac/Linux | Electron-приложения ([114](./114.md)) |
