...
Теория Electron — [Electron](./114.md); общая архитектура десктопа — [Архитектура десктопных приложений](./1.md). База по React — [272](/encyclopedia/5-languages/5-01-javascript/272) и [галерея компонентов](/lab/Примеры/1146).
...
Обзор процессов Electron — в [Electron](./114.md). API Node.js — [Первая программа на Node.js](/encyclopedia/5-languages/5-01-javascript/262). Общие правила десктопа (не блокировать UI) — [Особенности разработки десктопных приложений](./112.md).
...
**Что делает `npm run dev`:** обычно одновременно поднимает Vite (страница React) и Electron (окно, которое грузит этот URL). Общая схема dev-сервера — [Запуск и перезапуск приложений](/encyclopedia/1-basics/1-035-bazovaya-informatika/208). Вы правите `src/App.jsx` — окно обновляется без полной пересборки.
...
<ExternalCodeEmbed example="javascript/code-411-118-001" title="Main process — окно и IPC" minHeight={642} />
...
<ExternalCodeEmbed example="javascript/code-411-118-002" title="React — интерфейс" minHeight={534} />
...
| **Огромный установщик** | В bundle попали devDependencies | Настройте `files` в electron-builder — [Electron](./114.md). |
...
3. **Сборка установщика:** `electron-builder` — см. [Electron](./114.md).
...
- [React — компоненты-рецепты (Lab)](/lab/Примеры/1146)
...
**Первые шаги** ([маршрут подборки](/about/collections/pervye-shagi)) — [Первая программа на Axum](/encyclopedia/5-languages/5-13-rust/201), [Первая программа на React Native](/encyclopedia/4-code-dev/4-12-mobilnye-prilozheniya/11311), [Первая программа на Fiber](/encyclopedia/5-languages/5-10-go/2414), [Первая программа на Expo](/encyclopedia/4-code-dev/4-12-mobilnye-prilozheniya/11321), [Первая программа на Echo](/encyclopedia/5-languages/5-10-go/2413), [Первые шаги с SQL](/encyclopedia/3-data-markup/3-07-sql/101).
