...
**Что делает `npm run dev`:** обычно одновременно поднимает Vite (страница React) и Electron (окно, которое грузит этот URL). Общая схема dev-сервера — [Запуск и перезапуск приложений](/encyclopedia/1-basics/1-12-sovety-dlya-novichka/13). Вы правите `src/App.jsx` — окно обновляется без полной пересборки.
