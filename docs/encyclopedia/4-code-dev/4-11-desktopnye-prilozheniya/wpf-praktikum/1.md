...
> **Практикум, шаг 1 из 6.** Фиксируем базовую модель WPF перед MVVM и клиент-серверной частью. Если нужны **короткие примеры «скопировать и запустить»** (окно, кнопка, TextBox) — сначала [C# WinForms и WPF — простые окна](/lab/Примеры/1138). Полный локальный практикум с нуля — [119.md](../119.md); обзор платформы Windows — [116.md](../116.md).
...
В `.csproj` должно быть `<UseWPF>true</UseWPF>`. Дальше по [119.md](../119.md) можно собрать локальный список; в [шаге 2](./2) вынесем логику в ViewModel, в [шаге 4](./4) подключим Prism.
...
- [ ] Прочитан [119.md](../119.md) или повторены стили и `DataTemplate`.
