...
Пока программа лежит на диске, она в **энергонезависимой** памяти. После запуска ОС копирует образ в **ОЗУ**; процессор выполняет **машинные инструкции** по циклу **выборка → декодирование → исполнение** (fetch–decode–execute), пока программа не завершится штатно или с ошибкой. Один запущенный экземпляр — **процесс**; внутри него могут работать **потоки** — параллельные цепочки инструкций одной программы. **JIT-компиляция** переводит часто используемый байт-код в нативный машинный код уже во время работы (JVM, .NET), сочетая переносимость и скорость.
