...
Следующий шаг после настройки — ежедневные команды в [«Как работать с Git»](./112). Держать под рукой можно [12 команд](./115#12-komand) — `init`, `add`, `commit`, `push`, ветки и слияние. Готовые сценарии с разбором каждой строки — [лабораторная шпаргалка](/lab/Примеры/1123).
...
<div class="callout callout--warning">
  ...
  Установщик Git for Windows ставит <strong>Git Bash</strong> — там <code>git</code> обычно уже работает. В <strong>PowerShell</strong> и <strong>cmd</strong> команда появится только если при установке выбрали пункт про PATH или добавили путь вручную. Unix-утилиты (<code>grep</code>, <code>ls</code>) на Windows без Git Bash можно также поставить через <strong>Microsoft Coreutils</strong> (Preview, <code>winget install Microsoft.Coreutils</code>) — см. <a href="/encyclopedia/2-system-network/2-05-terminal/104#tot-zhe-grep-na-windows">grep на Windows</a>.
</div>
  </div>
...
<ExternalPlayEmbed example="basics/installer-wizard-play" title="Мастер установки" minHeight={480} playProps={{ appName: 'Установка Git for Windows', productName: 'Git', welcomeText: 'Мастер установит git.exe, Git Bash и опционально добавит Git в PATH для PowerShell и IDE.', extraOptionsLabel: 'Git from the command line and also from 3rd-party software (рекомендуется)', cardSubtitle: 'Далее — пошаговая установка с git-scm.com' }} />
...
4. На Windows часто помогает [Git Credential Manager](https://github.com/git-ecosystem/git-credential-manager) — он запоминает токен в менеджере учётных данных.
...
Токен **не коммитьте** в репозиторий. Если попал в Git — см. [секрет в истории](./1141#секрет-или-конфиденциальный-файл-в-git) и [`.gitignore`](./116).
...
2. Запустить агент и добавить ключ (Linux/macOS; на Windows — ssh-agent в PowerShell или встроенный OpenSSH).
3. Скопировать **публичный** ключ (`id_ed25519_github.pub`) в GitHub: **Settings → SSH and GPG keys → New SSH key**.
4. Проверить:
...
Несколько аккаунтов (GitHub + GitVerse и т.д.) — отдельные ключи и `~/.ssh/config`; см. [множественные сервисы Git](/encyclopedia/8-infra-security/8-03-zabota-o-kode-i-dannyh/1183). Общая теория SSH — в [основах ИБ](/encyclopedia/2-system-network/2-08-osnovy-informatsionnoy-bezopasnosti/116).

<div class="callout callout--tip">
  <div class="callout-title">Терминал до Git</div>

  <div class="callout-body">
  Команды `cd`, `ls`/`dir`, кавычки в путях — в разделе [«Терминал» (часть 2)](/encyclopedia/2-system-network/2-05-terminal/intro).
...
![image-10.png](https://assets.spirzen.ru/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/image-10.png)
...
| **Push origin** | Отправка на GitHub ([`git push`](./115#12-komand)) |
| Выпадающий список веток | [`git switch` / `checkout`](./115#12-komand) / создание ветки |
...
![image-13.png](https://assets.spirzen.ru/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/image-13.png)

При первом запуске Sourcetree **ищет установленный Git** — без Git for Windows установка неполная.

![image-14.png](https://assets.spirzen.ru/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/image-14.png)
...
![image-15.png](https://assets.spirzen.ru/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/image-15.png)
...
![image-16.png](https://assets.spirzen.ru/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/image-16.png)
...
![image-17.png](https://assets.spirzen.ru/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/image-17.png)
...
![image-18.png](https://assets.spirzen.ru/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/image-18.png)
...
![image-19.png](https://assets.spirzen.ru/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/image-19.png)
...
Дальше — [Как работать с Git](./112), типовые ошибки — [Типовые ситуации с Git](./1141), публикация статики — [GitHub Pages](/lab/Кейсы/3).
