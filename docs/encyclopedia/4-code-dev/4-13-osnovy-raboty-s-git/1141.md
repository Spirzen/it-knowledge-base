...
Эта глава — **практический справочник по симптомам**: "случилось X → сначала проверь Y → сделай Z". [12 команд на каждый день](/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/115#12-komand) — карманный набор; полная шпаргалка по CLI — в [справочнике 115](/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/115); базовый цикл и три зоны — в ["Как работать с Git"](/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/112); команда и PR — в [рекомендациях](/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/114); секреты и `.gitignore` — в [отдельном руководстве](/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/116). Краткие сценарии команд для копирования — [лабораторная шпаргалка](/lab/Примеры/1123).
...
<div class="callout callout--info">
  <div class="callout-title">Нужен только код без длинной карточки</div>

  <div class="callout-body">
  Готовые блоки команд с построчным разбором — [лабораторная «Git — шпаргалка сценариев»](/lab/Примеры/1123) ([push отклонён](/lab/Примеры/1123#push-отклонён), [откат](/lab/Примеры/1123#откат-reset), [stash](/lab/Примеры/1123#stash-временно-спрятать-правки)).

  Здесь — диагностика, зоны риска и пошаговые карточки.
</div>
</div>

| Симптом | Карточка |
|---------|----------|
| Забыл файл в последнем коммите | [Дополнить последний коммит](#дополнить-или-исправить-последний-коммит) · [amend в лаборатории](/lab/Примеры/1123#дополнить-последний-коммит-amend) |
...
| Срочно переключить ветку, работа не готова | [Stash](#временно-спрятать-правки-stash) · [лаборатория](/lab/Примеры/1123#stash-временно-спрятать-правки) |
...
| `push` отклонён (non-fast-forward) | [Push отклонён](#push-отклонён-non-fast-forward) · [лаборатория](/lab/Примеры/1123#push-отклонён) |
...
| После `reset --hard` пропали файлы | [Восстановление после hard reset](#после-git-reset---hard) · [reflog в лаборатории](/lab/Примеры/1123#восстановление-через-reflog) |
| Удалили ветку, коммиты нужны | [Удалённая ветка](#удалена-локальная-ветка) |
| Случайный merge в feature / на сервере | [Отмена merge](#отменить-слияние-merge) · [конфликт merge](/lab/Примеры/1123#merge-и-разрешение-конфликта) |
...
| Вклад в чужой open-source репозиторий | [Форк и upstream](#форк-и-синхронизация-с-upstream) · [лаборатория](/lab/Примеры/1123#fork-и-upstream) |
...
Подробнее про уровни Git и цепочку команд — в [главе про workflow](/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/112#git-workflow-chain).
...
<div class="callout callout--warning">
  ...
  <div class="callout-body">
  Неотслеживаемые файлы `reset --hard` не трогает; для них — `git clean`.

  Оба действия необратимы без `reflog` / резервной копии.

  Стоп-лист и порядок спасения — [Опасные скрипты](/encyclopedia/8-infra-security/8-03-zabota-o-kode-i-dannyh/101).
</div>
  </div>
...
| Тема | Где читать |
|------|------------|
| Базовый цикл, `add -p`, просмотр коммита | [Как работать с Git — Как работать с Git](/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/112) |
| PR, конфликты merge, инструменты | [Ветвление и слияние в Git — Ветвление и слияние](/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/113) |
| Культура команды, detached HEAD, краткое восстановление | [Рекомендации по использованию Git в команде — Рекомендации](/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/114) |
| Объектная модель, bisect (справочник), worktree, filter-repo | [Справочник-шпаргалка по Git — Шпаргалка](/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/115) |
| Секреты, `.gitignore`, утечки | [Файл .gitignore — .gitignore](/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git/116) |
