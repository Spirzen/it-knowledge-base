...
function Student:new(name)
    ...
    function interface:addGrade(grade)
        if grade >= 0 and grade <= 100 then
            table.insert(data.grades, grade)
            data.lastModified = os.date()
        end
    end
    ...
    function interface:getAverage()
        ...
        return #data.grades > 0 and sum / #data.grades or 0
    end
...
Функция `generate` работает с любыми отчетами, имеющими методы `start`, `data` и `endReport`. Добавление нового типа отчета (например, HTML) не потребует изменения кода генератора.
