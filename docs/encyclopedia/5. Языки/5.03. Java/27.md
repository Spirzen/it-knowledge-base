...
- **`spring-expression` (SpEL)** — мощный язык выражений, встроенный во время выполнения. Позволяет динамически вычислять значения в конфигурации: `@Value("#{systemProperties['user.region']}")`, `@Value("#{T(java.lang.Math).random() * 100}")`, вызов методов, доступ к коллекциям. Используется не только в XML/Java-конфигурации, но и в Spring Security, Spring Data и др.
...
- **`spring-orm`** — интеграция с ORM-фреймворками: Hibernate, JPA, JDO, iBatis/MyBatis. Spring *не заменяет ORM*, а *управляет его жизненным циклом* (сессии, фабрики), *обёртывает исключения* (HibernateException → DataAccessException) и *координирует транзакции*.
...
- DTO-проекции через `@Query` — возврат не сущностей, а плоских объектов.
...
Многие корпоративные системы до сих пор используют XML-конфигурацию. Миграция — не просто замена синтаксиса, а *архитектурное обновление*.
