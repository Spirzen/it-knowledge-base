...
В основе Flask лежит **WSGI** — *Web Server Gateway Interface*. Это не фреймворк и не библиотека, а *спецификация*, описывающая стандарт взаимодействия между веб-сервером (например, Apache, Nginx, Gunicorn) и Python-приложением.
...
Flask одинаково хорошо подходит как для full-stack приложений, так и для реализации RESTful API. Для этого достаточно возвращать из обработчиков не строки или HTML, а структурированные данные в формате JSON.
...
- URI должны именовать *ресурсы*, а не действия: `/users`, `/users/123`, `/users/123/posts`, а не `/getUser`, `/deleteUser`;
...
4. **Обработка ошибок**  
   ...
   В production важно логировать не только текст ошибки, но и код ответа сервера (`e.smtp_code`, `e.smtp_error`), поскольку он позволяет точно диагностировать причину (например, 550 — недоставляемый адрес, 421 — временная перегрузка).
