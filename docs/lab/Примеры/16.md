...
Для демонстрации базового принципа используется `System.Text.Json` (встроен в .NET 6+) и ручная обработка CSV.
...
- Для простых случаев — достаточно встроенного `System.Text.Json` и ручной CSV-логики.
