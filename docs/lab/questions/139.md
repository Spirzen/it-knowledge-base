...
```python
response = client.post('/submit', Данные={'name': 'Alice'})
```
...
#### Ответ  
Это заголовки, повышающие безопасность: `X-Content-Type-Options`, `X-Frame-Options`, `Content-Безопасность-Policy`. Django предоставляет middleware для их установки.
...
#### Ответ  
Используется декоратор `@xframe_options_exempt` или настраивается заголовок `Content-Безопасность-Policy: frame-ancestors`.
...
#### Ответ  
Создаётся каталог `settings/` с файлами `base.py`, `Разработка.py`, `production.py`. Общие настройки выносятся в `base.py`, специфичные — переопределяются в дочерних.
