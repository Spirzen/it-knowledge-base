...
**Связь с Excel:** на листе «Позиции» столбец «Сумма строки» = `quantity * price`, затем сводная таблица по `order_id` с полем «Сумма». В Pandas — `df.groupby('order_id').apply(..)`, см. [1113](/lab/Примеры/1113).
...
| В тренажёре ошибка на `WITH` | Движок тренажёра проще сервера | Выполнить в PostgreSQL — [111](/encyclopedia/3-data-markup/3-07-sql/111) |
