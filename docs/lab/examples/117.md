...
const Данные = await throttlePromises(fetchers, 5); // максимум 5 одновременных запросов
```
...
// Примеры:
const Данные = { user: { profile: { name: 'Timur' } } };
...
async function readJsonFile<T>(
  ...
  try {
    const Данные = await fs.promises.readFile(filePath, 'utf-8');
...
function asyncReducer<T>(state: AsyncState<T>, action: AsyncAction<T>): AsyncState<T> {
  switch (action.type) {
    ...
    case 'resolve': return { status: 'resolved', Данные: action.Данные };
...
return (
  <form onSubmit={handleSubmit(Данные => api.login(Данные))}>
    <Input
      value={values.email}
...
class TypedWorker {
  ...
  constructor() {
    ...
    this.worker.onmessage = (ev) => {
      const { id, payload } = ev.Данные as { id: number; payload: WorkerResponse };
...
self.onmessage = async (ev) => {
  const { id, payload } = ev.Данные as { id: number; payload: WorkerRequest };
  ...
  try {
    switch (payload.type) {
      ...
      case 'process':
        const sum = payload.Данные.reduce((a, b) => a + b, 0);
...
const kafkaPublisher = async (ev: OutboxEvent) => {
  const key = ev.type === 'user.created' ? ev.Данные.userId : ev.Данные.orderId;
  await kafka.send({
    topic: 'events',
