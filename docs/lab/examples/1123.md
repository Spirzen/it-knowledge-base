...
Команда `git status` отображает текущее состояние рабочей директории и области подготовленных файлов (staging area / index). Она показывает, какие изменения уже подготовлены к коммиту, какие изменены, но еще не добавлены в индекс, и какие файлы вовсе не отслеживаются системой Git.
