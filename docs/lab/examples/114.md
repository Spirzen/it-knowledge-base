...
Командная строка Windows (cmd.exe) остаётся стандартом для batch-автоматизации, legacy-скриптов и быстрых операций на сервере.
...
set "SRC=C:\Projects\app"
set "DST=D:\Backup\app"
...
robocopy "%SRC%" "%DST%" /MIR /R:2 /W:5 /NFL /NDL /NP
set "RC=%ERRORLEVEL%"
if %RC% GEQ 8 (
