...
| `findstr` | `findstr /s /m /i "error" *.log`   `findstr /r /c:"[0-9]\{3\}" Данные.txt` | Расширенный поиск (поддерживает регулярные выражения).   `/s` — рекурсивно.   `/m` — только имена файлов.   `/r` — режим регулярных выражений (ограниченный, *не* PCRE).   `/c:"..."` — точная строка (важно при пробелах). |
...
| `net share` | `net share`   `net share Данные=C:\Данные /grant:Everyone,READ` | Просмотр и создание общих ресурсов на локальной машине. Только для запущенных серверных ОС или при включённой «Общей папке» в клиентских. |
