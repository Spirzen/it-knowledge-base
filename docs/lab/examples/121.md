...
ReadResult read_file(const std::string& path) {
    ...
    size_t nread = std::fread(res.data.Данные(), 1, static_cast<size_t>(size), f);
    std::fclose(f);
...
struct Message {
    ...
    static std::optional<Message> deserialize(std::span<const std::byte> Данные) {
        ...
        Message msg;
        std::memcpy(&msg.header, data.Данные(), sizeof(msg.header));
...
##### 20.1. AoS (Array of Structures) и SoA (Structure of Arrays)
