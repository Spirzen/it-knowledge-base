...
Фрагмент `src/App.css` (или импорт из [113](/encyclopedia/3-data-markup/3-10-css/113#кнопки)):
...
Стили shimmer — [113](/encyclopedia/3-data-markup/3-10-css/113#скелетон-загрузки-skeleton) или [1116 — Skeleton](/lab/Примеры/1116#skeleton-shimmer).
...
**Задача:** обёртка с подсказкой по hover и **фокусу с клавиатуры** — CSS-only из [1155](/lab/Примеры/1155#подсказки-tooltip) недостаточен для touch; в React управляем видимостью явно.
...
export default function App() {
  return (
    <div className="app">
      <h1>Подсказки</h1>
      <Tooltip label="Сохранить черновик на сервере">
        <Button variant="primary">Сохранить</Button>
      </Tooltip>
    </div>
  );
...
`src/App.tsx` — демо всех четырёх компонентов:
...
| Средний | TypeScript + типы props | [30 — TypeScript и React](/encyclopedia/5-languages/5-01-javascript/30), [21 — типизация UI](/encyclopedia/5-languages/5-10-typescript/21#типизация-ui-компонентов) |
| Средний | UI из Galaxy → React | [1155](/lab/Примеры/1155), рецепты [Button](#ui-button), [Skeleton](#ui-skeleton) |
