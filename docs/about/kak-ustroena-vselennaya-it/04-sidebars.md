...
Сейчас в `docsSidebar` **десять** корневых блоков — четыре [категории](#категория) на spirzen.ru, пять внешних ссылок на порталы и один одиночный документ.

| Порядок | Пункт | Как задан в `sidebars.js` |
|---------|-------|---------------------------|
| 1 | Общее содержание | `type: 'doc'`, id `toc` |
| 2 | О проекте | [Ручная категория](#ручная-категория) `about/*` |
| 3 | Энциклопедия | autogenerated `encyclopedia` |
| 4 | Видеоигры | `type: 'link'` → games.spirzen.ru |
| 5 | Инструменты | `type: 'link'` → tools.spirzen.ru |
| 6 | Глоссарий | `type: 'link'` → terms.spirzen.ru |
| 7 | Лаборатория | `type: 'link'` → lab.spirzen.ru |
| 8 | Для детей | `type: 'link'` → kids.spirzen.ru |
| 9 | Контекст | autogenerated `context` |
| 10 | Философия | autogenerated `philosophy` |

Папки `docs/glossary/`, `docs/lab/`, `docs/tools/` и spinoff **9-03**, **9-04**, **9-11** в сборке spirzen **исключены** (`exclude` в `docusaurus.config.js`); старые URL ведут на порталы через `*ExternalRedirects.json`.
