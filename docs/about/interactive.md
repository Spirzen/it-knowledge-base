...
Особенность нашей энциклопедии заключается в том, что используется не только текст, но и множество интерактивных React-компонентов, которые реагируют на действия пользователя (клик, ввод текста) мгновенно, так как могут легко обновлять внутреннее состояние (state) без перезагрузки страницы, благодаря чему мы получаем живое изучение.

Такие кастомные React-демо встроены в статьи через [play.spirzen.ru](https://play.spirzen.ru/) (iframe, lazy load). Это не сторонние виджеты и не скриншоты — эмуляторы Office и терминала, пошаговые визуализаторы алгоритмов, схемы сети и инфраструктуры, тренажёры SQL и Git, игровые мини-демо и многое другое.

Поскольку интерактив расположен на другом сервисе, элемент `<iframe>` создаёт независимый встроенный браузерный контекст, который загружает совершенно отдельную HTML-страницу внутри вашего основного сайта. Как встраиваемые YouTube-ролики, карты или баннеры. Но вместо рекламы, я использую именно интересные возможности.

Ниже — **витрина лучших демо** в порядке возрастания сложности: сначала раздел "Основы", затем система и сеть, данные, код, языки, ИИ, проект и инфраструктура. Полный каталог привязан к статьям энциклопедии; здесь — ориентиры, с которых удобно начать. Полный реестр компонентов ведётся в репозитории (`info/demo-registry.md`). На сайте каждое демо живёт в своей статье — откройте ссылку под блоком, чтобы углубиться.

Главное - не смущайтесь от необходимости "кликать" на демо для запуска. Такой подход выбран осознанно, для скорости и обеспечения распределённости энциклопедии.
...
### WebEditor — HTML, CSS и JavaScript

Полноценный онлайн-редактор с отдельными вкладками HTML, CSS и JavaScript: живой предпросмотр обновляется при вводе. Удобен для экспериментов со всем веб-стеком сразу — не только с разметкой, как во встроенной песочнице выше.

Отдельное приложение экосистемы: [WebEditor](https://html.spirzen.ru/) ([исходники на GitHub](https://github.com/Spirzen/WebEditor)) — в статьях разделов HTML, CSS и JavaScript ссылка также стоит под встроенными демо.
