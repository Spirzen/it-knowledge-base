...
API-ключ (API key) — долгоживущий секретный идентификатор, который клиент передаёт при вызове удалённого API (заголовок `X-API-Key`, `Authorization: Bearer …` или query-параметр). Сервер сверяет ключ с реестром в БД и выдаёт доступ на уровне **приложения**, без контекста конкретного пользователя. Проще внедрить, чем OAuth и JWT, но при утечке действует до ручного отзыва. Сравнение с JWT — [авторизация в интеграционных сценариях](/encyclopedia/2-system-network/2-09-osnovy-integratsionnogo-vzaimodeystviya/1121#tokens-i-api-keys).
